...
int scamper_dealias_ipid(const scamper_dealias_probe_t **probes,
  ...
  if (probec == 0 || probes[0] == NULL || probes[0]->replyc != 1) return 0;

  prev = probes[0]->replies[0]->ipid;
  for (i = 1; i < probec; i++) {
    if ((p = probes[i]) == NULL) return 0;

    if (p->replyc != 1) return 0;
    ...
    if (cur > prev)
      ...
      diff = 0;
    if (diff < bs_mind) bs_mind = diff;
    if (diff > maxd) bs_maxd = diff;
    bs_sum += diff;
...
static void dealias_prefixscan_free(void *data) {
  ...
  if (prefixscan->xs != NULL) {
    for (i = 0; i < prefixscan->xc; i++)
      if (prefixscan->xs[i] != NULL) scamper_addr_free(prefixscan->xs[i]);
    free(prefixscan->xs);
  }
...
const char *scamper_dealias_probedef_method_tostr(
  ...
  static const char *m[] = {
      NULL,        "icmp-echo",     "tcp-ack", "udp", "tcp-ack-sport",
      "udp-dport", "tcp-syn-sport",
  };
...
void scamper_dealias_reply_free(scamper_dealias_reply_t *reply) {
  if (reply->src != NULL) scamper_addr_free(reply->src);
  free(reply);
  return;
}
...
uint32_t scamper_dealias_reply_count(const scamper_dealias_t *dealias) {
  ...
  for (i = 0; i < dealias->probec; i++) {
    if (dealias->probes[i] != NULL) rc += dealias->probes[i]->replyc;
  }
  return rc;
}
...
static int dealias_ipid16_inseq2(uint16_t a, uint16_t b, uint16_t fudge) {
  if (a == b || dealias_ipid16_diff(a, b) > fudge) return 0;
  return 1;
}
...
static int dealias_ipid32_inseq2(uint32_t a, uint32_t b, uint32_t fudge) {
  if (a == b || dealias_ipid32_diff(a, b) > fudge) return 0;
  return 1;
}
...
static int dealias_ipid16_bo(scamper_dealias_probe_t **probes, int probec) {
  ...
  for (i = 0; i < probec; i++) {
    if (i + 1 == probec || s[i]->def != s[i + 1]->def) {
      ...
      a = s[i]->replies[0]->ipid;
      b = s[i + 1]->replies[0]->ipid;
      u16 = dealias_ipid16_diff(a, b);
      if (u16 > max_nobs || max_nobs == 0) max_nobs = u16;
      u16 = dealias_ipid16_diff(byteswap16(a), byteswap16(b));
      if (u16 > max_bs || max_bs == 0) max_bs = u16;
      c++;
    }
  }
...
static int dealias_ipid16_inseq(scamper_dealias_probe_t **probes, int probec,
  ...
  if (probec == 2) {
    ...
    if (dealias_ipid16_inseq2(a, b, fudge) != 0) return 1;
    return 0;
  }
  ...
  for (i = 0; i + 2 < probec; i++) {
    ...
    if (dealias_ipid16_inseq3(a, b, c, fudge) == 0) return 0;
  }
...
static int dealias_ipid32_bo(scamper_dealias_probe_t **probes, int probec) {
  ...
  for (i = 0; i < probec; i++) {
    if (i + 1 == probec || s[i]->def != s[i + 1]->def) {
      ...
      a = s[i]->replies[0]->ipid32;
      b = s[i + 1]->replies[0]->ipid32;
      u32 = dealias_ipid32_diff(a, b);
      if (u32 > max_nobs || max_nobs == 0) max_nobs = u32;
      u32 = dealias_ipid32_diff(byteswap32(a), byteswap32(b));
      if (u32 > max_bs || max_bs == 0) max_bs = u32;
      c++;
    }
  }
...
static int dealias_ipid32_inseq(scamper_dealias_probe_t **probes, int probec,
  ...
  if (probec == 2) {
    ...
    if (dealias_ipid32_inseq2(a, b, fudge) != 0) return 1;
    return 0;
  }
  ...
  for (i = 0; i + 2 < probec; i++) {
    ...
    if (dealias_ipid32_inseq3(a, b, c, fudge) == 0) return 0;
  }
...
int scamper_dealias_ipid_inseq(scamper_dealias_probe_t **probes, int probec,
  ...
  if (bs == 3 && fudge == 0) {
    if ((i = bo[x](probes, probec)) == -1) return -1;
    return inseq[x](probes, probec, fudge, i);
  }

  if (bs == 2 || bs == 3) {
    if (inseq[x](probes, probec, fudge, 0) == 1) return 1;
...
int scamper_dealias_probes_alloc(scamper_dealias_t *dealias, uint32_t cnt) {
  ...
  if ((dealias->probes = malloc_zero(size)) == NULL) return -1;
  return 0;
}

int scamper_dealias_replies_alloc(scamper_dealias_probe_t *probe,
                                  uint16_t cnt) {
  size_t size = cnt * sizeof(scamper_dealias_reply_t *);
  if ((probe->replies = malloc_zero(size)) == NULL) return -1;
  return 0;
}
...
int scamper_dealias_radargun_probedefs_alloc(scamper_dealias_radargun_t *rg,
  ...
  if ((rg->probedefs = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_dealias_prefixscan_xs_add(scamper_dealias_t *dealias,
  ...
  if ((tmp = prefixscan->xc) == 65535) return -1;
...
int scamper_dealias_prefixscan_xs_alloc(scamper_dealias_prefixscan_t *p,
                                        uint16_t xc) {
  if ((p->xs = malloc_zero(sizeof(scamper_addr_t *) * xc)) != NULL) return 0;
  return -1;
}
...
int scamper_dealias_prefixscan_probedefs_alloc(scamper_dealias_prefixscan_t *p,
                                               uint32_t probedefc) {
  size_t len = probedefc * sizeof(scamper_dealias_probedef_t);
  if ((p->probedefs = malloc_zero(len)) != NULL) return 0;
  return -1;
}
...
int scamper_dealias_prefixscan_probedef_add(scamper_dealias_t *dealias,
  ...
  if (realloc_wrap((void **)&prefixscan->probedefs, size) != 0) return -1;

  /* add the probedef to the array */
  memcpy(&prefixscan->probedefs[prefixscan->probedefc], def,
         sizeof(scamper_dealias_probedef_t));
...
int scamper_dealias_radargun_fudge(scamper_dealias_t *dealias,
  ...
  if (dealias->method != SCAMPER_DEALIAS_METHOD_RADARGUN) goto err;
  ...
  for (x = 0; x < dealias->probec; x++) {
     ...
     */
    if (dr[pid].probec < 0) continue;

    if (pr->replyc > 1) {
      if (dr[pid].probes != NULL) free(dr[pid].probes);
      dr[pid].probec = -1;

      if (pr->def == def) goto done;
      continue;
    }
  ...
  if (dr[def->id].probec < 3) goto done;
  ...
  for (pid = 0; pid < rg->probedefc; pid++) {
    if (&rg->probedefs[pid] == def || dr[pid].probec < 3) continue;
    ...
    for (;;) {
      ...
      if ((inseq = dealias_fudge_inseq(pr_a, pr_b, bs, fudge)) == 0) break;

      if (j == drd->probec || k == dr[pid].probec) break;
    }
  ...
  *cnt = d;
  for (x = 0; x < rg->probedefc; x++)
    if (dr[x].probec > 0) free(dr[x].probes);
  free(dr);
  return 0;
...
const char *scamper_dealias_method_tostr(const scamper_dealias_t *d, char *b,
                                         size_t l) {
  static const char *m[] = {
      NULL, "mercator", "ally", "radargun", "prefixscan", "bump",
  };
  if (d->method >= sizeof(m) / sizeof(char *) || m[d->method] == NULL) {
    snprintf(b, l, "%d", d->method);
    return b;
...
void scamper_dealias_free(scamper_dealias_t *dealias) {
  static void (*const func[])(void *) = {
      dealias_mercator_free,   dealias_ally_free, dealias_radargun_free,
      dealias_prefixscan_free, dealias_bump_free,
  };

  uint32_t i;

  if (dealias == NULL) return;
