...
const char *scamper_do_dealias_usage(void) {
  return "dealias [-d dport] [-f fudge] [-m method] [-o replyc] [-O option]\n"
         "        [-p '[-c sum] [-d dp] [-F sp] [-i ip] [-M mtu] [-P meth] [-s "
         "size] [-t ttl]']\n"
...
static int dealias_ally_queue(const scamper_dealias_t *dealias,
                              dealias_state_t *state, const struct timeval *now,
                              struct timeval *tv) {
  if (state->ptb_tx.tv_sec == 0) return 0;
  timeval_add_s(tv, &state->ptb_tx, 1);
  if (timeval_cmp(tv, now) > 0) return 1;
...
static void dealias_queue(scamper_task_t *task) {
  static int (*const func[])(const scamper_dealias_t *, dealias_state_t *,
      ...
      NULL, dealias_ally_queue, NULL, NULL, NULL,
  };
  ...
  if (scamper_task_queue_isdone(task)) return;

  gettimeofday_wrap(&now);

  for (;;) {
    if ((p = dlist_head_item(state->recent_probes)) == NULL) break;
    timeval_add_s(&tv, &p->probe->tx, 10);
    if (timeval_cmp(&now, &tv) < 0) break;
...
static void dealias_ptb_free(dealias_ptb_t *ptb) {
  if (ptb == NULL) return;
  if (ptb->quote != NULL) free(ptb->quote);
  free(ptb);
  return;
}
...
static void dealias_target_free(dealias_target_t *tgt) {
  if (tgt == NULL) return;
  if (tgt->probes != NULL) dlist_free_cb(tgt->probes, free);
  if (tgt->addr != NULL) scamper_addr_free(tgt->addr);
  free(tgt);
  return;
}
...
static dealias_target_t *dealias_target_get(dealias_state_t *state,
  ...
  if ((tgt = dealias_target_find(state, addr)) != NULL) return tgt;
...
static int dealias_probedef_add(dealias_state_t *state,
  ...
  pd->def = def;
  if ((pd->target = dealias_target_get(state, def->dst)) == NULL) goto err;
...
static void dealias_prefixscan_array_free(scamper_addr_t **addrs, int addrc) {
  ...
  for (i = 0; i < addrc; i++)
    if (addrs[i] != NULL) scamper_addr_free(addrs[i]);
...
static int dealias_prefixscan_array(scamper_dealias_t *dealias,
  ...
  *outc = 0;
  ...
  if (prefixscan->prefix == 31) {
    ...
    if (dealias_prefixscan_array_add(dealias, &array, outc, &a) != 0) goto err;
...
static scamper_dealias_probe_t *dealias_probe_udp_find(dealias_state_t *state,
                                                       dealias_target_t *tgt,
                                                       uint16_t ipid,
                                                       uint16_t sport,
                                                       uint16_t dport) {
  ...
  for (n = dlist_head_node(tgt->probes); n != NULL; n = dlist_node_next(n)) {
    ...
    if (def->method == SCAMPER_DEALIAS_PROBEDEF_METHOD_UDP) {
      if (def->un.udp.dport == dport) return dp->probe;
    } else if (def->method == SCAMPER_DEALIAS_PROBEDEF_METHOD_UDP_DPORT) {
      if (dp->match_field == dport) return dp->probe;
    }
  }
...
static scamper_dealias_probe_t *dealias_probe_tcp_find2(dealias_state_t *state,
                                                        dealias_target_t *tgt,
                                                        uint16_t sport,
                                                        uint16_t dport) {
  scamper_dealias_probedef_t *def;
  ...
  for (n = dlist_head_node(tgt->probes); n != NULL; n = dlist_node_next(n)) {
    ...
    if (def->method == SCAMPER_DEALIAS_PROBEDEF_METHOD_TCP_ACK) {
      if (def->un.tcp.sport == sport) return dp->probe;
    } else if (SCAMPER_DEALIAS_PROBEDEF_VARY_TCP_SPORT(def)) {
      if (dp->match_field == sport) return dp->probe;
    }
  }
...
static scamper_dealias_probe_t *dealias_probe_tcp_find(dealias_state_t *state,
                                                       dealias_target_t *tgt,
                                                       uint16_t ipid,
                                                       uint16_t sport,
                                                       uint16_t dport) {
...
static scamper_dealias_probe_t *dealias_probe_icmp_find(
    dealias_state_t *state, dealias_target_t *tgt, uint16_t ipid, uint8_t type,
  ...
  if ((SCAMPER_ADDR_TYPE_IS_IPV4(tgt->addr) && type == ICMP_ECHO &&
       code == 0) ||
      (SCAMPER_ADDR_TYPE_IS_IPV6(tgt->addr) && type == ICMP6_ECHO_REQUEST &&
       code == 0))
  ...
  for (n = dlist_head_node(tgt->probes); n != NULL; n = dlist_node_next(n)) {
    ...
    if (SCAMPER_ADDR_TYPE_IS_IPV4(tgt->addr) && dp->probe->ipid != ipid)
      continue;
    if (def->method == method && def->un.icmp.id == id &&
        dp->match_field == seq)
      return dp->probe;
  }
...
static scamper_dealias_probe_t *dealias_probe_echoreq_find(
    dealias_state_t *state, dealias_target_t *tgt, uint16_t id, uint16_t seq) {
  scamper_dealias_probedef_t *def;
  dealias_probe_t *dp;
  dlist_node_t *n;
...
static dealias_probedef_t *dealias_mercator_def(scamper_dealias_t *dealias,
...
static dealias_probedef_t *dealias_ally_def(scamper_dealias_t *dealias,
                                            dealias_state_t *state) {
  return state->pds[state->probe];
}
...
static int dealias_ally_allzero(scamper_dealias_t *dealias) {
  ...
  for (i = 0; i < dealias->probec; i++) {
    ...
    for (j = 0; j < dealias->probes[i]->replyc; j++) {
      ...
      if (dealias->probes[i]->replies[j]->ipid != 0) return 0;
    }
  }
...
static int dealias_ally_handlereply_v6(scamper_task_t *task,
  ...
  if (state->ptb_tx.tv_sec != 0 || slist_count(state->ptbq) > 0) ptb = 1;

  /* is the probe going to be discarded? */
  for (sn = slist_head_node(state->discard); sn != NULL;
  ...
  if (probe->def->mtu != 0 && probe->def->mtu < dl->dl_ip_size &&
    ...
    if (dealias_ptb_add(state, dl, probe->def) != 0) return -1;
    dealias_queue(task);
    return 0;
  }
...
static void dealias_ally_handlereply(scamper_task_t *task,
  ...
  if (probe->replyc != 1 || !(SCAMPER_DEALIAS_REPLY_FROM_TARGET(probe, reply) ||
  ...
  if (dealias->probec < 2) return;

  /* find the probe in its place */
  for (k = 0; k < dealias->probec; k++)
    if (probe == dealias->probes[k]) break;
  if (k == dealias->probec) return;
  ...
  if (rc == 0) dealias_result(task, SCAMPER_DEALIAS_RESULT_NOTALIASES);
...
static void dealias_ally_handletimeout(scamper_task_t *task) {
  ...
  if (dealias->probec == ally->attempts) {
    for (k = 0; k < dealias->probec; k++)
      if (dealias->probes[k]->replyc != 1) break;
...
static dealias_probedef_t *dealias_radargun_def(scamper_dealias_t *dealias,
                                                dealias_state_t *state) {
...
static int dealias_radargun_postprobe(scamper_dealias_t *dealias,
  ...
  if (state->probe < rg->probedefc) {
    ...
    if (state->round < rg->attempts) {
      ...
      if ((rg->flags & SCAMPER_DEALIAS_RADARGUN_FLAG_SHUFFLE) != 0) {
        if (shuffle32(rgstate->order, rg->probedefc) != 0) return -1;
        rgstate->i = 0;
      }
    } else {
      /* we're all finished */
...
static dealias_probedef_t *dealias_prefixscan_def(scamper_dealias_t *dealias,
                                                  dealias_state_t *state) {
  return state->pds[state->probe];
}
...
static int dealias_prefixscan_next(scamper_task_t *task) {
  ...
  for (p = 0; p < dealias->probec; p++) defids[p] = dealias->probes[p]->def->id;
  ...
  if (dealias_probedef_add(state, def) != 0) goto err;
...
static void dealias_prefixscan_handlereply(scamper_task_t *task,
  ...
  if (probe != dealias->probes[dealias->probec - 1]) return;

  /* if the reply is not the first reply for this probe */
  if (probe->replyc != 1) return;

  if (probe->ipid == reply->ipid && ++pd->echo >= 2) {
    if (probe->def->id != 0) goto prefixscan_next;
   ...
   */
  if (scamper_task_queue_isprobe(task)) return;
  ...
  if (SCAMPER_DEALIAS_PROBEDEF_PROTO_IS_UDP(probe->def) &&
    ...
    if (probe->def->id == 0) {
       ...
       */
      if (array_find((void **)pfstate->aaliases, pfstate->aaliasc, reply->src,
                     (array_cmp_t)scamper_addr_cmp) == NULL) {
      ...
      if (scamper_addr_cmp(reply->src, prefixscan->a) == 0 ||
          array_find((void **)pfstate->aaliases, pfstate->aaliasc, reply->src,
                     (array_cmp_t)scamper_addr_cmp) != NULL) {
  ...
  if (seq < 2) {
    ...
    if (state->probedefc == 1) {
      ...
      if (dealias_prefixscan_next(task) != 0) goto err;

      /* if it turns out we don't need to probe, handle that */
      if (dealias->result == SCAMPER_DEALIAS_RESULT_ALIASES) return;
    }
  ...
  p = dealias->probec - 2;
  defid = probe->def->id;
  while (p >= 0 && dealias->probes[p]->def->id == defid) p--;
...
static dealias_probedef_t *dealias_bump_def(scamper_dealias_t *dealias,
                                            dealias_state_t *state) {
  return state->pds[state->probe];
}
...
static void dealias_bump_handletimeout(scamper_task_t *task) {
  ...
  if (bs->step < 2) {
    ...
    if (i * 2 > 65535) goto none;

    bs->bump = i * 2;
    if (bs->bump == 2) bs->bump++;

    if (bs->bump > bump->bump_limit) goto none;
...
static void do_dealias_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  static void (*const func[])(scamper_task_t *, scamper_dealias_probe_t *,
                              scamper_dealias_reply_t *, scamper_dl_rec_t *) = {
      dealias_mercator_handlereply, dealias_ally_handlereply,
      dealias_radargun_handlereply, dealias_prefixscan_handlereply,
  ...
  if (v4 && SCAMPER_DL_IS_TCP(dl)) {
    if (scamper_dl_rec_src(dl, &a) != 0 ||
      ...
      return;
    probe =
        dealias_probe_tcp_find2(state, tgt, dl->dl_tcp_dport, dl->dl_tcp_sport);
    ...
    if (SCAMPER_DL_IS_ICMP_TTL_EXP(dl) || SCAMPER_DL_IS_ICMP_UNREACH(dl) ||
        SCAMPER_DL_IS_ICMP_PACKET_TOO_BIG(dl)) {
      /* the IPID value used is expected to be of the form 0xabab */
      if (v4 && (dl->dl_icmp_ip_id & 0xff) != (dl->dl_icmp_ip_id >> 8)) return;
      ...
      if (dl->dl_icmp_ip_proto == IPPROTO_UDP)
        ...
        probe = dealias_probe_icmp_find(
            state, tgt, dl->dl_icmp_ip_id, dl->dl_icmp_icmp_type,
            dl->dl_icmp_icmp_code, dl->dl_icmp_icmp_id, dl->dl_icmp_icmp_seq);
      ...
      if (scamper_dl_rec_src(dl, &a) != 0 ||
          (tgt = dealias_target_find(state, &a)) == NULL)
        return;
      probe = dealias_probe_echoreq_find(state, tgt, dl->dl_icmp_id,
                                         dl->dl_icmp_seq);
    } else
      return;
  ...
  if (probe == NULL || scamper_dl_rec_src(dl, &a) != 0) return;
  ...
  } else if ((reply->src = scamper_addrcache_get(addrcache, a.type, a.addr)) ==
             NULL) {
...
static void do_dealias_handle_icmp(scamper_task_t *task,
  ...
  if ((state->flags & DEALIAS_STATE_FLAG_DL) != 0) return;
  ...
  if (SCAMPER_ICMP_RESP_IS_TTL_EXP(ir) || SCAMPER_ICMP_RESP_IS_UNREACH(ir) ||
    ...
    if (ir->ir_inner_ip_proto == IPPROTO_UDP)
      ...
      probe = dealias_probe_icmp_find(
          state, tgt, ir->ir_inner_ip_id, ir->ir_inner_icmp_type,
          ir->ir_inner_icmp_code, ir->ir_inner_icmp_id, ir->ir_inner_icmp_seq);
    ...
    if (scamper_icmp_resp_src(ir, &a) != 0) return;
  } else if (SCAMPER_ICMP_RESP_IS_ECHO_REPLY(ir) != 0) {
    if (scamper_icmp_resp_src(ir, &a) != 0 ||
        (tgt = dealias_target_find(state, &a)) == NULL)
      return;
    probe =
        dealias_probe_echoreq_find(state, tgt, ir->ir_icmp_id, ir->ir_icmp_seq);
  }
...
static void do_dealias_handle_timeout(scamper_task_t *task) {
  static void (*const func[])(scamper_task_t *) = {
      dealias_mercator_handletimeout, dealias_ally_handletimeout,
      dealias_radargun_handletimeout, dealias_prefixscan_handletimeout,
 ...
 */
static int dealias_state_probe(dealias_state_t *state, dealias_probedef_t *pdef,
                               scamper_dealias_probe_t *probe,
                               scamper_probe_t *pr) {
...
static void dealias_prefixscan_free(void *data) {
  ...
  if (pfstate->aaliases != NULL) {
    for (k = 0; k < pfstate->aaliasc; k++)
      if (pfstate->aaliases[k] != NULL) scamper_addr_free(pfstate->aaliases[k]);
...
static int dealias_prefixscan_alloc(scamper_dealias_t *dealias,
  ...
  if (dealias_prefixscan_array(dealias, &addrs, &addrc) != 0) goto err;
...
static void dealias_radargun_free(void *data) {
  ...
  if (rgstate->order != NULL) free(rgstate->order);
  free(rgstate);
  return;
}
...
static int dealias_radargun_alloc(scamper_dealias_radargun_t *rg,
  ...
  if ((rg->flags & SCAMPER_DEALIAS_RADARGUN_FLAG_SHUFFLE)) {
    ...
    for (i = 0; i < rg->probedefc; i++) rgstate->order[i] = i;
    if (shuffle32(rgstate->order, rg->probedefc) != 0) return -1;
  }
...
static void dealias_state_free(scamper_dealias_t *dealias,
  ...
  if (state->recent_probes != NULL) dlist_free(state->recent_probes);
  ...
  if (state->pds != NULL) {
    for (j = 0; j < state->pdc; j++)
      if (state->pds[j] != NULL) free(state->pds[j]);
    free(state->pds);
  }
...
static void do_dealias_probe(scamper_task_t *task) {
  static int (*const postprobe_func[])(scamper_dealias_t *,
                                       dealias_state_t *) = {
      dealias_mercator_postprobe, dealias_ally_postprobe,
      dealias_radargun_postprobe, dealias_prefixscan_postprobe,
      dealias_bump_postprobe,
  };
  static dealias_probedef_t *(*const def_func[])(scamper_dealias_t *,
                                                 dealias_state_t *) = {
      dealias_mercator_def,   dealias_ally_def, dealias_radargun_def,
      dealias_prefixscan_def, dealias_bump_def,
  };
  scamper_dealias_t *dealias = dealias_getdata(task);
  dealias_state_t *state = dealias_getstate(task);
  ...
  if (dealias->probec == 0) gettimeofday_wrap(&dealias->start);
  ...
  if ((pdef = def_func[dealias->method - 1](dealias, state)) == NULL) goto err;
  def = pdef->def;
  ...
  if (dealias_state_probe(state, pdef, dp, &probe) != 0) goto err;
  ...
  if (postprobe_func[dealias->method - 1](dealias, state) != 0) goto err;

  assert(state->id != 0);
  if (--state->id == 0) state->id = 255;
...
static int dealias_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case DEALIAS_OPT_DPORT:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 65535) return -1;
      break;
    ...
    case DEALIAS_OPT_ATTEMPTS:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 500) return -1;
      break;
    ...
    case DEALIAS_OPT_TTL:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 255) return -1;
      break;

    case DEALIAS_OPT_USERID:
      if (string_tolong(param, &tmp) != 0 || tmp < 0) return -1;
      break;
    ...
    case DEALIAS_OPT_WAIT_ROUND:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 180000) return -1;
      break;

    case DEALIAS_OPT_REPLYC:
      if (string_tolong(param, &tmp) != 0 || tmp < 3 || tmp > 255) return -1;
      break;
...
static int dealias_probedef_args(scamper_dealias_probedef_t *def, char *str) {
  ...
  if (scamper_options_parse(str, probedef_opts, probedef_opts_cnt, &opts_out,
                            &end) != 0) {
...
static int dealias_alloc_mercator(scamper_dealias_t *d, dealias_options_t *o) {
  ...
  if ((dst = scamper_addrcache_resolve(addrcache, AF_UNSPEC, o->addr)) ==
      NULL) {
...
static int dealias_alloc_ally(scamper_dealias_t *d, dealias_options_t *o) {
  ...
  if (o->probedefs != NULL) probedefc = slist_count(o->probedefs);
  ...
  if (o->fudge == 0 && o->inseq == 0) o->fudge = 200;

  if (probedefc > 0) {
    i = 0;
    for (sn = slist_head_node(o->probedefs); sn != NULL;
         sn = slist_node_next(sn)) {
...
static int dealias_alloc_radargun(scamper_dealias_t *d, dealias_options_t *o) {
  ...
  if (pdc == 0) {
    ...
    if (dealias_probedef_args(&pd0, (char *)slist_head_item(o->probedefs)) !=
        0) {
...
err:
  if (pd != NULL) {
    for (j = 0; j < pdc; j++)
      if (pd[j].dst != NULL) scamper_addr_free(pd[j].dst);
    free(pd);
  }
  if (pd_list != NULL)
...
static int dealias_alloc_prefixscan(scamper_dealias_t *d,
                                    dealias_options_t *o) {
  scamper_dealias_prefixscan_t *prefixscan;
  ...
  if (o->nobs != 0) flags |= SCAMPER_DEALIAS_PREFIXSCAN_FLAG_NOBS;
...
void *scamper_do_dealias_alloc(char *str) {
  static int (*const alloc_func[])(scamper_dealias_t *, dealias_options_t *) = {
      dealias_alloc_mercator,   dealias_alloc_ally, dealias_alloc_radargun,
      dealias_alloc_prefixscan, dealias_alloc_bump,
  };
  scamper_option_out_t *opts_out = NULL, *opt;
  ...
  if (o.wait_timeout == 0) o.wait_timeout = 5;
  ...
  if (alloc_func[method - 1](dealias, &o) != 0) goto err;
...
static int probedef2sig(scamper_task_t *task, scamper_dealias_probedef_t *def) {
  ...
  /* add it to the task */
  if (scamper_task_sig_add(task, sig) != 0) goto err;
...
scamper_task_t *scamper_do_dealias_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(dealias, &funcs)) == NULL) goto err;
  ...
  if ((state = malloc_zero(sizeof(dealias_state_t))) == NULL ||
      ...
      (state->targets = splaytree_alloc((splaytree_cmp_t)dealias_target_cmp)) ==
          NULL) {
  ...
  if (dealias->method == SCAMPER_DEALIAS_METHOD_MERCATOR) {
    ...
    if (probedef2sig(task, &mercator->probedef) != 0) goto err;
    state->probedefs = &mercator->probedef;
    state->probedefc = 1;
  } else if (dealias->method == SCAMPER_DEALIAS_METHOD_ALLY) {
    ally = dealias->data;
    for (i = 0; i < 2; i++)
      if (probedef2sig(task, &ally->probedefs[i]) != 0) goto err;
