...
static char *dealias_header_tostr(const scamper_dealias_t *dealias) {
  ...
  if (SCAMPER_DEALIAS_METHOD_IS_MERCATOR(dealias)) {
    mc = dealias->data;
    string_concat(buf, sizeof(buf), &off,
                  ", \"attempts\":%u, \"wait_timeout\":%u", mc->attempts,
                  mc->wait_timeout);
  } else if (SCAMPER_DEALIAS_METHOD_IS_ALLY(dealias)) {
    ally = dealias->data;
    string_concat(buf, sizeof(buf), &off,
                  ", \"wait_probe\":%u, \"wait_timeout\":%u", ally->wait_probe,
                  ally->wait_timeout);
    ...
    rg = dealias->data;
    string_concat(buf, sizeof(buf), &off,
                  ", \"attempts\":%u, \"wait_probe\":%u", rg->attempts,
                  rg->wait_probe);
    string_concat(buf, sizeof(buf), &off,
                  ", \"wait_round\":%u, \"wait_timeout\":%u", rg->wait_round,
                  rg->wait_timeout);
    if (rg->flags != 0)
    ...
    string_concat(buf, sizeof(buf), &off,
                  ", \"wait_probe\":%u, \"wait_timeout\":%u", pf->wait_probe,
                  pf->wait_timeout);
    if (pf->flags != 0)
      string_concat(buf, sizeof(buf), &off, "%s",
...
static char *dealias_probedef_tostr(const scamper_dealias_probedef_t *def) {
  char buf[256], tmp[64];
  size_t off = 0;
  string_concat(buf, sizeof(buf), &off, "{\"id\":%u, \"src\":\"%s\"", def->id,
                scamper_addr_tostr(def->src, tmp, sizeof(tmp)));
  string_concat(
      buf, sizeof(buf), &off, ", \"dst\":\"%s\", \"ttl\":%u, \"size\":%u",
  ...
  if (SCAMPER_DEALIAS_PROBEDEF_PROTO_IS_ICMP(def))
    ...
    string_concat(buf, sizeof(buf), &off,
                  ", \"udp_sport\":%u, \"udp_dport\":%u", def->un.udp.sport,
                  def->un.udp.dport);
...
static char *dealias_probe_tostr(const scamper_dealias_probe_t *probe) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(probe->def->dst))
    string_concat(header, sizeof(header), &header_len, ", \"ipid\":%u",
                  probe->ipid);
  ...
  if ((str = malloc_zero(len)) == NULL) goto done;
...
int scamper_file_json_dealias_write(const scamper_file_t *sf,
  ...
  if ((header = dealias_header_tostr(dealias)) == NULL) goto cleanup;
...
cleanup:
  ...
  if (pds != NULL) {
    for (i = 0; i < defc; i++)
      if (pds[i] != NULL) free(pds[i]);
    free(pds);
  }
  if (prs != NULL) {
    for (j = 0; j < dealias->probec; j++)
      if (prs[j] != NULL) free(prs[j]);
    free(prs);
  }
  return rc;
}
