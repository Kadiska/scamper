...
static const warts_var_t dealias_vars[] = {
    {WARTS_DEALIAS_LIST_ID, 4, -1}, {WARTS_DEALIAS_CYCLE_ID, 4, -1},
    {WARTS_DEALIAS_START, 8, -1},   {WARTS_DEALIAS_METHOD, 1, -1},
    {WARTS_DEALIAS_RESULT, 1, -1},  {WARTS_DEALIAS_PROBEC, 4, -1},
...
static const warts_var_t dealias_probe_vars[] = {
    {WARTS_DEALIAS_PROBE_DEF, 4, -1},    {WARTS_DEALIAS_PROBE_TX, 8, -1},
    {WARTS_DEALIAS_PROBE_REPLYC, 2, -1}, {WARTS_DEALIAS_PROBE_IPID, 2, -1},
...
static int warts_dealias_probedef_read(scamper_dealias_probedef_t *p,
  ...
  if (p->src == NULL || p->dst == NULL) return -1;
...
static int extract_dealias_prefixscan_xs(const uint8_t *buf, uint32_t *off,
  ...
  if (extract_uint16(buf, off, len, &xc, NULL) != 0) return -1;

  if (scamper_dealias_prefixscan_xs_alloc(pfs, xc) != 0) return -1;

  xs = pfs->xs;
  for (i = 0; i < xc; i++) {
    if (extract_addr(buf, off, len, &xs[i], param) != 0) return -1;
  }
...
static void insert_dealias_prefixscan_xs(uint8_t *buf, uint32_t *off,
  ...
  for (i = 0; i < p->xc; i++) insert_addr(buf, off, len, p->xs[i], param);
...
static int warts_dealias_prefixscan_state(const scamper_file_t *sf,
  ...
  if (p->probedefc > 0) {
    ...
    if ((state->probedefs = malloc_zero(size)) == NULL) return -1;
  }
...
static int warts_dealias_prefixscan_read(scamper_dealias_t *dealias,
  ...
  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) return -1;
  if (pfs.a == NULL || pfs.b == NULL) return -1;

  if (scamper_dealias_prefixscan_alloc(dealias) != 0) return -1;

  p = dealias->data;
  memcpy(p, &pfs, sizeof(pfs));

  /* by default we require five replies before inferring an alias */
  if (flag_isset(&buf[o], WARTS_DEALIAS_PREFIXSCAN_REPLYC) == 0) p->replyc = 5;
  ...
  if (p->probedefc > 0) {
    ...
    for (i = 0; i < p->probedefc; i++) {
      if (warts_dealias_probedef_read(&p->probedefs[i], state, table, buf, off,
                                      len) != 0)
        return -1;
    }
  }
...
static void warts_dealias_prefixscan_write(const void *data,
  ...
  for (i = 0; i < prefixscan->probedefc; i++) {
    warts_dealias_probedef_write(&prefixscan->probedefs[i],
                                 &state->probedefs[i], sf, table, buf, off,
                                 len);
  }
...
static int warts_dealias_radargun_read(scamper_dealias_t *dealias,
  ...
  if (scamper_dealias_radargun_alloc(dealias) != 0) return -1;

  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) return -1;
  if (probedefc == 0) return -1;

  rg = dealias->data;
  if (scamper_dealias_radargun_probedefs_alloc(rg, probedefc) != 0) return -1;
  ...
  for (i = 0; i < probedefc; i++) {
    if (warts_dealias_probedef_read(&rg->probedefs[i], state, table, buf, off,
                                    len) != 0)
      return -1;
  }
...
static void warts_dealias_radargun_write(const void *data,
  ...
  for (i = 0; i < rg->probedefc; i++) {
    warts_dealias_probedef_write(&rg->probedefs[i], &state->probedefs[i], sf,
                                 table, buf, off, len);
  }
...
static int warts_dealias_bump_read(scamper_dealias_t *dealias,
                                   ...
                                   warts_addrtable_t *table,
                                   scamper_dealias_probedef_t **defs,
                                   uint32_t *defc, uint8_t *buf, uint32_t *off,
                                   uint32_t len) {
  ...
  if (scamper_dealias_bump_alloc(dealias) != 0) return -1;
  ...
  if (warts_dealias_probedef_read(&bump->probedefs[0], state, table, buf, off,
                                  len) != 0 ||
      warts_dealias_probedef_read(&bump->probedefs[1], state, table, buf, off,
                                  len) != 0) {
    return -1;
  }
...
static void warts_dealias_bump_write(const void *data, const scamper_file_t *sf,
                                     warts_addrtable_t *table, uint8_t *buf,
                                     uint32_t *off, const uint32_t len,
                                     warts_dealias_data_t *state) {
  const scamper_dealias_bump_t *bump = data;
  ...
  warts_params_write(buf, off, len, state->flags, state->flags_len,
                     state->params_len, handlers, handler_cnt);
  warts_dealias_probedef_write(&bump->probedefs[0], &state->probedefs[0], sf,
                               table, buf, off, len);
  warts_dealias_probedef_write(&bump->probedefs[1], &state->probedefs[1], sf,
                               table, buf, off, len);
  return;
}
...
static int warts_dealias_ally_read(scamper_dealias_t *dealias,
  ...
  if (scamper_dealias_ally_alloc(dealias) != 0) return -1;
  ...
  if (warts_dealias_probedef_read(&ally->probedefs[0], state, table, buf, off,
                                  len) != 0 ||
      warts_dealias_probedef_read(&ally->probedefs[1], state, table, buf, off,
                                  len) != 0) {
    return -1;
  }
...
static void warts_dealias_ally_write(const void *data, const scamper_file_t *sf,
                                     warts_addrtable_t *table, uint8_t *buf,
                                     uint32_t *off, const uint32_t len,
                                     warts_dealias_data_t *state) {
  const scamper_dealias_ally_t *ally = data;
  ...
  warts_params_write(buf, off, len, state->flags, state->flags_len,
                     state->params_len, handlers, handler_cnt);
  warts_dealias_probedef_write(&ally->probedefs[0], &state->probedefs[0], sf,
                               table, buf, off, len);
  warts_dealias_probedef_write(&ally->probedefs[1], &state->probedefs[1], sf,
                               table, buf, off, len);
  return;
}
...
static int warts_dealias_mercator_read(scamper_dealias_t *dealias,
  ...
  if (scamper_dealias_mercator_alloc(dealias) != 0) return -1;
  ...
  if (warts_dealias_probedef_read(&mercator->probedef, state, table, buf, off,
                                  len) != 0) {
    return -1;
  }
...
static int warts_dealias_reply_state(const scamper_dealias_reply_t *reply,
  ...
  for (i = 0; i < sizeof(dealias_reply_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_DEALIAS_REPLY_SRC_GID) {
      ...
      if (SCAMPER_DEALIAS_REPLY_IS_ICMP(reply) == 0) continue;
      ...
      if (SCAMPER_DEALIAS_REPLY_IS_ICMP(reply)) continue;
    } else if (var->id == WARTS_DEALIAS_REPLY_TCP_FLAGS) {
      if (SCAMPER_DEALIAS_REPLY_IS_TCP(reply) == 0) continue;
    } else if (var->id == WARTS_DEALIAS_REPLY_IPID) {
      if (!SCAMPER_ADDR_TYPE_IS_IPV4(reply->src) || reply->ipid == 0) continue;
...
static int warts_dealias_probe_state(const scamper_file_t *sf,
  ...
  if (probe->replyc > 0) {
    ...
    if ((state->replies = malloc_zero(size)) == NULL) return -1;

    for (i = 0; i < probe->replyc; i++) {
      if (warts_dealias_reply_state(probe->replies[i], &state->replies[i], sf,
                                    table, len) != 0) {
        free(state->replies);
        state->replies = NULL;
        return -1;
...
static int warts_dealias_probe_read(scamper_dealias_probe_t *probe,
                                    warts_state_t *state,
                                    scamper_dealias_probedef_t *defs,
                                    uint32_t defc, warts_addrtable_t *table,
...
int scamper_file_warts_dealias_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  static int (*const read[])(scamper_dealias_t *, warts_state_t *,
      ...
      warts_dealias_mercator_read, warts_dealias_ally_read,
      warts_dealias_radargun_read, warts_dealias_prefixscan_read,
  ...
  if (read[dealias->method - 1](dealias, state, table, &defs, &defc, buf, &off,
                                hdr->len) != 0)
    goto err;

  if (dealias->probec == 0) goto done;
  ...
  for (i = 0; i < dealias->probec; i++) {
    ...
    if (warts_dealias_probe_read(probe, state, defs, defc, table, buf, &off,
                                 hdr->len) != 0) {
      goto err;
    }
  }
...
static void warts_dealias_probes_free(warts_dealias_probe_t *probes,
  ...
  if (probes != NULL) {
    for (i = 0; i < cnt; i++)
      if (probes[i].replies != NULL) free(probes[i].replies);
    free(probes);
  }
...
int scamper_file_warts_dealias_write(const scamper_file_t *sf,
  ...
  static int (*const state[])(const scamper_file_t *, const void *,
                              ...
                              uint32_t *) = {
      warts_dealias_mercator_state, warts_dealias_ally_state,
      warts_dealias_radargun_state, warts_dealias_prefixscan_state,
  ...
  static void (*const write[])(const void *, const scamper_file_t *,
      ...
      warts_dealias_mercator_write, warts_dealias_ally_write,
      warts_dealias_radargun_write, warts_dealias_prefixscan_write,
  ...
  if ((buf = malloc_zero(len)) == NULL) goto err;
  insert_wartshdr(buf, &off, len, SCAMPER_FILE_OBJ_DEALIAS);

  if (warts_dealias_params_write(dealias, sf, buf, &off, len, flags, flags_len,
                                 params_len) != 0) {
    goto err;
  }
