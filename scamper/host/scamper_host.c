...
scamper_host_rr_soa_t *scamper_host_rr_soa_alloc(const char *mn,
  ...
  if ((soa = malloc_zero(sizeof(scamper_host_rr_soa_t))) == NULL ||
      (soa->mname = strdup(mn)) == NULL || (soa->rname = strdup(rn)) == NULL) {
...
scamper_host_rr_mx_t *scamper_host_rr_mx_alloc(uint16_t pref,
                                               const char *exch) {
...
void scamper_host_query_free(scamper_host_query_t *query) {
  ...
  if (query->an != NULL) {
    for (r = 0; r < query->ancount; r++) scamper_host_rr_free(query->an[r]);
    free(query->an);
  }
  if (query->ns != NULL) {
    for (r = 0; r < query->nscount; r++) scamper_host_rr_free(query->ns[r]);
    free(query->ns);
  }
  if (query->ar != NULL) {
    for (r = 0; r < query->arcount; r++) scamper_host_rr_free(query->ar[r]);
    free(query->ar);
  }
...
int scamper_host_queries_alloc(scamper_host_t *host, int n) {
  ...
  if ((host->queries = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_host_query_rr_alloc(scamper_host_query_t *query) {
  ...
  if (query->ancount > 0) {
    ...
    if ((query->an = malloc_zero(len)) == NULL) return -1;
  }
  if (query->nscount > 0) {
    len = query->nscount * sizeof(scamper_host_rr_t *);
    if ((query->ns = malloc_zero(len)) == NULL) return -1;
  }
  if (query->arcount > 0) {
    len = query->arcount * sizeof(scamper_host_rr_t *);
    if ((query->ar = malloc_zero(len)) == NULL) return -1;
  }
  return 0;
}
