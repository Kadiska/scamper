...
const char *scamper_do_host_usage(void) {
  return "host [-r] [-R number] [-s server] [-t type] [-U userid] [-W wait] "
         "name\n";
}
...
static host_id_t *host_id_get(uint16_t id) {
  ...
  if ((hid = host_id_find(id)) != NULL) return hid;
...
static int host_query_add(uint16_t id, scamper_task_t *task) {
  ...
  if ((hid = host_id_get(id)) == NULL) goto err;
...
static void host_state_free(host_state_t *state) {
  ...
  if (state->qname != NULL) free(state->qname);
  if (state->cbs != NULL) dlist_free(state->cbs);
  if (state->pids != NULL) {
...
static host_state_t *host_state_alloc(scamper_task_t *task) {
  ...
  if (host->qtype == SCAMPER_HOST_TYPE_PTR) {
    ...
    if (SCAMPER_ADDR_TYPE_IS_IPV4(sa)) {
      ...
      snprintf(qname, sizeof(qname), "%u.%u.%u.%u.in-addr.arpa", u32 & 0xff,
      ...
      for (i = 3; i >= 0; i--) {
        ...
        string_concat(qname, sizeof(qname), &off, "%x.%x.%x.%x.%x.%x.%x.%x.",
                      u32 & 0xf, (u32 >> 4) & 0xf, (u32 >> 8) & 0xf,
                      (u32 >> 12) & 0xf, (u32 >> 16) & 0xf, (u32 >> 20) & 0xf,
                      (u32 >> 24) & 0xf, (u32 >> 28) & 0xf);
      }
#else
      for (i = 7; i >= 0; i--) {
        u16 = ntohs(in6->u.Word[i]);
        string_concat(qname, sizeof(qname), &off, "%x.%x.%x.%x.", u16 & 0xf,
                      (u16 >> 4) & 0xf, (u16 >> 8) & 0xf, (u16 >> 12) & 0xf);
      }
#endif
...
static int extract_name(char *name, size_t namelen, uint8_t *pktbuf,
  ...
  for (;;) {
    ...
    if (off + 1 + u8 >= pktlen) return -1;
    if (i + u8 >= namelen) return -1;
    memcpy(name + i, pktbuf + off + 1, u8);
    off += u8 + 1;
    i += u8;
    name[i++] = '.';
    if (ptr_used == 0) rc += u8 + 1;
  }
...
static int extract_soa(scamper_host_rr_t *rr, uint8_t *pktbuf, size_t pktlen,
  ...
  if (pktlen - off < 20) return -1;
  serial = bytes_ntohl(pktbuf + off);
  off += 4;
  refresh = bytes_ntohl(pktbuf + off);
  off += 4;
  retry = bytes_ntohl(pktbuf + off);
  off += 4;
  expire = bytes_ntohl(pktbuf + off);
  off += 4;
  minimum = bytes_ntohl(pktbuf + off);
  off += 4;
  assert(off <= pktlen);

  if ((soa = scamper_host_rr_soa_alloc(mname, rname)) == NULL) return -1;
  soa->serial = serial;
  soa->refresh = refresh;
  soa->retry = retry;
  soa->expire = expire;
...
static int extract_mx(scamper_host_rr_t *rr, uint8_t *pktbuf, size_t pktlen,
  ...
  if (pktlen - off < 2) return -1;

  preference = bytes_ntohs(pktbuf + off);
  off += 2;
  if (extract_name(exchange, sizeof(exchange), pktbuf, pktlen, off) <= 0)
    return -1;
  if ((mx = scamper_host_rr_mx_alloc(preference, exchange)) == NULL) return -1;
  rr->un.mx = mx;
...
static void do_host_read(const int fd, void *param) {
  ...
  if ((len = recv(fd, pktbuf, pktbuf_len, 0)) < 0) return;
  if (len < 12) return;
  ...
  if ((flags & 0x8000) == 0 || qdcount != 1) return;
  ...
  off += i;
  qtype = bytes_ntohs(pktbuf + off);
  off += 2;
  qclass = bytes_ntohs(pktbuf + off);
  off += 2;
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < x; j++) {
      ...
      type = bytes_ntohs(pktbuf + off);
      off += 2;
      class = bytes_ntohs(pktbuf + off);
      off += 2;
      ttl = bytes_ntohl(pktbuf + off);
      off += 4;
      rdlength = bytes_ntohs(pktbuf + off);
      off += 2;
      ...
      if (class == 1 &&
          (type == SCAMPER_HOST_TYPE_NS || type == SCAMPER_HOST_TYPE_CNAME ||
           type == SCAMPER_HOST_TYPE_PTR)) {
        if (extract_name(str, sizeof(str), pktbuf, len, off) <= 0) goto err;
        if ((rr->un.str = strdup(str)) == NULL) goto err;
      } else if (class == 1 && type == SCAMPER_HOST_TYPE_A) {
        if (rdlength != 4) goto err;
        memcpy(&in4, pktbuf + off, rdlength);
        inet_ntop(AF_INET, &in4, str, sizeof(str));
        if ((rr->un.addr = scamper_addr_alloc_ipv4(&in4)) == NULL) goto err;
      } else if (class == 1 && type == SCAMPER_HOST_TYPE_AAAA) {
        if (rdlength != 16) goto err;
        memcpy(&in6, pktbuf + off, rdlength);
        inet_ntop(AF_INET6, &in6, str, sizeof(str));
        if ((rr->un.addr = scamper_addr_alloc_ipv6(&in6)) == NULL) goto err;
      } else if (class == 1 && type == SCAMPER_HOST_TYPE_SOA) {
        if (extract_soa(rr, pktbuf, len, off) != 0) goto err;
      } else if (class == 1 && type == SCAMPER_HOST_TYPE_MX) {
        if (extract_mx(rr, pktbuf, len, off) != 0) goto err;
      }
    ...
    x = 0;
    while ((rr = slist_head_pop(rr_list)) != NULL) rrs[x++] = rr;
  }
...
static void do_host_probe(scamper_task_t *task) {
  ...
  if (state == NULL && (state = host_state_alloc(task)) == NULL) goto err;

  if (host->queries == NULL) {
    if (scamper_host_queries_alloc(host, host->retries + 1) != 0) goto err;
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(host->dst)) {
    if (dns4_fd == NULL) {
      ...
      if ((dns4_fd = scamper_fd_private(fd, NULL, do_host_read, NULL)) ==
          NULL) {
    ...
    if (dns4_sq == NULL) {
      if ((dns4_sq = scamper_queue_event(&tv, host_fd_close, dns4_fd)) ==
          NULL) {
    ...
    if (dns6_fd == NULL) {
      ...
      if ((dns6_fd = scamper_fd_private(fd, NULL, do_host_read, NULL)) ==
          NULL) {
    ...
    if (dns6_sq == NULL) {
      if ((dns6_sq = scamper_queue_event(&tv, host_fd_close, dns6_fd)) ==
          NULL) {
  ...
  id = dns_id++;
  if (host_query_add(id, task) != 0) goto err;
  ...
  if (host->qtype == SCAMPER_HOST_TYPE_A ||
    ...
    pktbuf[off++] = 0;
    bytes_htons(pktbuf + off, host->qtype);
    off += 2;
    bytes_htons(pktbuf + off, host->qclass);
    off += 2;
  } else
    return;
...
static int host_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case HOST_OPT_RETRIES:
      if (string_tolong(param, &tmp) != 0 || tmp < 0 || tmp > 3) return -1;
      break;

    case HOST_OPT_SERVER:
      if ((addr = scamper_addr_resolve(AF_INET, param)) == NULL) return -1;
    ...
    case HOST_OPT_WAIT:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 5) return -1;
      tmp *= 1000;
      break;
  }
...
scamper_task_t *scamper_do_host_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(host, &host_funcs)) == NULL) goto err;
  ...
  if ((host->src = scamper_getsrc(host->dst, 0)) == NULL) goto err;
...
void *scamper_do_host_alloc(char *str) {
  ...
  if (qtype == 0) {
    ...
    if ((name_addr = scamper_addr_resolve(AF_UNSPEC, name)) != NULL) goto err;
 ...
 *
 */
static scamper_host_do_t *scamper_host_do_add(scamper_task_t *task,
                                              void *param) {
...
static scamper_host_do_t *scamper_do_host_do_host(const char *qname,
  ...
  if ((hostdo = scamper_host_do_add(task, param)) == NULL) goto err;
...
scamper_host_do_t *scamper_do_host_do_a(const char *qname, void *param,
  ...
  if ((task = scamper_task_find(&sig)) != NULL) {
    if ((hostdo = scamper_host_do_add(task, param)) == NULL) return NULL;
    hostdo->un.a_cb = cb;
    return hostdo;
  }

  hostdo = scamper_do_host_do_host(qname, SCAMPER_HOST_TYPE_A, param);
  if (hostdo == NULL) return NULL;
  hostdo->un.a_cb = cb;
  return hostdo;
}
...
static void do_host_a_cb(scamper_host_t *host, host_state_t *state) {
  ...
  for (i = 0; i < host->qcount; i++) {
    for (j = 0; j < host->queries[i]->ancount; j++) {
      rr = host->queries[i]->an[j];
      if (rr->type == SCAMPER_HOST_TYPE_A) ac++;
    }
  }
  ...
  if (ac > 0 && (a = malloc(sizeof(scamper_addr_t *) * ac)) != NULL) {
    ...
    for (i = 0; i < host->qcount; i++) {
      for (j = 0; j < host->queries[i]->ancount; j++) {
        rr = host->queries[i]->an[j];
        if (rr->type == SCAMPER_HOST_TYPE_A) a[x++] = rr->un.addr;
      }
    }
    assert(x == ac);
  } else {
...
static int etc_resolv_line(char *line, void *param) {
  ...
  x = 10;
  if (strncasecmp(line, "nameserver", x) != 0) return 0;
  while (isspace(line[x]) != 0) x++;
  if (x == 10 || line[x] == '\0') return 0;
  ...
  if (strcasecmp(line + x, "fe80::1") == 0) return 0;
...
static int etc_resolv(void) {
  ...
  if (fd_lines(fd, etc_resolv_line, NULL) != 0) goto err;
  close(fd);
  fd = -1;
  if (default_ns == NULL) {
...
int scamper_do_host_setns(const char *nsip) {
  ...
  if (default_ns != NULL) scamper_addr_free(default_ns);
  default_ns = sa;

  return 0;
}

const scamper_addr_t *scamper_do_host_getns(void) { return default_ns; }
...
int scamper_do_host_init() {
  ...
  if ((nsip = scamper_option_nameserver_get()) != NULL) {
    if (scamper_do_host_setns(nsip) != 0) return -1;
