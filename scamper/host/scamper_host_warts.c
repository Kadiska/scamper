...
static const warts_var_t host_vars[] = {
    {WARTS_HOST_LIST, 4, -1},   {WARTS_HOST_CYCLE, 4, -1},
    {WARTS_HOST_USERID, 4, -1}, {WARTS_HOST_SRC, -1, -1},
    {WARTS_HOST_DST, -1, -1},   {WARTS_HOST_START, 8, -1},
    {WARTS_HOST_FLAGS, 2, -1},  {WARTS_HOST_WAIT, 2, -1},
    {WARTS_HOST_STOP, 1, -1},   {WARTS_HOST_RETRIES, 1, -1},
    {WARTS_HOST_QTYPE, 2, -1},  {WARTS_HOST_QCLASS, 2, -1},
    {WARTS_HOST_QNAME, -1, -1}, {WARTS_HOST_QCOUNT, 1, -1},
};
...
static const warts_var_t host_query_vars[] = {
    {WARTS_HOST_QUERY_TX, 8, -1},      {WARTS_HOST_QUERY_RX, 8, -1},
    {WARTS_HOST_QUERY_ID, 2, -1},      {WARTS_HOST_QUERY_ANCOUNT, 2, -1},
    {WARTS_HOST_QUERY_NSCOUNT, 2, -1}, {WARTS_HOST_QUERY_ARCOUNT, 2, -1},
};
...
static const warts_var_t host_rr_vars[] = {
    {WARTS_HOST_RR_CLASS, 2, -1}, {WARTS_HOST_RR_TYPE, 2, -1},
    {WARTS_HOST_RR_NAME, -1, -1}, {WARTS_HOST_RR_TTL, 4, -1},
...
static const warts_var_t host_rr_soa_vars[] = {
    {WARTS_HOST_RR_SOA_MNAME, -1, -1},  {WARTS_HOST_RR_SOA_RNAME, -1, -1},
    {WARTS_HOST_RR_SOA_SERIAL, 4, -1},  {WARTS_HOST_RR_SOA_REFRESH, 4, -1},
    {WARTS_HOST_RR_SOA_RETRY, 4, -1},   {WARTS_HOST_RR_SOA_EXPIRE, 4, -1},
...
static int warts_host_rr_mx_read(void **data, const uint8_t *buf, uint32_t *off,
  ...
  const int handler_cnt = sizeof(handlers) / sizeof(warts_param_reader_t);
  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) goto err;
  if (exchange == NULL) goto err;
  if ((mx = scamper_host_rr_mx_alloc(preference, exchange)) == NULL) goto err;
  free(exchange);
  *data = mx;
  return 0;
...
static int warts_host_rr_soa_read(void **data, const uint8_t *buf,
  ...
  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) goto done;
  if (mname == NULL || rname == NULL) goto done;
  if ((soa = scamper_host_rr_soa_alloc(mname, rname)) == NULL) goto done;
  soa->serial = serial;
  soa->refresh = refresh;
  soa->retry = retry;
  soa->expire = expire;
...
static int extract_rrdata(const uint8_t *buf, uint32_t *off, uint32_t len,
  ...
  if (extract_uint16(buf, off, len, &type, NULL) != 0) return -1;

  rrdata->type = type;
  if (type == SCAMPER_HOST_RR_DATA_TYPE_ADDR) {
    if (extract_addr(buf, off, len, (scamper_addr_t **)&rrdata->data, table) !=
        0)
      return -1;
    return 0;
    ...
    if (warts_host_rr_soa_read(&rrdata->data, buf, off, len) != 0) return -1;
    return 0;
  } else if (type == SCAMPER_HOST_RR_DATA_TYPE_MX) {
    if (warts_host_rr_mx_read(&rrdata->data, buf, off, len) != 0) return -1;
    return 0;
  }
  return -1;
}
...
static int warts_host_params_read(scamper_host_t *host,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (host->dst == NULL) return -1;

  return 0;
}

static int warts_host_params_write(
    const scamper_host_t *host, const scamper_file_t *sf,
...
int scamper_file_warts_host_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  if ((host = scamper_host_alloc()) == NULL) goto err;
  ...
  if (host->qcount > 0) {
    if (scamper_host_queries_alloc(host, host->qcount) != 0) goto err;
    for (i = 0; i < host->qcount; i++) {
      if ((host->queries[i] = query = scamper_host_query_alloc()) == NULL)
        goto err;
      if (warts_host_query_read(query, buf, &off, hdr->len) != 0) goto err;
      if (scamper_host_query_rr_alloc(query) != 0) goto err;
...
int scamper_file_warts_host_write(const scamper_file_t *sf,
  ...
  if (host->qcount > 0) {
    ...
    if (rrc > 0) {
      ...
      if ((rr_state = (warts_host_rr_t *)malloc_zero(size)) == NULL) goto err;
    }
  ...
  if (warts_host_params_write(host, sf, table, buf, &off, len, flags, flags_len,
                              params_len) != 0) {
    goto err;
  }
  ...
  if (host->qcount > 0) {
    ...
    for (i = 0; i < host->qcount; i++) {
      ...
      for (j = 0; j < query->ancount; j++)
        warts_host_rr_write(query->an[j], buf, &off, len, &rr_state[r++],
                            table);
      for (j = 0; j < query->nscount; j++)
        warts_host_rr_write(query->ns[j], buf, &off, len, &rr_state[r++],
                            table);
      for (j = 0; j < query->arcount; j++)
        warts_host_rr_write(query->ar[j], buf, &off, len, &rr_state[r++],
                            table);
    }
    free(query_state);
    query_state = NULL;
