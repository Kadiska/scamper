...
int scamper_neighbourdisc_replies_alloc(scamper_neighbourdisc_probe_t *probe,
  ...
  if ((probe->rxs = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_neighbourdisc_probe_add(scamper_neighbourdisc_t *nd,
  ...
  if (realloc_wrap((void **)&nd->probes, len) != 0) return -1;
  nd->probes[nd->probec++] = probe;
  return 0;
}

int scamper_neighbourdisc_probes_alloc(scamper_neighbourdisc_t *nd,
                                       uint16_t c) {
  size_t len = sizeof(scamper_neighbourdisc_probe_t *) * c;
  if ((nd->probes = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_neighbourdisc_ifname_set(scamper_neighbourdisc_t *nd,
                                     char *ifname) {
  if (nd->ifname != NULL) free(nd->ifname);

  if ((nd->ifname = strdup(ifname)) == NULL) return -1;
