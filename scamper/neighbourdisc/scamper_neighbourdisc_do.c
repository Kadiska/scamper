...
const char *scamper_do_neighbourdisc_usage(void) {
  return "neighbourdisc [-FQ] [-i if] [-o replyc] [-q attempts] [-S srcaddr] "
         "[-w wait]\n";
}
...
static void do_nd_probe_arp(scamper_task_t *task) {
  ...
  bytes_htons(pktbuf + off, ETHERTYPE_ARP);
  off += 2;
  bytes_htons(pktbuf + off, 0x0001);
  off += 2;
  bytes_htons(pktbuf + off, ETHERTYPE_IP);
  off += 2;
  pktbuf[off++] = 6;
  pktbuf[off++] = 4;
  bytes_htons(pktbuf + off, 0x0001);
...
static void do_nd_probe_nsol(scamper_task_t *task) {
  ...
  mem_concat(pktbuf, nd->src_mac->addr, 6, &off, pktbuf_len);
  bytes_htons(pktbuf + off, ETHERTYPE_IPV6);
  off += 2;

  /* IPv6 header: 40 bytes */
  ip6 = (struct ip6_hdr *)(pktbuf + off);
  ...
  icmp_off = off;
  icmp6 = (struct icmp6_hdr *)(pktbuf + off);
  ...
  for (i = ntohs(ip6->ip6_plen); i > 1; i -= 2) sum += *w++;
  if (i != 0) sum += ((uint8_t *)w)[0];
  sum = (sum >> 16) + (sum & 0xffff);
  sum += (sum >> 16);
  if ((u16 = ~sum) == 0) u16 = 0xffff;
...
static void do_nd_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_DL_IS_ARP(dl)) {
    ...
    while (opt_off + 8 <= dl->dl_icmp6_nd_opts_len) {
      if (opt[0] == 2 && opt[1] == 1 &&
          dl->dl_icmp6_nd_opts_len - opt_off >= 8) {
        mac = opt + 2;
        break;
      }
      if (opt[1] == 0) return;
  ...
  if (nd->dst_mac == NULL) nd->dst_mac = scamper_addr_use(reply->mac);
...
static void do_nd_probe(scamper_task_t *task) {
  ...
  if (state == NULL) {
    if (nd_state_alloc(task) != 0) goto err;
...
static void do_nd_free(scamper_task_t *task) {
  ...
  if (state != NULL && state->cbs != NULL) {
    ...
    while ((nddo = dlist_head_pop(state->cbs)) != NULL) {
      nddo->node = NULL;
      if (ip != NULL) nddo->cb(nddo->param, ip, mac);
      free(nddo);
    }
...
static int nd_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case ND_OPT_REPLYC:
      if (string_tolong(param, &tmp) != 0 || tmp < 0 || tmp > 65535) return -1;
      break;
...
scamper_task_t *scamper_do_neighbourdisc_alloctask(void *data,
  ...
  if ((task = scamper_task_alloc(nd, &nd_funcs)) == NULL) goto err;
...
void *scamper_do_neighbourdisc_alloc(char *str) {
  ...
  if (ifname == NULL) goto err;

  if ((nd = scamper_neighbourdisc_alloc()) == NULL) goto err;

  if ((nd->dst_ip = scamper_addrcache_resolve(addrcache, AF_UNSPEC, dst)) ==
      NULL)
    goto err;
  ...
  if (src != NULL) {
    if (SCAMPER_ADDR_TYPE_IS_IPV6(nd->dst_ip) == 0) goto err;
    nd->src_ip = scamper_addrcache_resolve(addrcache, AF_INET6, src);
    if (nd->src_ip == NULL) goto err;
  }

  if (scamper_neighbourdisc_ifname_set(nd, ifname) != 0) goto err;
...
scamper_neighbourdisc_do_t *scamper_do_neighbourdisc_do(
  ...
  if (scamper_if_getifname(ifname, sizeof(ifname), ifindex) != 0) goto err;
  ...
  if ((task = scamper_do_neighbourdisc_alloctask(nd, NULL, NULL)) == NULL)
    goto err;
  nd = NULL;
  if (scamper_task_sig_install(task) != 0) goto err;
  if (nd_state_alloc(task) != 0) goto err;
  do_nd_probe(task);
  if (scamper_task_queue_isdone(task)) goto err;
  return scamper_neighbourdisc_do_add(task, param, cb);
