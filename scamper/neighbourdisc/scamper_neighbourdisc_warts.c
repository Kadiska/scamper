...
static int warts_neighbourdisc_probe_state(const scamper_file_t *sf,
  ...
  for (i = 0; i < sizeof(neighbourdisc_probe_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_NEIGHBOURDISC_PROBE_RXC && probe->rxc == 0) continue;
  ...
  if (probe->rxc > 0) {
    ...
    if ((state->rxs = malloc_zero(size)) == NULL) return -1;

    for (i = 0; i < probe->rxc; i++) {
      if (warts_neighbourdisc_reply_state(probe->rxs[i], &state->rxs[i], table,
                                          len) != 0) {
        free(state->rxs);
        state->rxs = NULL;
        return -1;
...
static int warts_neighbourdisc_probe_read(scamper_neighbourdisc_probe_t *pr,
  ...
  if (pr->rxc == 0) return 0;

  if (scamper_neighbourdisc_replies_alloc(pr, pr->rxc) != 0) return -1;

  for (i = 0; i < pr->rxc; i++) {
    if ((reply = scamper_neighbourdisc_reply_alloc()) == NULL) return -1;
    pr->rxs[i] = reply;
...
static int warts_neighbourdisc_params_write(
    const scamper_neighbourdisc_t *nd, const scamper_file_t *sf,
    warts_addrtable_t *table, uint8_t *buf, uint32_t *off, const uint32_t len,
...
static int warts_neighbourdisc_params_read(scamper_neighbourdisc_t *nd,
  ...
  if (nd->src_mac == NULL || nd->dst_ip == NULL) return -1;
...
int scamper_file_warts_neighbourdisc_write(const scamper_file_t *sf,
  ...
  if (warts_neighbourdisc_params_write(nd, sf, table, buf, &off, len, flags,
                                       flags_len, params_len) != 0) {
    goto err;
  }
...
int scamper_file_warts_neighbourdisc_read(scamper_file_t *sf,
  ...
  if (warts_neighbourdisc_params_read(nd, table, state, buf, &off, hdr->len) !=
      0) {
    goto err;
  }

  if (nd->probec == 0) goto done;
  ...
  for (i = 0; i < nd->probec; i++) {
    ...
    if (warts_neighbourdisc_probe_read(probe, state, table, buf, &off,
                                       hdr->len) != 0) {
      goto err;
    }
  }
