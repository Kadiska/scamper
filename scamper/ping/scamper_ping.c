 ...
 *
 * $Id: scamper_ping.c,v 1.38 2021/08/28 21:36:31 mjl Exp $
 *
...
char *scamper_ping_method2str(const scamper_ping_t *ping, char *buf,
                              size_t len) {
  static char *m[] = {
      "icmp-echo", "tcp-ack", "tcp-ack-sport", "udp",     "udp-dport",
      "icmp-time", "tcp-syn", "tcp-synack",    "tcp-rst",
  };
...
void scamper_ping_v4ts_free(scamper_ping_v4ts_t *ts) {
  ...
  if (ts->ips != NULL) {
    for (i = 0; i < ts->ipc; i++)
      if (ts->ips[i] != NULL) scamper_addr_free(ts->ips[i]);
    free(ts->ips);
  }
...
scamper_ping_v4ts_t *scamper_ping_v4ts_alloc(uint8_t ipc) {
  ...
  if ((ts = malloc_zero(sizeof(scamper_ping_v4ts_t))) == NULL) goto err;
  ts->ipc = ipc;

  if ((ts->ips = malloc_zero(sizeof(scamper_addr_t *) * ipc)) == NULL) goto err;
...
int scamper_ping_replies_alloc(scamper_ping_t *ping, int count) {
  ...
  if ((ping->ping_replies = (scamper_ping_reply_t **)malloc_zero(size)) !=
      NULL) {
    return 0;
  }
...
scamper_ping_reply_v4ts_t *scamper_ping_reply_v4ts_alloc(uint8_t tsc, int ip) {
  ...
  if ((ts = malloc_zero(sizeof(scamper_ping_reply_v4ts_t))) == NULL) goto err;
  ts->tsc = tsc;

  if (tsc > 0) {
    if ((ts->tss = malloc_zero(sizeof(uint32_t) * tsc)) == NULL) goto err;
    if (ip != 0 &&
...
void scamper_ping_reply_v4rr_free(scamper_ping_reply_v4rr_t *rr) {
  ...
  if (rr->rr != NULL) {
    for (i = 0; i < rr->rrc; i++)
      if (rr->rr[i] != NULL) scamper_addr_free(rr->rr[i]);
    free(rr->rr);
  }
...
scamper_ping_reply_v4rr_t *scamper_ping_reply_v4rr_alloc(uint8_t rrc) {
  ...
  if ((rr = malloc_zero(sizeof(scamper_ping_reply_v4rr_t))) == NULL) goto err;
  rr->rrc = rrc;

  if ((rr->rr = malloc_zero(sizeof(scamper_addr_t *) * rrc)) == NULL) goto err;
...
void scamper_ping_reply_free(scamper_ping_reply_t *reply) {
  ...
  if (reply->addr != NULL) scamper_addr_free(reply->addr);

  if (reply->v4rr != NULL) scamper_ping_reply_v4rr_free(reply->v4rr);

  if (reply->v4ts != NULL) scamper_ping_reply_v4ts_free(reply->v4ts);

  if (reply->tsreply != NULL) scamper_ping_reply_tsreply_free(reply->tsreply);
