...
static void do_ping_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (dl->dl_ip_off != 0) return;

  if (SCAMPER_DL_IS_ICMP(dl)) {
    if ((ping->flags & SCAMPER_PING_FLAG_DL) == 0) return;
    ...
    if (SCAMPER_DL_IS_ICMP_ECHO_REPLY(dl) ||
      ...
      if ((SCAMPER_PING_METHOD_IS_ICMP_ECHO(ping) &&
        ...
        return;
      if (dl->dl_icmp_id != ping->probe_sport) return;
      seq = dl->dl_icmp_seq;
      if (seq < ping->probe_dport) seq = seq + 0x10000;
      ...
      if (SCAMPER_PING_METHOD_IS_ICMP(ping)) {
        if ((SCAMPER_PING_METHOD_IS_ICMP_ECHO(ping) &&
          ...
          return;
        if (dl->dl_icmp_icmp_id != ping->probe_sport) return;
        ...
        if (ping->probe_method == SCAMPER_PING_METHOD_TCP_ACK) {
          if (dl->dl_icmp_tcp_sport != ping->probe_sport) return;
        ...
        if (ping->probe_method == SCAMPER_PING_METHOD_UDP) {
          if (dl->dl_icmp_udp_dport != ping->probe_dport) return;
          if (ping->dst->type == SCAMPER_ADDR_TYPE_IPV4)
            seq = match_ipid(task, dl->dl_icmp_ip_id);
          else
            seq = state->seq - 1;
    ...
    if (ping->probe_method == SCAMPER_PING_METHOD_TCP_ACK ||
      ...
      if (dl->dl_tcp_dport != ping->probe_sport) return;
      ...
      seq = dl->dl_tcp_dport;
      if (dl->dl_tcp_dport < ping->probe_sport) seq = seq + 0x10000;
      seq = seq - ping->probe_sport;
    }
  } else if (SCAMPER_DL_IS_UDP(dl)) {
    if (SCAMPER_PING_METHOD_IS_UDP(ping) == 0) return;
  ...
  if ((reply->addr = ping_addr(ping, state, dl->dl_ip_src)) == NULL) goto err;
...
static void do_ping_handle_icmp(scamper_task_t *task, scamper_icmp_resp_t *ir) {
  ...
  if (SCAMPER_ICMP_RESP_IS_ECHO_REPLY(ir) ||
      SCAMPER_ICMP_RESP_IS_TIME_REPLY(ir)) {
    ...
    if (((SCAMPER_PING_METHOD_IS_ICMP_ECHO(ping) &&
      ...
      return;
    if (ir->ir_icmp_id != ping->probe_sport) return;
    ...
    if (ir->ir_inner_ip_off != 0) return;

    if (SCAMPER_PING_METHOD_IS_ICMP(ping)) {
      if (ir->ir_inner_icmp_id != ping->probe_sport) return;
      ...
      if (ping->probe_method == SCAMPER_PING_METHOD_TCP_ACK) {
        if (ir->ir_inner_tcp_sport != ping->probe_sport) return;
        if (ping->dst->type == SCAMPER_ADDR_TYPE_IPV4)
      ...
      if (ping->probe_method == SCAMPER_PING_METHOD_UDP) {
        if (ir->ir_inner_udp_dport != ping->probe_dport) return;
        if (ping->dst->type == SCAMPER_ADDR_TYPE_IPV4)
          seq = match_ipid(task, ir->ir_inner_ip_id);
        else
          seq = state->seq - 1;
  ...
  if ((reply->addr = ping_addr(ping, state, addr.addr)) == NULL) goto err;
  ...
  if (SCAMPER_ICMP_RESP_IS_TIME_REPLY(ir)) {
    if ((reply->tsreply = scamper_ping_reply_tsreply_alloc()) == NULL) goto err;
  ...
  if (ir->ir_af == AF_INET) {
    ...
    if (rrs != NULL && rrc > 0) {
      if ((v4rr = scamper_ping_reply_v4rr_alloc(rrc)) == NULL) goto err;
      reply->v4rr = v4rr;

      for (i = 0; i < rrc; i++)
        if ((v4rr->rr[i] = scamper_addr_alloc_ipv4(&rrs[i])) == NULL) goto err;
    }
   ...
   * the replies counter we keep state with
   */
  if (ping->ping_replies[seq] == NULL) state->replies++;
...
static int ping_state_payload(scamper_ping_t *ping, ping_state_t *state) {
  ...
  if (SCAMPER_PING_METHOD_IS_ICMP(ping)) {
    ...
    if (SCAMPER_PING_METHOD_IS_ICMP_TIME(ping)) state->payload_len -= 12;
  } else if (SCAMPER_PING_METHOD_IS_TCP(ping))
  ...
  if (state->payload_len == 0) return 0;

  if ((state->payload = malloc_zero(state->payload_len)) == NULL) return -1;
  ...
  if ((ping->flags & SCAMPER_PING_FLAG_SPOOF) != 0 &&
    ...
    if ((src = scamper_getsrc(ping->dst, 0)) == NULL) return -1;
...
static void ping_state_free(ping_state_t *state) {
  ...
  if (state->probes != NULL) {
    for (i = 0; i < state->seq; i++)
      if (state->probes[i] != NULL) free(state->probes[i]);
    free(state->probes);
  }

  if (state->payload != NULL) free(state->payload);

  if (state->last_addr != NULL) scamper_addr_free(state->last_addr);
...
static int ping_state_alloc(scamper_task_t *task) {
  ...
  if (ping_state_payload(ping, state) != 0) goto err;
...
static void do_ping_probe(scamper_task_t *task) {
  ...
  if (state == NULL) {
    if (ping_state_alloc(task) != 0) goto err;
  ...
  if (state->mode == PING_MODE_PROBE) {
    ...
    if (ping->dst->type == SCAMPER_ADDR_TYPE_IPV4) probe.pr_ip_off = IP_DF;
    ...
    if (SCAMPER_PING_METHOD_IS_ICMP(ping)) {
      ...
      if ((ping->flags & SCAMPER_PING_FLAG_ICMPSUM) != 0) {
        ...
        if ((ping->flags & SCAMPER_PING_FLAG_SPOOF) != 0) i += 4;
     ...
     * record
     */
    if ((pp = malloc_zero(sizeof(ping_probe_t))) == NULL) goto err;
...
static int validate_probe_wait(char *s_str, long long *out) {
  ...
  if (string_tollong(s_str, &s) == -1 || s < 0 ||
      s > SCAMPER_DO_PING_PROBEWAIT_MAX)
    return -1;
  ...
  *out = (s * 1000000) + us;
  if (*out < SCAMPER_DO_PING_PROBEWAIT_US_MIN) return -1;
...
static int ping_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case PING_OPT_PAYLOAD:
      ...
      if (i == 0 || (i % 2) != 0) goto err;
      tmp = i / 2;
      if (tmp > 1000) goto err;
      break;
    ...
    case PING_OPT_PROBEICMPSUM:
      if (string_tollong(param, &tmp) == -1 || tmp < 0 || tmp > 65535) goto err;
      break;
    ...
    case PING_OPT_PROBEWAIT:
      if (validate_probe_wait(param, &tmp) != 0) goto err;
      break;
    ...
    case PING_OPT_USERID:
      if (string_tollong(param, &tmp) != 0 || tmp < 0) goto err;
      break;
...
static int ping_tsopt(scamper_ping_t *ping, uint32_t *flags, char *tsopt) {
  ...
  if (strncasecmp(tsopt, "tsprespec", 9) == 0 && *ptr == '=') {
    ...
    if ((ts = scamper_ping_v4ts_alloc(i)) == NULL) return -1;
...
void *scamper_do_ping_alloc(char *str) {
  ...
  if (pattern_len != 0 && payload_len != 0) goto err;
   ...
   */
  if (tsopt != NULL) {
    if (ping->dst->type != SCAMPER_ADDR_TYPE_IPV4) goto err;

    if ((flags & SCAMPER_PING_FLAG_V4RR) != 0) goto err;

    if (ping_tsopt(ping, &flags, tsopt) != 0) goto err;
  }
  ...
  if (SCAMPER_PING_METHOD_IS_ICMP(ping)) {
    cmps += 8;
    if (flags & SCAMPER_PING_FLAG_ICMPSUM) cmps += 2;
    if (SCAMPER_PING_METHOD_IS_ICMP_TIME(ping)) cmps += 12;
    if (payload_len != 0) cmps += payload_len;
    ...
    if (probe_size == 0) {
      probe_size = cmps;
      if (payload_len == 0) probe_size += 12;
    }
  } else
    goto err;

  if (probe_size < cmps) goto err;

  /* TBT method is about getting IPv6 fragments */
  if (flags & SCAMPER_PING_FLAG_TBT) {
    if (ping->dst->type == SCAMPER_ADDR_TYPE_IPV4) goto err;
    if (reply_pmtu == 0 || probe_size <= reply_pmtu) goto err;
  }
  ...
  if (src != NULL && (ping->src = scamper_addr_resolve(af, src)) == NULL)
    goto err;

  if (rtr != NULL && (ping->rtr = scamper_addr_resolve(af, rtr)) == NULL)
    goto err;

  /* copy in the data bytes, if any */
  if (pattern_len != 0) {
    if (scamper_ping_setdata(ping, pattern, pattern_len) != 0) goto err;
  } else if (payload_len != 0) {
    if (scamper_ping_setdata(ping, payload, payload_len) != 0) goto err;
  }
  ...
  if (SCAMPER_PING_METHOD_IS_TCP(ping)) {
    if (A == 0 && random_u32(&probe_tcpack) != 0) goto err;
    ...
    if (ping->probe_method == SCAMPER_PING_METHOD_TCP_SYN ||
      ...
      if (random_u32(&ping->probe_tcpseq) != 0) goto err;
    }
  }
...
static void do_ping_free(scamper_task_t *task) {
  ...
  if ((ping = ping_getdata(task)) != NULL) scamper_ping_free(ping);

  if ((state = ping_getstate(task)) != NULL) ping_state_free(state);
...
scamper_task_t *scamper_do_ping_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(ping, &ping_funcs)) == NULL) goto err;
...
void scamper_do_ping_cleanup() { return; }
