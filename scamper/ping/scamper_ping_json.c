...
static char *ping_probe_data(const scamper_ping_t *ping, char *buf,
                             size_t len) {
  size_t off = 0;
  uint16_t i;
  for (i = 0; i + 4 < ping->probe_datalen; i += 4)
    string_concat(buf, len, &off, "%02x%02x%02x%02x", ping->probe_data[i + 0],
                  ping->probe_data[i + 1], ping->probe_data[i + 2],
                  ping->probe_data[i + 3]);
...
static char *ping_header(const scamper_ping_t *ping) {
  static const char *flags[] = {"v4rr",   "spoof",     "payload",
  ...
  string_concat(buf, sizeof(buf), &off,
                ...
                ping->ping_sent, ping->probe_size, ping->userid,
                ping->probe_ttl, ping->probe_wait);
  ...
  if (SCAMPER_PING_METHOD_IS_TCP(ping))
    string_concat(buf, sizeof(buf), &off, ", \"tcp_seq\":%u, \"tcp_ack\":%u",
  ...
  if (ping->flags != 0) {
    ...
    for (u8 = 0; u8 < 9; u8++) {
      if ((ping->flags & (0x1 << u8)) == 0) continue;
      if (c > 0) string_concat(buf, sizeof(buf), &off, ",");
  ...
  if (SCAMPER_PING_METHOD_IS_ICMP(ping) &&
    ...
    string_concat(buf, sizeof(buf), &off, ", \"icmp_csum\": %u",
                  ping->probe_icmpsum);
...
static char *ping_reply_proto(const scamper_ping_reply_t *reply, char *buf,
                              size_t len) {
  if (reply->reply_proto == IPPROTO_ICMP ||
...
static char *ping_reply(const scamper_ping_t *ping,
  ...
  if (reply->tx.tv_sec != 0) {
    ...
    string_concat(buf, sizeof(buf), &off, ", \"tx\":{\"sec\":%u, \"usec\":%u}",
                  reply->tx.tv_sec, reply->tx.tv_usec);
    string_concat(buf, sizeof(buf), &off, ", \"rx\":{\"sec\":%u, \"usec\":%u}",
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(reply->addr)) {
    string_concat(buf, sizeof(buf), &off,
                  ", \"probe_ipid\":%u, \"reply_ipid\":%u", reply->probe_ipid,
                  reply->reply_ipid);
  ...
  if (SCAMPER_PING_REPLY_IS_ICMP(reply)) {
    string_concat(buf, sizeof(buf), &off,
                  ", \"icmp_type\":%u, \"icmp_code\":%u", reply->icmp_type,
                  reply->icmp_code);
...
static char *ping_stats(const scamper_ping_t *ping) {
  ...
  if (scamper_ping_stats(ping, &stats) != 0) return NULL;
  ...
  if (ping->ping_sent != 0) {
    ...
    if (stats.nreplies == 0)
      ...
      string_concat(
          buf, sizeof(buf), &off, "%.2f",
          (float)(ping->ping_sent - stats.nreplies) / ping->ping_sent);
  }
  if (stats.nreplies > 0) {
...
int scamper_file_json_ping_write(const scamper_file_t *sf,
  ...
  if ((header = ping_header(ping)) == NULL) goto cleanup;
  ...
  if (reply_count > 0) {
    ...
    for (i = 0, j = 0; i < ping->ping_sent; i++) {
      ...
      while (reply != NULL) {
        ...
        if ((replies[j] = ping_reply(ping, reply)) == NULL) goto cleanup;
  ...
  len += 2; /* ], */
  if ((stats = ping_stats(ping)) != NULL) len += (stats_len = strlen(stats));
  len += 2; /* }\n */

  if ((str = malloc_zero(len)) == NULL) goto cleanup;
  memcpy(str + wc, header, header_len);
  wc += header_len;
  memcpy(str + wc, ", \"responses\":[", 15);
  wc += 15;
