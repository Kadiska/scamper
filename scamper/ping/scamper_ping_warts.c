...
static const warts_var_t ping_vars[] = {
    {WARTS_PING_LIST_ID, 4, -1},       {WARTS_PING_CYCLE_ID, 4, -1},
    {WARTS_PING_ADDR_SRC_GID, 4, -1},  {WARTS_PING_ADDR_DST_GID, 4, -1},
    {WARTS_PING_START, 8, -1},         {WARTS_PING_STOP_R, 1, -1},
    {WARTS_PING_STOP_D, 1, -1},        {WARTS_PING_DATA_LEN, 2, -1},
    {WARTS_PING_DATA_BYTES, -1, -1},   {WARTS_PING_PROBE_COUNT, 2, -1},
    {WARTS_PING_PROBE_SIZE, 2, -1},    {WARTS_PING_PROBE_WAIT, 1, -1},
    {WARTS_PING_PROBE_TTL, 1, -1},     {WARTS_PING_REPLY_COUNT, 2, -1},
    {WARTS_PING_PING_SENT, 2, -1},     {WARTS_PING_PROBE_METHOD, 1, -1},
    {WARTS_PING_PROBE_SPORT, 2, -1},   {WARTS_PING_PROBE_DPORT, 2, -1},
    {WARTS_PING_USERID, 4, -1},        {WARTS_PING_ADDR_SRC, -1, -1},
    {WARTS_PING_ADDR_DST, -1, -1},     {WARTS_PING_FLAGS8, 1, -1},
    {WARTS_PING_PROBE_TOS, 1, -1},     {WARTS_PING_PROBE_TSPS, -1, -1},
    {WARTS_PING_PROBE_ICMPSUM, 2, -1}, {WARTS_PING_REPLY_PMTU, 2, -1},
    {WARTS_PING_PROBE_TIMEOUT, 1, -1}, {WARTS_PING_PROBE_WAIT_US, 4, -1},
    {WARTS_PING_PROBE_TCPACK, 4, -1},  {WARTS_PING_FLAGS, 4, -1},
    {WARTS_PING_PROBE_TCPSEQ, 4, -1},  {WARTS_PING_ADDR_RTR, -1, -1},
};
...
static void insert_ping_reply_v4rr(uint8_t *buf, uint32_t *off,
  ...
  assert(len - *off >= 1);
  buf[(*off)++] = rr->rrc;
  for (i = 0; i < rr->rrc; i++) insert_addr(buf, off, len, rr->rr[i], param);
...
static int extract_ping_reply_v4rr(const uint8_t *buf, uint32_t *off,
  ...
  if ((*out = scamper_ping_reply_v4rr_alloc(rrc)) == NULL) return -1;

  for (i = 0; i < rrc; i++) {
    if (extract_addr(buf, off, len, &addr, param) != 0) return -1;
    (*out)->rr[i] = addr;
  }
...
static void insert_ping_reply_v4ts(uint8_t *buf, uint32_t *off,
  ...
  for (i = 0; i < ts->tsc; i++) insert_uint32(buf, off, len, &ts->tss[i], NULL);

  for (i = 0; i < ipc; i++) insert_addr(buf, off, len, ts->ips[i], param);
...
static int extract_ping_reply_v4ts(const uint8_t *buf, uint32_t *off,
  ...
  tsc = buf[(*off)++];
  ipc = buf[(*off)++];
  if (ipc != 0 && ipc != tsc) return -1;
  if ((*out = scamper_ping_reply_v4ts_alloc(tsc, ipc != 0 ? 1 : 0)) == NULL)
    return -1;

  for (i = 0; i < tsc; i++) {
    if (extract_uint32(buf, off, len, &u32, NULL) != 0) return -1;
    (*out)->tss[i] = u32;
  }
...
static int extract_ping_reply_tsreply(uint8_t *buf, uint32_t *off,
  ...
  if (*off >= len || len - *off < 12) return -1;
  if ((tsreply = scamper_ping_reply_tsreply_alloc()) == NULL) return -1;
  ...
  *params_len = 0;
  ...
  for (i = 0; i < sizeof(ping_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_PING_ADDR_SRC_GID ||
        ...
        (var->id == WARTS_PING_PROBE_TIMEOUT &&
...
static void insert_ping_probe_tsps(uint8_t *buf, uint32_t *off,
  ...
  assert(len - *off >= 1);
  buf[(*off)++] = ts->ipc;
  for (i = 0; i < ts->ipc; i++) insert_addr(buf, off, len, ts->ips[i], param);
...
static int extract_ping_probe_tsps(const uint8_t *buf, uint32_t *off,
  ...
  if ((*out = scamper_ping_v4ts_alloc(ipc)) == NULL) return -1;
...
static int warts_ping_params_read(scamper_ping_t *ping, warts_state_t *state,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (ping->src == NULL || ping->dst == NULL) return -1;
...
static int warts_ping_params_write(
    const scamper_ping_t *ping, const scamper_file_t *sf,
...
int scamper_file_warts_ping_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  for (i = 0; i < reply_count; i++) {
    ...
    if (warts_ping_reply_read(ping, reply, state, table, buf, &off, hdr->len) !=
        0) {
      goto err;
    }
...
int scamper_file_warts_ping_write(const scamper_file_t *sf,
  ...
  if ((reply_count = scamper_ping_reply_count(ping)) > 0) {
    ...
    for (i = 0, j = 0; i < ping->ping_sent; i++) {
      for (reply = ping->ping_replies[i]; reply != NULL; reply = reply->next) {
        if (warts_ping_reply_state(sf, ping, reply, &reply_state[j++], table,
                                   &len) == -1) {
          goto err;
        }
      }
  ...
  if (warts_ping_params_write(ping, sf, table, buf, &off, len, flags, flags_len,
                              params_len) == -1) {
    goto err;
  }
