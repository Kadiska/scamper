...
static void usage(uint32_t opt_mask) {
  ...
  if ((opt_mask & OPT_CYCLEID) != 0) usage_str('C', "cycle id");
...
static int ppswindow_set(int p, int w) {
  ...
  if (p != pps) {
    ...
    if (p != 0)
      ...
      wait_between = 0;
    probe_window = 250000;
    if (wait_between > 250000) probe_window += wait_between;
  }
...
static int check_options(int argc, char *argv[]) {
  static const scamper_multicall_t multicall[] = {
      {"scamper-trace", "trace", scamper_do_trace_arg_validate,
       scamper_do_trace_usage},
      {"scamper-ping", "ping", scamper_do_ping_arg_validate,
       scamper_do_ping_usage},
      {"scamper-tracelb", "tracelb", scamper_do_tracelb_arg_validate,
       scamper_do_tracelb_usage},
      {"scamper-dealias", "dealias", scamper_do_dealias_arg_validate,
       scamper_do_dealias_usage},
      {"scamper-sting", "sting", scamper_do_sting_arg_validate,
       scamper_do_sting_usage},
      {"scamper-neighbourdisc", "neighbourdisc",
       scamper_do_neighbourdisc_arg_validate, scamper_do_neighbourdisc_usage},
      {"scamper-tbit", "tbit", scamper_do_tbit_arg_validate,
       scamper_do_tbit_usage},
      {"scamper-sniff", "sniff", scamper_do_sniff_arg_validate,
       scamper_do_sniff_usage},
      {"scamper-host", "host", scamper_do_host_arg_validate,
       scamper_do_host_usage},
  };
  int i;
  ...
  for (m = 0; m < sizeof(multicall) / sizeof(scamper_multicall_t); m++) {
    ...
    if (argv0 >= len &&
        strcmp(argv[0] + argv0 - len, multicall[m].argv0) == 0) {
  ...
  if (options & OPT_LISTID &&
      set_opt(OPT_LISTID, opt_listid, listid_set) != 0) {
    usage(OPT_LISTID);
    return -1;
  }
  ...
  o = options & (OPT_CTRL_INET | OPT_CTRL_UNIX | OPT_CTRL_REMOTE | OPT_IP |
                 OPT_CMDLIST | OPT_INFILE);
  ...
  if (options & (OPT_CTRL_INET | OPT_CTRL_REMOTE | OPT_CTRL_UNIX)) {
    ...
    if (options & OPT_CTRL_INET) {
      ...
      if (arglist_len != 0 || string_addrport(opt_ctrl_inet, &ctrl_inet_addr,
                                              &ctrl_inet_port) != 0) {
        usage(OPT_CTRL_INET);
        return -1;
      }
    ...
    if (options & OPT_CTRL_REMOTE) {
       ...
       * leftover args
       */
      if (arglist_len != 0 || string_addrport(opt_ctrl_remote, &ctrl_rem_name,
                                              &ctrl_rem_port) != 0) {
    ...
    if ((options &
         (OPT_CTRL_INET | OPT_CTRL_REMOTE | OPT_CTRL_UNIX | OPT_INFILE)) ||
...
int scamper_option_pps_get() { return pps; }

int scamper_option_pps_set(const int p) { return ppswindow_set(p, window); }

int scamper_option_window_get() { return window; }

int scamper_option_window_set(const int w) { return ppswindow_set(pps, w); }

const char *scamper_option_monitorname_get() { return monitorname; }
...
const char *scamper_option_nameserver_get(void) { return nameserver; }
...
uint16_t scamper_sport_default(void) { return default_sport; }
...
static int scamper_timeout(struct timeval *timeout, struct timeval *nextprobe,
  ...
  if (scamper_queue_readycount() > 0 ||
    ...
    if (exit_when_done != 0 && scamper_sources_isempty() == 1) return 2;
  }
...
static int scamper(int argc, char *argv[]) {
  ...
  if (scamper_osinfo_init() != 0) return -1;
  ...
  if ((options & OPT_PIDFILE) != 0 && scamper_pidfile() != 0) return -1;

  random_seed();

  if (firewall != NULL) {
    if (scamper_firewall_init(firewall) != 0) return -1;
    free(firewall);
    firewall = NULL;
  }
  ...
  if (scamper_probe_init() != 0) return -1;

  /* initialise the queues that hold the current tasks */
  if (scamper_queue_init() == -1) return -1;
  ...
  if (scamper_do_trace_init() != 0 || scamper_do_ping_init() != 0 ||
      scamper_do_tracelb_init() != 0 || scamper_do_dealias_init() != 0 ||
      scamper_do_sting_init() != 0 || scamper_do_neighbourdisc_init() != 0 ||
      scamper_do_tbit_init() != 0 || scamper_do_sniff_init() != 0 ||
      scamper_do_host_init() != 0) {
    return -1;
  }

  if (options & (OPT_CTRL_INET | OPT_CTRL_UNIX | OPT_CTRL_REMOTE)) {
    if (scamper_control_init() != 0) return -1;
    if (options & OPT_CTRL_INET &&
  ...
  if (options & OPT_LISTNAME) ssp.name = listname;
  if (options & OPT_LISTID) ssp.list_id = listid;
  if (options & OPT_CYCLEID) ssp.cycle_id = cycleid;
  ...
  if (options & (OPT_IP | OPT_CMDLIST)) {
    if ((source = scamper_source_cmdline_alloc(&ssp, command, arglist,
                                               arglist_len)) == NULL) {
      return -1;
    }
  } else if ((options & (OPT_CTRL_INET | OPT_CTRL_UNIX | OPT_CTRL_REMOTE)) ==
             0) {
    if (intype == NULL)
  ...
  for (;;) {
    ...
    if (scamper_queue_event_proc(&tv) != 0) return -1;
    ...
    if (scamper_queue_readycount() > 0 || scamper_sources_isready() == 1) {
      ...
      for (;;) {
        ...
        if (timeval_cmp(&nextprobe, &tv) > 0) break;
        ...
        if ((task = scamper_queue_select()) == NULL) {
           ...
           * add any new tasks
           */
          if (window != 0 && scamper_queue_windowcount() >= window) break;

          /*
           * if there are no more tasks ready to be added yet, there's
           * nothing more to be done in the loop
           */
          if (scamper_sources_gettask(&task) != 0 || task == NULL) break;
        }
...
#ifndef WITHOUT_DEBUGFILE
  if (options & OPT_DEBUGFILE) scamper_debug_close();
#endif
...
BOOL load_npcap_dll() {
  ...
  UINT length_npcap_dir =
      _tcslen(system_directory) + _tcslen(_T("\\Npcap")) + 1;
...
#if defined(DMALLOC)
  free(malloc(1));
#elif !defined(NDEBUG) && defined(__FreeBSD__) && \
    __FreeBSD_version >= 500014 && __FreeBSD_version < 1000011
  _malloc_options = "AJ";
#endif
