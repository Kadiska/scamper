...
static void ipv4_tostr(const scamper_addr_t *addr, char *buf,
                       const size_t len) {
...
static int ipv4_prefix(const scamper_addr_t *sa, const scamper_addr_t *sb) {
  ...
  for (i = 32; i > 0; i--) {
    if (((a->s_addr ^ b->s_addr) & htonl(uint32_netmask[i - 1])) == 0) break;
  }

  return i;
}

static int ipv4_prefixhosts(const scamper_addr_t *sa,
                            const scamper_addr_t *sb) {
  const struct in_addr *a = sa->addr;
  const struct in_addr *b = sb->addr;
  struct in_addr c;
  int i;
...
static int ipv4_islinklocal(const scamper_addr_t *sa) {
  const struct in_addr *a = sa->addr;
  if ((ntohl(a->s_addr) & 0xffff0000) == 0xa9fe0000) return 1;
  return 0;
}
...
static int ipv4_netaddr(const scamper_addr_t *sa, void *net, int netlen) {
  const struct in_addr *a = sa->addr;
  struct in_addr p;
  if (netlen <= 0 || netlen > 32 || sa == NULL || net == NULL) return -1;
...
static int ipv4_isreserved(const scamper_addr_t *a) {
  ...
  int i;
  for (i = 0; i < prefc; i++)
    if ((addr & prefs[i][1]) == prefs[i][0]) return 1;
  return 0;
}
...
static void ipv6_tostr(const scamper_addr_t *addr, char *buf,
                       const size_t len) {
...
static int ipv6_inprefix(const scamper_addr_t *sa, const void *p, int len) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (((addr->s6_addr32[i] ^ prefix->s6_addr32[i]) & mask) != 0) return 0;
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (((addr->u.Word[i] ^ prefix->u.Word[i]) & mask) != 0) return 0;
...
static int ipv6_prefix(const scamper_addr_t *sa, const scamper_addr_t *sb) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 32; j++) {
      if (((ua ^ ub) & htonl(uint32_netmask[j])) != 0) return x;
      x++;
    }
  }
#else
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 16; j++) {
      if (((ua ^ ub) & htons(uint16_mask[j])) != 0) return x;
      x++;
    }
  }
#endif
...
static int ipv6_netaddr(const scamper_addr_t *sa, void *net, int nl) {
  ...
  if (nl <= 0 || nl > 128 || sa == NULL || net == NULL) return -1;
  memset(&p, 0, sizeof(p));
...
static int ipv6_isreserved(const scamper_addr_t *sa) {
  ...
  if ((a->s6_addr[0] & 0xe0) != 0x20) return 1;

  /* 2002::/16 (6to4) */
  if (a->s6_addr[1] == 0x02) return 1;

  /* 2001::/16 (many) */
  if (a->s6_addr[1] == 0x01) {
    if (a->s6_addr[2] == 0) {
      /* 2001::/32 (teredo) */
      if (a->s6_addr[3] == 0) return 1;
      ...
      /* 2001:3::/32 (AMT) */
      if (a->s6_addr[3] == 0x3) return 1;

      /* 2001:4:112::/48 (AS112-v6) */
      if (a->s6_addr[4] == 0x4 && a->s6_addr[5] == 0x1 && a->s6_addr[6] == 0x12)
        return 1;

      /* 2001:10::/28 (ORCHID) and 2001:20::/28 (ORCHIDv2) */
      if (((a->s6_addr[3] & 0xf0) == 0x10) || ((a->s6_addr[3] & 0xf0) == 0x20))
        return 1;
    }

    /* 2001:db8::/32 (documentation */
    if (a->s6_addr[2] == 0x0d && a->s6_addr[3] == 0xb8) return 1;
  }
...
#ifndef _WIN32
  for (i = 0; i < 4; i++) {
    if ((v = ntohl(a->s6_addr32[i] ^ b->s6_addr32[i])) == 0) continue;
...
#else
  for (i = 0; i < 8; i++) {
    if ((v = ntohs(a->u.Word[i] ^ b->u.Word[i])) == 0) continue;
...
static int ethernet_bit(const scamper_addr_t *addr, int bit) {
  static const uint8_t mask[] = {0x01, 0x80, 0x40, 0x20,
                                 0x10, 0x08, 0x04, 0x02};
...
static void firewire_tostr(const scamper_addr_t *addr, char *buf,
  ...
  snprintf(buf, len, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", lla[0], lla[1],
           lla[2], lla[3], lla[4], lla[5], lla[6], lla[7]);
  return;
}
...
int scamper_addr_prefix(const scamper_addr_t *a, const scamper_addr_t *b) {
  if (a->type != b->type || handlers[a->type - 1].prefix == NULL) return -1;
...
int scamper_addr_isrfc1918(const scamper_addr_t *sa) {
  ...
  if (sa->type != SCAMPER_ADDR_TYPE_IPV4) return 0;
...
int scamper_addr_is6to4(const scamper_addr_t *sa) {
  ...
  if (sa->type != SCAMPER_ADDR_TYPE_IPV6) return 0;

  a = sa->addr;
#ifndef _WIN32
  if (a->s6_addr[0] == 0x20 && a->s6_addr[1] == 0x02) return 1;
#else
  if (a->u.Word[0] == htons(0x2002)) return 1;
#endif
...
int scamper_addr_isunicast(const scamper_addr_t *sa) {
  if (handlers[sa->type - 1].isunicast == NULL) return -1;
  return handlers[sa->type - 1].isunicast(sa);
}

int scamper_addr_isreserved(const scamper_addr_t *sa) {
  if (handlers[sa->type - 1].isreserved == NULL) return -1;
...
scamper_addr_t *scamper_addrcache_get(scamper_addrcache_t *ac, const int type,
  ...
  if ((sa = scamper_addr_alloc(type, addr)) != NULL) {
    if (splaytree_insert(ac->tree[type - 1], sa) == NULL) goto err;
    sa->internal = ac;
  }
...
scamper_addrcache_t *scamper_addrcache_alloc() {
  ...
  if ((ac = malloc_zero(sizeof(scamper_addrcache_t))) == NULL) return NULL;
