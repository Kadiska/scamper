...
static int dl_parse_ip(scamper_dl_rec_t *dl, uint8_t *pktbuf, size_t pktlen) {
  ...
  if ((pkt[0] >> 4) == 4) /* IPv4 */
  {
    ...
    if ((u16 & IP_DF) != 0) dl->dl_ip_flags |= SCAMPER_DL_IP_FLAG_DF;
    if ((u16 & IP_MF) != 0) dl->dl_ip_flags |= SCAMPER_DL_IP_FLAG_MF;
  ...
  } else if ((pkt[0] >> 4) == 6) /* IPv6 */
  {
    ...
    if ((iplen = sizeof(struct ip6_hdr)) > len) return 0;
    ...
    for (;;) {
      switch (dl->dl_ip_proto) {
        ...
        case IPPROTO_ROUTING:
          if (sizeof(struct ip6_ext) > len) return 0;
          ip6_exthdr = (struct ip6_ext *)pkt;
          if ((extlen = (ip6_exthdr->ip6e_len * 8) + 8) > len) return 0;
          dl->dl_ip_proto = ip6_exthdr->ip6e_nxt;
          break;

        case IPPROTO_FRAGMENT:
          if ((extlen = sizeof(struct ip6_frag)) > len) return 0;
  ...
  if (dl->dl_ip_proto == IPPROTO_UDP) {
    ...
    if (dl->dl_tcp_hl >= 20 && len >= dl->dl_tcp_hl) {
      ...
      while (off < dl->dl_tcp_hl) {
        ...
        if (off + tmp[1] > dl->dl_tcp_hl) break;
      ...
      if (dl->dl_tcp_datalen > 0) dl->dl_tcp_data = pkt + dl->dl_tcp_hl;
    }
...
static int dlt_en10mb_cb(scamper_dl_rec_t *dl, uint8_t *pkt, size_t len) {
  ...
  if (u16 == ETHERTYPE_IP || u16 == ETHERTYPE_IPV6) {
    if (dl_parse_ip(dl, pkt + 14, len - 14) == 0) return 0;
  } else if (u16 == ETHERTYPE_ARP) {
    /* need to at least have a header */
    if (14 + 8 >= len) return 0;
    ...
    if (14 + 8 + (dl->dl_arp_hln * 2) + (dl->dl_arp_pln * 2) > len) return 0;
...
static int dl_bpf_node_init(const scamper_fd_t *fdn, scamper_dl_t *node) {
  ...
  scamper_debug(__func__, "bpf if %s index %d buflen %d datalink %d", ifname,
                ifindex, node->readbuf_len, node->type);
...
static int dl_bpf_init(void) {
  ...
  if (osinfo->os_id == SCAMPER_OSINFO_OS_FREEBSD && osinfo->os_rel_dots >= 2 &&
      osinfo->os_rel[0] == 4 &&
...
static int dl_bpf_read(const int fd, scamper_dl_t *node) {
  ...
  while ((len = read(fd, buf, node->readbuf_len)) == -1) {
    ...
    printerror(__func__, "read %d bytes from fd %d failed", node->readbuf_len,
               fd);
    return -1;
  }
...
static int dl_bpf_tx(const scamper_dl_t *node, const uint8_t *pkt,
                     const size_t len) {
  ssize_t wb;
...
static int dl_linux_filter(scamper_dl_t *node, struct bpf_insn *insns,
                           int len) {
  ...
  if (pcap_setfilter(pcap, &filter) != 0) return -1;
  return 0;
}
...
static int dl_dlpi_ack(const int fd, void *ack, int primitive) {
  ...
  if (dlp->dl_primitive != primitive) {
    scamper_debug(__func__, "expected %d, got %d", primitive,
                  dlp->dl_primitive);
    return -1;
  }
...
static int dl_dlpi_node_init(const scamper_fd_t *fdn, scamper_dl_t *node) {
  ...
  if (strioctl(fd, SBIOCSTIME, &tv, sizeof(tv)) == -1) {
    printerror(__func__, "could not SBIOCSTIME %d.%06d", tv.tv_sec, tv.tv_usec);
    return -1;
  }
  ...
  scamper_debug(__func__, "dlpi if %s index %d datalink %d", ifname, ifindex,
                node->type);
#endif
...
static int dl_dlpi_tx(const scamper_dl_t *node, const uint8_t *pkt,
                      const size_t len) {
  struct strbuf data;
  int fd;

  if ((fd = scamper_fd_fd_get(node->fdn)) < 0) return -1;
...
void scamper_dl_rec_udp_print(const scamper_dl_rec_t *dl) {
  ...
  scamper_debug(NULL, "from %s %sudp %d:%d len %d",
                addr_tostr(dl->dl_af, dl->dl_ip_src, addr, sizeof(addr)), ipid,
                dl->dl_tcp_sport, dl->dl_tcp_dport, dl->dl_ip_size);
  return;
}

void scamper_dl_rec_tcp_print(const scamper_dl_rec_t *dl) {
  static const char *tcpflags[] = {"fin", "syn", "rst", "psh",
                                   "ack", "urg", "ece", "cwr"};
  uint8_t u8;
  size_t off;
  char addr[64];
  ...
  scamper_debug(NULL, "from %s %stcp %d:%d %s %s len %d",
                addr_tostr(dl->dl_af, dl->dl_ip_src, addr, sizeof(addr)), ipid,
                dl->dl_tcp_sport, dl->dl_tcp_dport, flags, pos, dl->dl_ip_size);
...
void scamper_dl_rec_icmp_print(const scamper_dl_rec_t *dl) {
  ...
  if (dl->dl_af == AF_INET) {
    ...
    snprintf(ip, sizeof(ip), "from %s size %d ttl %d tos 0x%02x ipid 0x%04x",
             addr, dl->dl_ip_size, dl->dl_ip_ttl, dl->dl_ip_tos, dl->dl_ip_id);
    ...
    switch (dl->dl_icmp_type) {
      ...
      case ICMP_ECHOREPLY:
        t = "echo reply";
        snprintf(cbuf, sizeof(cbuf), "id %d seq %d", dl->dl_icmp_id,
                 dl->dl_icmp_seq);
        c = cbuf;
        break;
  ...
  if (t == NULL) {
    snprintf(icmp, sizeof(icmp), "icmp %d code %d", dl->dl_icmp_type,
             dl->dl_icmp_code);
  } else if (c == NULL) {
  ...
  if (dl->dl_icmp_ip_dst != NULL) {
    if (dl->dl_af == AF_INET) {
      ...
      snprintf(inner_ip, sizeof(inner_ip),
               ...
               dl->dl_icmp_ip_size, dl->dl_icmp_ip_ttl, dl->dl_icmp_ip_tos,
               dl->dl_icmp_ip_id);
    } else {
      addr_tostr(AF_INET6, dl->dl_icmp_ip_dst, addr, sizeof(addr));
      snprintf(inner_ip, sizeof(inner_ip), " to %s size %d hlim %d flow 0x%05x",
               addr, dl->dl_icmp_ip_size, dl->dl_icmp_ip_hlim,
               dl->dl_icmp_ip_flow);
    }
    ...
    switch (dl->dl_icmp_ip_proto) {
      ...
      case IPPROTO_TCP:
        snprintf(inner_transport, sizeof(inner_transport),
                 " proto TCP sport %d dport %d seq %08x", dl->dl_icmp_tcp_sport,
                 dl->dl_icmp_tcp_dport, dl->dl_icmp_tcp_seq);
        break;
