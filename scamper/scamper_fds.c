...
#define SCAMPER_FD_TYPE_IS_TCP(fd) \
  ((fd)->type == SCAMPER_FD_TYPE_TCP4 || (fd)->type == SCAMPER_FD_TYPE_TCP6)

#define SCAMPER_FD_TYPE_IS_IPV4(fd)                \
  ((fd)->type == SCAMPER_FD_TYPE_PROBE_ICMP4 ||    \
   (fd)->type == SCAMPER_FD_TYPE_PROBE_ICMP4ERR || \
   (fd)->type == SCAMPER_FD_TYPE_UDP4 ||           \
   (fd)->type == SCAMPER_FD_TYPE_UDP4DG || (fd)->type == SCAMPER_FD_TYPE_TCP4)
...
#define SCAMPER_FD_TYPE_IS_DL(fd) ((fd)->type == SCAMPER_FD_TYPE_DL)
...
static char *fd_addr_tostr(char *buf, size_t len, int af, void *addr) {
  ...
  if (addr == NULL || addr_tostr(af, addr, tmp, sizeof(tmp)) == NULL) return "";
...
static void fd_free(scamper_fd_t *fdn) {
  ...
  if (fdn->read.node != NULL) dlist_node_pop(fdn->read.list, fdn->read.node);

  if (fdn->write.node != NULL) dlist_node_pop(fdn->write.list, fdn->write.node);

  if (fdn->rc0 != NULL) dlist_node_pop(refcnt_0, fdn->rc0);

  if (fdn->fd_tree_node != NULL)
    splaytree_remove_node(fd_tree, fdn->fd_tree_node);

  if (fdn->fd_list_node != NULL) dlist_node_pop(fd_list, fdn->fd_list_node);

  if (SCAMPER_FD_TYPE_IS_ICMP(fdn)) {
    if (fdn->fd_icmp_addr != NULL) free(fdn->fd_icmp_addr);
  } else if (SCAMPER_FD_TYPE_IS_UDP(fdn)) {
    if (fdn->fd_udp_addr != NULL) free(fdn->fd_udp_addr);
  } else if (SCAMPER_FD_TYPE_IS_TCP(fdn)) {
    if (fdn->fd_tcp_addr != NULL) free(fdn->fd_tcp_addr);
  } else if (SCAMPER_FD_TYPE_IS_DL(fdn)) {
    if (fdn->fd_dl_dl != NULL) scamper_dl_state_free(fdn->fd_dl_dl);
  }
 ...
 */
static int fds_select_assemble(dlist_t *fds, slist_t **file_list, fd_set *fdset,
                               fd_set **fdsp, int *nfds, const int event,
                               scamper_fd_poll_t **fdps, HANDLE *events,
                               int *count_event) {
  ...
  while (node != NULL) {
    ...
    if (*nfds < fdp->fdn->fd) *nfds = fdp->fdn->fd;
  }
...
static void fds_files_check(dlist_t *fds, slist_t *list) {
  ...
  dlist_lock(fds);
  while ((fdp = slist_head_pop(list)) != NULL) fdp->cb(fdp->fdn, fdp->param);
  dlist_unlock(fds);
...
static int fds_select(struct timeval *timeout) {
  ...
  if (fds_select_assemble(read_fds, &rfiles, &rfds, &rfdsp, &nfds, FD_READ,
                          fdps, events, &count_event) != 0)
    goto err;
  if (fds_select_assemble(write_fds, &wfiles, &wfds, &wfdsp, &nfds, FD_WRITE,
                          fdps, events, &count_event) != 0)
    goto err;
...
static void fds_poll_check(short event, int rc, int count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if ((poll_fds[i].events & event) == 0) continue;

    /* skip over if there is no event of this type */
    if ((poll_fds[i].revents & (event | POLLHUP | POLLERR)) == 0) continue;
...
static int fds_poll(struct timeval *tv) {
  ...
  while (n != NULL) {
    ...
    } else if (fd->type == SCAMPER_FD_TYPE_PCAP_ICMP4 ||
               fd->type == SCAMPER_FD_TYPE_PCAP_ICMP6) {
  ...
  if (tv != NULL) {
    ...
    if (timeout == 0 && tv->tv_usec != 0) timeout++;
  } else {
    timeout = -1;
  }
  ...
  if (rc > 0) {
    if (in != 0) fds_poll_check(POLLIN, rc < in ? rc : in, count);
    if (out != 0) fds_poll_check(POLLOUT, rc < out ? rc : out, count);
  }
...
static int fds_kqueue(struct timeval *tv) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (fd < 0 || fd >= fd_array_s) continue;
    if ((fdp = fd_array[fd]) == NULL) continue;
...
static int fd_addr_cmp(int type, void *a, void *b) {
  assert(type == SCAMPER_FD_TYPE_TCP4 || type == SCAMPER_FD_TYPE_TCP6 ||
         ...
         type == SCAMPER_FD_TYPE_PROBE_ICMP6 ||
  ...
  if (type == SCAMPER_FD_TYPE_TCP4 || type == SCAMPER_FD_TYPE_UDP4 ||
      type == SCAMPER_FD_TYPE_UDP4DG || type == SCAMPER_FD_TYPE_PCAP_ICMP4 ||
      type == SCAMPER_FD_TYPE_PCAP_ICMP6 ||
      type == SCAMPER_FD_TYPE_PROBE_ICMP4 ||
      type == SCAMPER_FD_TYPE_PROBE_ICMP4ERR)
...
#endif
{
  ...
  if (fd + 1 > fd_array_s) {
    ...
    if (realloc_wrap((void **)&fd_array, size) != 0) goto err;
    for (i = fd_array_s; i < fd + 1; i++) fd_array[i] = NULL;
    fd_array_s = fd + 1;
  }
...
static scamper_fd_t *fd_tcp(int type, void *addr, uint16_t sport) {
  ...
  if ((fdn = fd_find(&findme)) != NULL) return fdn;
...
int scamper_fds_poll(struct timeval *timeout) {
  ...
  if (dlist_count(refcnt_0) > 0) {
    ...
    while ((fdn = (scamper_fd_t *)dlist_head_item(refcnt_0)) != NULL) {
      ...
      if (timeval_cmp(&fdn->tv, &tv) > 0) break;
    ...
    if (fdn != NULL) {
      ...
      if (timeout == NULL || timeval_cmp(&tv, timeout) < 0) timeout = &tv;
    }
  }
...
int scamper_fd_fd_get(const scamper_fd_t *fdn) {
  if (fdn->raw == NULL) return fdn->fd;
  return fdn->raw->fd;
}

void *scamper_fd_pcap_get(const scamper_fd_t *fdn) { return fdn->fd_dl_pcap; }
...
#ifdef HAVE_KQUEUE
    if (kq != -1) fds_kqueue_set(fdn, EVFILT_READ, EV_ADD);
#endif

#ifdef HAVE_EPOLL
    if (ep != -1) fds_epoll_ctl(fdn, EPOLLIN, EPOLL_CTL_ADD);
#endif
...
#ifdef HAVE_KQUEUE
    if (kq != -1) fds_kqueue_set(fdn, EVFILT_WRITE, EV_ADD);
#endif

#ifdef HAVE_EPOLL
    if (ep != -1) fds_epoll_ctl(fdn, EPOLLOUT, EPOLL_CTL_ADD);
#endif
...
scamper_fd_t *scamper_fd_udp4(void *addr, uint16_t sport) {
  ...
  if ((fd = fd_udp(SCAMPER_FD_TYPE_UDP4DG, addr, sport)) == NULL) return NULL;
...
scamper_fd_t *scamper_fd_ip4(void) { return fd_null(SCAMPER_FD_TYPE_IP4); }
...
#ifdef _WIN32
  if ((fdn = fd_alloc(SCAMPER_FD_TYPE_DL, 4096 + ++virtual_counter)) == NULL)
    goto err;
#else
  if ((fdn = fd_alloc(SCAMPER_FD_TYPE_DL, pcap_get_selectable_fd(pcap))) ==
      NULL)
    goto err;
#endif
...
scamper_fd_t *scamper_fd_private(int fd, void *param, scamper_fd_cb_t read_cb,
  ...
  if ((fdn->fd_list_node = dlist_tail_push(fd_list, fdn)) == NULL) goto err;
...
int scamper_fds_init() {
  ...
  if (scamper_option_epoll()) {
    pollfunc = fds_epoll;
    if (fds_epoll_init() != 0) return -1;
  }
#endif

  if (scamper_option_select() || pollfunc == NULL) pollfunc = fds_select;
...
void scamper_fds_cleanup() {
  ...
  /* clean up the lists */
  cleanup_list(read_fds);
  read_fds = NULL;
  cleanup_list(write_fds);
  write_fds = NULL;
  cleanup_list(read_queue);
  read_queue = NULL;
  cleanup_list(write_queue);
  write_queue = NULL;
