...
struct handler {
  ...
  int (*read)(scamper_file_t *sf, scamper_file_filter_t *filter, uint16_t *type,
              void **data);
  ...
  int (*write_ping)(const scamper_file_t *sf, const struct scamper_ping *ping);
  ...
  int (*write_tbit)(const scamper_file_t *sf, const struct scamper_tbit *tbit);

  int (*write_sniff)(const scamper_file_t *sf,
                     const struct scamper_sniff *sniff);

  int (*write_host)(const scamper_file_t *sf, const struct scamper_host *host);
...
int scamper_file_getfd(const scamper_file_t *sf) { return sf->fd; }

void *scamper_file_getstate(const scamper_file_t *sf) { return sf->state; }

char *scamper_file_getfilename(scamper_file_t *sf) { return sf->filename; }
...
int scamper_file_write_tracelb(scamper_file_t *sf,
  ...
  if (sf->type != SCAMPER_FILE_NONE &&
      handlers[sf->type].write_tracelb != NULL) {
...
int scamper_file_write_dealias(scamper_file_t *sf,
                               const struct scamper_dealias *dealias) {
  if (sf->type != SCAMPER_FILE_NONE &&
      handlers[sf->type].write_dealias != NULL) {
...
int scamper_file_write_obj(scamper_file_t *sf, uint16_t type,
                           const void *data) {
  ...
  if (type > 13 || func[type] == NULL) return -1;
 ...
 */
scamper_file_filter_t *scamper_file_filter_alloc(uint16_t *types,
                                                 uint16_t num) {
...
static int file_type_get(char *type) {
  int i;
  if (type == NULL) return SCAMPER_FILE_NONE;
  for (i = 0; i < handler_cnt; i++)
    if (strcasecmp(type, handlers[i].type) == 0) return i;
...
static int file_type_detect(scamper_file_t *sf) {
  int i;
  for (i = 0; i < handler_cnt; i++)
    if (handlers[i].detect != NULL && handlers[i].detect(sf) == 1) return i;
  return SCAMPER_FILE_NONE;
}
...
static int file_open_read(scamper_file_t *sf) {
  ...
  if (sf->fd != -1) {
    if (fstat(sf->fd, &sb) != 0) return -1;

    if (sb.st_size != 0 && (sb.st_mode & S_IFIFO) == 0)
      sf->type = file_type_detect(sf);
  }

  if (sf->type == SCAMPER_FILE_NONE) return -1;

  if (handlers[sf->type].init_read == NULL) return -1;
...
static int file_open_append(scamper_file_t *sf) {
  ...
  if ((sb.st_mode & S_IFIFO) != 0) return -1;
...
static scamper_file_t *file_open(int fd, char *fn, char mode, int type) {
  ...
  if (mode == 'r')
    open_func = file_open_read;
  else if (mode == 'w')
    open_func = file_open_write;
  else if (mode == 'a')
    open_func = file_open_append;
  else
    return NULL;
