...
static int arts_read_hdr(const scamper_file_t *sf, arts_header_t *ah) {
  ...
  if (attr_len > sizeof(buf) && (tmp = malloc_zero(attr_len)) == NULL) goto err;
...
static scamper_trace_hop_t *arts_hop_reply(scamper_addr_t *addr, uint32_t rtt,
                                           uint8_t distance) {
...
static int arts_hop_read(scamper_trace_hop_t *hop, const uint8_t *buf,
  ...
  if ((hop->hop_addr = scamper_addr_alloc_ipv4(buf + i)) == NULL) return -1;
  i += 4;
...
static scamper_trace_hop_t *arts_hops_read(const arts_header_t *ah,
  ...
  while (count-- > 0) {
    ...
    if ((rc = arts_hop_read(hop, buf + i, ah)) <= 0) goto err;
    i += rc;
  }
...
static scamper_list_t *arts_list_get(arts_state_t *state, uint32_t id) {
  ...
  if ((list = splaytree_find(state->list_tree, &findme)) == NULL) {
    if ((list = scamper_list_alloc(id, NULL, NULL, NULL)) == NULL) return NULL;
...
static int arts_cycle_cmp(const scamper_cycle_t *a, const scamper_cycle_t *b) {
  int i;
  if ((i = arts_list_cmp(a->list, b->list)) != 0) return i;
  if (a->id < b->id) return -1;
  if (a->id > b->id) return 1;
  return 0;
}
...
static scamper_cycle_t *arts_cycle_get(arts_state_t *state,
  ...
  if ((cycle = splaytree_find(state->cycle_tree, &findme)) == NULL) {
    if ((cycle = scamper_cycle_alloc(list)) == NULL) return NULL;
    cycle->id = id;
...
static scamper_trace_t *arts_read_trace(const scamper_file_t *sf,
  ...
  if ((trace->src = scamper_addr_alloc_ipv4(buf + i)) == NULL) goto err;
  i += 4;

  if ((trace->dst = scamper_addr_alloc_ipv4(buf + i)) == NULL) goto err;
  i += 4;
  ...
  if (ah->ver >= 3) {
    ...
    i += 4;
    junk32 = ntohl(junk32);
    if ((trace->list = arts_list_get(state, junk32)) == NULL) goto err;
  ...
  if ((hop = hops) != NULL) {
    for (;;) {
      if (max_hop < hop->hop_probe_ttl) max_hop = hop->hop_probe_ttl;
  ...
  if ((uint32_t)i != ah->data_length) goto err;
  free(buf);
  buf = NULL;

  if (max_hop == 0) return trace;

  if (scamper_trace_hops_alloc(trace, max_hop) == -1) goto err;
  ...
  if (destination_replied != 0) {
    if ((hop = arts_hop_reply(trace->dst, rtt, hop_distance)) == NULL) goto err;
...
static int arts_skip(scamper_file_t *sf, uint32_t bytes) {
  ...
  if (state->ispipe == 0) {
    if (lseek(fd, bytes, SEEK_CUR) != -1) return 0;
  ...
  while (bytes != 0) {
    ...
    if (read_wrap(fd, buf, NULL, len) != 0) return -1;
    bytes -= len;
  }
...
int scamper_file_arts_read(scamper_file_t *sf, scamper_file_filter_t *filter,
  ...
  for (;;) {
    ...
    if (ah.data_length == 0) return -1;

    if (ah.id == ARTS_IP_PATH &&
        scamper_file_filter_isset(filter, SCAMPER_FILE_OBJ_TRACE)) {
      if ((*data = arts_read_trace(sf, &ah)) == NULL) return -1;
      *type = SCAMPER_FILE_OBJ_TRACE;
      return 0;
    }

    /* skip over */
    if (arts_skip(sf, ah.data_length) != 0) return -1;
  }
...
int scamper_file_arts_init_read(scamper_file_t *sf) {
  ...
  if ((s = (arts_state_t *)malloc_zero(sizeof(arts_state_t))) == NULL ||
      (s->list_tree = splaytree_alloc((splaytree_cmp_t)arts_list_cmp)) ==
          NULL ||
      (s->cycle_tree = splaytree_alloc((splaytree_cmp_t)arts_cycle_cmp)) ==
          NULL) {
    arts_state_free(s);
    return -1;
  }
