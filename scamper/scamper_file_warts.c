...
int warts_str_size(const char *str) { return strlen(str) + 1; }
...
static warts_addr_t *warts_addr_alloc(scamper_addr_t *addr, uint32_t id) {
  warts_addr_t *wa;
  if ((wa = malloc(sizeof(warts_addr_t))) == NULL) return NULL;
...
uint32_t warts_addr_size(warts_addrtable_t *t, scamper_addr_t *addr) {
  ...
  fm.addr = addr;
  if (splaytree_find(t->tree, &fm) != NULL) return 1 + 4;
...
warts_addrtable_t *warts_addrtable_alloc_byaddr(void) {
  ...
  if ((table = malloc(sizeof(warts_addrtable_t))) == NULL) return NULL;
  table->addrs = NULL;
  table->addrc = 0;
  if ((table->tree = splaytree_alloc((splaytree_cmp_t)warts_addr_cmp)) ==
      NULL) {
    free(table);
    return NULL;
  }
...
void warts_addrtable_free(warts_addrtable_t *table) {
  ...
  if (table->addrs != NULL) {
    for (i = 0; i < table->addrc; i++) warts_addr_free(table->addrs[i]);
    free(table->addrs);
  }
  free(table);
  return;
...
int extract_addr(const uint8_t *buf, uint32_t *off, const uint32_t len,
                 scamper_addr_t **out, void *param) {
  warts_addrtable_t *table = param;
  warts_addr_t *wa;
  uint32_t u32;
  ...
  if (size == 0) {
    ...
    memcpy(&u32, &buf[*off], 4);
    u32 = ntohl(u32);
    if (table->addrc < 0 || u32 >= (uint32_t)table->addrc) return -1;
   ...
   */
  type = buf[(*off)++];
  if (type == 0 || type > SCAMPER_ADDR_TYPE_MAX) return -1;
...
int extract_string(const uint8_t *buf, uint32_t *off, const uint32_t len,
                   char **out, void *param) {
  uint32_t i;
...
int extract_uint16(const uint8_t *buf, uint32_t *off, const uint32_t len,
                   uint16_t *out, void *param) {
  if (*off >= len || len - *off < 2) return -1;
  memcpy(out, buf + *off, 2);
  *off += 2;
  *out = ntohs(*out);
  return 0;
}

int extract_uint32(const uint8_t *buf, uint32_t *off, const uint32_t len,
                   uint32_t *out, void *param) {
...
int extract_int32(const uint8_t *buf, uint32_t *off, const uint32_t len,
                  int32_t *out, void *param) {
  uint32_t u32;
  if (*off >= len || len - *off < 4) return -1;
  memcpy(&u32, buf + *off, 4);
  *off += 4;
...
int extract_byte(const uint8_t *buf, uint32_t *off, const uint32_t len,
                 uint8_t *out, void *param) {
  if (*off >= len || len - *off < 1) return -1;
  *out = buf[(*off)++];
  return 0;
}

int extract_bytes_ptr(const uint8_t *buf, uint32_t *off, const uint32_t len,
                      const uint8_t **out, uint16_t *req) {
  if (*off >= len || len - *off < *req) return -1;
...
int extract_bytes_alloc(const uint8_t *buf, uint32_t *off, const uint32_t len,
  ...
  if (*req == 0) {
    *out = NULL;
    ...
    if ((*out = malloc_zero(*req)) == NULL) return -1;
...
int extract_addr_gid(const uint8_t *buf, uint32_t *off, const uint32_t len,
...
int extract_list(const uint8_t *buf, uint32_t *off, const uint32_t len,
...
int extract_cycle(const uint8_t *buf, uint32_t *off, const uint32_t len,
                  scamper_cycle_t **cycle, warts_state_t *state) {
  uint32_t id;

  if (extract_uint32(buf, off, len, &id, NULL) != 0) return -1;

  if (id >= state->cycle_count || state->cycle_table[id] == NULL) return -1;
  *cycle = scamper_cycle_use(state->cycle_table[id]->cycle);

  return 0;
}

int extract_timeval(const uint8_t *buf, uint32_t *off, const uint32_t len,
...
void warts_params_write(uint8_t *buf, uint32_t *off, const uint32_t len,
                        const uint8_t *flags, const uint16_t flags_len,
  ...
  *buf = NULL;
  ...
  if (rf != NULL) {
    if ((ret = rf(scamper_file_getreadparam(sf), buf, len)) == 0 || ret == -2) {
      if (ret == -2) scamper_file_seteof(sf);
      return 0;
    }
    return -1;
  }
  ...
  if ((tmp = malloc_zero(len)) == NULL) return -1;
...
int warts_write(const scamper_file_t *sf, const void *buf, size_t len) {
  ...
  if (wf == NULL) {
    ...
    if (state->isreg && (off = lseek(fd, 0, SEEK_CUR)) == (off_t)-1) return -1;
    ...
    if (write_wrap(fd, buf, NULL, len) != 0) {
       ...
       */
      if (state->isreg) {
        if (ftruncate(fd, off) != 0) return -1;
      }
      return -1;
    }
  } else {
...
int warts_addr_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  if (hdr->len > 2) goto err;

  if ((state->addr_count % WARTS_ADDR_TABLEGROW) == 0) {
    size =
        sizeof(scamper_addr_t *) * (state->addr_count + WARTS_ADDR_TABLEGROW);
  ...
  if (buf == NULL) {
    if (addr_out != NULL) *addr_out = NULL;
    return 0;
  }
   ...
   * think it should be.
   */
  if (state->addr_count % 255 != buf[0]) goto err;

  /* sanity check the type of address */
  if (buf[1] == 0 || buf[1] > SCAMPER_ADDR_TYPE_MAX) goto err;

  /* allocate a scamper address using the record read from disk */
  if ((addr = scamper_addr_alloc(buf[1], buf + 2)) == NULL) goto err;

  state->addr_table[state->addr_count++] = addr;
  free(buf);

  if (addr_out != NULL) *addr_out = addr;
 ...
 *
 */
int warts_list_params_read(scamper_list_t *list, uint8_t *buf, uint32_t *off,
                           uint32_t len) {
...
void warts_list_params_write(const scamper_list_t *list, uint8_t *buf,
...
void warts_cycle_params_write(const scamper_cycle_t *cycle, uint8_t *buf,
                              uint32_t *off, const uint32_t len,
                              const uint8_t *flags, const uint16_t flags_len,
                              const uint16_t params_len) {
  warts_param_writer_t handlers[] = {
...
int warts_cycle_params_read(scamper_cycle_t *cycle, uint8_t *buf, uint32_t *off,
                            uint32_t len) {
  warts_param_reader_t handlers[] = {
...
int warts_cycle_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  if ((state->cycle_count % WARTS_CYCLE_TABLEGROW) == 0) {
    size =
        sizeof(warts_list_t *) * (state->cycle_count + WARTS_CYCLE_TABLEGROW);
 ...
 */
int warts_cycle_stop_write(const scamper_file_t *sf, scamper_cycle_t *cycle) {
  uint32_t wc_id;
  uint8_t *buf = NULL;
  uint32_t off = 0, len;
  uint8_t flag = 0;
...
int warts_icmpext_read(const uint8_t *buf, uint32_t *off, uint32_t len,
  ...
  *off += 2;
  ...
  if (len - *off < tmp) return -1;

  while (tmp >= 4) {
    memcpy(&u16, &buf[*off], 2);
    u16 = ntohs(u16);
...
void warts_icmpext_write(uint8_t *buf, uint32_t *off, const uint32_t len,
  ...
  for (ie = exts; ie != NULL; ie = ie->ie_next) {
    ...
    u16 = htons(ie->ie_dl);
    memcpy(&buf[*off + 2 + tmp], &u16, 2);
    tmp += 2;

    /* write the class num/type fields */
    buf[*off + 2 + tmp] = ie->ie_cn;
    tmp++;
    buf[*off + 2 + tmp] = ie->ie_ct;
    tmp++;
...
int scamper_file_warts_read(scamper_file_t *sf, scamper_file_filter_t *filter,
  ...
  for (;;) {
    ...
    if (state->hdr.type == 0) {
      ...
      if (tmp == -1 || hdr.magic != WARTS_MAGIC || hdr.type == 0) goto err;
    } else {
      hdr = state->hdr;
    }
    ...
    *data = NULL;
    ...
    if (hdr.type == SCAMPER_FILE_OBJ_ADDR ||
      ...
      if (objread[hdr.type](sf, &hdr, &ptr) != 0) goto err;
      ...
      buf = NULL;
      if (warts_read(sf, &buf, hdr.len) != 0) goto err;
      if (buf == NULL) {
        /* partial read.  return for now */
        state->hdr = hdr;
        return 0;
      ...
      if (hdr.type >= sizeof(objread) / sizeof(warts_obj_read_t) ||
          objread[hdr.type] == NULL || objread[hdr.type](sf, &hdr, data) != 0)
        goto err;
...
err:
  fprintf(stderr, "off 0x%s magic 0x%04x type 0x%04x len 0x%08x\n",
...
int scamper_file_warts_init_write(scamper_file_t *sf) {
  ...
  if (fd != -1) {
    ...
    if (S_ISREG(sb.st_mode)) s->isreg = 1;
  }
  ...
  if ((s->cycle_tree = splaytree_alloc((splaytree_cmp_t)warts_cycle_cmp)) ==
      NULL)
    goto err;
  s->cycle_count = 1;
...
int scamper_file_warts_init_append(scamper_file_t *sf) {
  ...
  for (;;) {
    ...
    switch (hdr.type) {
      case SCAMPER_FILE_OBJ_ADDR:
        if (warts_addr_read(sf, &hdr, &addr) != 0 || addr == NULL) return -1;
        break;

      case SCAMPER_FILE_OBJ_LIST:
        if (warts_list_read(sf, &hdr, &list) != 0 || list == NULL) return -1;
        break;

      case SCAMPER_FILE_OBJ_CYCLE_START:
      case SCAMPER_FILE_OBJ_CYCLE_DEF:
        if (warts_cycle_read(sf, &hdr, &cycle) != 0 || cycle == NULL) return -1;
        break;
  ...
  for (j = 1; j < s->list_count; j++)
    if (splaytree_insert(s->list_tree, s->list_table[j]) == NULL) return -1;
  free(s->list_table);
  s->list_table = NULL;
  ...
  for (j = 1; j < s->cycle_count; j++) {
    ...
    if (s->cycle_table[j] == NULL) continue;
    if (splaytree_insert(s->cycle_tree, s->cycle_table[j]) == NULL) return -1;
  }
  free(s->cycle_table);
  s->cycle_table = NULL;
...
void scamper_file_warts_free_state(scamper_file_t *sf) {
  ...
  warts_free_state(state->list_tree, (void **)state->list_table,
                   state->list_count, (splaytree_free_t)warts_list_free);

  warts_free_state(state->cycle_tree, (void **)state->cycle_table,
                   state->cycle_count, (splaytree_free_t)warts_cycle_free);

  if (state->addr_table != NULL) {
    for (i = 1; i < state->addr_count; i++)
      if (state->addr_table[i] != NULL) scamper_addr_free(state->addr_table[i]);
