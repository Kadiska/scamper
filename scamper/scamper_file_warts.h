...
#define WARTS_VAR_MFB(array) \
  ((WARTS_VAR_COUNT(array) / 7) + (WARTS_VAR_COUNT(array) % 7 == 0 ? 0 : 1))
...
typedef int (*wpr_t)(const uint8_t *, uint32_t *, const uint32_t, void *,
                     void *);
typedef void (*wpw_t)(uint8_t *, uint32_t *, const uint32_t, const void *,
                      void *);
