...
static int firewall_rule_cmp(const scamper_firewall_rule_t *a,
  ...
  if (a->type == SCAMPER_FIREWALL_RULE_TYPE_5TUPLE) {
    ...
    if (a->sfw_5tuple_dst == NULL && b->sfw_5tuple_dst == NULL) return 0;
    if (a->sfw_5tuple_dst != NULL && b->sfw_5tuple_dst == NULL) return -1;
    if (a->sfw_5tuple_dst == NULL && b->sfw_5tuple_dst != NULL) return 1;
...
static void firewall_rule_free(scamper_firewall_rule_t *sfw) {
  ...
  if (sfw->sfw_5tuple_src != NULL) scamper_addr_free(sfw->sfw_5tuple_src);
  if (sfw->sfw_5tuple_dst != NULL) scamper_addr_free(sfw->sfw_5tuple_dst);
  free(sfw);

  return;
}

static scamper_firewall_rule_t *firewall_rule_dup(
    scamper_firewall_rule_t *sfw) {
  scamper_firewall_rule_t *dup;

  if ((dup = memdup(sfw, sizeof(scamper_firewall_rule_t))) == NULL) return NULL;

  scamper_addr_use(dup->sfw_5tuple_src);
  if (dup->sfw_5tuple_dst != NULL) scamper_addr_use(dup->sfw_5tuple_dst);
...
static void firewall_entry_free(scamper_firewall_entry_t *entry) {
  if (entry->node != NULL) splaytree_remove_node(entries, entry->node);
...
static int firewall_entries_alloc(void) {
  if ((entries = splaytree_alloc((splaytree_cmp_t)firewall_entry_cmp)) ==
      NULL) {
...
static int ipfw_sysctl_check(void) {
  ...
  if (sysctlbyname(name, &i, &len, NULL, 0) != 0) {
    ...
    if ((osinfo->os_id == SCAMPER_OSINFO_OS_FREEBSD &&
         osinfo->os_rel_dots >= 2 && osinfo->os_rel[0] == 6 &&
         osinfo->os_rel[1] < 3) ||
        (osinfo->os_id == SCAMPER_OSINFO_OS_DARWIN && osinfo->os_rel_dots > 0 &&
         osinfo->os_rel[0] == 8)) {
...
int scamper_firewall_ipfw_init(void) {
  if (fws != -1 || ipfw_sysctl_check() != 0 || ipfw_inited != 0) return -1;
...
int scamper_firewall_ipfw_add(int n, int af, int p, void *s, void *d, int sp,
  ...
  if (d == NULL) {
    if (af == AF_INET) insn->opcode = O_IP_DST_ME;
#if __FreeBSD_version >= 600000
    else if (af == AF_INET6)
      insn->opcode = O_IP6_DST_ME;
#endif
...
#if __FreeBSD_version >= 600000
  if (af == AF_INET6) ipfw_deny_ip6_ext6hdr_frag(n, s, d);
#endif
...
int scamper_firewall_ipfw_init(void) {
  ...
  if (ipfw_have_ipv4 != 0 &&
      (fw4s = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) < 0) {
    printerror(__func__, "could not open socket for ipfw");
    return -1;
  }
  if (ipfw_have_ipv6 != 0 &&
      (fw6s = socket(AF_INET6, SOCK_RAW, IPPROTO_RAW)) < 0) {
...
int scamper_firewall_ipfw_add(int n, int af, int p, void *s, void *d, int sp,
  ...
  if (af == AF_INET) {
    ...
    for (i = 0; i < 4; i++) fw6.fw_smsk.s6_addr32[i] = ~0;
    memcpy(&fw6.fw_dst, d, 16);
    for (i = 0; i < 4; i++) fw6.fw_dmsk.s6_addr32[i] = ~0;
    fw6.fw_pts[0] = sp;
...
static int ipfw_init(char *opts) {
  ...
  if (ipfw_sysctl_check() != 0) return -1;

  if (firewall_entries_alloc() != 0) return -1;

  if (firewall_freeslots_alloc(start, end) != 0) return -1;

#ifdef WITHOUT_PRIVSEP
  if (scamper_firewall_ipfw_init() != 0) return -1;
#else
  if (scamper_privsep_ipfw_init() != 0) return -1;
#endif
...
#ifdef WITHOUT_PRIVSEP
  if (scamper_firewall_ipfw_add(ruleno, af, p, s, d, sp, dp) != 0) return -1;
#else
  if (scamper_privsep_ipfw_add(ruleno, af, p, s, d, sp, dp) != 0) return -1;
#endif
...
int scamper_firewall_pf_init(const char *name) {
  ...
  if (pf_fd != -1 || pf_inited != 0) return -1;
  if ((len = strlen(name)) == 0 || string_isprint(name, len) == 0) return -1;
...
int scamper_firewall_pf_del(int ruleno) {
  ...
  off = 0;
  string_concat(pfte.anchor, sizeof(pfte.anchor), &off, "%s/%d.%d", pf_name,
                pf_pid, ruleno);
...
#ifdef WITHOUT_PRIVSEP
  if (scamper_firewall_pf_add(n, af, p, s, d, sp, dp) != 0) return -1;
#else
  if (scamper_privsep_pf_add(n, af, p, s, d, sp, dp) != 0) return -1;
#endif
  return 0;
}
...
static int pf_init(char *opts) {
  ...
  if (string_isnumber(num_str) == 0 || string_tolong(num_str, &num) != 0 ||
      num < 1 || num > 65535) {
  ...
  if (firewall_freeslots_alloc(1, num) != 0) return -1;

#ifdef WITHOUT_PRIVSEP
  if (scamper_firewall_pf_init(name_str) != 0) return -1;
#else
  if (scamper_privsep_pf_init(name_str) != 0) return -1;
#endif
...
scamper_firewall_entry_t *scamper_firewall_entry_get(
  ...
  if ((sfw->sfw_5tuple_proto != IPPROTO_TCP &&
      ...
      sfw->sfw_5tuple_sport == 0 || sfw->sfw_5tuple_dport == 0 ||
  ...
  if ((entry = heap_remove(freeslots)) == NULL) goto err;
...
#ifdef HAVE_IPFW
  if (ipfw_use != 0) {
    if (ipfw_add(entry->slot, sfw) != 0) goto err;
    return entry;
  }
#endif
...
#ifdef HAVE_PF
  if (pf_use != 0) {
    if (pf_add(entry->slot, sfw) != 0) goto err;
    return entry;
  }
#endif
...
err:
  if (entry != NULL) {
    if (entry->rule != NULL) firewall_rule_free(entry->rule);
    free(entry);
  }
...
#if defined(HAVE_PF)
  if (pf_use != 0) {
    if (pf_del(entry->slot) != 0) return -1;
    goto done;
  }
#endif
...
void scamper_firewall_entry_free(scamper_firewall_entry_t *entry) {
   ...
   */
  if (entry->slot >= 0) firewall_rule_delete(entry);
#endif
  return;
}

#if defined(HAVE_IPFW) || defined(HAVE_PF)
static int firewall_entry_cleanup(void *param,
                                  scamper_firewall_entry_t *entry) {
...
void scamper_firewall_cleanup(void) {
  ...
  if (freeslots != NULL) {
    while ((entry = heap_remove(freeslots)) != NULL) firewall_entry_free(entry);
...
#else
    if (ipfw_inited != 0) scamper_privsep_ipfw_cleanup();
#endif
  }
#endif
...
#else
    if (pf_inited != 0) scamper_privsep_pf_cleanup();
#endif
  }
#endif
