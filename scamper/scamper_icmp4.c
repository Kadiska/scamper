...
void scamper_icmp4_read_cb(const int fd, void *param) {
  ...
  if (scamper_icmp4_recv(fd, &ir) == 0) scamper_icmp_resp_handle(&ir);
...
void scamper_icmp4_read_err_cb(const int fd, void *param) {
  ...
  scamper_icmp_resp_t ir;
  memset(&ir, 0, sizeof(ir));
  if (scamper_icmp4_recv_err(fd, &ir) == 0) scamper_icmp_resp_handle(&ir);
  scamper_icmp_resp_clean(&ir);
#endif
  return;
}
...
#else
int scamper_icmp4_open_err(const void *addr) { return -1; }
#endif
