 ...
 * scamper_icmp_resp.c
 *
 * $Id: scamper_icmp_resp.c,v 1.34 2020/03/17 07:32:16 mjl Exp $
 *
...
void scamper_icmp_resp_print(const scamper_icmp_resp_t *ir) {
  ...
  if (ir->ir_af == AF_INET) {
    ...
    switch (ir->ir_icmp_type) {
      ...
      case ICMP_ECHOREPLY:
        t = "echo reply";
        snprintf(cbuf, sizeof(cbuf), "id %d seq %d", ir->ir_icmp_id,
                 ir->ir_icmp_seq);
        c = cbuf;
        break;
  ...
  } else /* if(ir->ir_af == AF_INET6) */
  {
    ...
    snprintf(ip, sizeof(ip), "from %s size %d hlim %d", addr, ir->ir_ip_size,
             ir->ir_ip_hlim);
  ...
  if (t == NULL) {
    snprintf(icmp, sizeof(icmp), "icmp %d code %d", ir->ir_icmp_type,
             ir->ir_icmp_code);
  } else if (c == NULL) {
  ...
  if (ir->ir_flags & SCAMPER_ICMP_RESP_FLAG_INNER_IP) {
    ...
    } else /* if(ir->ir_af == AF_INET6) */
    {
      ...
      snprintf(inner_ip, sizeof(inner_ip), " to %s size %d hlim %d flow 0x%05x",
               addr, ir->ir_inner_ip_size, ir->ir_inner_ip_hlim,
               ir->ir_inner_ip_flow);
    }
...
int scamper_icmp_resp_inner_dst(scamper_icmp_resp_t *resp,
                                scamper_addr_t *addr) {
  if (resp->ir_af == AF_INET) {
    addr->type = SCAMPER_ADDR_TYPE_IPV4;
...
void scamper_icmp_resp_clean(scamper_icmp_resp_t *ir) {
  if (ir->ir_ext != NULL) free(ir->ir_ext);

  if (ir->ir_ipopt_rrs != NULL) free(ir->ir_ipopt_rrs);

  if (ir->ir_ipopt_tsips != NULL) free(ir->ir_ipopt_tsips);

  if (ir->ir_ipopt_tstss != NULL) free(ir->ir_ipopt_tstss);

  if (ir->ir_inner_ipopt_rrs != NULL) free(ir->ir_inner_ipopt_rrs);

  if (ir->ir_inner_ipopt_tsips != NULL) free(ir->ir_inner_ipopt_tsips);

  if (ir->ir_inner_ipopt_tstss != NULL) free(ir->ir_inner_ipopt_tstss);
...
void scamper_icmp_resp_handle(scamper_icmp_resp_t *resp) {
  ...
  if (SCAMPER_ICMP_RESP_IS_TTL_EXP(resp) ||
    ...
    if (!SCAMPER_ICMP_RESP_INNER_IS_SET(resp)) return;
    if (scamper_icmp_resp_inner_dst(resp, &addr) != 0) return;
    ...
    if (scamper_icmp_resp_src(resp, &addr) != 0) return;
  } else {
    return;
  }
