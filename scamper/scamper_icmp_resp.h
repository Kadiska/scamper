...
#define SCAMPER_ICMP_RESP_INNER_IS_TCP(ir) ((ir->ir_inner_ip_proto == 6))

#define SCAMPER_ICMP_RESP_INNER_IS_UDP(ir) ((ir->ir_inner_ip_proto == 17))

#define SCAMPER_ICMP_RESP_INNER_IS_ICMP_ECHO_REQ(ir)        \
  ((ir->ir_af == AF_INET && ir->ir_inner_ip_proto == 1 &&   \
    ir->ir_inner_icmp_type == 8) ||                         \
   (ir->ir_af == AF_INET6 && ir->ir_inner_ip_proto == 58 && \
    ir->ir_inner_icmp_type == 128))

#define SCAMPER_ICMP_RESP_INNER_IS_ICMP_TIME_REQ(ir)     \
  (ir->ir_af == AF_INET && ir->ir_inner_ip_proto == 1 && \
   ir->ir_inner_icmp_type == 13)
