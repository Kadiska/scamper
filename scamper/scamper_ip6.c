...
int scamper_ip6_build(scamper_probe_t *probe, uint8_t *buf, size_t *len) {
  ...
  if (off <= *len) {
    ...
    ip6->ip6_flow =
        htonl(0x6 << 28 | probe->pr_ip_tos << 20 | probe->pr_ip_flow);
   ...
   */
  if (off <= *len) {
    if (nxthdrval[probe->pr_ipopts[0].type] == -1) return -1;
  ...
  for (i = 0; i < probe->pr_ipoptc; i++) {
    if (off + 1 < *len) {
      ...
      if (i == probe->pr_ipoptc - 1) {
        ...
        if (nxthdrval[probe->pr_ipopts[i + 1].type] == -1) return -1;
...
int scamper_ip6_frag_build(scamper_probe_t *probe, uint8_t *buf, size_t *len) {
  ...
  if (probe->pr_len != 0) memcpy(buf + ip6hlen, probe->pr_data, probe->pr_len);
  *len = req;
