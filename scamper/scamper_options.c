...
static int opt_add(scamper_option_out_t **head, scamper_option_out_t **tail,
  ...
  if (*tail == NULL) {
    assert(*head == NULL);
    if ((*head = malloc_zero(sizeof(scamper_option_out_t))) == NULL) return -1;
    *tail = *head;
  } else {
...
static int opt_parse_param(int type, char **str, char **next) {
  ...
  if (type == SCAMPER_OPTION_TYPE_NUM) {
    ...
    if (*tmp != '\0' && isspace((int)*tmp) == 0) goto err;
...
int scamper_options_c2id(const scamper_option_in_t *opts, const int cnt,
  ...
  int i;
  for (i = 0; i < cnt; i++) {
    if (c == opts[i].c) return opts[i].id;
  }
  return -1;
}
  ...
  do {
    ...
    if (str[1] == '-') {
      ...
      if (i != cnt) {
        ...
        goto err; /* no match for this long option */
    } else {
...
int scamper_options_validate(const scamper_option_in_t *opts, const int cnt,
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (needp == 1) {
      ...
      if (validate(optid, argv[i], NULL) != 0) goto err;
    }
  }
