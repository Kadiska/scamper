...
const scamper_osinfo_t *scamper_osinfo_get(void) { return osinfo; }
...
#ifdef _WIN32
int scamper_osinfo_init(void) {
  if ((osinfo = malloc_zero(sizeof(scamper_osinfo_t))) == NULL) goto err;
  if ((osinfo->os = strdup("Windows")) == NULL) goto err;
