...
static int privsep_open_icmp(uint16_t plen, const uint8_t *param) {
  ...
  if (type == AF_INET) return scamper_icmp4_open_fd();
  if (type == AF_INET6) return scamper_icmp6_open_fd();
...
static int privsep_open_sock(uint16_t plen, const uint8_t *param) {
  ...
  off = 0;
  memcpy(&domain, param + off, sizeof(domain));
  off += sizeof(domain);
  memcpy(&protocol, param + off, sizeof(protocol));
  off += sizeof(protocol);
  memcpy(&port, param + off, sizeof(port));
  off += sizeof(port);

  if (off != plen) goto inval;
...
static int privsep_ipfw_add(uint16_t plen, const uint8_t *param) {
  ...
  memcpy(&af, param + off, sizeof(af));
  off += sizeof(af);
  if (af == AF_INET) {
    s = &s4;
    al = 4;
    if (df == 0) {
      if (plen != 1 + (sizeof(int) * 5) + sizeof(struct in_addr)) goto inval;
    } else {
    ...
    al = 16;
    if (df == 0) {
      if (plen != 1 + (sizeof(int) * 5) + sizeof(struct in6_addr)) goto inval;
    } else {
  ...
  memcpy(&n, param + off, sizeof(n));
  off += sizeof(n);
  memcpy(&p, param + off, sizeof(p));
  off += sizeof(p);
  memcpy(s, param + off, al);
  off += al;
  ...
  memcpy(&sp, param + off, sizeof(sp));
  off += sizeof(sp);
  memcpy(&dp, param + off, sizeof(dp));
  off += sizeof(dp);

  if (off != plen) goto inval;
  if (sp < 0 || sp > 65535 || dp < 0 || dp > 65535) goto inval;
  if (p != IPPROTO_TCP && p != IPPROTO_UDP) goto inval;
...
static int privsep_pf_add(uint16_t plen, const uint8_t *param) {
  ...
  off += sizeof(af);
  if (af == AF_INET) {
    if (plen != (sizeof(int) * 5) + (sizeof(struct in_addr) * 2)) goto inval;
    s = &s4;
    d = &d4;
    al = 4;
  } else if (af == AF_INET6) {
    if (plen != (sizeof(int) * 5) + (sizeof(struct in6_addr) * 2)) goto inval;
    s = &s6;
    d = &d6;
    al = 16;
  } else
...
static int privsep_unlink(uint16_t plen, const uint8_t *param) {
  ...
  if (plen < 2 || name[plen - 1] != '\0') return -1;

  uid = getuid();
  euid = geteuid();
  if (seteuid(uid) != 0) return -1;
  unlink(name);
  if (seteuid(euid) != 0) exit(-errno);
...
static int privsep_open_unix(uint16_t plen, const uint8_t *param) {
  ...
  if (sockaddr_compose_un((struct sockaddr *)&sn, name) != 0) return -1;
...
static int privsep_open_file(uint16_t plen, const uint8_t *param) {
  ...
  if (flags & O_CREAT) {
     ...
     */
    if (plen < off + sizeof(mode_t) + 2) return -1;
...
static int privsep_send_rc(int rc, int error, uint8_t msg_type) {
  ...
  scamper_debug(__func__, "rc: %d, error: %d, msg_type: 0x%02x", rc, error,
                msg_type);
  ...
  if (sendmsg(root_fd, &msg, 0) == -1) return -1;
...
static int privsep_send_fd(int fd, int error, uint8_t msg_type) {
  ...
  scamper_debug(__func__, "fd: %d error: %d msg_type: 0x%02x", fd, error,
                msg_type);
...
static int privsep_do(void) {
  ...
  for (;;) {
    ...
    if ((ret = read_wrap(root_fd, (uint8_t *)&msg, NULL, sizeof(msg))) != 0) {
      if (ret == -1) printerror(__func__, "could not read msg hdr");
      break;
    }

    /* if we've been told to exit, then do so now */
    if (msg.type == SCAMPER_PRIVSEP_EXIT) break;
    ...
    if (funcs[msg.type].txfunc(fd, error, msg.type) != 0) break;

    if (funcs[msg.type].txfunc == privsep_send_fd && fd != -1) close(fd);
  }
...
static int privsep_dotask(uint16_t type, uint16_t len, const uint8_t *param) {
  ...
  if (privsep_lame_send(type, len, param) == -1) return -1;

  if (read_wrap(lame_fd, buf, NULL, sizeof(buf)) == -1) return -1;
...
static int privsep_getfd_3int(const uint16_t type, const int p1, const int p2,
                              const int p3) {
  uint8_t param[sizeof(p1) + sizeof(p2) + sizeof(p3)];
  size_t off = 0;
  memcpy(param + off, &p1, sizeof(p1));
  off += sizeof(p1);
  memcpy(param + off, &p2, sizeof(p2));
  off += sizeof(p2);
...
int scamper_privsep_open_file(const char *file, const int flags,
  ...
  if (flags & O_CREAT) len += sizeof(mode);
  ...
  if ((param = malloc_zero(len)) == NULL) return -1;

  /* copy in the flags parameter, and the mode parameter if necessary */
  memcpy(param, &flags, sizeof(flags));
  off = sizeof(flags);
  if (flags & O_CREAT) {
...
int scamper_privsep_open_tcp(const int domain, const int port) {
  return privsep_getfd_3int(SCAMPER_PRIVSEP_OPEN_SOCK, domain, IPPROTO_TCP,
                            port);
}

int scamper_privsep_open_udp(const int domain, const int port) {
  return privsep_getfd_3int(SCAMPER_PRIVSEP_OPEN_SOCK, domain, IPPROTO_UDP,
                            port);
}
...
int scamper_privsep_ipfw_add(int n, int af, int p, void *s, void *d, int sp,
  ...
  memcpy(param + len, &af, sizeof(af));
  len += sizeof(af);
  memcpy(param + len, &n, sizeof(n));
  len += sizeof(n);
  memcpy(param + len, &p, sizeof(p));
  len += sizeof(p);
  memcpy(param + len, s, al);
  len += al;
  if (d != NULL) {
    memcpy(param + len, d, al);
    len += al;
  }
  memcpy(param + len, &sp, sizeof(sp));
  len += sizeof(sp);
  memcpy(param + len, &dp, sizeof(dp));
  len += sizeof(dp);
...
int scamper_privsep_ipfw_del(int n, int af) {
  ...
  uint16_t len = 0;
  memcpy(param + len, &n, sizeof(int));
  len += sizeof(n);
  memcpy(param + len, &af, sizeof(int));
  len += sizeof(af);
