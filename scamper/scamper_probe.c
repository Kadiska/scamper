...
static char *tcp_flags(char *buf, size_t len, scamper_probe_t *probe) {
  ...
  buf[0] = '\0';
  if (probe->pr_len != 0) flags &= ~(TH_ACK);
...
static char *tcp_pos(char *buf, size_t len, scamper_probe_t *probe) {
  ...
  if (probe->pr_len > 0) string_concat(buf, len, &off, "(%u)", probe->pr_len);
  return buf;
}
...
static void probe_print(scamper_probe_t *probe) {
  ...
  if (probe->pr_ip_dst->type == SCAMPER_ADDR_TYPE_IPV4) {
    if (scamper_ip4_hlen(probe, &iphl) != 0) return;

    if ((probe->pr_ip_off & IP_OFFMASK) != 0) {
      scamper_debug("tx", "frag %s %04x:%d ttl %d, len %d", addr,
                    probe->pr_ip_id, probe->pr_ip_off << 3, probe->pr_ip_ttl,
                    iphl + probe->pr_len);
      return;
    }
    ...
    switch (probe->pr_ip_proto) {
      case IPPROTO_UDP:
        scamper_debug("tx", "udp %s, ttl %d, %d:%d, len %d", addr,
                      probe->pr_ip_ttl, probe->pr_udp_sport,
                      probe->pr_udp_dport, iphl + 8 + probe->pr_len);
        break;

      case IPPROTO_TCP:
        scamper_debug("tx", "tcp %s%s, ttl %d, %d:%d%s, ipid %04x, %s, len %d",
                      addr, tos, probe->pr_ip_ttl, probe->pr_tcp_sport,
                      probe->pr_tcp_dport, tcp_flags(tcp, sizeof(tcp), probe),
      ...
      case IPPROTO_ICMP:
        if (probe->pr_icmp_type == ICMP_ECHO) {
          ...
          scamper_debug("tx", "icmp %s echo, ttl %d%s, seq %d, len %d", addr,
                        probe->pr_ip_ttl, icmp, probe->pr_icmp_seq,
                        iphl + 8 + probe->pr_len);
          ...
          scamper_debug("tx", "icmp %s %s, len %d", addr, icmp,
                        iphl + 8 + probe->pr_len);
        } else if (probe->pr_icmp_type == ICMP_TSTAMP) {
          scamper_debug("tx", "icmp %s ts, ttl %d, seq %d, len %d", addr,
                        probe->pr_ip_ttl, probe->pr_icmp_seq, iphl + 20);
        } else {
          scamper_debug("tx", "icmp %s type %d, code %d, len %d", addr,
                        probe->pr_icmp_type, probe->pr_icmp_code,
                        iphl + 8 + probe->pr_len);
        }
        break;
    }
  } else if (probe->pr_ip_dst->type == SCAMPER_ADDR_TYPE_IPV6) {
    if (scamper_ip6_hlen(probe, &iphl) != 0) return;

    if (probe->pr_ip_off != 0) {
      scamper_debug("tx", "frag %s off %04x, ttl %d, len %d", addr,
                    probe->pr_ip_off, probe->pr_ip_ttl, iphl + probe->pr_len);
      return;
    }
    ...
    switch (probe->pr_ip_proto) {
      ...
      case IPPROTO_TCP:
        scamper_debug("tx", "tcp %s%s, ttl %d, %d:%d%s, %s, len %d", addr, tos,
                      probe->pr_ip_ttl, probe->pr_tcp_sport,
                      probe->pr_tcp_dport, tcp_flags(tcp, sizeof(tcp), probe),
...
static scamper_probe_t *probe_dup(scamper_probe_t *pr) {
  ...
  if ((pd = memdup(pr, sizeof(scamper_probe_t))) == NULL) goto err;
  ...
  pd->pr_ipopts = NULL;
  pd->pr_data = NULL;
  if (pr->pr_ip_src != NULL) pd->pr_ip_src = scamper_addr_use(pr->pr_ip_src);
  if (pr->pr_ip_dst != NULL) pd->pr_ip_dst = scamper_addr_use(pr->pr_ip_dst);
  if (pr->pr_rtr != NULL) pd->pr_rtr = scamper_addr_use(pr->pr_rtr);

  if (pr->pr_ipoptc > 0) {
    len = pr->pr_ipoptc * sizeof(scamper_probe_ipopt_t);
    if ((pd->pr_ipopts = memdup(pr->pr_ipopts, len)) == NULL) goto err;
  }

  if (pr->pr_len > 0) {
    if ((pd->pr_data = memdup(pr->pr_data, pr->pr_len)) == NULL) goto err;
  }
...
static void probe_state_free_cb(void *vpt) {
  ...
  if (pt == NULL) return;
  if (pt->pr != NULL) probe_free(pt->pr);
  if (pt->buf != NULL && pt->buf != pktbuf) free(pt->buf);
  if (pt->tx_rt != NULL) scamper_route_free(pt->tx_rt);
  if (pt->rx_rt != NULL) scamper_route_free(pt->rx_rt);
  if (pt->dlhdr != NULL) scamper_dlhdr_free(pt->dlhdr);
  if (pt->dst != NULL) scamper_addr_free(pt->dst);
  free(pt);
  return;
}
...
static void probe_state_free(probe_state_t *pt) {
  if (pt == NULL) return;
  if (pt->anc != NULL) scamper_task_anc_del(pt->task, pt->anc);
...
static probe_state_t *probe_state_alloc(scamper_probe_t *pr) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(pr->pr_ip_dst)) {
    ...
    if (pr->pr_ip_off != 0) build_func = scamper_ip6_frag_build;
...
static int probe_dl_tx(probe_state_t *pt) {
  ...
  if (pt->dlhdr->len > 0) memcpy(pkt, pt->dlhdr->buf, pt->dlhdr->len);
  ...
  if (pt->buf == pktbuf) gettimeofday_wrap(&pt->tv);
...
static void probe_rx_rt_cb(scamper_route_t *rt) {
  ...
  if (probe_dl_tx(pt) != 0) goto err;

  if (pt->anc != NULL) probe_state_free(pt);
  return;
...
static int probe_task_dl(scamper_probe_t *pr, scamper_task_t *task) {
  ...
  if (pt->mode != PROBE_MODE_TX) {
    ...
    if ((pt->anc = scamper_task_anc_add(task, pt, probe_state_free_cb)) ==
        NULL) {
      pr->pr_errno = errno;
      goto err;
    }
...
int scamper_probe_task(scamper_probe_t *pr, scamper_task_t *task) {
   ...
   */
  if (pr->pr_ip_proto == IPPROTO_TCP || pr->pr_rtr != NULL ||
  ...
  if (dl != 0) {
    if (probe_task_dl(pr, task) != 0) goto err;
  } else if (SCAMPER_ADDR_TYPE_IS_IPV4(pr->pr_ip_dst)) {
    if (probe_task_ipv4(pr, task, icmp) != 0) goto err;
  } else if (SCAMPER_ADDR_TYPE_IS_IPV6(pr->pr_ip_dst)) {
    if (probe_task_ipv6(pr, task, icmp) != 0) goto err;
  } else {
    pr->pr_errno = EINVAL;
    goto err;
  }
...
int scamper_probe(scamper_probe_t *probe) {
  ...
  if (probe->pr_ip_dst->type == SCAMPER_ADDR_TYPE_IPV4) {
    if ((probe->pr_ip_off & IP_OFFMASK) != 0) {
      ...
      build_func = scamper_tcp4_build;
      if (probe->pr_fd != -1) send_func = scamper_tcp4_probe;
  ...
  if (probe->pr_dl == NULL) {
    if (send_func != NULL) return send_func(probe);
...
int scamper_probe_init(void) {
  ...
  if (SCAMPER_OSINFO_IS_SUNOS(osinfo)) ipid_dl = 1;
  if (scamper_option_planetlab() || scamper_option_rawtcp()) rawtcp = 1;
  return 0;
}
