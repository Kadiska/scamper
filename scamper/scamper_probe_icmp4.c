...
void scamper_probe_icmp4_read_cb(void *fd_, void *param) {
  ...
  if (scamper_probe_icmp4_recv(fd, &ir) == 0) scamper_icmp_resp_handle(&ir);
...
void scamper_probe_icmp4_read_err_cb(void *fd_, void *param) {
  ...
  scamper_icmp_resp_t ir;
  memset(&ir, 0, sizeof(ir));
  if (scamper_probe_icmp4_recv_err(fd, &ir) == 0) scamper_icmp_resp_handle(&ir);
  scamper_icmp_resp_clean(&ir);
#endif
  return;
}
...
#else
int scamper_probe_icmp4_open_err(const void *addr) { return -1; }
#endif
...
int scamper_probe_icmp4_open(const void *addr) {
  ...
  filter.data = ~((1 << ICMP_DEST_UNREACH) | (1 << ICMP_TIME_EXCEEDED) |
                  (1 << ICMP_ECHOREPLY) | (1 << ICMP_TSTAMPREPLY) |
                  (1 << ICMP_PARAMPROB));
