/*
 * scamper_probe_icmp6.c
 *
 * $Id: scamper_probe_icmp6.c,v 1.100 2020/03/17 07:32:16 mjl Exp $
 *
...
int scamper_probe_icmp6_build(scamper_probe_t *probe, uint8_t *buf,
  ...
  if (req <= *len) {
    ...
    icmp->icmp6_cksum = scamper_probe_icmp6_cksum(probe);
...
int scamper_probe_icmp6_recv(int fd, scamper_icmp_resp_t *resp) {
  ...
  if ((pbuflen = recvfrom(fd, rxbuf, sizeof(rxbuf), 0, (struct sockaddr *)&from,
                          &fromlen)) < 0) {
...
#else
    icmp6_recv_ip_outer(fd, resp, icmp, &from,
                        pbuflen + sizeof(struct ip6_hdr));
#endif
...
void scamper_probe_icmp6_read_cb(void *fd_, void *param) {
  int fd = *((int *)fd_);
  scamper_icmp_resp_t ir;

  memset(&ir, 0, sizeof(ir));

  if (scamper_probe_icmp6_recv(fd, &ir) == 0) scamper_icmp_resp_handle(&ir);
...
void scamper_probe_icmp6_cleanup() {
  if (txbuf != NULL) {
    free(txbuf);
    txbuf = NULL;
  }
...
int scamper_probe_icmp6_open(const void *addr) {
  ...
  fd = scamper_probe_icmp6_open_fd();
#else
  ...
  opt = 1;
  if (setsockopt(fd, IPPROTO_IPV6, IPV6_DONTFRAG, (char *)&opt, sizeof(opt)) ==
      -1) {
  ...
  opt = 1;
  if (setsockopt(fd, IPPROTO_IPV6, IPV6_RECVHOPLIMIT, &opt, sizeof(opt)) ==
      -1) {
    printerror(__func__, "could not set IPV6_RECVHOPLIMIT");
  }
#elif defined(IPV6_HOPLIMIT)
  opt = 1;
  if (setsockopt(fd, IPPROTO_IPV6, IPV6_HOPLIMIT, (char *)&opt, sizeof(opt)) ==
      -1) {
...
err:
  if (fd != -1) scamper_probe_icmp6_close(fd);
  return -1;
}
