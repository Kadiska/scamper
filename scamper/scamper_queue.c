...
int scamper_queue_isprobe(scamper_queue_t *sq) {
  if (sq->queue == probe_queue) return 1;
  return 0;
}
...
int scamper_queue_iswait(scamper_queue_t *sq) {
  if (sq->queue == wait_queue) return 1;
  return 0;
}
...
int scamper_queue_isdone(scamper_queue_t *sq) {
  if (sq->queue == done_queue) return 1;
  return 0;
}
...
struct scamper_task *scamper_queue_getdone(const struct timeval *tv) {
  ...
  if ((sq = (scamper_queue_t *)heap_head_item(done_queue)) == NULL) return NULL;
...
int scamper_queue_readycount() {
  ...
  if (heap_count(wait_queue) > 0) {
    ...
    while ((sq = heap_head_item(wait_queue)) != NULL) {
      if (timeval_cmp(&tv, &sq->timeout) < 0) break;

      queue_unlink(sq);

      scamper_task_handletimeout(sq->un.task);

      if (sq->queue == NULL) queue_link(sq, probe_queue);
    }
  }
...
void scamper_queue_empty() {
  ...
  while ((sq = (scamper_queue_t *)heap_remove(wait_queue)) != NULL) count--;

  while ((sq = (scamper_queue_t *)dlist_head_pop(probe_queue)) != NULL) count--;

  return;
}

int scamper_queue_count() { return count; }

scamper_queue_t *scamper_queue_alloc(scamper_task_t *task) {
  scamper_queue_t *sq;
  if ((sq = malloc_zero(sizeof(scamper_queue_t))) != NULL) sq->un.task = task;
  return sq;
}
