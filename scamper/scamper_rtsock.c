...
static rtsock_pair_t *rtsock_pair_alloc(scamper_route_t *route, int seq) {
  rtsock_pair_t *pair;
  if ((pair = malloc_zero(sizeof(rtsock_pair_t))) == NULL) return NULL;
  pair->route = route;
  pair->seq = seq;
...
static void rtsock_pair_free(rtsock_pair_t *pair) {
  ...
  if (pair->node != NULL) dlist_node_pop(pairs, pair->node);
  free(pair);
  return;
}
...
size_t scamper_rtsock_roundup(size_t len) {
  ...
  if (broken == -1) {
    ...
    if (osinfo->os_id == SCAMPER_OSINFO_OS_DARWIN && osinfo->os_rel_dots > 0 &&
        osinfo->os_rel[0] >= 10)
      broken = 1;
    else
      broken = 0;
...
static int scamper_rtsock_getifindex(int fd, scamper_addr_t *dst) {
  ...
  if ((slen = sockaddr_len((struct sockaddr *)&sas)) <= 0) return -1;
...
int scamper_rtsock_getroute(scamper_fd_t *fdn, scamper_route_t *route) {
  ...
  if ((fd = scamper_fd_fd_get(fdn)) == -1) return -1;

  /* ask the question */
  if (scamper_rtsock_getifindex(fd, route->dst) != 0) return -1;

  /* keep track of the question */
  if (rtsock_pair_alloc(route, seq++) == NULL) return -1;
  return 0;
}
...
static void rtsock_parsemsg(uint8_t *buf, size_t len) {
  ...
  if (nlmsg->nlmsg_pid != pid) return;

  if ((pair = rtsock_pair_get(nlmsg->nlmsg_seq)) == NULL) return;
  route = pair->route;
...
static void rtsock_parsemsg(uint8_t *buf, size_t len) {
  ...
  while (x < len) {
    ...
    if (rtm->rtm_pid != pid || rtm->rtm_msglen > len - x ||
        rtm->rtm_type != RTM_GET || (rtm->rtm_flags & RTF_DONE) == 0 ||
...
void scamper_rtsock_read_cb(void *fd_, void *param) {
  ...
  if (len > 0) rtsock_parsemsg(buf, len);
...
static int scamper_rtsock_getroute4(scamper_route_t *route) {
  ...
  if ((dw = GetBestRoute2(NULL, BestIfIndex, NULL, &address, 0, &fw,
                          &BestSourceAddress)) != NO_ERROR) {
    route->error = dw;
    return -1;
  }
...
static int scamper_rtsock_getroute6(scamper_route_t *route) {
  ...
  if ((memcmp(&fw.NextHop.Ipv6.sin6_addr, empty,
              sizeof(fw.NextHop.Ipv6.sin6_addr))) != 0) {
    if ((route->gw = scamper_addrcache_get_ipv6(
             addrcache, &(fw.NextHop.Ipv6.sin6_addr))) == NULL) {
      route->error = errno;
      return -1;
    }
  }
...
#ifndef _WIN32
  if (route->internal != NULL) rtsock_pair_free(route->internal);
#endif
  if (route->dst != NULL) scamper_addr_free(route->dst);
  if (route->gw != NULL) scamper_addr_free(route->gw);
  free(route);
  return;
}
...
scamper_route_t *scamper_route_alloc(scamper_addr_t *dst, void *param,
  ...
  scamper_route_t *route;
  if ((route = malloc_zero(sizeof(scamper_route_t))) == NULL) return NULL;
