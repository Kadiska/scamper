...
#ifdef O_NONBLOCK
  if (fcntl_set(fd, O_NONBLOCK) == -1) goto err;
#endif
...
static int ssf_read_line(void *param, uint8_t *buf, size_t len) {
  ...
  if (string_isprint(str, len) == 0) goto err;
  ...
  for (;;) {
    ...
    bits[bitc++] = str;
    while (isdigit((int)*str) != 0 || *str == '.') str++;
  ...
  if (scamper_source_command(source, cb) != 0) goto err;
