...
static int source_assert(void *item, void *param) {
  ...
  assert(s->refcnt > 0);
  if (s->list != NULL) assert(s->list->refcnt > 0);
  if (s->cycle != NULL) assert(s->cycle->refcnt > 0);
  if (s->sof != NULL) assert(scamper_outfile_getrefcnt(s->sof) > 0);
...
static scamper_sourcetask_t *sourcetask_alloc(scamper_source_t *source,
  ...
  if ((st = malloc_zero(sizeof(scamper_sourcetask_t))) == NULL) goto err;
  if ((st->node = dlist_tail_push(source->tasks, st)) == NULL) goto err;
...
static int command_cycle(scamper_source_t *source, scamper_cycle_t *cycle) {
  ...
  if ((command = command_alloc(COMMAND_CYCLE)) == NULL) goto err;
  command->un.cycle = cycle;
  if (dlist_tail_push(source->commands, command) == NULL) goto err;
  source->cycle_points++;
...
static int source_active_attach(scamper_source_t *source) {
  if (source->list_ == active) return 0;

  if (source->list_ == finished) return -1;

  if (source->list_ == blocked) {
    /* if the source has a zero priority, it must remain blocked */
    if (source->priority == 0) return 0;
    source_blocked_detach(source);
  }

  if ((source->list_node = clist_tail_push(active, source)) == NULL) return -1;
  source->list_ = active;
...
static int source_blocked_attach(scamper_source_t *source) {
  if (source->list_ == blocked) return 0;

  if (source->list_ == finished) return -1;

  if (source->list_node != NULL) source_active_detach(source);

  if ((source->list_node = dlist_tail_push(blocked, source)) == NULL) return -1;
...
static int source_finished_attach(scamper_source_t *source) {
  if (source->list_ == finished) return 0;
...
static void source_command_unhold(void *cookie) {
  ...
  free(onhold);
  if ((cmd = command_alloc(COMMAND_TASK)) == NULL) goto err;
  cmd->un.sourcetask = st;
  st = NULL;
  if (dlist_head_push(source->commands, cmd) == NULL) goto err;
...
static int source_command_onhold(scamper_source_t *source,
  ...
  if ((onhold = malloc_zero(sizeof(command_onhold_t))) == NULL ||
      ...
      (onhold->cookie =
           scamper_task_onhold(block, onhold, source_command_unhold)) == NULL) {
    goto err;
  }
...
static int source_task_install(scamper_source_t *source,
  ...
  if ((block = scamper_task_sig_block(task)) == NULL) {
    if (scamper_task_sig_install(task) != 0) return -1;
    *out = task;
  } else {
    if (source_command_onhold(source, block, st) != 0) return -1;
    *out = NULL;
  }
...
static int command_probe_handle(scamper_source_t *source, command_t *command,
   ...
   */
  if ((st = sourcetask_alloc(source, task)) == NULL) goto err;
  task = NULL;
  scamper_task_setsourcetask(st->task, st);

  if (source_task_install(source, st, task_out) != 0) goto err;
...
static void source_cycle_finish(scamper_cycle_t *cycle,
  ...
  if (source != NULL) source->cycle_points--;
...
static int source_cycle(scamper_source_t *source, uint32_t cycle_id) {
  ...
  if (source->cyclemon != NULL) scamper_cyclemon_unuse(source->cyclemon);
...
static void source_flush_commands(scamper_source_t *source) {
  ...
  if (source->data != NULL) source->freedata(source->data);
...
static void source_detach(scamper_source_t *source) {
  ...
  if (source->tree_node != NULL) {
    ...
    if (source_refcnt_dec(source) == 0) source_free(source);
  }
...
int scamper_source_isfinished(scamper_source_t *source) {
  ...
  if (source->commands != NULL && dlist_count(source->commands) > 0) return 0;

  /* if there are commands that are on hold, the source cannot be finished */
  if (source->onhold != NULL && dlist_count(source->onhold) > 0) return 0;

  /* if there are still tasks underway, the source is not finished */
  if (source->tasks != NULL && dlist_count(source->tasks) > 0) return 0;
...
static void source_free(scamper_source_t *source) {
  ...
  if (source->commands != NULL) source_flush_commands(source);

  /* empty the source of tasks */
  if (source->tasks != NULL) source_flush_tasks(source);
...
char *scamper_source_tostr(const scamper_source_t *src, char *buf, size_t len) {
  ...
  if (src->list == NULL || src->list->name == NULL) return NULL;
...
int scamper_source_getcommandcount(const scamper_source_t *source) {
  if (source->commands != NULL) return dlist_count(source->commands);
  return -1;
}
...
int scamper_source_gettaskcount(const scamper_source_t *source) {
  if (source->tasks != NULL) return dlist_count(source->tasks);
  return -1;
}
...
int scamper_source_halttask(scamper_source_t *source, uint32_t id) {
  ...
  fm.id = id;
  if (source->idtree == NULL) return -1;
  if ((st = splaytree_find(source->idtree, &fm)) == NULL) return -1;

  scamper_task_halt(st->task);

  for (no = dlist_head_node(source->commands); no != NULL;
       no = dlist_node_next(no)) {
...
int scamper_source_command2(scamper_source_t *s, const char *command,
  ...
  if ((data = command_func_allocdata(f, command)) == NULL) goto err;
  if ((task = f->alloctask(data, s->list, s->cycle)) == NULL) goto err;
  data = NULL;
   ...
   * pass the cyclemon structure to the task
   */
  if ((st = sourcetask_alloc(s, task)) == NULL) goto err;
...
int scamper_source_command(scamper_source_t *source, const char *command) {
  ...
  if ((func = command_func_get(command)) == NULL) goto err;
  if ((data = command_func_allocdata(func, command)) == NULL) goto err;

  if ((cmd = command_alloc(COMMAND_PROBE)) == NULL) goto err;
  cmd->un.pr.funcs = func;
  cmd->un.pr.data = data;
  cmd->un.pr.cyclemon = scamper_cyclemon_use(source->cyclemon);

  if (dlist_tail_push(source->commands, cmd) == NULL) goto err;
...
void scamper_sourcetask_free(scamper_sourcetask_t *st) {
  ...
  if (st->node != NULL) dlist_node_pop(source->tasks, st->node);
  if (st->idnode != NULL) splaytree_remove_node(source->idtree, st->idnode);
...
void scamper_source_free(scamper_source_t *source) {
   ...
   */
  if (source_refcnt_dec(source) != 0) return;
...
int scamper_sources_gettask(scamper_task_t **task) {
  ...
  while ((source = dlist_head_item(finished)) != NULL) source_detach(source);
   ...
   */
  if (source_cur != NULL && source_cnt >= source_cur->priority) source_next();

  while ((source = source_cur) != NULL) {
    assert(source->priority > 0);

    while ((command = dlist_head_pop(source->commands)) != NULL) {
      if (source->take != NULL) source->take(source->data);

      switch (command->type) {
        case COMMAND_PROBE:
          if (command_probe_handle(source, command, task) != 0) goto err;
          if (*task == NULL) continue;
          source_cnt++;
          goto done;

        case COMMAND_TASK:
          if (command_task_handle(source, command, task) != 0) goto err;
          if (*task == NULL) continue;
          source_cnt++;
          goto done;
...
int scamper_sources_add(scamper_source_t *source) {
  ...
  if (source_active_attach(source) != 0) goto err;
...
int scamper_sources_init(void) {
  if ((active = clist_alloc()) == NULL) return -1;

  if ((blocked = dlist_alloc()) == NULL) return -1;

  if ((finished = dlist_alloc()) == NULL) return -1;
