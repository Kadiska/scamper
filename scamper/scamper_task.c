...
static int host_cmp(const s2t_t *a, const s2t_t *b) {
  ...
  if ((i = strcasecmp(a->sig->sig_host_name, b->sig->sig_host_name)) != 0)
    return i;
  if (a->sig->sig_host_type < b->sig->sig_host_type) return -1;
  if (a->sig->sig_host_type > b->sig->sig_host_type) return 1;
  return 0;
}
...
static void tx_nd_check(scamper_dl_rec_t *dl) {
  ...
  sig.sig_tx_nd_ip = &ip;
  fm.sig = &sig;
  if ((s2t = patricia_find(pt, &fm)) == NULL) return;
...
static void sniff_check(scamper_dl_rec_t *dl) {
  ...
  for (n = dlist_head_node(sniff); n != NULL; n = dlist_node_next(n)) {
    s2t = dlist_node_item(n);
    sig = s2t->sig;
    if (sig->sig_sniff_icmp_id != id) continue;
    if (scamper_addr_cmp(sig->sig_sniff_src, &src) != 0) continue;
...
char *scamper_task_sig_tostr(scamper_task_sig_t *sig, char *buf, size_t len) {
  ...
  if (sig->sig_type == SCAMPER_TASK_SIG_TYPE_TX_IP)
    ...
    string_concat(buf, len, &off, "host %s %u", sig->sig_host_name,
                  sig->sig_host_type);
  else
    return NULL;
...
scamper_task_anc_t *scamper_task_anc_add(scamper_task_t *task, void *data,
  ...
  if (task->ancillary == NULL && (task->ancillary = dlist_alloc()) == NULL)
    return NULL;
  if ((anc = malloc_zero(sizeof(scamper_task_anc_t))) == NULL) return NULL;
  anc->data = data;
...
int scamper_task_sig_add(scamper_task_t *task, scamper_task_sig_t *sig) {
  s2t_t *s2t;
  if ((s2t = malloc_zero(sizeof(s2t_t))) == NULL) return -1;
  s2t->sig = sig;
  s2t->task = task;
...
scamper_task_t *scamper_task_sig_block(scamper_task_t *task) {
  ...
  for (n = slist_head_node(task->siglist); n != NULL; n = slist_node_next(n)) {
    s2t = slist_node_item(n);
    sig = s2t->sig;
    if ((tf = scamper_task_find(sig)) != NULL && tf != task) return tf;
  }
...
void *scamper_task_onhold(scamper_task_t *task, void *param,
  ...
  if (task->onhold == NULL && (task->onhold = dlist_alloc()) == NULL) goto err;
  if ((toh = malloc_zero(sizeof(task_onhold_t))) == NULL) goto err;
  if ((cookie = dlist_tail_push(task->onhold, toh)) == NULL) goto err;
...
int scamper_task_dehold(scamper_task_t *task, void *cookie) {
  ...
  if ((toh = dlist_node_pop(task->onhold, cookie)) == NULL) return -1;
  free(toh);
  return 0;
}
...
scamper_task_t *scamper_task_alloc(void *data, scamper_task_funcs_t *funcs) {
  ...
  if ((task->queue = scamper_queue_alloc(task)) == NULL) goto err;

  if ((task->siglist = slist_alloc()) == NULL) goto err;
...
void scamper_task_free(scamper_task_t *task) {
  ...
  if (task->funcs != NULL) task->funcs->task_free(task);
  ...
  if (task->fds != NULL) {
    for (i = 0; i < task->fdc; i++) scamper_fd_free(task->fds[i]);
    free(task->fds);
  }
...
void *scamper_task_getdata(const scamper_task_t *task) { return task->data; }

void *scamper_task_getstate(const scamper_task_t *task) { return task->state; }
...
void scamper_task_handleicmp(scamper_task_t *task, scamper_icmp_resp_t *resp) {
  if (task->funcs->handle_icmp != NULL) task->funcs->handle_icmp(task, resp);
  return;
}
...
void scamper_task_handletimeout(scamper_task_t *task) {
  if (task->funcs->handle_timeout != NULL) task->funcs->handle_timeout(task);
  return;
}
...
static scamper_fd_t *task_fd(scamper_task_t *t, scamper_fd_t *fd) {
  ...
  if (array_find((void **)t->fds, t->fdc, fd, (array_cmp_t)task_fd_cmp) ==
      NULL) {
...
int scamper_task_init(void) {
  if ((tx_ip4 =
           patricia_alloc((patricia_bit_t)tx_ip_bit, (patricia_cmp_t)tx_ip_cmp,
  ...
  if ((tx_ip6 =
    ...
    return -1;
  if ((tx_nd4 =
           patricia_alloc((patricia_bit_t)tx_nd_bit, (patricia_cmp_t)tx_nd_cmp,
  ...
  if ((tx_nd6 =
           patricia_alloc((patricia_bit_t)tx_nd_bit, (patricia_cmp_t)tx_nd_cmp,
                          (patricia_fbd_t)tx_nd_fbd)) == NULL)
    return -1;
  if ((host = splaytree_alloc((splaytree_cmp_t)host_cmp)) == NULL) return -1;
  if ((sniff = dlist_alloc()) == NULL) return -1;
  return 0;
}

void scamper_task_cleanup(void) {
  if (tx_ip4 != NULL) {
    patricia_free(tx_ip4);
    tx_ip4 = NULL;
  }
  if (tx_ip6 != NULL) {
    patricia_free(tx_ip6);
    tx_ip6 = NULL;
  }
  if (tx_nd4 != NULL) {
    patricia_free(tx_nd4);
    tx_nd4 = NULL;
  }
  if (tx_nd6 != NULL) {
    patricia_free(tx_nd6);
    tx_nd6 = NULL;
  }
  if (host != NULL) {
    splaytree_free(host, NULL);
    host = NULL;
  }
  if (sniff != NULL) {
    dlist_free(sniff);
