...
static void tcp_cksum(scamper_probe_t *probe, struct tcphdr *tcp, size_t len) {
  ...
  if (len != 0) sum += ((uint8_t *)w)[0];

  /* fold the checksum */
  sum = (sum >> 16) + (sum & 0xffff);
  sum += (sum >> 16);

  if ((tcp->th_sum = ~sum) == 0) tcp->th_sum = 0xffff;
...
static void tcp4_build(scamper_probe_t *probe, uint8_t *buf) {
  ...
  if ((probe->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_TS) != 0) {
    ...
    while ((tcphlen % 4) != 0) tcphlen += tcp_nop(buf + tcphlen);
  }

  if (probe->pr_tcp_sackb != 0) tcphlen += tcp_sack(buf + tcphlen, probe);
  ...
  if (probe->pr_len > 0) memcpy(buf + tcphlen, probe->pr_data, probe->pr_len);
...
size_t scamper_tcp4_hlen(scamper_probe_t *pr) {
  ...
  if (pr->pr_tcp_flags & TH_SYN) {
    if (pr->pr_tcp_mss != 0) len += 4;
    if (pr->pr_tcp_wscale != 0) len += 3;
    if ((pr->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_SACK) != 0) len += 2;
  ...
  if (pr->pr_tcp_sackb != 0) len += ((8 * pr->pr_tcp_sackb) + 2);
  while ((len % 4) != 0) len++;
  assert(len <= 60);
  return len;
}
