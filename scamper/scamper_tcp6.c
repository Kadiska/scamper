...
size_t scamper_tcp6_hlen(scamper_probe_t *pr) {
  ...
  if (pr->pr_tcp_flags & TH_SYN) {
    if (pr->pr_tcp_mss != 0) tcphlen += 4;
    if (pr->pr_tcp_wscale != 0) tcphlen += 3;
    if ((pr->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_SACK) != 0) tcphlen += 2;
  ...
  if ((pr->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_TS) != 0) {
    ...
    if (pr->pr_tcp_sackb != 0)
      while ((tcphlen % 4) != 0) tcphlen++;
  }
  if (pr->pr_tcp_sackb != 0) tcphlen += ((8 * pr->pr_tcp_sackb) + 2);
  while ((tcphlen % 4) != 0) tcphlen++;
  assert(tcphlen <= 60);
  return tcphlen;
}
...
int scamper_tcp6_build(scamper_probe_t *probe, uint8_t *buf, size_t *len) {
  ...
  if (req <= *len) {
    ...
    if ((probe->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_TS) != 0) {
      ...
      while ((tcphlen % 4) != 0) tcphlen += tcp_nop(buf + ip6hlen + tcphlen);
    }
