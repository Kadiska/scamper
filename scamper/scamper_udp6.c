...
static int scamper_udp6_read_err(void *fd_, scamper_icmp_resp_t *resp) {
  ...
  if (msg.msg_controllen < sizeof(struct cmsghdr)) return -1;
  ...
  while (cm != NULL) {
    if (cm->cmsg_level == SOL_SOCKET && cm->cmsg_type == SCM_TIMESTAMP) {
      ...
      if (ee->ee_origin == SO_EE_ORIGIN_ICMP6) {
        ...
        memcpy(&resp->ir_ip_src.v6, &sin6->sin6_addr, sizeof(struct in6_addr));
      }
    }
  ...
  if (ee == NULL || (resp->ir_icmp_type != ICMP6_TIME_EXCEEDED &&
...
void scamper_udp6_read_err_cb(void *fd_, void *param) {
  ...
  if (scamper_udp6_read_err(&fd, &ir) == 0 &&
      scamper_fd_sport((const scamper_fd_t *)param, &ir.ir_inner_udp_sport) ==
          0)
...
int scamper_udp6_open_err(const void *addr, int sport) {
  ...
  if ((fd = scamper_udp6_open(addr, sport)) == -1) return -1;
...
#else
int scamper_udp6_open_err(const void *addr, int sport) { return -1; }
#endif
