...
static struct iovec *iov_alloc(void) {
  struct iovec *iov;
  if ((iov = malloc(sizeof(struct iovec))) == NULL) return NULL;
...
static int writebuf_tx(scamper_writebuf_t *wb, int fd) {
  ...
  if ((iovs = slist_count(wb->iovs)) <= 0) return 0;
...
static int writebuf_tx(scamper_writebuf_t *wb, int fd) {
  ...
  if (slist_count(wb->iovs) == 0) return 0;

  iov = slist_head_item(wb->iovs);
  if ((size = send(fd, iov->iov_base, iov->iov_len, 0)) == -1) return -1;
...
int scamper_writebuf_gtzero(const scamper_writebuf_t *wb) {
  if (slist_count(wb->iovs) > 0) return 1;
  return 0;
}
...
size_t scamper_writebuf_len2(const scamper_writebuf_t *wb, char *str,
                             size_t len) {
  slist_node_t *node;
  struct iovec *iov;
  size_t k = 0, off = 0;
  ...
  for (node = slist_head_node(wb->iovs); node != NULL;
       node = slist_node_next(node)) {
    iov = slist_node_item(node);
    k += iov->iov_len;
    c++;
 ...
 * rest of scamper.
 */
int scamper_writebuf_send(scamper_writebuf_t *wb, const char *data,
                          size_t len) {
  size_t s, x;
...
scamper_writebuf_t *scamper_writebuf_alloc(void) {
  ...
  if ((wb = malloc_zero(sizeof(scamper_writebuf_t))) == NULL ||
      (wb->iovs = slist_alloc()) == NULL || (wb->tail = iov_alloc()) == NULL)
    goto err;
  return wb;
