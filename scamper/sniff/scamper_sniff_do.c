...
const char *scamper_do_sniff_usage(void) {
  return "sniff [-c limit-pktc] [-G limit-time] [-S ipaddr] [-U userid] "
         "<expression>\n";
}
...
static void sniff_finish(scamper_task_t *task, int reason) {
  ...
  if (state != NULL && state->list != NULL &&
    ...
    i = 0;
    while ((pkt = slist_head_pop(state->list)) != NULL) sniff->pkts[i++] = pkt;
    assert(i == rc);
  }
...
static void do_sniff_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_DL_IS_ICMP(dl)) {
    if ((SCAMPER_DL_IS_ICMP_ECHO(dl) && dl->dl_icmp_id == sniff->icmpid &&
  ...
  if ((i = slist_count(state->list)) < 0) goto err;
...
static void sniff_state_free(sniff_state_t *state) {
  ...
  if (state->list != NULL) slist_free(state->list);
...
static int sniff_state_alloc(scamper_task_t *task) {
  ...
  if ((state = malloc_zero(sizeof(sniff_state_t))) == NULL) goto err;
...
static int sniff_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case SNIFF_OPT_LIMIT_PKTC:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 5000) goto err;
      break;

    case SNIFF_OPT_LIMIT_TIME:
      if (string_tolong(param, &tmp) != 0 || tmp < 0 || tmp > 1200) goto err;
      break;
...
void *scamper_do_sniff_alloc(char *str) {
  ...
  if (expr == NULL) goto err;

  if (strncasecmp(expr, "icmp[icmpid] == ", 16) != 0 ||
      string_isnumber(expr + 16) == 0 ||
      string_tolong(expr + 16, &icmpid) != 0 || icmpid < 0 || icmpid > 65535) {
  ...
  if ((sniff = scamper_sniff_alloc()) == NULL) goto err;

  if ((sniff->src = scamper_addrcache_resolve(addrcache, AF_UNSPEC, src)) ==
      NULL) {
...
static void do_sniff_free(scamper_task_t *task) {
  ...
  if ((sniff = sniff_getdata(task)) != NULL) scamper_sniff_free(sniff);

  if ((state = sniff_getstate(task)) != NULL) sniff_state_free(state);
...
scamper_task_t *scamper_do_sniff_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(sniff, &sniff_funcs)) == NULL) goto err;
...
void scamper_do_sniff_cleanup() { return; }
