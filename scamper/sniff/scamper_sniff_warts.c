...
static const warts_var_t sniff_vars[] = {
    {WARTS_SNIFF_LIST, 4, -1},        {WARTS_SNIFF_CYCLE, 4, -1},
    {WARTS_SNIFF_USERID, 4, -1},      {WARTS_SNIFF_SRC, -1, -1},
    {WARTS_SNIFF_START, 8, -1},       {WARTS_SNIFF_FINISH, 8, -1},
    {WARTS_SNIFF_STOP_REASON, 1, -1}, {WARTS_SNIFF_LIMIT_PKTC, 4, -1},
    {WARTS_SNIFF_LIMIT_TIME, 2, -1},  {WARTS_SNIFF_PKTC, 4, -1},
...
static int warts_sniff_params_read(scamper_sniff_t *sniff,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (sniff->src == NULL) return -1;
  return 0;
}

static int warts_sniff_params_write(
    const scamper_sniff_t *sniff, const scamper_file_t *sf,
...
int scamper_file_warts_sniff_write(const scamper_file_t *sf,
  ...
  if (sniff->pktc > 0) {
    ...
    if ((pkts = (warts_sniff_pkt_t *)malloc_zero(size)) == NULL) goto err;
  ...
  if (warts_sniff_params_write(sniff, sf, table, buf, &off, len, flags,
