 ...
 * scamper_do_sting.c
 *
 * $Id: scamper_sting_do.c,v 1.49 2020/03/17 07:32:16 mjl Exp $
 *
...
static void handletimeout_hole(scamper_task_t *task) {
   ...
   * and give up
   */
  if (state->attempt == sting->dataretx) state->mode = MODE_RST;
...
static void handletcp_hole(scamper_task_t *task, scamper_dl_rec_t *dl) {
   ...
   */
  if (state->isn >= dl->dl_tcp_ack) goto err;
...
static void do_sting_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_DL_IS_TCP(dl) == 0 || dl->dl_tcp_sport != sting->dport ||
...
static int sting_state_alloc(scamper_task_t *task) {
  ...
  if ((state->probes = malloc_zero(size)) == NULL) goto err;
...
static void do_sting_probe(scamper_task_t *task) {
  ...
  if (state == NULL) {
    ...
    if (sting_state_alloc(task) != 0) goto err;
  ...
  if ((pkt = scamper_sting_pkt_alloc(SCAMPER_STING_PKT_FLAG_TX, probe.pr_tx_raw,
...
static int sting_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case STING_OPT_DIST:
      if (string_tolong(param, &tmp) != 0 || tmp < SCAMPER_DO_STING_DIST_MIN ||
    ...
    case STING_OPT_MEAN:
      if (string_tolong(param, &tmp) != 0 || tmp < SCAMPER_DO_STING_MEAN_MIN ||
    ...
    case STING_OPT_INTER:
      if (string_tolong(param, &tmp) != 0 || tmp < SCAMPER_DO_STING_INTER_MIN ||
...
void *scamper_do_sting_alloc(char *str) {
  ...
  if ((sting->dst = scamper_addrcache_resolve(addrcache, AF_UNSPEC, addr)) ==
      NULL) {
...
scamper_task_t *scamper_do_sting_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(sting, &sting_funcs)) == NULL) goto err;
  ...
  if (sting->src == NULL &&
      (sting->src = scamper_getsrc(sting->dst, 0)) == NULL)
    goto err;
...
void scamper_do_sting_cleanup(void) { return; }
