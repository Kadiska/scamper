...
static const warts_var_t sting_vars[] = {
    {WARTS_STING_LIST, 4, -1},     {WARTS_STING_CYCLE, 4, -1},
    {WARTS_STING_USERID, 4, -1},   {WARTS_STING_SRC, -1, -1},
    {WARTS_STING_DST, -1, -1},     {WARTS_STING_SPORT, 2, -1},
    {WARTS_STING_DPORT, 2, -1},    {WARTS_STING_COUNT, 2, -1},
    {WARTS_STING_MEAN, 2, -1},     {WARTS_STING_INTER, 2, -1},
    {WARTS_STING_DIST, 1, -1},     {WARTS_STING_SYNRETX, 1, -1},
    {WARTS_STING_DATARETX, 1, -1}, {WARTS_STING_SEQSKIP, 1, -1},
    {WARTS_STING_DATALEN, 2, -1},  {WARTS_STING_DATA, -1, -1},
    {WARTS_STING_START, 8, -1},    {WARTS_STING_HSRTT, 8, -1},
    {WARTS_STING_DATAACKC, 2, -1}, {WARTS_STING_HOLEC, 2, -1},
    {WARTS_STING_PKTC, 4, -1},     {WARTS_STING_RESULT, 1, -1},
};
...
static int warts_sting_params_read(scamper_sting_t *sting,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (sting->src == NULL || sting->dst == NULL) return -1;
  return 0;
}

static int warts_sting_params_write(
    const scamper_sting_t *sting, const scamper_file_t *sf,
...
int scamper_file_warts_sting_write(const scamper_file_t *sf,
  ...
  if (sting->pktc > 0) {
    ...
    if ((pkts = (warts_sting_pkt_t *)malloc_zero(size)) == NULL) goto err;
  ...
  if (warts_sting_params_write(sting, sf, table, buf, &off, len, flags,
