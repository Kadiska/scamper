...
int scamper_tbit_data_seqoff(uint32_t rcv_nxt, uint32_t seq) {
  if (seq >= rcv_nxt) return seq - rcv_nxt;
...
int scamper_tbit_fo_setcookie(scamper_tbit_t *tbit, uint8_t *cookie,
                              uint8_t len) {
  if ((tbit->fo_cookie = memdup(cookie, len)) == NULL) return -1;
...
int scamper_tbit_fo_getcookie(scamper_tbit_t *tbit, uint8_t *c, uint8_t *l) {
  ...
  for (i = 0; i < tbit->pktc; i++) {
    pkt = tbit->pkts[i];
    if (pkt->dir != SCAMPER_TBIT_PKT_DIR_RX) continue;

    v = (pkt->data[0] >> 4);
    if (v == 4) {
      iphlen = (pkt->data[0] & 0xf) * 4;
      if (pkt->data[9] != IPPROTO_TCP) continue;
      if ((bytes_ntohs(pkt->data + 6) & 0x1fff) != 0) continue;
    } else if (v == 6) {
      iphlen = 40;
      u8 = pkt->data[6];
      for (;;) {
        switch (u8) {
          ...
          case IPPROTO_FRAGMENT:
            ...
            if ((bytes_ntohs(pkt->data + iphlen + 2) & 0xfff8) != 0) break;
            u8 = pkt->data[iphlen + 0];
            iphlen += 8;
            continue;
        }
    ...
    while (u8 < tcphlen) {
      ...
      if (pktptr[1] == 0 || u8 + pktptr[1] > tcphlen) break;
...
void scamper_tbit_tcpqe_free(scamper_tbit_tcpqe_t *qe, void (*ff)(void *)) {
  if (qe == NULL) return;
  if (ff != NULL && qe->data != NULL) ff(qe->data);
  free(qe);
  return;
}
...
uint32_t scamper_tbit_tcpq_tail(const scamper_tbit_tcpq_t *tcpq) {
  ...
  for (i = 0; i < tcpq->tqec; i++) {
    qe = tcpq->tqes[i]->qe;
    edge = qe->seq + qe->len;
    if ((u32 = scamper_tbit_data_seqoff(tcpq->seq, edge)) > range) range = u32;
  }
...
scamper_tbit_tcpq_t *scamper_tbit_tcpq_alloc(uint32_t isn) {
  ...
  if ((q = malloc_zero(sizeof(scamper_tbit_tcpq_t))) == NULL) goto err;
  q->seq = isn;
  return q;
err:
...
void scamper_tbit_tcpq_free(scamper_tbit_tcpq_t *q, void (*ff)(void *)) {
  if (q == NULL) return;
  if (q->tqes != NULL) scamper_tbit_tcpq_flush(q, ff);
  free(q);
  return;
}

int scamper_tbit_tcpq_seg(scamper_tbit_tcpq_t *q, uint32_t *seq,
                          uint16_t *len) {
  tqe_t *tqe;
  assert(q->tqec >= 0);
  if (q->tqec == 0) return -1;
  tqe = q->tqes[0];
...
scamper_tbit_tcpqe_t *scamper_tbit_tcpq_pop(scamper_tbit_tcpq_t *q) {
  ...
  if (q->tqec == 0) return NULL;

  tqe = q->tqes[0];
  qe = tqe->qe;
  free(tqe);

  if (--q->tqec > 0) memmove(q->tqes, q->tqes + 1, sizeof(tqe_t *) * q->tqec);

  off = scamper_tbit_data_seqoff(q->seq, qe->seq);
  if (off < 0 && off + qe->len <= 0) return qe;

  len = qe->len + off;
  for (i = 0; i < q->tqec; i++) q->tqes[i]->off -= len;
  q->seq += len;
...
int scamper_tbit_tcpq_add(scamper_tbit_tcpq_t *q, uint32_t seq, uint8_t flags,
  ...
  if ((tqe = malloc_zero(sizeof(tqe_t))) == NULL) goto err;
  if ((tqe->qe = malloc_zero(sizeof(scamper_tbit_tcpqe_t))) == NULL) goto err;
  tqe->off = scamper_tbit_data_seqoff(q->seq, seq);
  tqe->qe->seq = seq;
  tqe->qe->flags = flags;
  tqe->qe->len = len;
  tqe->qe->data = data;
  if (array_insert((void ***)&q->tqes, &q->tqec, tqe, (array_cmp_t)tqe_cmp) !=
      0)
    goto err;
  return 0;
...
int scamper_tbit_tcpq_sack(scamper_tbit_tcpq_t *q, uint32_t *sack, int count) {
  ...
  assert(q->tqec >= 0);
  if (q->tqec == 0) return 0;

  qe = q->tqes[0]->qe;
  if (qe->len == 0) return 0;
  ...
  for (i = 1; i < q->tqec && c < count; i++) {
    qe = q->tqes[i]->qe;
    if (qe->len == 0) continue;
    if ((off = scamper_tbit_data_seqoff(right, qe->seq)) <= 0) {
      off = abs(off);
      if (qe->len > off) right = right + qe->len - off;
      continue;
    }
...
int scamper_tbit_data_inrange(uint32_t rcv_nxt, uint32_t seq, uint16_t len) {
  if ((SEQ_LEQ(rcv_nxt, seq) && SEQ_LT(seq, rcv_nxt + 65535)) ||
      (SEQ_LEQ(rcv_nxt, seq + len - 1) &&
       SEQ_LT(seq + len - 1, rcv_nxt + 65535)))
    return 1;
  return 0;
}
...
int scamper_tbit_pkt_tcpdatabytes(const scamper_tbit_pkt_t *pkt, uint16_t *bc) {
  ...
  if (scamper_tbit_pkt_iph(pkt, &proto, &iphlen, &iplen) != 0) return -1;
...
int scamper_tbit_pkt_tcpack(const scamper_tbit_pkt_t *pkt, uint32_t *ack) {
  ...
  uint16_t iplen;
  if (scamper_tbit_pkt_iph(pkt, &proto, &iphlen, &iplen) != 0) return -1;
  if (proto != IPPROTO_TCP || (pkt->data[iphlen + 13] & TH_ACK) == 0) return -1;
...
int scamper_tbit_icw_size(const scamper_tbit_t *tbit, uint32_t *icw_out) {
  ...
  if (tbit->result != SCAMPER_TBIT_RESULT_ICW_SUCCESS || tbit->pktc < 1)
    goto done;

  for (i = 1; i < tbit->pktc; i++) {
    pkt = tbit->pkts[i];
    if (pkt->dir == SCAMPER_TBIT_PKT_DIR_RX) break;
  }
  if (i == tbit->pktc ||
  ...
  if ((q = scamper_tbit_tcpq_alloc(start_seq)) == NULL) goto done;

  for (i++; i < tbit->pktc; i++) {
    pkt = tbit->pkts[i];
    if (pkt->dir != SCAMPER_TBIT_PKT_DIR_RX) continue;
    if (scamper_tbit_pkt_iph(pkt, &proto, &iphlen, &iplen) != 0) break;
    if (proto != IPPROTO_TCP) break;
    ...
    if ((datalen = iplen - iphlen - tcphlen) == 0 && (flags & TH_FIN) == 0)
      continue;
    if (scamper_tbit_data_inrange(start_seq, seq, datalen) == 0) continue;
    ...
    if (scamper_tbit_tcpq_add(q, seq, flags, datalen, NULL) != 0) break;
  }
...
int scamper_tbit_stats(const scamper_tbit_t *tbit,
                       scamper_tbit_stats_t *stats) {
  ...
  if (i == tbit->pktc) return 0;

  if (scamper_tbit_pkt_iph(pkt, &proto, &iphlen, &iplen) != 0) goto err;
  if (proto != IPPROTO_TCP) goto err;
  if ((pkt->data[iphlen + 13] & (TH_SYN | TH_ACK)) != (TH_SYN | TH_ACK))
    goto err;
  ...
  if ((q = scamper_tbit_tcpq_alloc(rcv_nxt)) == NULL) goto err;
  ...
  for (i++; i < tbit->pktc; i++) {
    ...
    if (scamper_tbit_data_inrange(rcv_nxt, seq, datalen) == 0) continue;

    if (scamper_tbit_tcpq_add(q, seq, flags, datalen, NULL) != 0) goto err;
    ...
    while (scamper_tbit_tcpq_seg(q, &seq, &datalen) == 0) {
      ...
      if ((off = scamper_tbit_data_seqoff(rcv_nxt, seq)) > 0) break;
...
char *scamper_tbit_type2str(const scamper_tbit_t *tbit, char *buf, size_t len) {
  static char *t[] = {
      NULL,  "pmtud",      "ecn",       "null",      "sack-rcvr", "icw",
      "abc", "blind-data", "blind-rst", "blind-syn", "blind-fin",
  };
...
scamper_tbit_pkt_t *scamper_tbit_pkt_alloc(uint8_t dir, uint8_t *data,
  ...
  if ((pkt = malloc_zero(sizeof(scamper_tbit_pkt_t))) == NULL) goto err;

  pkt->dir = dir;
  if (len != 0 && data != NULL) {
    if ((pkt->data = memdup(data, len)) == NULL) goto err;
    pkt->len = len;
  }
...
int scamper_tbit_record_pkt(scamper_tbit_t *tbit, scamper_tbit_pkt_t *pkt) {
  ...
  if (realloc_wrap((void **)&tbit->pkts, len) != 0) return -1;
...
void scamper_tbit_pmtud_free(scamper_tbit_pmtud_t *pmtud) {
  if (pmtud == NULL) return;
  if (pmtud->ptbsrc != NULL) scamper_addr_free(pmtud->ptbsrc);
  free(pmtud);
  return;
}
...
void scamper_tbit_free(scamper_tbit_t *tbit) {
  ...
  if (tbit->pkts != NULL) {
    for (i = 0; i < tbit->pktc; i++) scamper_tbit_pkt_free(tbit->pkts[i]);
    free(tbit->pkts);
  }
