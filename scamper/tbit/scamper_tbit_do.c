...
const char *scamper_do_tbit_usage(void) {
  return "tbit [-t type] [-p app] [-d dport] [-s sport] [-b asn] [-f cookie]\n"
...
static void tbit_result(scamper_task_t *task, uint8_t result) {
  ...
  if (state->flags & TBIT_STATE_FLAG_RST_SEEN) d = 1;
...
static void tbit_classify(scamper_task_t *task) {
  ...
  if (tbit->type == SCAMPER_TBIT_TYPE_PMTUD) {
    pmtud = tbit->data;
    if (pmtud->options & SCAMPER_TBIT_PMTUD_OPTION_BLACKHOLE) bh = 1;
...
static int tbit_reassemble(scamper_task_t *task, scamper_dl_rec_t **out,
  ...
  if (dl->dl_ip_datalen == 0 || dl->dl_ip_proto != IPPROTO_TCP) {
    scamper_debug(__func__, "ignoring fragment %d %d", dl->dl_ip_datalen,
                  dl->dl_ip_proto);
    return 0;
  }
  ...
  if (frag == NULL) {
    ...
    if (SCAMPER_DL_IS_IP_MF(dl) == 0) frags->gotlast = 1;
  }
  ...
  if (newp->dl_tcp_datalen > 0) newp->dl_tcp_data = data + newp->dl_tcp_hl;
...
static tbit_probe_t *tp_tcp(tbit_state_t *state) {
  tbit_probe_t *tp;
  if ((tp = tp_alloc(state, TBIT_PROBE_TYPE_TCP)) == NULL) return NULL;
  tp->tp_flags = TH_ACK;
...
static void tp_flush(tbit_state_t *state, tbit_probe_t *tp_stop) {
  tbit_probe_t *tp;
  for (;;) {
    if ((tp = slist_head_item(state->tx)) == NULL || tp == tp_stop) break;
...
static int tbit_segment(tbit_state_t *state, const uint8_t *data,
                        uint16_t len) {
...
static int tbit_data_inrange(tbit_state_t *state, uint32_t seq, uint32_t len) {
  if (scamper_tbit_data_inrange(state->rcv_nxt, seq, len) != 0) return 1;
...
static int tbit_rxq(tbit_state_t *state, const scamper_dl_rec_t *dl) {
  ...
  if (scamper_tbit_tcpq_add(state->rxq, dl->dl_tcp_seq, dl->dl_tcp_flags,
    ...
    printerror(__func__, "could not add %u/%2x/%u", dl->dl_tcp_seq,
               dl->dl_tcp_flags, dl->dl_tcp_datalen);
    goto err;
  }
...
static int tbit_ssl_wbio_write(tbit_state_t *state) {
  ...
  if ((pending = BIO_pending(state->wbio)) < 0) return -1;
  ...
  while (off < pending) {
    ...
    if (tbit_segment(state, buf, rc) != 0) return -1;
  }
...
#if defined(HAVE_OPENSSL)
  if (state->ssl != NULL) return tbit_ssl_write(state, (uint8_t *)buf, off);
#endif

  if (tbit_segment(state, (uint8_t *)buf, off) != 0) return -1;
  return off;
}

static int tbit_app_http_rx(scamper_task_t *task, uint8_t *data,
                            uint16_t dlen) {
  tbit_state_t *state = tbit_getstate(task);
  int rc;
  ...
  if (state->flags & TBIT_STATE_FLAG_NOMOREDATA) return 0;
...
#endif
    if ((rc = tbit_app_http_req(task)) < 0) return -1;
...
static int tbit_app_bgp_req(scamper_task_t *task) {
  ...
  /* bgp header */
  for (off = 0; off < 16; off++) buf[off] = 0xff;
  bytes_htons(buf + off, 29);
  off += 2;       /* length */
  buf[off++] = 1; /* type = BGP_OPEN */
  buf[off++] = 4; /* version = 4 */

  /* bgp open */
  bytes_htons(buf + off, (uint16_t)bgp->asn);
  off += 2;
  bytes_htons(buf + off, 180);
  off += 2; /* hold time */
  bytes_htonl(buf + off, 12345);
  off += 4;       /* id */
...
static int tbit_app_bgp_rx(scamper_task_t *task, uint8_t *data, uint16_t dlen) {
  ...
  if (state->mode == MODE_SYN || state->mode == MODE_RTSOCK) {
    if ((rc = tbit_app_bgp_req(task)) < 0) return -1;
    state->flags |= TBIT_STATE_FLAG_NOMOREDATA;
    return rc;
  }
...
static int tbit_app_rx(scamper_task_t *task, uint8_t *data, uint16_t len) {
  static int (*const func[])(scamper_task_t *, uint8_t *, uint16_t) = {
      NULL, tbit_app_http_rx, NULL, NULL, NULL, tbit_app_bgp_rx,
  };
  scamper_tbit_t *tbit = tbit_getdata(task);
...
static void dl_syn(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (tbit->fo_cookie != NULL) {
    ...
    if ((rc = tbit_app_rx(task, NULL, 0)) < 0) goto err;
  }

  /* send a pure ack to their data */
  if ((tp = tp_tcp(state)) == NULL) goto err;
...
static void dl_fin(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if ((state->flags & TBIT_STATE_FLAG_FIN_SEEN) &&
    ...
    if (tbit_tcpclosed(state)) {
      if (tbit->result == SCAMPER_TBIT_RESULT_NONE) tbit_classify(task);
      state->mode = MODE_DONE;
      tbit_queue(task);
    }
    return;
  ...
  if (dl->dl_tcp_datalen == 0 && (dl->dl_tcp_flags & TH_FIN) == 0) return;
  ...
  if (tbit_rxq(state, dl) != 0) goto err;
  ...
  while (scamper_tbit_tcpq_seg(state->rxq, &seq, &datalen) == 0) {
    ...
    if ((x = scamper_tbit_data_seqoff(state->rcv_nxt, seq)) > 0) break;
    ...
    if ((state->flags & TBIT_STATE_FLAG_FIN_ACKED) == 0) tp->tp_flags |= TH_FIN;
...
ack:
  ...
  if (state->mode != MODE_DONE && (state->flags & TBIT_STATE_FLAG_SACK) != 0) {
    x = 4;
    if ((state->flags & TBIT_STATE_FLAG_TCPTS) != 0) x--;
...
static int dl_data_pmtud(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_DL_IS_IP_FRAG(dl) != 0) frag = 1;
  ...
  if (dl->dl_tcp_datalen > 0) state->flags |= TBIT_STATE_FLAG_SEEN_DATA;

  if (skip) {
    if (dl->dl_tcp_datalen > 0) {
      state->rcv_nxt += dl->dl_tcp_datalen;

      if (ipv4 && SCAMPER_DL_IS_IP_DF(dl) == 0 && size > pmtud->mtu &&
          SCAMPER_DL_IS_IP_REASS(dl) == 0)
...
static int dl_data_ecn(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if ((dl->dl_tcp_flags & TH_FIN) != 0) fin = 1;
...
static int dl_data_null(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  while (scamper_tbit_tcpq_seg(state->rxq, &seq, &datalen) == 0) {
    ...
    if (datalen > 0) state->flags |= TBIT_STATE_FLAG_SEEN_DATA;
    ...
    if (qe->len > 0) state->rcv_nxt += (qe->len - off);
    if ((rc = tbit_app_rx(task, qe->data + off, qe->len - off)) < 0) goto err;
    scamper_tbit_tcpqe_free(qe, free);
    qe = NULL;
...
static int sack_rcvr_next(scamper_task_t *task) {
  ...
  if ((tp = tp_tcp(state)) == NULL) return -1;
  tp->tp_len = 1;
...
static int dl_data_sack_rcvr(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (dl->dl_tcp_seq != state->rcv_nxt) return 0;
  ...
  if (sack_rcvr_next(task) != 0) goto err;
...
static void timeout_data_sack_rcvr(scamper_task_t *task) {
  ...
  state->sackr_timeout++;
  if (sack_rcvr_next(task) != 0) tbit_handleerror(task, errno);
...
static int dl_data_icw(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (state->flags & TBIT_STATE_FLAG_NOMOREDATA) {
    if ((state->icw_flags & TBIT_STATE_ICW_FLAG_SEQ) == 0) {
      icw = tbit->data;
      icw->start_seq = scamper_tbit_data_seqoff(state->sa_seq, state->rcv_nxt);
  ...
  while (scamper_tbit_tcpq_seg(state->rxq, &seq, &datalen) == 0) {
    ...
    if ((off = scamper_tbit_data_seqoff(state->rcv_nxt, seq)) > 0) {
      if ((tp = tp_tcp(state)) == NULL) goto err;
      if (state->mode == MODE_MOREDATA) tp->tp_flags |= TH_FIN;
      break;
    }
...
static int dl_data_blinddata(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (state->mode == MODE_DATA) return dl_data_null(task, dl);
  if (state->mode != MODE_BLIND) return 0;
  ...
  if (state->blind_step == 0) {
    if (seg->len != state->blind_part2 + state->blind_part3) return 0;
    state->blind_step++;
    state->attempt = 0;
    ...
    if (seg->len != state->blind_part3) return 0;
    state->blind_step++;
    state->attempt = 0;
    tp_flush(state, NULL);
...
static int dl_data_blindfin(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (state->blind_step == 0) {
    if (seg->len != state->blind_part2) return 0;
    state->blind_step++;
    state->attempt = 0;
    ...
    if (dl->dl_tcp_ack < state->blind_ack) {
      scamper_debug(__func__, "old ack %u %u", dl->dl_tcp_ack,
                    state->blind_ack);
      return 0;
    }
    ...
    if (dl->dl_tcp_datalen > 0) {
      if (tbit_app_rx(task, dl->dl_tcp_data, dl->dl_tcp_datalen) < 0) goto err;
      state->rcv_nxt += dl->dl_tcp_datalen;
      if ((tp = tp_tcp(state)) == NULL) goto err;
    }
...
static void dl_data(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  /* is the data in range? */
  if (tbit_data_inrange(state, dl->dl_tcp_seq, dl->dl_tcp_datalen) == 0) return;

  /* remove segment data from the send queue */
  if (dl->dl_tcp_ack > state->snd_nxt) {
    if ((seg = slist_head_item(state->segments)) == NULL) return;
...
static void dl_pmtud(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV6(tbit->dst) && pmtud->mtu < 1280) mtu = 0;

  /* if an out of sequence packet is received, ack it */
  if (dl->dl_tcp_seq != state->rcv_nxt) {
    if (dl->dl_ip_size > mtu) return;
    if (tp_tcp(state) == NULL) goto err;
    tbit_queue(task);
    return;
  }
  ...
  if (mtu != 0) {
    ...
    if (SCAMPER_DL_IS_IP_FRAG(dl) || SCAMPER_DL_IS_IP_REASS(dl)) success = 1;
  }
...
static void dl_blackhole(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (mtu != 0) {
    ...
    if (SCAMPER_DL_IS_IP_FRAG(dl) || SCAMPER_DL_IS_IP_REASS(dl)) ack = 1;
  }
...
static void dl_moredata(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (tbit->type == SCAMPER_TBIT_TYPE_ICW) assert(state->icw_count == 2);
  ...
  while (scamper_tbit_tcpq_seg(state->rxq, &seq, &datalen) == 0) {
    ...
    fin = (qe->flags & TH_FIN) != 0 ? 1 : 0;
    if (qe->len > 0) state->rcv_nxt += (qe->len - off);
    if (tbit_app_rx(task, qe->data + off, qe->len - off) < 0) goto err;
    scamper_tbit_tcpqe_free(qe, free);
    qe = NULL;
...
static int blind_check_rst(const scamper_task_t *t,
...
static void do_tbit_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (dl->dl_ip_off != 0 || SCAMPER_DL_IS_IP_MF(dl)) {
    ...
    if (pkt == NULL || scamper_tbit_record_pkt(tbit, pkt) != 0) goto err;
    if (tbit_reassemble(task, &newp, dl) != 0) goto err;
    if (newp == NULL) return;
    dl = newp;
  }

  /* Unless it is an inbound TCP packet for the flow, ignore it */
  if (SCAMPER_DL_IS_TCP(dl) == 0 || dl->dl_tcp_sport != tbit->dport ||
      dl->dl_tcp_dport != tbit->sport ||
  ...
  if (func[state->mode] == NULL) goto done;
  ...
  if (state->mode != MODE_SYN) {
    ...
    if ((dl->dl_tcp_flags & TH_ACK) == 0) goto done;
  }
  ...
  if (newp != NULL && newp != dl) {
    if (newp->dl_ip_data != NULL) free(newp->dl_ip_data);
    free(newp);
  }
  return;
...
static void do_tbit_handle_timeout(scamper_task_t *task) {
  ...
  if (func[state->mode] != NULL) func[state->mode](task);
...
static void tbit_state_free(scamper_task_t *task) {
  ...
  if (tbit->type == SCAMPER_TBIT_TYPE_PMTUD) {
    if (state->pmtud_ptb_data != NULL) free(state->pmtud_ptb_data);
  }

  if (state->segments != NULL)
    slist_free_cb(state->segments, (slist_free_t)tbit_segment_free);

  if (state->tx != NULL) slist_free_cb(state->tx, (slist_free_t)tp_free);

  if (state->rxq != NULL) scamper_tbit_tcpq_free(state->rxq, free);

  if (state->frags != NULL) {
    for (i = 0; i < state->fragc; i++) tbit_frags_free(state->frags[i]);
    free(state->frags);
  }
  ...
  if (state->ssl != NULL) {
    ...
    if (state->wbio != NULL) BIO_free(state->wbio);
    if (state->rbio != NULL) BIO_free(state->rbio);
  }
#endif
...
static int tbit_state_alloc(scamper_task_t *task) {
  ...
  if (tbit->dst->type == SCAMPER_ADDR_TYPE_IPV4) random_u16(&state->ipid);
...
static int tbit_tx_tcp(scamper_task_t *task, scamper_probe_t *pr,
  ...
  if (tp != NULL) {
    if (tp->tp_len > 0) {
      if ((seg = slist_head_item(state->segments)) == NULL) return 0;
    ...
    if (state->mode == MODE_FIN) pr->pr_tcp_flags |= TH_FIN;
  }
  ...
  if (tbit->type == SCAMPER_TBIT_TYPE_ECN && (pr->pr_tcp_flags & TH_FIN) == 0 &&
      pr->pr_len > 0) {
...
static int tbit_tx_ptb(scamper_task_t *task, scamper_probe_t *pr,
  ...
  if (pmtud->ptbsrc != NULL) pr->pr_ip_src = pmtud->ptbsrc;
...
static void do_tbit_probe(scamper_task_t *task) {
  ...
  if (state == NULL) {
    ...
    if (tbit_state_alloc(task) != 0) goto err;
  ...
  if ((pkt = scamper_tbit_pkt_alloc(SCAMPER_TBIT_PKT_DIR_TX, probe.pr_tx_raw,
                                    probe.pr_tx_rawlen, &probe.pr_tx)) ==
          NULL ||
  ...
  if (wait > 0) timeval_add_ms(&state->timeout, &probe.pr_tx, wait);
...
static int tbit_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case TBIT_OPT_MTU:
    case TBIT_OPT_ASN:
      if (string_tolong(param, &tmp) != 0 || tmp < 0 || tmp > 65535) goto err;
      break;
    ...
    case TBIT_OPT_FO:
      ...
      if ((tmp % 8) != 0) return -1;
      tmp = tmp / 2;
      if (tmp < 4 || tmp > 16) return -1;
      break;

    case TBIT_OPT_WSCALE:
      if (string_tolong(param, &tmp) != 0 || tmp > 14 || tmp < 0) goto err;
      break;

    case TBIT_OPT_ATTEMPTS:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 4) goto err;
      break;

    case TBIT_OPT_OFFSET:
      if (string_tolong(param, &tmp) != 0) goto err;
#if SIZEOF_LONG > 4
      if (tmp > 2147483647 || tmp < -2147483647) goto err;
#endif
      break;

    case TBIT_OPT_TTL:
      if (string_tolong(param, &tmp) != 0 || tmp < 1 || tmp > 255) goto err;
      break;
...
static int tbit_app_http(scamper_tbit_t *tbit, tbit_options_t *o) {
  ...
  if (strncasecmp(o->url, "http://", 7) == 0) {
    ...
    if (scamper_option_notls() != 0) return -1;
    dport = 443;
    host = o->url + 8;
  ...
  if (*ptr == ':') {
    ...
    *ptr = '\0';
    ...
    if (string_tolong(port, &lo) != 0 || lo < 1 || lo > 65535) return -1;
    dport = (uint16_t)lo;
    *ptr = '/';
  }
  ...
  *(ptr - 1) = '\0';
  ...
  if (file == NULL) file = "/";
  if (tbit->dport == 0) tbit->dport = dport;
...
static int tbit_alloc_pmtud(scamper_tbit_t *tbit, tbit_options_t *o) {
  ...
  if ((pmtud = scamper_tbit_pmtud_alloc()) == NULL) return -1;
  tbit->data = pmtud;
...
static int tbit_alloc_icw(scamper_tbit_t *tbit, tbit_options_t *o) {
  if ((tbit->data = scamper_tbit_icw_alloc()) == NULL) return -1;
...
static int tbit_alloc_blind(scamper_tbit_t *tbit, tbit_options_t *o) {
  ...
  if ((blind = scamper_tbit_blind_alloc()) == NULL) return -1;
  tbit->data = blind;
...
static int tbit_alloc_null(scamper_tbit_t *tbit, tbit_options_t *o) {
  ...
  if (u != 0 && countbits32(u) != 1) return -1;

  if ((null = scamper_tbit_null_alloc()) == NULL) return -1;
  tbit->data = null;
  ...
  if (o->options & (TBIT_OPT_OPTION_IPTS_SYN | TBIT_OPT_OPTION_IPRR_SYN)) {
    if (SCAMPER_ADDR_TYPE_IS_IPV4(tbit->dst) == 0) return -1;
...
void *scamper_do_tbit_alloc(char *str) {
  ...
  static int (*const app_func[])(scamper_tbit_t *, tbit_options_t *) = {
      NULL,         tbit_app_http, tbit_app_smtp,
      tbit_app_dns, tbit_app_ftp,  tbit_app_bgp,
  };
  ...
  if (type == SCAMPER_TBIT_TYPE_SACK_RCVR) {
    if (o.dat_retx != 0 && o.dat_retx != 1) goto err;
  ...
  if ((tbit->dst = scamper_addrcache_resolve(addrcache, AF_UNSPEC, addr)) ==
      NULL) {
  ...
  if (o.src != NULL) {
    ...
    if (i != AF_INET && i != AF_INET6) goto err;
  ...
  if (app_func[o.app] != NULL && app_func[o.app](tbit, &o) != 0) goto err;

  if (type_func[type] != NULL && type_func[type](tbit, &o) != 0) goto err;
...
scamper_task_t *scamper_do_tbit_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(data, &tbit_funcs)) == NULL) goto err;
...
int scamper_do_tbit_init(void) {
  ...
  if (scamper_option_notls() == 0) {
    if ((ssl_ctx = SSL_CTX_new(SSLv23_client_method())) == NULL) return -1;
