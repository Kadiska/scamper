...
static char *tbit_header_tostr(const scamper_tbit_t *tbit,
  ...
  string_concat(buf, sizeof(buf), &off,
                ...
                scamper_tbit_type2str(tbit, tmp, sizeof(tmp)), tbit->userid);
  ...
  if (tbit->options != 0)
    string_concat(
        buf, sizeof(buf), &off, ", \"options\":[%s]",
        tbit_bits_encode(tmp, sizeof(tmp), tbit->options, 16, tbit_options,
  ...
  if (state->flags & TBIT_STATE_FLAG_CISN)
    string_concat(buf, sizeof(buf), &off, ", \"client_isn\":%u",
                  state->client_isn);
  if (state->flags & TBIT_STATE_FLAG_SISN)
    string_concat(buf, sizeof(buf), &off, ", \"server_isn\":%u",
                  state->server_isn);

  if (tbit->type == SCAMPER_TBIT_TYPE_PMTUD) {
    pmtud = tbit->data;
    string_concat(buf, sizeof(buf), &off, ", \"mtu\":%u, \"ptb_retx\":%u",
                  pmtud->mtu, pmtud->ptb_retx);
    if (pmtud->ptbsrc != NULL)
      string_concat(buf, sizeof(buf), &off, ", \"ptbsrc\":\"%s\"",
                    scamper_addr_tostr(pmtud->ptbsrc, tmp, sizeof(tmp)));
    string_concat(
        buf, sizeof(buf), &off, ", \"pmtud_options\":[%s]",
        tbit_bits_encode(tmp, sizeof(tmp), pmtud->options, 8, pmtud_options,
                         sizeof(pmtud_options) / sizeof(char *)));
  } else if (tbit->type == SCAMPER_TBIT_TYPE_NULL) {
    null = tbit->data;
    string_concat(
        buf, sizeof(buf), &off, ", \"null_options\":[%s]",
        tbit_bits_encode(tmp, sizeof(tmp), null->options, 16, null_options,
                         sizeof(null_options) / sizeof(char *)));
    string_concat(
        buf, sizeof(buf), &off, ", \"null_results\":[%s]",
        tbit_bits_encode(tmp, sizeof(tmp), null->results, 16, null_results,
    ...
    blind = tbit->data;
    string_concat(buf, sizeof(buf), &off,
                  ", \"blind_off\":%d, \"blind_retx\":%u", blind->off,
                  blind->retx);
  }
  ...
  if (tbit->app_proto == SCAMPER_TBIT_APP_HTTP && tbit->app_data != NULL) {
    ...
    if (http->host != NULL && http->file != NULL)
      string_concat(buf, sizeof(buf), &off, ", \"http_url\":\"%s://%s%s\"", str,
                    http->host, http->file);
    else if (http->host != NULL)
      string_concat(buf, sizeof(buf), &off, ", \"http_url\":\"%s://%s\"", str,
                    http->host);
  } else if (tbit->app_proto == SCAMPER_TBIT_APP_BGP &&
             tbit->app_data != NULL) {
    bgp = tbit->app_data;
    string_concat(buf, sizeof(buf), &off, ", \"app\":\"bgp\", \"bgp_asn\":%u",
                  bgp->asn);
  }
...
static char *tbit_pkt_tostr(const scamper_tbit_t *tbit,
                            ...
                            tbit_state_t *state) {
  static const char *tcpflags_str[] = {"fin", "syn", "rst", "psh",
                                       "ack", "urg", "ece", "cwr"};
  struct timeval tv;
  ...
  string_concat(buf, sizeof(buf), &off,
                "{\"dir\":%s, \"tv_sec\":%u, \"tv_usec\":%u, \"len\":%u", tmp,
                tv.tv_sec, tv.tv_usec, pkt->len);
  ...
  if (v == 4) {
    ...
    for (;;) {
      switch (proto) {
        ...
        case IPPROTO_FRAGMENT:
          if (pkt->data[iphlen + 3] & 0x1) frag_mf = 1;
    ...
    goto done; /* not v4 or v6 */

  string_concat(buf, sizeof(buf), &off,
                ", \"ip_hlen\":%u, \"ip_ecn\":%u, \"ip_ttl\":%u", iphlen, ecn,
                ttl);

  if (v == 4 || (v == 6 && frag != 0))
    string_concat(buf, sizeof(buf), &off,
                  ", \"frag_id\":%u, \"frag_off\":%u, \"frag_mf\":%u", frag_id,
                  frag_off, frag_mf);
  if (v == 4)
    string_concat(buf, sizeof(buf), &off, ", \"frag_df\":%u",
                  (pkt->data[6] & 0x40) >> 7);

  if (frag_off != 0) goto done;
  ...
  if (proto == IPPROTO_TCP) {
    ...
    while (u8 < tcphlen) {
      ...
      if (pp[1] == 0 || u8 + pp[1] > tcphlen) break;
      if (pp[0] == 3 && pp[1] == 3) {
        string_concat(buf, sizeof(buf), &off,
      ...
      if (pp[0] == 5 &&
          (pp[1] == 10 || pp[1] == 18 || pp[1] == 26 || pp[1] == 34)) {
        if (pkt->dir == SCAMPER_TBIT_PKT_DIR_TX)
        ...
        for (u16 = 0; u16 < (pp[1] - 2) / 8; u16++)
          string_concat(buf, sizeof(buf), &off, "%s{\"left\":%u, \"right\":%u}",
                        u16 != 0 ? ", " : "",
      ...
      if (pp[0] == 34 && pp[1] >= 2) {
        ...
        if (pp[1] > 2) {
          ...
          for (u16 = 0; u16 < pp[1] - 2; u16++)
            string_concat(buf, sizeof(buf), &off, "%02x", pp[2 + u16]);
          string_concat(buf, sizeof(buf), &off, "\"}");
        }
        tcpoptc++;
      }
      if (pp[0] == 254 && pp[1] >= 4 && pp[2] == 0xF9 && pp[3] == 0x89) {
        ...
        if (pp[1] > 4) {
          ...
          for (u16 = 0; u16 < pp[1] - 4; u16++)
            string_concat(buf, sizeof(buf), &off, "%02x", pp[4 + u16]);
          string_concat(buf, sizeof(buf), &off, "\"}");
        }
        tcpoptc++;
      }
...
int scamper_file_json_tbit_write(const scamper_file_t *sf,
  ...
  if ((header = tbit_header_tostr(tbit, &state)) == NULL) goto cleanup;
  len += (header_len = strlen(header));
  len += 2; /* }\n" */

  if ((str = malloc_zero(len)) == NULL) goto cleanup;
  memcpy(str + wc, header, header_len);
  wc += header_len;
  memcpy(str + wc, ", \"pkts\":[", 10);
  wc += 10;
  for (i = 0; i < tbit->pktc; i++) {
    if (i > 0) {
      memcpy(str + wc, ", ", 2);
