...
int scamper_file_text_tbit_write(const scamper_file_t *sf,
  ...
  string_concat(buf, sizeof(buf), &soff,
                ...
                tbit->server_mss, scamper_tbit_res2str(tbit, tmp, sizeof(tmp)));
  ...
  if (tbit->app_proto == SCAMPER_TBIT_APP_HTTP && tbit->app_data != NULL) {
    ...
    if (http->host != NULL && http->file != NULL)
      string_concat(buf, sizeof(buf), &soff, ", url: %s://%s%s", str,
                    http->host, http->file);
    else if (http->host != NULL)
      string_concat(buf, sizeof(buf), &soff, ", url: %s://%s", str, http->host);
    else
  ...
  for (i = 0; i < tbit->pktc; i++) {
    ...
    string_concat(buf, sizeof(buf), &soff, " [%3d.%03d] %s ", (int)diff.tv_sec,
    ...
    if (off != 0) {
      string_concat(buf, sizeof(buf), &soff, "%-13s %4dF%22s %s%s", "", len, "",
                    ipid, fstr);
      ...
      while (u8 < tcphlen) {
        ...
        if (ptr[1] == 0 || u8 + ptr[1] > tcphlen) break;
      ...
      string_concat(buf, sizeof(buf), &soff, "%-13s %4d%s", tfstr, len,
                    frag != 0 ? "F" : " ");

      toff = 0;
      string_concat(tmp, sizeof(tmp), &toff, " seq = %u:%u", seq, ack);
      if (datalen != 0) string_concat(tmp, sizeof(tmp), &toff, "(%d)", datalen);
      ...
      if (ecn == 3)
        string_concat(buf, sizeof(buf), &soff, " CE");
      else if (ecn != 0)
        string_concat(buf, sizeof(buf), &soff, " ECT");
    } else if (proto == IPPROTO_ICMP) {
      if (pkt->data[iphlen + 0] == 3 && pkt->data[iphlen + 1] == 4) {
        u16 = bytes_ntohs(pkt->data + iphlen + 6);
        string_concat(buf, sizeof(buf), &soff, "%-13s %4d  mtu = %d", "PTB",
                      len, u16);
      }
