...
static const warts_var_t tbit_vars[] = {
    {WARTS_TBIT_LIST, 4, -1},     {WARTS_TBIT_CYCLE, 4, -1},
    {WARTS_TBIT_USERID, 4, -1},   {WARTS_TBIT_SRC, -1, -1},
    {WARTS_TBIT_DST, -1, -1},     {WARTS_TBIT_SPORT, 2, -1},
    {WARTS_TBIT_DPORT, 2, -1},    {WARTS_TBIT_START, 8, -1},
    {WARTS_TBIT_RESULT, 2, -1},   {WARTS_TBIT_TYPE, 1, -1},
    {WARTS_TBIT_APPPROTO, 1, -1}, {WARTS_TBIT_CMSS, 2, -1},
    {WARTS_TBIT_SMSS, 2, -1},     {WARTS_TBIT_SYNRETX, 1, -1},
    {WARTS_TBIT_DATARETX, 1, -1}, {WARTS_TBIT_PKTC16, 2, -1},
    {WARTS_TBIT_PKTC, 4, -1},     {WARTS_TBIT_COOKIE, -1, -1},
    {WARTS_TBIT_WSCALE, 1, -1},   {WARTS_TBIT_OPTIONS, 4, -1},
    {WARTS_TBIT_TTL, 1, -1},
};
...
static void warts_tbit_icw_params(const scamper_tbit_t *tbit,
  ...
  for (i = 0; i < sizeof(tbit_icw_vars) / sizeof(warts_var_t); i++) {
    var = &tbit_icw_vars[i];
    if (var->id == WARTS_TBIT_ICW_FIRSTSEQ && icw->start_seq == 0) continue;
    flag_set(state->flags, var->id, &max_id);
    assert(var->size >= 0);
    state->params_len += var->size;
  }
...
static void warts_tbit_null_params(const scamper_tbit_t *tbit,
  ...
  for (i = 0; i < sizeof(tbit_null_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TBIT_NULL_OPTIONS && null->options == 0) continue;
    if (var->id == WARTS_TBIT_NULL_RESULTS && null->results == 0) continue;
...
static void warts_tbit_pmtud_params(const scamper_tbit_t *tbit,
  ...
  for (i = 0; i < sizeof(tbit_pmtud_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TBIT_PMTUD_MTU && pmtud->mtu == 0) continue;
    if (var->id == WARTS_TBIT_PMTUD_PTBRETX && pmtud->ptb_retx == 0) continue;
    if (var->id == WARTS_TBIT_PMTUD_OPTIONS && pmtud->options == 0) continue;
    if (var->id == WARTS_TBIT_PMTUD_PTBSRC && pmtud->ptbsrc == NULL) continue;
...
static void warts_tbit_app_http_params(const scamper_tbit_t *tbit,
  ...
  for (i = 0; i < sizeof(tbit_app_http_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TBIT_APP_HTTP_HOST && http->host == NULL) continue;
    if (var->id == WARTS_TBIT_APP_HTTP_FILE && http->file == NULL) continue;
    if (var->id == WARTS_TBIT_APP_HTTP_TYPE && http->type == 0) continue;
...
static int warts_tbit_app_http_read(scamper_tbit_t *tbit, const uint8_t *buf,
  ...
  if ((http = scamper_tbit_app_http_alloc(type, host, file)) == NULL) goto err;
  if (host != NULL) {
    free(host);
    host = NULL;
  }
...
static void warts_tbit_app_bgp_params(const scamper_tbit_t *tbit,
  ...
  for (i = 0; i < sizeof(tbit_app_bgp_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TBIT_APP_BGP_ASN && bgp->asn == 0) continue;
...
int extract_cookie(const uint8_t *buf, uint32_t *off, const uint32_t len,
  ...
  if (cookielen > 16 || cookielen > len - *off) return -1;
...
static int warts_tbit_params_read(scamper_tbit_t *tbit,
  ...
  if (tbit->src == NULL || tbit->dst == NULL) return -1;
  ...
  if (flag_isset(&buf[o], WARTS_TBIT_TTL) == 0) tbit->ttl = 255;

  return 0;
}

static int warts_tbit_params_write(
    const scamper_tbit_t *tbit, const scamper_file_t *sf,
...
int scamper_file_warts_tbit_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  switch (tbit->type) {
    case SCAMPER_TBIT_TYPE_PMTUD:
      if ((tbit->data = scamper_tbit_pmtud_alloc()) == NULL) goto err;
      break;

    case SCAMPER_TBIT_TYPE_NULL:
      if ((tbit->data = scamper_tbit_null_alloc()) == NULL) goto err;
      break;

    case SCAMPER_TBIT_TYPE_ICW:
      if ((tbit->data = scamper_tbit_icw_alloc()) == NULL) goto err;
      break;
    ...
    case SCAMPER_TBIT_TYPE_BLIND_FIN:
      if ((tbit->data = scamper_tbit_blind_alloc()) == NULL) goto err;
      break;
  }
  ...
  if (tbit->pktc > 0) {
    ...
    if (scamper_tbit_pkts_alloc(tbit, tbit->pktc) != 0) goto err;
    ...
    for (i = 0; i < tbit->pktc; i++) {
      ...
      if (tbit->pkts[i] == NULL) goto err;
    }
  }

  for (;;) {
    if (extract_uint16(buf, &off, hdr->len, &junk16, NULL) != 0) goto err;
    if (junk16 == WARTS_TBIT_STRUCT_EOF) break;
    if (extract_uint32(buf, &off, hdr->len, &junk32, NULL) != 0) goto err;

    if (hdr->len - off < junk32) goto err;
    ...
    if (junk16 == WARTS_TBIT_STRUCT_TYPE) {
      switch (tbit->type) {
        ...
        case SCAMPER_TBIT_TYPE_NULL:
          if (warts_tbit_null_read(tbit, buf, &i, hdr->len) != 0) goto err;
          break;

        case SCAMPER_TBIT_TYPE_ICW:
          if (warts_tbit_icw_read(tbit, buf, &i, hdr->len) != 0) goto err;
          break;
        ...
        case SCAMPER_TBIT_TYPE_BLIND_DATA:
        case SCAMPER_TBIT_TYPE_BLIND_FIN:
          if (warts_tbit_blind_read(tbit, buf, &i, hdr->len) != 0) goto err;
          break;
      }
    } else if (junk16 == WARTS_TBIT_STRUCT_APP) {
      if (tbit->app_proto == SCAMPER_TBIT_APP_HTTP) {
        if (warts_tbit_app_http_read(tbit, buf, &i, hdr->len) != 0) goto err;
      } else if (tbit->app_proto == SCAMPER_TBIT_APP_BGP) {
        if (warts_tbit_app_bgp_read(tbit, buf, &i, hdr->len) != 0) goto err;
      }
    } else
      goto err;
...
int scamper_file_warts_tbit_write(const scamper_file_t *sf,
  ...
  if (tbit->pktc > 0) {
    ...
    if ((pkts = (warts_tbit_pkt_t *)malloc_zero(size)) == NULL) goto err;
  ...
  if (warts_tbit_params_write(tbit, sf, table, buf, &off, len, flags, flags_len,
                              params_len) != 0) {
    goto err;
  }
  ...
  if (warts_write(sf, buf, len) == -1) goto err;
