...
int scamper_trace_pmtud_hop_count(const scamper_trace_t *trace) {
  ...
  int count = 0;
  if (trace == NULL || trace->pmtud == NULL) return -1;
  for (hop = trace->pmtud->hops; hop != NULL; hop = hop->hop_next) count++;
  return count;
}
...
int scamper_trace_pmtud_n_alloc_c(scamper_trace_pmtud_t *pmtud, uint8_t count) {
  ...
  if ((pmtud->notes = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_trace_pmtud_n_add(scamper_trace_pmtud_t *pmtud,
  ...
  if (realloc_wrap((void **)&pmtud->notes, len) != 0) return -1;
...
int scamper_trace_lastditch_hop_count(const scamper_trace_t *trace) {
  scamper_trace_hop_t *hop;
  int count = 0;
  if (trace == NULL || trace->lastditch == NULL) return -1;
  for (hop = trace->lastditch; hop != NULL; hop = hop->hop_next) count++;
  return count;
}
...
void scamper_trace_dtree_free(scamper_trace_t *trace) {
  ...
  if (trace->dtree == NULL) return;

  if (trace->dtree->lss_stop != NULL) scamper_addr_free(trace->dtree->lss_stop);
  if (trace->dtree->gss_stop != NULL) scamper_addr_free(trace->dtree->gss_stop);
  if (trace->dtree->lss != NULL) free(trace->dtree->lss);

  if (trace->dtree->gss != NULL) {
    for (i = 0; i < trace->dtree->gssc; i++)
      if (trace->dtree->gss[i] != NULL) scamper_addr_free(trace->dtree->gss[i]);
...
int scamper_trace_dtree_gss_alloc(scamper_trace_t *trace, uint16_t cnt) {
  if (trace->dtree == NULL || trace->dtree->gss != NULL) return -1;
...
scamper_addr_t *scamper_trace_dtree_gss_find(const scamper_trace_t *trace,
  ...
  if (trace->dtree == NULL) return NULL;
  return array_find((void **)trace->dtree->gss, trace->dtree->gssc, iface,
...
void scamper_trace_hop_free(scamper_trace_hop_t *hop) {
  ...
  if (hop->hop_name != NULL) free(hop->hop_name);
...
int scamper_trace_hop_count(const scamper_trace_t *trace) {
  ...
  for (i = 0; i < trace->hop_count; i++)
    for (hop = trace->hops[i]; hop != NULL; hop = hop->hop_next) hops++;
...
const char *scamper_trace_type_tostr(const scamper_trace_t *t, char *b,
                                     size_t l) {
  static const char *m[] = {
      NULL,        "icmp-echo", "udp", "tcp", "icmp-echo-paris",
      "udp-paris", "tcp-ack",
  };
...
const char *scamper_trace_stop_tostr(const scamper_trace_t *t, char *b,
                                     size_t l) {
  static const char *r[] = {
      "NONE",     "COMPLETED", "UNREACH",  "ICMP", "LOOP",
      "GAPLIMIT", "ERROR",     "HOPLIMIT", "GSS",  "HALTED",
  };
  if (t->stop_reason >= sizeof(r) / sizeof(char *) ||
      r[t->stop_reason] == NULL) {
...
int scamper_trace_iscomplete(const scamper_trace_t *trace) {
  ...
  if (trace->stop_reason != SCAMPER_TRACE_STOP_COMPLETED) return 0;

  for (i = trace->firsthop - 1; i < trace->hop_count; i++)
    if (trace->hops[i] == NULL) return 0;
...
static int trace_hop_firstaddr(const scamper_trace_t *trace,
  ...
  while (tmp != hop) {
    if (scamper_trace_hop_addr_cmp(tmp, hop) == 0) return 0;
    tmp = tmp->hop_next;
  }
...
int scamper_trace_loop(const scamper_trace_t *trace, const int n,
  ...
  while (i < trace->hop_count) {
    if (hop == NULL) {
      ...
      while (i < trace->hop_count) {
        if ((hop = trace->hops[i]) != NULL) break;
        i++;
      }
