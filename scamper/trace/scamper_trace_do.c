...
#define MODE_IS_PMTUD(mode)                                          \
  ((mode) == MODE_PMTUD_DEFAULT || (mode) == MODE_PMTUD_SILENT_L2 || \
   (mode) == MODE_PMTUD_SILENT_TTL || (mode) == MODE_PMTUD_BADSUGG)

#define MODE_IS_PARALLEL(mode) \
  ((mode) == MODE_PARALLEL || (mode) == MODE_PARALLEL_FINISH)

#define MODE_IS_DTREE(mode)                                  \
  ((mode) == MODE_DTREE_FIRST || (mode) == MODE_DTREE_FWD || \
...
const char *scamper_do_trace_usage(void) {
  return "trace [-MQT] [-c confidence] [-d dport] [-f firsthop]\n"
         "      [-g gaplimit] [-G gapaction] [-l loops] [-m maxttl] [-N "
         "squeries]\n"
         "      [-o offset] [-O options] [-p payload] [-P method] [-q "
         "attempts]\n"
...
static void trace_stop(scamper_trace_t *trace, const uint8_t reason,
  ...
  if (trace->stop_reason != SCAMPER_TRACE_STOP_NONE) {
    scamper_debug(__func__, "reason %d/%d precedes %d/%d", trace->stop_reason,
                  trace->stop_data, reason, data);
    return;
  }
...
static int trace_queue(scamper_task_t *task, const struct timeval *now) {
  ...
  /* set a timeout based on when the next probe in the window expires */
  if (MODE_IS_PARALLEL(state->mode)) {
    ...
    while ((hs = dlist_head_item(state->window)) != NULL) {
      ...
      if (timeval_cmp(&next_tx, now) > 0) break;
      dlist_head_pop(state->window);

      probe = state->probes[hs->id];
      assert(probe->rx == 0);
      ...
      if (probe->ttl > trace->hop_count) trace->hop_count = probe->ttl;
      free(hs);
    }

    /* probe ready to go right now */
    if (slist_count(state->probeq) > 0) goto probe;
    ...
    if (state->mode == MODE_PARALLEL) {
      ...
      if (dlist_count(state->window) == 0 && slist_count(state->probeq) == 0)
        goto done;
    }
    ...
    if (trace->hop_count - (trace->firsthop - 1) >= trace->gaplimit &&
        dlist_count(state->window) == 0 && slist_count(state->probeq) == 0) {
      /* see if there are any responses for the possible gaplimit hops */
      for (i = 0; i < trace->gaplimit; i++)
        if (trace->hops[trace->hop_count - 1 - i] != NULL) break;
    ...
    hs = dlist_head_item(state->window);
    assert(hs != NULL);
    timeval_add_s(&next_tx, &hs->last_tx, trace->wait);
    if (timeval_cmp(&next_tx, now) <= 0) goto probe;
...
probe:
   ...
   */
  if (trace->wait_probe == 0) return scamper_task_queue_probe(task);

  /* enforce the minimum delay between probes */
  timeval_add_cs(&next_tx, &state->last_tx, trace->wait_probe);
  if (timeval_cmp(&next_tx, now) <= 0) return scamper_task_queue_probe(task);
  return scamper_task_queue_wait_tv(task, &next_tx);
...
static void trace_lss_free(trace_lss_t *lss) {
  ...
  if (lss->name != NULL) free(lss->name);
  if (lss->tree != NULL)
...
static trace_lss_t *trace_lss_get(char *name) {
  ...
  findme.name = name;
  if ((lss = splaytree_find(lsses, &findme)) != NULL) return lss;
...
static int pmtud_L2_init(trace_state_t *state) {
   ...
   */
    ...
    for (idx = 0; idx < L2_cnt - 1; idx++)
      if (size > L2[idx].mtu && size <= L2[idx + 1].mtu) break;
  }
...
static scamper_trace_hop_t *hop_find(const scamper_trace_t *trace,
  ...
  for (i = trace->firsthop - 1; i < trace->hop_count; i++) {
    for (hop = trace->hops[i]; hop != NULL; hop = hop->hop_next) {
      if (scamper_addr_cmp(hop->hop_addr, addr) == 0) return hop;
    }
  }
...
static int pmtud_TTL_init(scamper_task_t *task) {
  ...
  if (pmtud_TTL_set_probettl(task, lower, upper) == 0) return 0;
...
static int dtree_lss_in(trace_state_t *state, scamper_addr_t *iface) {
  assert(state != NULL && state->lsst != NULL);
  if (splaytree_find(state->lsst->tree, iface) != NULL) return 1;
  return 0;
}
...
static int trace_isloop(const scamper_trace_t *trace,
   ...
   * check it again.
   */
  for (tmp = trace->hops[hop->hop_probe_ttl - 1]; tmp != hop;
       tmp = tmp->hop_next)
    if (scamper_addr_cmp(hop->hop_addr, tmp->hop_addr) == 0) return 0;
  ...
  for (i = trace->firsthop - 1; i < trace->hop_count; i++) {
    ...
    if (i == hop->hop_probe_ttl - 1) continue;
    ...
    for (tmp = trace->hops[i]; tmp != NULL; tmp = tmp->hop_next) {
      ...
      if (scamper_addr_cmp(hop->hop_addr, tmp->hop_addr) == 0) {
        ...
        state->loopc++;
        if (state->loopc >= trace->loops) return 1;
...
static int trace_hop_ptr(const scamper_task_t *task, scamper_trace_hop_t *hop) {
  ...
  if ((trace->flags & SCAMPER_TRACE_FLAG_PTR) == 0) return 0;
...
static scamper_trace_hop_t *trace_hop(const scamper_task_t *task,
                                      const trace_probe_t *probe, const int af,
                                      const void *addr) {
...
static scamper_trace_hop_t *trace_icmp_hop(const scamper_task_t *task,
  ...
  if (scamper_icmp_resp_src(ir, &addr) != 0) goto err;

  /* create a generic hop record without any special bits filled out */
  if ((hop = trace_hop(task, probe, ir->ir_af, addr.addr)) == NULL) goto err;
  ...
  if (SCAMPER_ICMP_RESP_INNER_IS_SET(ir)) {
     ...
     */
    if (ir->ir_af == AF_INET) hop->hop_icmp_q_tos = ir->ir_inner_ip_tos;
  }

  /* if ICMP extensions are included, then parse and include them. */
  if (ir->ir_ext != NULL && scamper_icmpext_parse(&hop->hop_icmpext, ir->ir_ext,
                                                  ir->ir_extlen) != 0) {
    goto err;
  }
...
static scamper_trace_hop_t *trace_dl_hop(scamper_task_t *task,
  ...
  /* create a generic hop record without any special bits filled out */
  if ((hop = trace_hop(task, pr, dl->dl_af, dl->dl_ip_src)) == NULL) goto err;

  /* fill out the basic bits of the hop structure */
  hop->hop_reply_size = dl->dl_ip_size;
  hop->hop_reply_ttl = dl->dl_ip_ttl;
  hop->hop_flags |=
      (SCAMPER_TRACE_HOP_FLAG_REPLY_TTL | SCAMPER_TRACE_HOP_FLAG_TS_DL_RX);
...
static uint8_t trace_first_mode(const scamper_trace_t *trace) {
  if (SCAMPER_TRACE_IS_DOUBLETREE(trace)) return MODE_DTREE_FIRST;
  if (trace->squeries > 1) return MODE_PARALLEL;
  return MODE_TRACE;
}
...
static void trace_next_mode(scamper_task_t *task, const struct timeval *now) {
  ...
  if (SCAMPER_TRACE_IS_PMTUD(trace)) {
    ...
    if (scamper_if_getmtu(ifindex, &ifmtu) == -1 || ifmtu <= state->header_size)
      goto done;
    if (scamper_trace_pmtud_alloc(trace) != 0) goto done;
...
static int handleicmp_trace(scamper_task_t *task, scamper_icmp_resp_t *ir,
  ...
  assert(state->mode == MODE_TRACE || state->mode == MODE_DTREE_FWD ||
         state->mode == MODE_DTREE_BACK);

  /* we should only have to deal with probes sent while in the trace state */
  if (probe->mode != MODE_TRACE && probe->mode != MODE_DTREE_FWD &&
      probe->mode != MODE_DTREE_BACK) {
    return 0;
  }

  /* create a hop record and insert it into the trace */
  if ((hop = trace_icmp_hop(task, probe, ir)) == NULL) return -1;
  ...
  if (SCAMPER_TRACE_IS_ALLATTEMPTS(trace)) {
    ...
    if (state->n <= TRACE_CONFIDENCE_MAX_N) {
       ...
       * probes to send for this hop, then send the next one
       */
      if (state->attempt < k(state)) goto probe;
    }
  ...
  if (trace->confidence == 0 && SCAMPER_TRACE_IS_ALLATTEMPTS(trace) == 0) {
    ...
    hop = trace->hops[trace->hop_count - 1];
    assert(hop != NULL);
    while (hop != NULL) {
      trace_stop_reason(trace, hop, state, &stop_reason, &stop_data);
      if (stop_reason != SCAMPER_TRACE_STOP_NONE) {
...
static int handleicmp_pmtud_default(scamper_task_t *task,
   ...
   */
  if (probe->mode != MODE_PMTUD_DEFAULT) return 0;
...
static int handleicmp_pmtud_silent_L2(scamper_task_t *task,
  ...
  if (probe->size < l2->lower || l2->upper <= probe->size) {
    scamper_debug(__func__, "L2 search %d < %d || %d <= %d", probe->size,
                  l2->lower, l2->upper, probe->size);
    return 0;
  }
...
static int handleicmp_pmtud_silent_TTL(scamper_task_t *task,
  ...
  if (SCAMPER_ICMP_RESP_IS_TTL_EXP(ir)) {
    ...
    if (pmtud_TTL_set_probettl(task, probe->ttl, state->pmtud->TTL->upper) ==
        0) {
      /*
       * if we are not finished with PMTU yet, put the trace back in
       * the queue
       */
      if (pmtud_L2_search_end(task) == 1) return 0;
    }
...
static int handleicmp_pmtud_badsugg(scamper_task_t *task,
  ...
  if (scamper_icmp_resp_src(ir, &addr) != 0) return -1;
...
static int handleicmp_parallel(scamper_task_t *task, scamper_icmp_resp_t *ir,
                               trace_probe_t *probe) {
  scamper_trace_t *trace = trace_getdata(task);
  trace_state_t *state = trace_getstate(task);
  scamper_trace_hop_t *hop = NULL;
  ...
  if (MODE_IS_PARALLEL(state->mode) == 0) return 0;
  ...
  if (probe->ttl > state->max_ttl) state->max_ttl = probe->ttl;

  /* remove the trace_hop_state_t from the window */
  for (dn = dlist_head_node(state->window); dn != NULL;
  ...
  if (state->mode == MODE_PARALLEL_FINISH && slist_count(state->probeq) == 0 &&
...
static void do_trace_handle_icmp(scamper_task_t *task,
                                 scamper_icmp_resp_t *ir) {
   ...
   * stop now
   */
  if (func[state->mode] == NULL) return;
  ...
  if (trace->offset != 0) {
    if (ir->ir_inner_data == NULL) return;
    ...
    if (trace->dst->type == SCAMPER_ADDR_TYPE_IPV4) {
      ...
      if (ir->ir_inner_datalen < 8) return;
      ...
      if ((ir->ir_inner_ip_id >> 16) != trace->sport) return;

      if (ir->ir_inner_datalen < 4) return;
    ...
    if (trace->type == SCAMPER_TRACE_TYPE_UDP) {
      ...
      if (ir->ir_af == AF_INET) {
        ...
        if (ir->ir_inner_ip_flow == 0) return;
    ...
    if (ir->ir_af == AF_INET) {
      ...
      if (trace_ipid_fudge(state, ir->ir_inner_ip_id, &id) != 0) return;
    } else {
      if (ir->ir_inner_ip_flow == 0) return;
      id = ir->ir_inner_ip_flow - 1;
    }
...
static void timeout_trace(scamper_task_t *task) {
  ...
  if (SCAMPER_TRACE_IS_ALLATTEMPTS(trace) || trace->confidence != 0) {
    for (hop = trace->hops[trace->hop_count - 1]; hop != NULL;
         hop = hop->hop_next) {
      /*
...
static void timeout_dtree_back(scamper_task_t *task) {
  ...
  if (state->lsst != NULL &&
    ...
    for (hop = trace->hops[state->ttl - 1]; hop != NULL; hop = hop->hop_next) {
      if (dtree_lss_in(state, hop->hop_addr) != 0) goto next_mode;
    }
  }
...
static int handletp_trace(scamper_task_t *task, scamper_dl_rec_t *dl,
  ...
  /* we should only have to deal with probes sent while in the trace state */
  if (probe->mode != MODE_TRACE) return 0;

  /* create a hop record based off the TCP data */
  if ((hop = trace_dl_hop(task, probe, dl)) == NULL) return -1;
  trace_hopins(&trace->hops[hop->hop_probe_ttl - 1], hop);
  ...
  if (SCAMPER_TRACE_IS_ALLATTEMPTS(trace)) {
    if (probe->attempt != trace->attempts) goto probe;
    trace->hop_count++;
  } else if (trace->confidence != 0) {
...
static void dlin_trace(scamper_trace_t *trace, scamper_dl_rec_t *dl,
  ...
  for (hop = trace->hops[probe->ttl - 1]; hop != NULL; hop = hop->hop_next) {
    ...
    scamper_debug(__func__, "hop %d.%06d dl_rec %d.%06d diff %d",
                  hop->hop_rtt.tv_sec, hop->hop_rtt.tv_usec, tv.tv_sec,
                  tv.tv_usec, timeval_diff_us(&hop->hop_rtt, &tv));
...
static void do_trace_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if ((dl->dl_flags & SCAMPER_DL_REC_FLAG_TIMESTAMP) == 0) return;

  if (SCAMPER_DL_IS_IP(dl) == 0) return;
  ...
  if (trace->type == SCAMPER_TRACE_TYPE_UDP ||
    ...
    if (dl->dl_ip_proto == IPPROTO_UDP) {
      ...
      if (dl->dl_icmp_ip_proto != IPPROTO_UDP) return;
      if (dl->dl_icmp_udp_sport != trace->sport) return;
      ...
      if (trace->type == SCAMPER_TRACE_TYPE_UDP) {
        ...
        if (dl->dl_icmp_udp_dport != trace->dport) return;

        if (dl->dl_af == AF_INET) {
          if (ntohs(dl->dl_icmp_udp_sum) == dl->dl_icmp_ip_id &&
              dl->dl_icmp_udp_sum != 0) {
            probe_id = ntohs(dl->dl_icmp_udp_sum) - 1;
          } else if (trace_ipid_fudge(state, dl->dl_icmp_ip_id, &probe_id) !=
                     0) {
            return;
          }
        } else if ((trace->flags & SCAMPER_TRACE_FLAG_CONSTPAYLOAD) == 0) {
          if (dl->dl_icmp_udp_sum == 0) return;
          probe_id = ntohs(dl->dl_icmp_udp_sum) - 1;
        } else {
      ...
      return;
             ...
             trace->type == SCAMPER_TRACE_TYPE_ICMP_ECHO_PARIS) {
    if (SCAMPER_DL_IS_ICMP(dl) == 0) return;

    if (SCAMPER_DL_IS_ICMP_ECHO_REQUEST(dl)) {
      if (dl->dl_icmp_id != trace->sport) return;
      ...
      if (dl->dl_icmp_icmp_id != trace->sport) return;
    ...
    if (dl->dl_ip_proto == IPPROTO_TCP) {
      ...
      if (dl->dl_af == AF_INET) {
        if (trace_ipid_fudge(state, dl->dl_icmp_ip_id, &probe_id) != 0) return;
      } else {
        if (dl->dl_icmp_ip_flow == 0) return;
  ...
  if (probe_id >= state->id_next) return;
  ...
  if (direction == 0) {
    ...
    scamper_debug(__func__, "probe %d.%06d dl %d.%06d diff %d",
                  probe->tx_tv.tv_sec, probe->tx_tv.tv_usec, dl->dl_tv.tv_sec,
                  dl->dl_tv.tv_usec,
...
static void trace_handle_rt(scamper_route_t *rt) {
  ...
  if (state->mode != MODE_RTSOCK || state->route != rt) goto done;
...
static int trace_state_alloc(scamper_task_t *task) {
  ...
  state->n = 2;
  if (trace->confidence == 99) state->confidence = 1;
  ...
  if (trace->dtree != NULL && trace->dtree->lss != NULL) {
    if ((state->lsst = trace_lss_get(trace->dtree->lss)) == NULL) goto err;
  }
...
#ifndef _WIN32
    if ((state->rtsock = scamper_fd_rtsock()) == NULL) goto err;
#endif
  } else {
    state->mode = trace_first_mode(trace);
  }
  ...
  if (scamper_option_icmp_rxerr() != 0) {
    ...
    if (state->icmp == NULL || state->probe == NULL) goto err;
  } else {
    if (trace->dst->type == SCAMPER_ADDR_TYPE_IPV4) {
...
static void do_trace_probe(scamper_task_t *task) {
  ...
  if (state != NULL) {
    assert(MODE_IS_PARALLEL(state->mode) || state->attempt < trace->attempts ||
           trace->confidence != 0);
    ...
    if (trace_state_alloc(task) != 0) goto err;
  ...
  if (probe.pr_ip_ttl >= state->alloc_hops &&
      (state->mode == MODE_TRACE || MODE_IS_DTREE(state->mode) ||
       MODE_IS_PARALLEL(state->mode))) {
    /*
    ...
    for (i = state->alloc_hops; i < u16; i++) trace->hops[i] = NULL;
  ...
  if (state->dl != NULL &&
      (MODE_IS_PMTUD(state->mode) || trace->offset != 0 || trace->rtr != NULL ||
       (SCAMPER_TRACE_TYPE_IS_UDP_PARIS(trace) && sunos != 0) ||
  ...
  if (trace->payload_len == 0 || MODE_IS_PMTUD(state->mode)) {
    if (probe.pr_len > 0) memset(probe.pr_data, 0, probe.pr_len);
  } else {
...
static int trace_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case TRACE_OPT_TOS:
      if (string_tolong(param, &tmp) == -1 || tmp < SCAMPER_DO_TRACE_TOS_MIN ||
    ...
    case TRACE_OPT_WAIT:
      if (string_tolong(param, &tmp) == -1 || tmp < SCAMPER_DO_TRACE_WAIT_MIN ||
    ...
    case TRACE_OPT_CONFIDENCE:
      if (string_tolong(param, &tmp) != 0 || (tmp != 95 && tmp != 99)) goto err;
      break;
    ...
    case TRACE_OPT_USERID:
      if (string_tolong(param, &tmp) != 0 || tmp < 0) goto err;
      break;
...
void *scamper_do_trace_alloc(char *str) {
  ...
  if (firsthop > hoplimit && hoplimit != 0) goto err;
  ...
  if (confidence != 0 && (flags & SCAMPER_TRACE_FLAG_ALLATTEMPTS)) goto err;
  ...
  if (SCAMPER_TRACE_TYPE_IS_TCP(trace) && trace->payload_len > 0) goto err;
  ...
  if (trace->squeries > trace->gaplimit) goto err;
...
int scamper_do_trace_dtree_lss_clear(char *name) {
  ...
  findme.name = name;
  if ((lss = splaytree_find(lsses, &findme)) == NULL) return -1;
...
scamper_task_t *scamper_do_trace_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(trace, &trace_funcs)) == NULL) goto err;
...
int scamper_do_trace_init(void) {
  ...
  if (SCAMPER_OSINFO_IS_SUNOS(osinfo)) sunos = 1;
