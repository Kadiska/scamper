...
static char *hop_tostr(const scamper_trace_t *trace, scamper_trace_hop_t *hop) {
  ...
  string_concat(buf, sizeof(buf), &off, ", \"reply_ttl\":%u, \"reply_tos\":%u",
                hop->hop_reply_ttl, hop->hop_reply_tos);

  if ((trace->flags & SCAMPER_TRACE_FLAG_RXERR) == 0) {
    string_concat(buf, sizeof(buf), &off,
                  ", \"reply_ipid\":%u, \"reply_size\":%u", hop->hop_reply_ipid,
                  hop->hop_reply_size);
  }

  if (SCAMPER_TRACE_HOP_IS_ICMP(hop)) {
    string_concat(buf, sizeof(buf), &off,
                  ", \"icmp_type\":%u, \"icmp_code\":%u", hop->hop_icmp_type,
                  hop->hop_icmp_code);
  ...
  if (hop->hop_icmpext != NULL) {
    ...
    for (ie = hop->hop_icmpext; ie != NULL; ie = ie->ie_next) {
      if (ie != hop->hop_icmpext) string_concat(buf, sizeof(buf), &off, ",");
      string_concat(buf, sizeof(buf), &off,
                    "{\"ie_cn\":%u,\"ie_ct\":%u,\"ie_dl\":%u", ie->ie_cn,
                    ie->ie_ct, ie->ie_dl);
...
static char *header_tostr(const scamper_trace_t *trace) {
  ...
  string_concat(
      buf, sizeof(buf), &off, ", \"stop_reason\":\"%s\", \"stop_data\":%u",
      scamper_trace_stop_tostr(trace, tmp, sizeof(tmp)), trace->stop_data);
...
int scamper_file_json_trace_write(const scamper_file_t *sf,
  ...
  for (i = trace->firsthop - 1; i < trace->hop_count; i++)
    for (hop = trace->hops[i]; hop != NULL; hop = hop->hop_next) hopc++;
  if (hopc > 0) {
    len += 11; /* , "hops":[] */
