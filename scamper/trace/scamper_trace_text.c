...
static char *hop_tostr(const scamper_trace_t *trace, const int h) {
  ...
  replyc = 0;
  for (hop = trace->hops[h]; hop != NULL; hop = hop->hop_next) replyc++;
  ...
  if (replyc == 1) {
    ...
    snprintf(str_hop, sizeof(str_hop), "%2d  %s  %s ms%s", h + 1, str_addr,
             str_rtt, str_icmp);
  ...
  if ((str_addrs = malloc_zero(len)) == NULL) goto out;
  if ((str_rtts = malloc_zero(len)) == NULL) goto out;

  /* keep track of the length of each string in the arrays */
  len = sizeof(size_t) * replyc;
  if ((len_addrs = malloc_zero(len)) == NULL) goto out;
  if ((len_rtts = malloc_zero(len)) == NULL) goto out;
  ...
  for (hop = trace->hops[h]; hop != NULL; hop = hop->hop_next) {
    ...
    len = strlen(str_addr);
    if ((str_addrs[i] = malloc_zero(len + 1)) == NULL) goto out;
    ...
    if ((str_rtts[i] = malloc_zero(len + 1)) == NULL) goto out;
  ...
  if ((str = malloc_zero(len)) == NULL) goto out;
...
static int pmtud_ver1(const scamper_trace_t *trace, char **mtus) {
  ...
  if ((hop = trace->pmtud->hops) == NULL) {
    ...
    if (trace->pmtud->outmtu != 0) size = trace->pmtud->outmtu;
  }
  ...
  for (i = 0; i < trace->hop_count; i++) {
    ...
    if (scamper_addr_cmp(hop->hop_addr, trace->hops[i]->hop_addr) == 0) {
      if ((mtus[i] = mtu_tostr(mtu, size)) == NULL) return -1;

      if (SCAMPER_TRACE_HOP_IS_ICMP_PTB(hop)) mtu = hop->hop_icmp_nhmtu;
    ...
    if (i >= hop->hop_probe_ttl - hop->hop_icmp_q_ttl) {
      if (SCAMPER_TRACE_HOP_IS_ICMP_PTB(hop)) size = mtu = hop->hop_icmp_nhmtu;
  ...
  while (n < pmtud->notec) {
    ...
    if (note->type == SCAMPER_TRACE_PMTUD_N_TYPE_SILENCE) {
      ...
      for (i = h; i < trace->hop_count; i++) {
        if (trace->hops[i] == NULL) continue;
        if (scamper_addr_cmp(trace->hops[i]->hop_addr, ha) == 0) break;
      }
      ...
      if (i == trace->hop_count) {
        ...
        /* handle wrap */
        if (i >= (trace->hop_count - 1)) i = trace->hop_count - 2;
        if (i < 0) i = 0;
      }
    }
    ...
    if (SCAMPER_TRACE_HOP_IS_ICMP_PTB(hop)) {
      ...
      if (addr[0] != '\0' || note->nhmtu != hop->hop_icmp_nhmtu) {
        ...
        free(mtus[i]);
        if ((mtus[i] = strdup(buf)) == NULL) return -1;
      }
...
int scamper_file_text_trace_write(const scamper_file_t *sf,
  ...
  if (trace->hop_count > 0) {
    ...
    for (i = 0; i < trace->hop_count; i++) {
      if ((hops[i] = hop_tostr(trace, i)) == NULL) goto cleanup;
      len += strlen(hops[i]);
    }
    ...
    if (trace->pmtud != NULL && trace->pmtud->ver >= 1 &&
      ...
      if (pmtud_tostr[trace->pmtud->ver](trace, mtus) != 0) goto cleanup;

      for (i = 0; i < trace->hop_count; i++)
        if (mtus[i] != NULL) len += strlen(mtus[i]);
    }
