...
static const warts_var_t trace_vars[] = {
    {WARTS_TRACE_LIST_ID, 4, -1},      {WARTS_TRACE_CYCLE_ID, 4, -1},
    {WARTS_TRACE_ADDR_SRC_GID, 4, -1}, {WARTS_TRACE_ADDR_DST_GID, 4, -1},
    {WARTS_TRACE_START, 8, -1},        {WARTS_TRACE_STOP_R, 1, -1},
    {WARTS_TRACE_STOP_D, 1, -1},       {WARTS_TRACE_FLAGS8, 1, -1},
    {WARTS_TRACE_ATTEMPTS, 1, -1},     {WARTS_TRACE_HOPLIMIT, 1, -1},
    {WARTS_TRACE_TYPE, 1, -1},         {WARTS_TRACE_PROBE_S, 2, -1},
    {WARTS_TRACE_PORT_SRC, 2, -1},     {WARTS_TRACE_PORT_DST, 2, -1},
    {WARTS_TRACE_FIRSTHOP, 1, -1},     {WARTS_TRACE_TOS, 1, -1},
    {WARTS_TRACE_WAIT, 1, -1},         {WARTS_TRACE_LOOPS, 1, -1},
    {WARTS_TRACE_HOPCOUNT, 2, -1},     {WARTS_TRACE_GAPLIMIT, 1, -1},
    {WARTS_TRACE_GAPACTION, 1, -1},    {WARTS_TRACE_LOOPACTION, 1, -1},
    {WARTS_TRACE_PROBEC, 2, -1},       {WARTS_TRACE_WAITPROBE, 1, -1},
    {WARTS_TRACE_CONFIDENCE, 1, -1},   {WARTS_TRACE_ADDR_SRC, -1, -1},
    {WARTS_TRACE_ADDR_DST, -1, -1},    {WARTS_TRACE_USERID, 4, -1},
    {WARTS_TRACE_OFFSET, 2, -1},       {WARTS_TRACE_ADDR_RTR, -1, -1},
    {WARTS_TRACE_SQUERIES, 1, -1},     {WARTS_TRACE_FLAGS, 4, -1},
};
...
static const warts_var_t pmtud_vars[] = {
    {WARTS_TRACE_PMTUD_IFMTU, 2, -1},  {WARTS_TRACE_PMTUD_PMTU, 2, -1},
    {WARTS_TRACE_PMTUD_OUTMTU, 2, -1}, {WARTS_TRACE_PMTUD_VER, 1, -1},
...
static const warts_var_t hop_vars[] = {
    {WARTS_TRACE_HOP_ADDR_GID, 4, -1},    {WARTS_TRACE_HOP_PROBE_TTL, 1, -1},
    {WARTS_TRACE_HOP_REPLY_TTL, 1, -1},   {WARTS_TRACE_HOP_FLAGS, 1, -1},
    {WARTS_TRACE_HOP_PROBE_ID, 1, -1},    {WARTS_TRACE_HOP_RTT, 4, -1},
    {WARTS_TRACE_HOP_ICMP_TC, 2, -1},     {WARTS_TRACE_HOP_PROBE_SIZE, 2, -1},
    {WARTS_TRACE_HOP_REPLY_SIZE, 2, -1},  {WARTS_TRACE_HOP_REPLY_IPID, 2, -1},
    {WARTS_TRACE_HOP_REPLY_IPTOS, 1, -1}, {WARTS_TRACE_HOP_NHMTU, 2, -1},
    {WARTS_TRACE_HOP_Q_IPLEN, 2, -1},     {WARTS_TRACE_HOP_Q_IPTTL, 1, -1},
    {WARTS_TRACE_HOP_TCP_FLAGS, 1, -1},   {WARTS_TRACE_HOP_Q_IPTOS, 1, -1},
    {WARTS_TRACE_HOP_ICMPEXT, -1, -1},    {WARTS_TRACE_HOP_ADDR, -1, -1},
    {WARTS_TRACE_HOP_TX, 8, -1},          {WARTS_TRACE_HOP_NAME, -1, -1},
};
...
static int warts_trace_params_read(scamper_trace_t *trace, warts_state_t *state,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (trace->dst == NULL) return -1;
  if (trace->firsthop == 0) trace->firsthop = 1;
  if (trace->squeries < 2) trace->squeries = 1;
...
static int warts_trace_params_write(
    const scamper_trace_t *trace, const scamper_file_t *sf,
    warts_addrtable_t *table, uint8_t *buf, uint32_t *off, const uint32_t len,
    const uint8_t *flags, const uint16_t flags_len, const uint16_t params_len) {
...
static void warts_trace_hop_params(const scamper_trace_t *trace,
  ...
  *params_len = 0;
  ...
  for (i = 0; i < sizeof(hop_vars) / sizeof(warts_var_t); i++) {
    ...
    /* not used any more */
    if (var->id == WARTS_TRACE_HOP_ADDR_GID) continue;

    if (var->id == WARTS_TRACE_HOP_ADDR) {
      if (hop->hop_addr == NULL) continue;
    } else if (var->id == WARTS_TRACE_HOP_TCP_FLAGS) {
      if (SCAMPER_TRACE_HOP_IS_TCP(hop) == 0) continue;
    } else if (var->id == WARTS_TRACE_HOP_ICMP_TC) {
      if (SCAMPER_TRACE_HOP_IS_ICMP(hop) == 0) continue;
    } else if (var->id == WARTS_TRACE_HOP_Q_IPLEN) {
      if (SCAMPER_TRACE_HOP_IS_ICMP_Q(hop) == 0) continue;
      if (hop->hop_icmp_q_ipl == trace->probe_size) continue;
    } else if (var->id == WARTS_TRACE_HOP_Q_IPTTL) {
      if (SCAMPER_TRACE_HOP_IS_ICMP_Q(hop) == 0) continue;
      if (hop->hop_icmp_q_ttl == 1) continue;
    } else if (var->id == WARTS_TRACE_HOP_Q_IPTOS) {
      if (SCAMPER_TRACE_HOP_IS_ICMP_Q(hop) == 0) continue;
      if (hop->hop_addr->type != SCAMPER_ADDR_TYPE_IPV4) continue;
    } else if (var->id == WARTS_TRACE_HOP_NHMTU) {
      if (SCAMPER_TRACE_HOP_IS_ICMP_PTB(hop) == 0) continue;
    } else if (var->id == WARTS_TRACE_HOP_ICMPEXT) {
      if (hop->hop_icmpext == NULL) continue;
    } else if (var->id == WARTS_TRACE_HOP_REPLY_IPID) {
      if (hop->hop_reply_ipid == 0) continue;
    } else if (var->id == WARTS_TRACE_HOP_TX) {
      if (hop->hop_tx.tv_sec == 0) continue;
    } else if (var->id == WARTS_TRACE_HOP_NAME) {
      if (hop->hop_name == NULL) continue;
    }
...
static int warts_trace_hop_read(scamper_trace_hop_t *hop, warts_state_t *state,
  ...
  if (hop->hop_addr == NULL) return -1;
  if (hop->hop_probe_ttl == 0) return -1;
...
static int warts_trace_hops_read(scamper_trace_hop_t **hops,
  ...
  for (i = 0; i < count; i++) {
    ...
    if (hop == NULL) goto err;

    if (warts_trace_hop_read(hop, state, table, buf, off, len) != 0) goto err;
  }
...
static void warts_trace_pmtud_n_params(const scamper_trace_pmtud_t *pmtud,
  ...
  for (i = 0; i < sizeof(pmtud_n_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TRACE_PMTUD_N_TYPE) {
      ...
      if (n->nhmtu == 0) continue;
    } else if (var->id == WARTS_TRACE_PMTUD_N_HOP) {
      if (n->hop == NULL) continue;
...
static int warts_trace_pmtud_n_read(const scamper_trace_pmtud_t *pmtud,
  ...
  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) return -1;
...
static int warts_trace_pmtud_state(const scamper_trace_t *trace,
  ...
  if (state->params_len != 0) state->len += 2;
  ...
  if (trace->pmtud->notec > 0) {
    ...
    for (i = 0; i < trace->pmtud->notec; i++) {
      ...
      if (note->params_len != 0) state->len += 2;
    }
  }
...
static int warts_trace_pmtud_read(scamper_trace_t *trace, warts_state_t *state,
  ...
  if (scamper_trace_pmtud_alloc(trace) != 0) goto err;

  if (warts_params_read(buf, off, len, handlers, handler_cnt) != 0) goto err;
  ...
  if (extract_uint16(buf, off, len, &count, NULL) != 0) goto err;
  if (count != 0) {
  ...
  if (trace->pmtud->notec != 0) {
    ...
    for (u8 = 0; u8 < trace->pmtud->notec; u8++) {
      if ((n = scamper_trace_pmtud_n_alloc()) == NULL) goto err;
      if (warts_trace_pmtud_n_read(trace->pmtud, n, buf, off, len) != 0)
        goto err;
      trace->pmtud->notes[u8] = n;
      n = NULL;
    }
  }
...
static int warts_trace_lastditch_read(scamper_trace_t *trace,
  ...
  if (warts_params_read(buf, off, len, NULL, 0) != 0) goto err;
...
int scamper_file_warts_trace_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  if ((table = warts_addrtable_alloc_byid()) == NULL) goto err;
  ...
  if (warts_trace_hops_read(&hops, state, table, buf, &off, hdr->len, count) !=
      0)
    goto err;
  ...
  for (i = 0, hop = hops; i < count; i++) {
    if (hop->hop_probe_ttl > max_ttl) max_ttl = hop->hop_probe_ttl;
    hop = hop->hop_next;
  }
  ...
  if (trace->hop_count != 0) {
    if (trace->hop_count < max_ttl) goto err;
    if (trace->hop_count > 255) goto err;
  } else {
  ...
  for (;;) {
    if (extract_uint16(buf, &off, hdr->len, &u16, NULL) != 0) goto err;
    if (u16 == WARTS_TRACE_ATTR_EOF) break;
    ...
    if (type == WARTS_TRACE_ATTR_PMTUD) {
      ...
      i = off;
      if (warts_trace_lastditch_read(trace, state, table, buf, &i, hdr->len) !=
          0)
        goto err;
...
int scamper_file_warts_trace_write(const scamper_file_t *sf,
  ...
  if ((table = warts_addrtable_alloc_byaddr()) == NULL) goto err;

  /* figure out which trace data items we'll store in this record */
  warts_trace_params(trace, table, trace_flags, &trace_flags_len,
                     &trace_params_len);
  ...
  if (trace->pmtud != NULL) {
    if ((pmtud = malloc_zero(sizeof(warts_trace_pmtud_t))) == NULL) goto err;

    if (warts_trace_pmtud_state(trace, pmtud, table) != 0) goto err;
  ...
  if (trace->lastditch != NULL) {
    ...
    if ((ld_state = (warts_trace_hop_t *)malloc_zero(size)) == NULL) goto err;
  ...
  if (trace->dtree != NULL) {
    ...
    if (warts_trace_dtree_params(sf, trace, table, &dtree_state) != 0) goto err;
