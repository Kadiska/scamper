...
void scamper_tracelb_probeset_summary_free(
  ...
  if (sum->addrs != NULL) {
    for (i = 0; i < sum->addrc; i++)
      if (sum->addrs[i] != NULL) scamper_addr_free(sum->addrs[i]);
    free(sum->addrs);
  }
  free(sum);
  return;
...
scamper_tracelb_probeset_summary_t *scamper_tracelb_probeset_summary_alloc(
    ...
    if (probe->rxc > 0) {
      for (j = 0; j < probe->rxc; j++) {
        ...
        if (array_find((void **)sum->addrs, sum->addrc, addr,
          ...
          continue;
        if (array_insert((void ***)&sum->addrs, &sum->addrc, addr,
...
int scamper_tracelb_node_cmp(const scamper_tracelb_node_t *a,
  ...
  if ((i = scamper_addr_human_cmp(a->addr, b->addr)) != 0) return i;
...
int scamper_tracelb_link_cmp(const scamper_tracelb_link_t *a,
  ...
  if ((i = scamper_tracelb_node_cmp(a->from, b->from)) != 0) return i;
...
static int tracelb_node_index(const scamper_tracelb_t *trace,
  ...
  for (i = 0; i < trace->nodec; i++) {
    if (trace->nodes[i] == node) return i;
  }
  return -1;
}
...
int scamper_tracelb_node_convergencepoint(const scamper_tracelb_t *trace,
   ...
   */
  if ((loop = malloc_zero(sizeof(int) * trace->nodec)) == NULL) return -1;
  n = nn = from;
  loop[n] = 1;
  ...
  for (;;) {
    ...
    if ((n = tracelb_node_index(trace, node->links[0]->to)) == -1) break;
...
scamper_tracelb_node_t *scamper_tracelb_node_alloc(scamper_addr_t *addr) {
  ...
  if ((node = malloc_zero(sizeof(scamper_tracelb_node_t))) != NULL) {
    if (addr != NULL) node->addr = scamper_addr_use(addr);
  }
  return node;
}
...
void scamper_tracelb_node_free(scamper_tracelb_node_t *node) {
  ...
  if (node->links != NULL) free(node->links);

  if (node->addr != NULL) scamper_addr_free(node->addr);
...
scamper_tracelb_node_t *scamper_tracelb_node_find(
    scamper_tracelb_t *trace, scamper_tracelb_node_t *node) {
  uint16_t i;

  for (i = 0; i < trace->nodec; i++) {
    if (trace->nodes[i]->addr == NULL) continue;
...
scamper_tracelb_reply_t *scamper_tracelb_reply_alloc(scamper_addr_t *addr) {
  ...
  if (addr != NULL) reply->reply_from = scamper_addr_use(addr);
...
void scamper_tracelb_reply_free(scamper_tracelb_reply_t *reply) {
  ...
  if (reply->reply_from != NULL) scamper_addr_free(reply->reply_from);
...
void scamper_tracelb_probe_free(scamper_tracelb_probe_t *probe) {
  ...
  if (probe->rxs != NULL) {
    for (i = 0; i < probe->rxc; i++) scamper_tracelb_reply_free(probe->rxs[i]);
...
int scamper_tracelb_probe_reply(scamper_tracelb_probe_t *probe,
  ...
  if (realloc_wrap((void **)&probe->rxs, len) != 0) return -1;
...
int scamper_tracelb_probeset_probes_alloc(scamper_tracelb_probeset_t *set,
  ...
  if ((set->probes = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_tracelb_probeset_add(scamper_tracelb_probeset_t *probeset,
  ...
  if (realloc_wrap((void **)&probeset->probes, len) != 0) return -1;
...
scamper_tracelb_link_t *scamper_tracelb_link_find(
    const scamper_tracelb_t *tr, scamper_tracelb_link_t *link) {
...
int scamper_tracelb_link_add(scamper_tracelb_t *trace,
   ...
   */
  for (i = 0; i < trace->nodec; i++) {
    if ((node = trace->nodes[i]) == link->from) break;
  }
  if (i == trace->nodec) return -1;
  assert(node != NULL);
...
int scamper_tracelb_link_zerottlfwd(const scamper_tracelb_link_t *link) {
  if (link->from->addr == NULL) return 0;
  ...
  if (link->from->q_ttl != 0 || link->to->q_ttl != 1) return 0;
...
int scamper_tracelb_link_probesets_alloc(scamper_tracelb_link_t *link,
  ...
  if ((link->sets = malloc_zero(len)) == NULL) return -1;
  return 0;
}
...
int scamper_tracelb_node_links_alloc(scamper_tracelb_node_t *node,
                                     uint16_t count) {
  size_t size = sizeof(scamper_tracelb_link_t *) * count;
  if ((node->links = malloc_zero(size)) != NULL) return 0;
  return -1;
}
...
int scamper_tracelb_probe_replies_alloc(scamper_tracelb_probe_t *probe,
  ...
  if ((probe->rxs = malloc_zero(size)) != NULL) return 0;
  return -1;
}
...
const char *scamper_tracelb_type_tostr(const scamper_tracelb_t *trace) {
  if (trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT) return "udp-dport";
  if (trace->type == SCAMPER_TRACELB_TYPE_ICMP_ECHO) return "icmp-echo";
  if (trace->type == SCAMPER_TRACELB_TYPE_UDP_SPORT) return "udp-sport";
  if (trace->type == SCAMPER_TRACELB_TYPE_TCP_SPORT) return "tcp-sport";
  if (trace->type == SCAMPER_TRACELB_TYPE_TCP_ACK_SPORT) return "tcp-ack-sport";
  return NULL;
}
  ...
  while (q > 0) {
    ...
    for (i = 0; i < qt; i++) {
      ...
      for (j = 0; j < nq[i]->linkc; j++) {
        for (k = 0; k < q; k++) {
          if (nq[i]->links[j]->to == nq[k]) break;
        }

        if (k != q) continue;

        for (k = 0; k < n; k++) {
          if (nq[i]->links[j]->to == nodes[k]) break;
        }
