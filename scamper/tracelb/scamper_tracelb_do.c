 ...
 *
 * $Id: scamper_tracelb_do.c,v 1.283 2020/06/12 21:11:55 mjl Exp $
 *
...
static int k(tracelb_state_t *state, int n) {
  ...
  static const int k[][2] = {
      {0, 0},     {0, 0},     {6, 8},     {11, 15},   {16, 21},   {21, 28},
      {27, 36},   {33, 43},   {38, 51},   {44, 58},   {51, 66},   {57, 74},
      {63, 82},   {70, 90},   {76, 98},   {83, 106},  {90, 115},  {96, 123},
      {103, 132}, {110, 140}, {117, 149}, {124, 157}, {131, 166}, {138, 175},
      {145, 183}, {152, 192}, {159, 201}, {167, 210}, {174, 219}, {181, 228},
      {189, 237}, {196, 246}, {203, 255}, {211, 264}, {218, 273}, {226, 282},
      {233, 291}, {241, 300}, {248, 309}, {256, 319}, {264, 328}, {271, 337},
      {279, 347}, {287, 356}, {294, 365}, {302, 375}, {310, 384}, {318, 393},
      {326, 403}, {333, 412}, {341, 422}, {349, 431}, {357, 441}, {365, 450},
      {373, 460}, {381, 470}, {389, 479}, {397, 489}, {405, 499}, {413, 508},
      {421, 518}, {429, 528}, {437, 537}, {445, 547}, {453, 557}, {462, 566},
      {470, 576}, {478, 586}, {486, 596}, {494, 606}, {502, 616}, {511, 625},
      {519, 635}, {527, 645}, {535, 655}, {544, 665}, {552, 675}, {560, 685},
      {569, 695}, {577, 705}, {585, 715}, {594, 725}, {602, 735}, {610, 745},
      {619, 755}, {627, 765}, {635, 775}, {644, 785}, {652, 795}, {661, 805},
      {669, 815}, {678, 825}, {686, 835}, {695, 845}, {703, 855}, {712, 866},
      {720, 876}, {729, 886}, {737, 896}, {746, 906},
  };
...
static int tracelb_paths_loop_fwd(tracelb_path_t *path, tracelb_path_t *node) {
  ...
  for (i = 0; i < path->fwdc; i++) {
    if (path->fwd[i] == node) return 1;
    if (tracelb_paths_loop_fwd(path->fwd[i], node) != 0) return 1;
  }
  return 0;
}
...
static int tracelb_paths_loop_back(tracelb_path_t *path, tracelb_path_t *node) {
  ...
  for (i = 0; i < path->backc; i++) {
    if (path->back[i] == node) return 1;
    if (tracelb_paths_loop_back(path->back[i], node) != 0) return 1;
  }
  return 0;
}
...
static void tracelb_paths_assert(tracelb_state_t *state) {
  ...
  for (p = 0; p < state->pathc; p++) {
    ...
    if (path->linkc == 0 && path->backc > 0) {
      for (i = 0; i < path->backc; i++) assert(path->back[i]->linkc > 0);
    }
    ...
    for (i = 0; i < path->backc; i++) {
      ...
      if (path->back[i]->distance > d) d = path->back[i]->distance;

      for (j = 0; j < path->back[i]->fwdc; j++) {
        assert(path->distance > path->back[i]->distance);
        if (path->back[i]->fwd[j] == path) break;
      }

      assert(j != path->back[i]->fwdc);
    }

    if (path->backc > 0) assert(d + 1 == path->distance);

    for (i = 0; i < path->fwdc; i++) {
      assert(path->fwd[i]->backc > 0);
      for (j = 0; j < path->fwd[i]->backc; j++) {
        if (path->fwd[i]->back[j] == path) break;
      }
  ...
  for (p = 0; p < state->pathc; p++) {
    ...
    if (path->linkc > 0) {
      if (path->links[0]->link->from->addr != NULL) {
        scamper_addr_tostr(path->links[0]->link->from->addr, addr,
                           sizeof(addr));
...
static tracelb_link_t *tracelb_isloop_addr(const tracelb_path_t *path,
  ...
  if (path->linkc > 0) {
    ...
    for (;;) {
      ...
      if ((link = tlbl->link) == NULL || link->from->addr == NULL) goto next;

      /* if the from address is the same, then we have a loop */
      if (scamper_addr_cmp(link->from->addr, addr) == 0) return tlbl;
  ...
  for (i = 0; i < path->backc; i++)
    if ((tlbl = tracelb_isloop_addr(path->back[i], addr)) != NULL) return tlbl;
...
static tracelb_link_t *tracelb_isloop(const tracelb_path_t *path) {
   ...
   */
  if (scamper_tracelb_link_zerottlfwd(path->links[path->linkc - 1]->link) !=
      0) {
    return 0;
  }

  return tracelb_isloop_addr(path,
                             path->links[path->linkc - 1]->link->to->addr);
}
...
static scamper_addr_t *tracelb_addr(tracelb_state_t *state, int type,
  ...
  fm.type = type;
  fm.addr = addr;
  if ((a = splaytree_find(state->addrs, &fm)) != NULL) return a;
...
static void tracelb_set_visited0(tracelb_state_t *state) {
  int i;
  for (i = 0; i < state->pathc; i++) state->paths[i]->visited = 0;
  return;
}
...
static void tracelb_bringfwd_free(tracelb_branch_t *br) {
  ...
  if (br->bringfwd != NULL) {
    for (i = 0; i < br->bringfwdc; i++)
      if (br->bringfwd[i] != NULL) free(br->bringfwd[i]);
    free(br->bringfwd);
    br->bringfwd = NULL;
  }
  br->bringfwdc = 0;
 ...
 */
static int tracelb_bringfwd_dft(tracelb_branch_t *branch,
                                tracelb_path_t *path) {
  int i, x, rc = 0;

  assert(path != NULL);

  if (path->visited != 0) return 1;

  if (path->backc == 0) {
    if (tracelb_bringfwd_add(branch, path) != 0) return -1;
    path->visited = 1;
    return 1;
  }
...
static int tracelb_bringfwd_set(tracelb_state_t *state, tracelb_branch_t *br,
  ...
  for (i = 0; i < br->bringfwdc; i++) {
    ...
    if (path == tlbl->path) {
      ...
      scamper_debug(__func__, "set %d i %d k %d < %d", set, i,
                    br->bringfwd[i]->k, k(state, n));
      break;
    }
  }
...
static tracelb_flowid_t *tracelb_link_flowid_get(tracelb_link_t *link) {
  ...
  if (link->flowids == NULL) return NULL;
...
err:
  if (newnode != NULL) {
    if (newnode->node != NULL) scamper_tracelb_node_free(newnode->node);
    free(newnode);
  }
  return -1;
}
...
static uint8_t tracelb_path_length(const tracelb_path_t *path) {
  ...
  while (path->backc > 0) {
    ...
    for (i = 1; i < path->backc; i++) {
      if (shortest->linkc > path->back[i]->linkc) shortest = path->back[i];
    }
    len += shortest->linkc;
    path = shortest;
  }
...
static int tracelb_bringfwd_cmp(const tracelb_bringfwd_t *a,
  ...
  if (a->path->distance < b->path->distance) return 1;
  if (a->path->distance > b->path->distance) return -1;
  return 0;
}
...
static int tracelb_branch_waiting(tracelb_state_t *state,
                                  tracelb_branch_t *br) {
...
static void tracelb_branch_reset(tracelb_branch_t *branch) {
  ...
  if (branch->mode != MODE_PERPACKET) {
    ...
    if (branch->probes != NULL) {
      for (i = 0; i < branch->probec; i++) branch->probes[i]->branch = NULL;
      free(branch->probes);
      branch->probes = NULL;
    }
    branch->probec = 0;
...
static void tracelb_branch_free(tracelb_state_t *state, tracelb_branch_t *br) {
  ...
  if (br->probes != NULL) {
    for (i = 0; i < br->probec; i++) br->probes[i]->branch = NULL;
    free(br->probes);
  }

  tracelb_bringfwd_free(br);

  if (br->newnodes != NULL) free(br->newnodes);
...
static int tracelb_path_add(tracelb_state_t *state, tracelb_path_t *path) {
  ...
  if (tracelb_branch_waiting(state, branch) != 0) goto err;
...
static void tracelb_path_distance_1(tracelb_path_t *path, int distance) {
  ...
  path->visited++;
  if (path->distance >= distance) return;
 ...
 */
static void tracelb_path_distance(tracelb_state_t *state, tracelb_path_t *path,
                                  int distance) {
...
static int tracelb_paths_splice(tracelb_state_t *state, tracelb_path_t *path0,
  ...
  for (i = 0; i < newp->backc; i++) {
    for (j = 0; j < newp->back[i]->fwdc; j++) {
      if (newp->back[i]->fwd[j] == path) newp->back[i]->fwd[j] = newp;
    }
  }
...
static int tracelb_paths_splice_bynode(tracelb_state_t *state,
  ...
  link = tlbl->link;
  for (i = 0; i < state->linkc; i++) {
    if (state->links[i] != tlbl && state->links[i]->link->to != NULL &&
...
static void tracelb_node_ptr_cb(void *param, const char *name) {
  ...
  if (name != NULL) th->node->name = strdup(name);
...
static int tracelb_node_ptr(scamper_task_t *task,
                            scamper_tracelb_node_t *node) {
  scamper_tracelb_t *trace = tracelb_getdata(task);
  tracelb_state_t *state = tracelb_getstate(task);
  tracelb_host_t *th = NULL;

  if ((trace->flags & SCAMPER_TRACELB_FLAG_PTR) == 0 || node->addr == NULL)
    return 0;
...
static int tracelb_process_hops(scamper_task_t *task, tracelb_branch_t *br) {
  ...
  for (i = 0; i < br->newnodec; i++) {
    ...
    if ((set = scamper_tracelb_probeset_alloc()) == NULL ||
      ...
      if (br->mode != MODE_CLUMP) scamper_tracelb_link_free(link);
      goto err;
    }
      ...
      if (j == br->probec || br->probes[j]->probe->flowid != flowid) {
        ...
        if (record != 0) {
          ...
          if (timxceed != 0 && br->probes[k]->mode != MODE_PERPACKET) {
            pr = br->probes[k];
            if (tracelb_flowids_list_add(flowids, pr) != 0) goto err;
          }
          ...
          while (k < j) {
            pr = br->probes[k];
            if (scamper_tracelb_probeset_add(set, pr->probe) != 0) goto err;
            k++;
          }
        }

        if (j == br->probec) break;
    ...
    if (tracelb_link_continue(trace, newp, link) == 0) continue;
...
static int tracelb_process_clump(scamper_task_t *task, tracelb_branch_t *br) {
  ...
  if (path0 == NULL) {
    ...
    if ((path0 = tracelb_path_alloc(state, 0)) == NULL) goto err;
    br->path = path0;
    ...
    if (trace->gaplimit < 2) {
      ...
      for (i = link->hopc - 1; i >= 0; i--) {
        if (k == trace->gaplimit) break;

        set = link->sets[i];
        for (j = 0; j < set->probec; j++)
          if (set->probes[j]->rxc > 0) break;

        if (j == set->probec) k++;
      }
      if (k == trace->gaplimit) halt = 1;
    }
  }
  ...
  for (i = 0; i <= br->probec; i++) {
    ...
    if (halt != 0) continue;
    for (j = 0; j < probe->rxc; j++) {
      if (SCAMPER_TRACELB_REPLY_IS_ICMP_TTL_EXP(probe->rxs[j]) == 0) halt = 1;
    }
  }
  ...
  if (halt == 0) {
    ...
    if (tracelb_branch_waiting(state, br) != 0) goto err;
  } else {
...
static int tracelb_process_perpacket(scamper_task_t *task,
  ...
  tracelb_branch_reset(br);
  timeval_add_cs(&br->next_tx, &br->last_tx, trace->wait_probe);
  if (tracelb_branch_active(state, br) != 0) return -1;
...
static void tracelb_process_probes(scamper_task_t *task, tracelb_branch_t *br) {
  ...
  for (i = 0; i < br->probec; i++) {
    pr = br->probes[i];
    probe = pr->probe;
    if (pr->mode == MODE_FIRSTADDR)
      ms = "firstaddr";
    else if (pr->mode == MODE_FIRSTHOP)
      ms = "firsthop";
    else if (pr->mode == MODE_HOPPROBE)
      ms = "hopprobe";
    else if (pr->mode == MODE_BRINGFWD)
      ms = "bringfwd";
    else if (pr->mode == MODE_BRINGFWD0)
      ms = "bringfwd0";
    else if (pr->mode == MODE_CLUMP)
      ms = "clump";
    else if (pr->mode == MODE_PERPACKET)
      ms = "perpacket";
    else
      ms = "???";
    scamper_debug(__func__, "flow %d ttl %d rx %d %s", probe->flowid,
                  probe->ttl, probe->rxc, ms);
  }
#endif

  /* remove the branch from the active heap, if it is not already removed */
  if (br->heapnode != NULL) heap_delete(state->active, br->heapnode);
...
static int tracelb_probe_vals(scamper_task_t *task, tracelb_branch_t *branch,
  ...
  if (tracelb_path_flowid(path0, pr->probe) != 0) {
    ...
    for (i = 0; i < branch->bringfwdc; i++) branch->bringfwd[i]->k = 0;
  ...
  if (branch->bringfwd == NULL || branch->bringfwd[0]->path != path0) {
     ...
     */
    tracelb_set_visited0(state);
    if (tracelb_bringfwd_dft(branch, path0) == -1) return -1;
    array_qsort((void **)branch->bringfwd, branch->bringfwdc,
                (array_cmp_t)tracelb_bringfwd_cmp);

#ifndef NDEBUG
    if (branch->bringfwdc > 0) {
      i = branch->bringfwdc - 1;
      for (;;) {
  ...
  scamper_debug(__func__, "bringfwd: ttl %d flowid %d", pr->probe->ttl,
                pr->probe->flowid);
  return 0;
}
...
static scamper_tracelb_reply_t *handleicmp_reply(const scamper_icmp_resp_t *ir,
  ...
  if (ir->ir_ext != NULL &&
      scamper_icmpext_parse(&reply->reply_icmp_ext, ir->ir_ext,
                            ir->ir_extlen) != 0) {
...
static int hopprobe_handlereply(scamper_task_t *task, tracelb_probe_t *pr,
  ...
  if (tracelb_newnode_find(branch, reply) == NULL) {
    if (tracelb_newnode_add(branch, pr->probe) != 0) return -1;
  ...
  if (branch->n >= TRACELB_CONFIDENCE_MAX_N ||
    ...
    timeval_add_cs(&branch->next_tx, &branch->last_tx, trace->wait_probe);
    if (tracelb_branch_active(state, branch) != 0) return -1;
...
static void handleicmp_perpacket(scamper_task_t *task, scamper_icmp_resp_t *ir,
  ...
  if (process == 0) {
    heap_delete(state->active, branch->heapnode);
    timeval_add_cs(&branch->next_tx, &branch->last_tx, trace->wait_probe);
    if (tracelb_branch_active(state, branch) != 0) goto err;
    tracelb_queue(task);
  } else {
...
static void handleicmp_bringfwd(scamper_task_t *task, scamper_icmp_resp_t *ir,
  ...
  if (pr->mode != cpr->mode || pr->probe->flowid != cpr->probe->flowid ||
  ...
  if ((tlbl = tracelb_link_find(state, &findme)) == NULL) {
    ...
    if (tracelb_link_flowid_add_tail(tlbl, pr->probe) != 0) goto err;

    for (i = 0; i < branch->bringfwdc; i++) {
      if (branch->bringfwd[i]->path->linkc == 0) continue;

      if (branch->bringfwd[i]->path->links[0] == tlbl) break;
    }
...
static void do_tracelb_handle_icmp(scamper_task_t *task,
   ...
   * is in use.
   */
  if (ir->ir_fd != scamper_fd_fd_get(state->icmp)) return;
  ...
  if (SCAMPER_TRACELB_TYPE_IS_UDP(trace)) {
     ...
     * ignore the packet
     */
    if (ir->ir_inner_ip_proto != IPPROTO_UDP) return;

    if (trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT) {
      /* if the dport varies, the sport should match */
      if (ir->ir_inner_udp_sport != trace->sport) return;
    } else {
      /* if the sport varies, the dport should match */
      if (ir->ir_inner_udp_dport != trace->dport) return;
    }
    ...
    if (ir->ir_af == AF_INET) {
      ...
      if (ir->ir_inner_udp_sum == 0) return;
      id = ntohs(ir->ir_inner_udp_sum) - 1;
    } else
      return;
    ...
    if (SCAMPER_ICMP_RESP_IS_ECHO_REPLY(ir) == 0) {
      ...
      if (ir->ir_icmp_id != trace->sport || ir->ir_icmp_seq >= state->id_next) {
        return;
      }
 ...
 */
static void handletimeout_firstaddr(scamper_task_t *task,
                                    tracelb_branch_t *br) {
  scamper_tracelb_t *trace = tracelb_getdata(task);
  tracelb_state_t *state = tracelb_getstate(task);
  scamper_tracelb_node_t *node = NULL;
 ...
 *
 */
static void handletimeout_perpacket(scamper_task_t *task,
                                    tracelb_branch_t *br) {
...
static void do_tracelb_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl) {
  ...
  if (SCAMPER_DL_IS_TCP(dl) == 0) return;
...
static void tracelb_handle_rt(scamper_route_t *rt) {
...
#ifndef _WIN32
  if (state->rtsock == NULL) goto done;
#endif

  if (state->route != rt) goto done;
  ...
  if (state->route == rt) state->route = NULL;
  return;
}
...
static void tracelb_state_free(scamper_tracelb_t *trace,
                               tracelb_state_t *state) {
  tracelb_branch_t *br;
  tracelb_probe_t *pr;
  tracelb_host_t *th;
  int i;
  ...
  if (state->paths != NULL) {
    for (i = 0; i < state->pathc; i++) tracelb_path_free(state->paths[i]);
    free(state->paths);
  }

  /* free the link records */
  if (state->links != NULL) {
    for (i = 0; i < state->linkc; i++) tracelb_link_free(state->links[i]);
    free(state->links);
  }
...
static void do_tracelb_free(scamper_task_t *task) {
  ...
  /* free any state kept */
  if (state != NULL) tracelb_state_free(trace, state);
...
static int tracelb_state_alloc(scamper_task_t *task) {
  ...
  if ((state->addrs = splaytree_alloc((splaytree_cmp_t)scamper_addr_cmp)) ==
      NULL) {
    printerror(__func__, "could not alloc addr tree");
    goto err;
  }

  if ((state->active = heap_alloc((heap_cmp_t)tracelb_branch_active_cmp)) ==
      NULL) {
    printerror(__func__, "could not alloc active heap");
    goto err;
  }
  heap_onremove(state->active, tracelb_branch_onremove);
  if ((state->waiting = heap_alloc((heap_cmp_t)tracelb_branch_waiting_cmp)) ==
      NULL) {
  ...
  if (trace->dst->type == SCAMPER_ADDR_TYPE_IPV4) {
    if (trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT) {
      ...
      if ((state->probe = scamper_fd_probe_icmp4(addr)) == NULL) goto err;
    ...
    if (trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT) {
      ...
      if ((state->probe = scamper_fd_probe_icmp6(addr)) == NULL) goto err;
...
static void do_tracelb_probe(scamper_task_t *task) {
  ...
  if (state == NULL) {
    ...
    if (tracelb_state_alloc(task) != 0) goto err;
  ...
  if (branch->mode == MODE_RTSOCK) {
    if (trace->rtr != NULL)
      state->route = scamper_route_alloc(trace->rtr, task, tracelb_handle_rt);
    else
      state->route = scamper_route_alloc(trace->dst, task, tracelb_handle_rt);
    if (state->route == NULL) goto err;

#ifndef _WIN32
    if (scamper_rtsock_getroute(state->rtsock, state->route) != 0) goto err;
#else
    if (scamper_rtsock_getroute(state->route) != 0) goto err;
#endif
  ...
  if (tpl != NULL && tpl->probe->rxc == 0 &&
     ...
     */
    if (tracelb_probe_vals(task, branch, tp) < 0) goto err;
  ...
  if (trace->dst->type == SCAMPER_ADDR_TYPE_IPV4) probe.pr_ip_off = IP_DF;
  ...
  if (tracelb_probe_add(state, branch, tp) != 0) goto err;
...
static int tracelb_arg_param_validate(int optid, char *param, long long *out) {
  ...
  switch (optid) {
    ...
    case TRACE_OPT_OPTION:
      if (strcasecmp(param, "ptr") != 0) goto err;
      break;
    ...
    case TRACE_OPT_TOS:
      if (string_tolong(param, &tmp) != 0 || tmp < SCAMPER_DO_TRACELB_TOS_MIN ||
          tmp > SCAMPER_DO_TRACELB_TOS_MAX) {
        goto err;
      }
      break;
...
void *scamper_do_tracelb_alloc(char *str) {
  ...
  if (af != AF_INET && af != AF_INET6) goto err;

  if (rtr != NULL && (trace->rtr = scamper_addr_resolve(af, rtr)) == NULL)
    goto err;
...
scamper_task_t *scamper_do_tracelb_alloctask(void *data, scamper_list_t *list,
  ...
  if ((task = scamper_task_alloc(trace, &funcs)) == NULL) goto err;
  ...
  if (trace->src == NULL &&
      (trace->src = scamper_getsrc(trace->dst, 0)) == NULL)
    goto err;
