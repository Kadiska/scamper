...
static char *header_tostr(const scamper_tracelb_t *trace) {
  ...
  string_concat(buf, sizeof(buf), &off,
                ", \"wait_timeout\":%u, \"wait_probe\":%u", trace->wait_timeout,
                trace->wait_probe * 10);
  string_concat(buf, sizeof(buf), &off, ", \"probec\":%u, \"probec_max\":%u",
                trace->probec, trace->probec_max);
  string_concat(buf, sizeof(buf), &off, ", \"nodec\":%u, \"linkc\":%u",
...
static char *reply_tostr(const scamper_tracelb_probe_t *probe,
  ...
  if (SCAMPER_TRACELB_REPLY_IS_TCP(reply)) {
    ...
    if (SCAMPER_TRACELB_REPLY_IS_ICMP_UNREACH(reply) ||
      ...
      string_concat(buf, sizeof(buf), &off, ", \"icmp_q_ttl\":%u",
...
static char *probe_tostr(const scamper_tracelb_probe_t *probe,
  ...
  for (i = 0; i < probe->rxc; i++)
    if (scamper_addr_cmp(addr, probe->rxs[i]->reply_from) == 0) rxc++;

  string_concat(buf, sizeof(buf), &off,
                "{\"tx\":{\"sec\":%u, \"usec\":%u}, \"replyc\":%u,"
                " \"ttl\":%u, \"attempt\":%u, \"flowid\":%u",
                probe->tx.tv_sec, probe->tx.tv_usec, rxc, probe->ttl,
                probe->attempt, probe->flowid);

  if (rxc > 0) {
    /* make room for the replies array */
    string_concat(buf, sizeof(buf), &off, ", \"replies\":[");
    if ((rxs = malloc_zero(rxc * sizeof(char *))) == NULL) goto err;
    j = 0;
    for (i = 0; i < probe->rxc; i++) {
      if (scamper_addr_cmp(addr, probe->rxs[i]->reply_from) != 0) continue;
      if (j > 0) len++; /* , */
      if ((rxs[j] = reply_tostr(probe, probe->rxs[i])) == NULL) goto err;
      len += strlen(rxs[j]);
      j++;
    }
    len++; /* ] */
  ...
  if ((dup = malloc(len)) == NULL) goto err;
  off = 0;
...
err:
  if (rxs != NULL) {
    for (i = 0; i < rxc; i++)
      if (probe->rxs[i] != NULL) free(rxs[i]);
    free(rxs);
  }
...
static int strlist_add(strlist_t **tail, char *str, size_t *off) {
  ...
  if ((sl = malloc(sizeof(strlist_t))) == NULL) return -1;
  sl->next = NULL;
...
static char *probeset_summary_tojson(scamper_tracelb_probeset_summary_t *sum,
  ...
  if (sum->nullc > 0 && sum->addrc == 0) return strdup("{\"addr\":\"*\"}");
  ...
  for (i = 0; i < sum->addrc; i++) {
    ...
    if (strlist_add(&tail, buf, &len) != 0) goto err;
    ...
    for (j = 0; j < set->probec; j++) {
      ...
      for (k = 0; k < probe->rxc; k++)
        if (scamper_addr_cmp(sum->addrs[i], probe->rxs[k]->reply_from) == 0)
          break;
      if (k == probe->rxc) continue;
      if (p > 0 && strlist_add(&tail, ",", &len) != 0) goto err;
      dup = probe_tostr(probe, sum->addrs[i]);
      if (strlist_add(&tail, dup, &len) != 0) goto err;
      free(dup);
      dup = NULL;
      p++;
    }

    if (strlist_add(&tail, "]}", &len) != 0) goto err;
  }
  ...
  if (node->linkc > 1) {
    ...
    for (j = 0; j < node->linkc; j++) {
      ...
      for (k = 0; k < link->sets[0]->probec; k++) {
        if (k > 0 && strlist_add(&tail, ",", &len) != 0) goto err;
    ...
    for (j = 0; j < link->hopc - 1; j++) {
      ...
      string_concat(buf, sizeof(buf), &off, "[");
      if (strlist_add(&tail, buf, &len) != 0) goto err;
      if ((dup = probeset_summary_tojson(sum, set)) == NULL) goto err;
    ...
    if (link->hopc > 1 && strlist_add(&tail, ",", &len) != 0) goto err;
    if (link->to != NULL) {
      off = 0;
      string_concat(buf, sizeof(buf), &off, "[{\"addr\":\"%s\", \"probes\":[",
                    scamper_addr_tostr(link->to->addr, tmp, sizeof(tmp)));
...
int scamper_file_json_tracelb_write(const scamper_file_t *sf,
  ...
  if ((header = header_tostr(trace)) == NULL) goto cleanup;
  len = strlen(header);
  ...
  if (trace->nodec > 0) {
    ...
    for (i = 0; i < trace->nodec; i++) {
      if (trace->nodes[i]->linkc < 1) continue;
      if ((nodes[nodec] = node_tostr(trace->nodes[i])) == NULL) goto cleanup;
      if (nodec > 0) len++; /* , */
