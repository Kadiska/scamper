...
static const warts_var_t tracelb_vars[] = {
    {WARTS_TRACELB_LIST_ID, 4, -1},      {WARTS_TRACELB_CYCLE_ID, 4, -1},
    {WARTS_TRACELB_ADDR_SRC_GID, 4, -1}, {WARTS_TRACELB_ADDR_DST_GID, 4, -1},
    {WARTS_TRACELB_START, 8, -1},        {WARTS_TRACELB_SPORT, 2, -1},
    {WARTS_TRACELB_DPORT, 2, -1},        {WARTS_TRACELB_PROBE_SIZE, 2, -1},
    {WARTS_TRACELB_TYPE, 1, -1},         {WARTS_TRACELB_FIRSTHOP, 1, -1},
    {WARTS_TRACELB_WAIT_TIMEOUT, 1, -1}, {WARTS_TRACELB_WAIT_PROBE, 1, -1},
    {WARTS_TRACELB_ATTEMPTS, 1, -1},     {WARTS_TRACELB_CONFIDENCE, 1, -1},
    {WARTS_TRACELB_TOS, 1, -1},          {WARTS_TRACELB_NODEC, 2, -1},
    {WARTS_TRACELB_LINKC, 2, -1},        {WARTS_TRACELB_PROBEC, 4, -1},
    {WARTS_TRACELB_PROBECMAX, 4, -1},    {WARTS_TRACELB_GAPLIMIT, 1, -1},
    {WARTS_TRACELB_ADDR_SRC, -1, -1},    {WARTS_TRACELB_ADDR_DST, -1, -1},
    {WARTS_TRACELB_USERID, 4, -1},       {WARTS_TRACELB_FLAGS, 1, -1},
...
static const warts_var_t tracelb_node_vars[] = {
    ...
    {WARTS_TRACELB_NODE_FLAGS, 1, -1},    {WARTS_TRACELB_NODE_LINKC, 2, -1},
    {WARTS_TRACELB_NODE_QTTL, 1, -1},     {WARTS_TRACELB_NODE_ADDR, -1, -1},
...
static const warts_var_t tracelb_probe_vars[] = {
    {WARTS_TRACELB_PROBE_TX, 8, -1},  {WARTS_TRACELB_PROBE_FLOWID, 2, -1},
    {WARTS_TRACELB_PROBE_TTL, 1, -1}, {WARTS_TRACELB_PROBE_ATTEMPT, 1, -1},
...
static int warts_tracelb_params_read(scamper_tracelb_t *trace,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (trace->src == NULL || trace->dst == NULL) return -1;
  return 0;
}

static int warts_tracelb_params_write(
    const scamper_tracelb_t *trace, const scamper_file_t *sf,
...
static int warts_tracelb_node_state(const scamper_file_t *sf,
  ...
  for (i = 0; i < sizeof(tracelb_node_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TRACELB_NODE_ADDR_GID) {
      ...
      if (SCAMPER_TRACELB_NODE_QTTL(node) == 0) continue;
...
static int warts_tracelb_reply_state(const scamper_file_t *sf,
  ...
  for (i = 0; i < sizeof(tracelb_reply_vars) / sizeof(warts_var_t); i++) {
    ...
    if (var->id == WARTS_TRACELB_REPLY_FROM_GID) {
      ...
      if ((reply->reply_flags & SCAMPER_TRACELB_REPLY_FLAG_TCP) != 0) continue;
...
static int warts_tracelb_reply_read(scamper_tracelb_reply_t *reply,
  ...
  if ((rc = warts_params_read(buf, off, len, handlers, handler_cnt)) != 0)
    return rc;
  if (reply->reply_from == NULL) return -1;
  return 0;
}
...
static int warts_tracelb_probe_read(scamper_tracelb_probe_t *probe,
  ...
  if (probe->rxc > 0) {
    if (scamper_tracelb_probe_replies_alloc(probe, probe->rxc) != 0) return -1;

    for (i = 0; i < probe->rxc; i++) {
      if ((reply = scamper_tracelb_reply_alloc(NULL)) == NULL) return -1;
      probe->rxs[i] = reply;
...
static void warts_tracelb_probe_write(const scamper_tracelb_probe_t *probe,
  ...
  for (i = 0; i < probe->rxc; i++) {
    warts_tracelb_reply_write(probe->rxs[i], &state->replies[i], table, buf,
                              off, len);
  }
...
static int warts_tracelb_probeset_read(scamper_tracelb_probeset_t *set,
  ...
  if (set->probec > 0) {
    if (scamper_tracelb_probeset_probes_alloc(set, set->probec) != 0) return -1;

    for (i = 0; i < set->probec; i++) {
      if ((set->probes[i] = scamper_tracelb_probe_alloc()) == NULL ||
          warts_tracelb_probe_read(set->probes[i], state, table, buf, off,
                                   len) != 0) {
        return -1;
      }
    }
...
static void warts_tracelb_probeset_write(const scamper_tracelb_probeset_t *set,
  ...
  for (i = 0; i < set->probec; i++) {
    warts_tracelb_probe_write(set->probes[i], &state->probes[i], table, buf,
                              off, len);
  }
...
static int warts_tracelb_link_state(const scamper_file_t *sf,
  ...
  for (k = 0; k < sizeof(tracelb_link_vars) / sizeof(warts_var_t); k++) {
    var = &tracelb_link_vars[k];

    /* if the link does not include a `to' node, skip it */
    if (var->id == WARTS_TRACELB_LINK_TO && link->to == NULL) continue;
...
static int warts_tracelb_link_read(scamper_tracelb_t *trace,
  ...
  if (from >= trace->nodec) return -1;
  link->from = trace->nodes[from];

  if (flag_isset(&buf[o], WARTS_TRACELB_LINK_TO) != 0) {
    if (to >= trace->nodec) return -1;
    link->to = trace->nodes[to];
  } else
    link->to = NULL;

  if (link->hopc > 0) {
    if (scamper_tracelb_link_probesets_alloc(link, link->hopc) != 0) return -1;

    for (i = 0; i < link->hopc; i++) {
      if ((set = scamper_tracelb_probeset_alloc()) == NULL) return -1;
      link->sets[i] = set;
...
static void warts_tracelb_link_write(const scamper_tracelb_link_t *link,
  ...
  for (i = 0; i < link->hopc; i++) {
    warts_tracelb_probeset_write(link->sets[i], &state->sets[i], table, buf,
                                 off, len);
  }
...
int scamper_file_warts_tracelb_read(scamper_file_t *sf, const warts_hdr_t *hdr,
  ...
  if (warts_tracelb_params_read(trace, state, table, buf, &off, hdr->len) !=
      0) {
    goto err;
  }
  ...
  if (trace->nodec > 0) {
    ...
    for (i = 0; i < trace->nodec; i++) {
      ...
      if (warts_tracelb_node_read(trace->nodes[i], state, table, buf, &off,
                                  hdr->len) != 0)
        goto err;
    }
  }
  ...
  if (trace->linkc > 0) {
    ...
    for (i = 0; i < trace->linkc; i++) {
      if ((trace->links[i] = scamper_tracelb_link_alloc()) == NULL) goto err;

      if (warts_tracelb_link_read(trace, trace->links[i], state, table, buf,
                                  &off, hdr->len) != 0)
        goto err;
    }
  }
  ...
  if (trace->nodec > 0) {
    ...
    for (i = 0; i < trace->linkc; i++) {
      for (j = 0; j < trace->nodec; j++) {
        if (trace->links[i]->from == trace->nodes[j]) break;
      }

      if (j == trace->nodec) goto err;
      ...
      if (nlc[j] == node->linkc) goto err;

      node->links[nlc[j]++] = trace->links[i];
    }

    for (i = 0; i < trace->nodec; i++) {
      if (nlc[i] != trace->nodes[i]->linkc) goto err;
    }
...
int scamper_file_warts_tracelb_write(const scamper_file_t *sf,
  ...
  if (trace->nodec > 0) {
    ...
    for (i = 0; i < trace->nodec; i++) {
      len2 = len;
      node = trace->nodes[i];
      if (warts_tracelb_node_state(sf, node, table, &node_state[i], &len2) !=
          0) {
        goto err;
      }
  ...
  if (trace->linkc > 0) {
    ...
    for (i = 0; i < trace->linkc; i++) {
      len2 = len;
      link = trace->links[i];
      if (warts_tracelb_link_state(sf, trace, link, &link_state[i], table,
                                   &len2) != 0) {
        goto err;
      }
  ...
  /* write trace nodes */
  for (i = 0; i < trace->nodec; i++) {
    warts_tracelb_node_write(trace->nodes[i], &node_state[i], table, buf, &off,
                             len);
  }
  if (node_state != NULL) {
    free(node_state);
