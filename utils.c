...
int sockaddr_len(const struct sockaddr *sa) { return sa->sa_len; }
#else
...
int sockaddr_compose_un(struct sockaddr *sa, const char *file) {
  ...
  if (strlen(file) + 1 > sizeof(sn->sun_path)) return -1;
...
int sockaddr_compose_str(struct sockaddr *sa, const char *addr,
                         const int port) {
  ...
  if (getaddrinfo(addr, NULL, &hints, &res0) != 0 || res0 == NULL) return rc;
...
static char *link_tostr(const struct sockaddr_dl *sdl, char *buf,
                        const size_t len) {
...
#ifndef _WIN32
    if (inet_ntop(AF_INET, &((const struct sockaddr_in *)sa)->sin_addr, addr,
                  sizeof(addr)) == NULL) {
      return NULL;
    }
#else
    if (getnameinfo(sa, sizeof(struct sockaddr_in), addr, sizeof(addr), NULL, 0,
                    NI_NUMERICHOST) != 0) {
      return NULL;
    }
#endif
...
#ifndef _WIN32
    if (inet_ntop(AF_INET6, &((const struct sockaddr_in6 *)sa)->sin6_addr, addr,
                  sizeof(addr)) == NULL) {
      return NULL;
    }
#else
    if (getnameinfo(sa, sizeof(struct sockaddr_in6), addr, sizeof(addr), NULL,
                    0, NI_NUMERICHOST) != 0) {
      return NULL;
    }
#endif
...
const char *addr_tostr(int af, const void *addr, char *buf, size_t len) {
  ...
  if (sockaddr_compose((struct sockaddr *)&sas, af, addr, 0) != 0) return NULL;

  if (getnameinfo((const struct sockaddr *)&sas, sizeof(sas), buf, len, NULL, 0,
                  NI_NUMERICHOST) != 0) {
    return NULL;
  }
...
void *malloc_zero_dm(const size_t size, const char *file, const int line) {
  void *ptr;
  if ((ptr = dmalloc_malloc(file, line, size, DMALLOC_FUNC_MALLOC, 0, 0)) !=
      NULL) {
    memset(ptr, 0, size);
  }
  return ptr;
...
int array_findpos(void **array, int nmemb, const void *item, array_cmp_t cmp) {
  ...
  if (r == 0) {
    if (cmp(array[0], item) == 0) return 0;
    return -1;
  }
...
static int array_insert_0(void **array, int *nmemb, void *item,
  ...
  array[*nmemb] = item;
  *nmemb = *nmemb + 1;
  if (cmp != NULL) array_qsort(array, *nmemb, cmp);
  return 0;
}
...
int array_insert(void ***array, int *nmemb, void *item, array_cmp_t cmp) {
  ...
  if (realloc_wrap((void **)array, len) != 0) return -1;
...
#ifdef DMALLOC
int array_insert_dm(void ***array, int *nmemb, void *item, array_cmp_t cmp,
                    const char *file, const int line) {
  size_t len;

  assert(nmemb != NULL && *nmemb >= 0);
  len = ((*nmemb) + 1) * sizeof(void *);
  if (realloc_wrap_dm((void **)array, len, file, line) != 0) return -1;
...
int timeval_inrange_us(const struct timeval *a, const struct timeval *b,
                       int c) {
  struct timeval tv;
  int rc = timeval_cmp(a, b);
  if (rc < 0) {
    timeval_add_us(&tv, a, c);
    if (timeval_cmp(&tv, b) < 0) return 0;
  } else if (rc > 0) {
    timeval_add_us(&tv, b, c);
    if (timeval_cmp(&tv, a) < 0) return 0;
  }
  return 1;
}
...
char *json_esc(const char *in, char *out, size_t len) {
  ...
  while (*in != '\0') {
    if (isprint(*in) == 0) break;

    switch (*in) {
      case '"':
        if (off + 2 >= len) goto done;
        out[off++] = '\\';
        out[off++] = '"';
        break;
...
const char *string_findlc(const char *str, const char *find) {
  ...
  for (;;) {
    for (i = 0; find[i] != '\0'; i++)
      if (tolower((int)sp[i]) != find[i]) break;
    if (find[i] == '\0') return sp;
    if (sp[i] == '\0') break;
    sp++;
  }
...
char *string_nullterm(char *buf, const char *delim, char **next) {
  ...
  if (delim == NULL || *delim == '\0' || (tmp = buf) == NULL) return NULL;
...
char *string_lastof(char *str, const char *delim) {
  ...
  if (delim == NULL || *delim == '\0' || str == NULL) return NULL;
...
int string_addrport(const char *in, char **first, int *port) {
  ...
  if (string_isnumber(in)) {
    if (string_tolong(in, &lo) == -1 || lo < 1 || lo > 65535) goto err;
    *first = NULL;
    *port = lo;
    return 0;
  }

  if ((dup = strdup(in)) == NULL) goto err;
  ...
  if (dup[0] == '[') {
    ...
    string_nullterm_char(dup, ':', &ptr);
    if (ptr == NULL || (first_tmp = strdup(dup)) == NULL) goto err;
  }

  if (string_tolong(ptr, &lo) != 0 || lo < 1 || lo > 65535) goto err;
...
static int uudecode_4(uint8_t *out, const char *in, size_t c) {
  ...
  if (c > 2) out[2] = (((a - 32) & 0x3f) << 6 & 0xc0) | ((b - 32) & 0x3f);
...
int uudecode_line(const char *in, size_t ilen, uint8_t *out, size_t *olen) {
  ...
  for (;;) {
    ...
    if (ilen - j < 4) goto err;

    /* decode 4 characters into 3 bytes */
    if (uudecode_4(out + i, in + j, o - i) != 0) goto err;
...
void *uudecode(const char *in, size_t len) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (uudecode_4(out + j, in + i, x - j) != 0) goto err;
...
size_t uuencode_bytes(const uint8_t *in, size_t len, size_t *off, uint8_t *out,
                      size_t olen) {
  static const uint8_t b[] = {2,  6,  6,  6,  10, 10, 10, 14, 14, 14, 18, 18,
                              18, 22, 22, 22, 26, 26, 26, 30, 30, 30, 34, 34,
                              34, 38, 38, 38, 42, 42, 42, 46, 46, 46, 50, 50,
                              50, 54, 54, 54, 58, 58, 58, 62, 62, 62};
...
int uuencode(const uint8_t *in, size_t ilen, uint8_t **out, size_t *olen) {
  ...
  if ((ptr = malloc(len)) == NULL) return -1;
  *out = ptr;
  *olen = len;
...
uint16_t byteswap16(const uint16_t word) { return ((word >> 8) | (word << 8)); }

uint32_t byteswap32(const uint32_t word) {
  return ((word << 24) | (word >> 24) | ((word & 0xff00) << 8) |
          ((word >> 8) & 0xff00));
}
...
int fd_lines(int fd, int (*func)(char *, void *), void *param) {
  ...
  readbuf_len = 8192;
  readbuf_off = 0;
  if ((readbuf = malloc(readbuf_len)) == NULL) goto done;

  while ((ss = read(fd, readbuf + readbuf_off,
                    readbuf_len - readbuf_off - 1)) >= 0) {
    start = 0;
    off = 0;
    end = readbuf_off + ss;

    while (off <= end) {
      if (off == end && ss != 0) break;
      if (readbuf[off] == '\n' || (off == end && start < off)) {
        readbuf[off] = '\0';
        if (func(readbuf + start, param) != 0) goto done;
        start = ++off;
      } else {
        ++off;
      }
    ...
    if (ss == 0) {
      ...
      readbuf_len += 8192;
      readbuf_off = off;
      if (realloc_wrap((void **)&readbuf, readbuf_len) != 0) goto done;
    } else {
