...
static const sc_dump_t dump_funcs[] = {
    {NULL, NULL, NULL, NULL},
    {"dump aliases inferred with Ally (IPID-based)", NULL, process_1_ally,
     finish_1},
    {"dump aliases inferred with Mercator (CSA-based)", process_2_ping, NULL,
     finish_2},
    {"dump aliases inferred with both Ally and Mercator", process_3_ping,
     process_3_ally, finish_3},
};
...
static void usage(uint32_t opt_mask) {
  ...
  if (opt_mask & OPT_PROBEWAIT) fprintf(stderr, "     -i inter-probe gap\n");

  if (opt_mask & OPT_FUDGE) fprintf(stderr, "     -f fudge\n");

  if (opt_mask & OPT_OPTIONS) {
    fprintf(stderr, "     -O: options\n");
    fprintf(stderr,
            "         nobs: do not consider byte swapped IPID values\n");
  ...
  if (opt_mask & OPT_TEXT) fprintf(stderr, "     -t logfile\n");

  if (opt_mask & OPT_WAIT) fprintf(stderr, "     -w waittime\n");
...
static void status(char *format, ...) {
  ...
  snprintf(pref, sizeof(pref), "p %d, w %d, v %d", probing, heap_count(waiting),
           slist_count(virgin));
...
static int sc_waittest(sc_test_t *test) {
  ...
  if (heap_insert(waiting, wt) == NULL) return -1;
...
static sc_addr2router_t *sc_addr2router_add(splaytree_t *tree, sc_router_t *r,
  ...
  if (slist_tail_push(r->addrs, a2r) == NULL) return NULL;
  return a2r;
}

static sc_addr2router_t *sc_addr2router_find(splaytree_t *t,
                                             scamper_addr_t *a) {
  sc_addr2router_t fm;
  fm.addr = a;
...
static void sc_ipidseq_free(sc_ipidseq_t *seq) {
  if (seq == NULL) return;
  if (seq->addr != NULL) scamper_addr_free(seq->addr);
  free(seq);
  return;
}
...
static sc_ipidseq_t *sc_ipidseq_alloc(scamper_addr_t *addr) {
  ...
  if ((seq = malloc_zero(sizeof(sc_ipidseq_t))) == NULL) return NULL;
...
static int sc_ipidseq_method(sc_ipidseq_t *a, sc_ipidseq_t *b) {
  if (a->icmp == IPID_INCR && a->icmp == b->icmp) return METHOD_ICMP;
  if (a->tcp == IPID_INCR && a->tcp == b->tcp) return METHOD_TCP;
  if (a->udp == IPID_INCR && a->udp == b->udp) return METHOD_UDP;
  return 0;
}
...
static sc_addr2router_t *sc_allytest_find(sc_allytest_t *at,
                                          scamper_addr_t *a) {
  if (at->router_tree == NULL) return NULL;
  ...
  if (a2r_a == NULL && a2r_b == NULL) {
    ...
    if (a2r_a->router != a2r_b->router) {
      ...
      while ((a2r_a = slist_head_pop(r->addrs)) != NULL) {
        ...
        if (slist_tail_push(a2r_b->router->addrs, a2r_a) == NULL) return -1;
      }
      dlist_node_pop(at->router_list, r->node);
      sc_router_free(r);
    }
  } else if (a2r_a != NULL) {
    if (sc_allytest_addr2router(at, a2r_a->router, b) == NULL) return -1;
  } else {
    if (sc_allytest_addr2router(at, a2r_b->router, a) == NULL) return -1;
  }
...
static sc_test_t *sc_pingtest_new(scamper_addr_t *addr) {
  ...
  if ((pt->target->test = sc_test_alloc(TEST_PING, pt)) == NULL) goto err;
...
static int addressfile_line(char *buf, void *param) {
  ...
  if (slist_count(list) < 2) goto err;

  splaytree_free(tree, NULL);
  tree = NULL;
  if (sc_allytest_new(list) != 0) goto err;
  return 0;
...
static int process_ping(sc_test_t *test, scamper_ping_t *ping) {
  ...
  if (pt->step < 3) {
    if (sc_waittest(test) != 0) goto err;
...
static int process_ally(sc_test_t *test, scamper_dealias_t *dealias) {
  ...
  string_concat(msg, sizeof(msg), &off,
                "set ally %s:", scamper_addr_tostr(a, buf, sizeof(buf)));
...
static int sc_test_ping(sc_test_t *test, char *cmd, size_t len) {
  ...
  if ((found = sc_target_find(pt->target)) != NULL && found->test != test) {
    ...
    if (sc_target_add(pt->target) != 0) return -1;
  }
...
static int sc_test_ipidseq(sc_test_t *test, scamper_addr_t *addr, char *cmd,
                           size_t len) {
  sc_pingtest_t *pt;
  sc_target_t *found;
  sc_test_t *tt;
  ...
  if ((tt = sc_pingtest_new(addr)) == NULL) return -1;
  pt = tt->data;
...
static int sc_test_ally(sc_test_t *test, char *cmd, size_t len) {
  ...
  if (at->a == NULL) {
    if ((at->a = sc_target_alloc(a)) == NULL) return -1;
    at->a->test = test;
  }
  if (at->b == NULL) {
    if ((at->b = sc_target_alloc(b)) == NULL) return -1;
    at->b->test = test;
  }
  ...
  if (flags & FLAG_NOBS) string_concat(cmd, len, &off, " -O nobs");
  string_concat(cmd, len, &off, " -W %d -q %d -p '-P %s' %s %s\n", probe_wait,
                attempts, method[at->method - 1],
...
static int do_files(void) {
  ...
  if ((outfile_fd = open(outfile_name, fd_flags, mode)) == -1) return -1;
...
static int pair_add(scamper_addr_t *a, scamper_addr_t *b) {
  ...
  if ((pair = malloc(sizeof(sc_pair_t))) == NULL) return -1;
...
static int tc_add(scamper_addr_t *a, scamper_addr_t *b) {
  ...
  if (dump_t == NULL && ((dump_t = splaytree_alloc(
                              (splaytree_cmp_t)sc_addr2router_cmp)) == NULL ||
  ...
  if (a2r_a != NULL && a2r_b != NULL) {
    ...
    while ((a2r_x = slist_head_pop(r_b->addrs)) != NULL) {
      if (slist_tail_push(r_a->addrs, a2r_x) == NULL) goto err;
      a2r_x->router = r_a;
    }
    ...
    r_a = a2r_a->router;
    if (sc_addr2router_add(dump_t, r_a, b) == NULL) goto err;
  } else if (a2r_b != NULL) {
    r_b = a2r_b->router;
    if (sc_addr2router_add(dump_t, r_b, a) == NULL) goto err;
  } else {
...
static int dump_process_ping(const scamper_ping_t *ping) {
  ...
  if (SCAMPER_PING_METHOD_IS_UDP(ping) == 0) return 0;
      ...
      if (SCAMPER_PING_REPLY_IS_ICMP_UNREACH_PORT(r) == 0 &&
        ...
        if ((flags & FLAG_TC) == 0) {
          if (pair_find(r->addr, ping->dst) == NULL) {
            if (pair_add(r->addr, ping->dst) != 0) return -1;
            printf("%s %s\n", scamper_addr_tostr(r->addr, a, sizeof(a)),
                   scamper_addr_tostr(ping->dst, b, sizeof(b)));
          }
        } else {
          if (tc_add(r->addr, ping->dst) != 0) return -1;
        }
      }
      r = r->next;
    }
...
static int ally_data(void) {
  ...
  if ((ipidseqs = splaytree_alloc((splaytree_cmp_t)sc_ipidseq_cmp)) == NULL)
    return -1;
  if ((virgin = slist_alloc()) == NULL) return -1;
  if ((waiting = heap_alloc((heap_cmp_t)sc_waittest_cmp)) == NULL) return -1;
  ...
  if ((scamper_lp = scamper_linepoll_alloc(do_scamperread_line, NULL)) ==
          NULL ||
...
int main(int argc, char *argv[]) {
  ...
  if (ally_init() != 0) return -1;

  if (options & OPT_DUMP) return ally_read();
