...
static void print_header_comments(void) {
  ...
  printf(
      ...
      "======================================================================="
      "\n"
      "# There is one trace per line, with the following tab-separated "
      "fields:\n"
      "#\n"
      "#\n");
  ...
  if ((options & OPT_OLDFORMAT) == 0) {
    printf(
        "# %2d. Key -- Indicates the type of line and determines the meaning "
        "of the\n"
        "#            remaining fields.  This will always be 'T' for an IP "
        "trace.\n"
        "#\n",
        i++);

    u32 = (OPT_HIDESRC | OPT_HIDEDST | OPT_HIDELIST | OPT_HIDECYCLE |
           OPT_HIDETIME);
    ...
    if ((options & OPT_HIDESRC) == 0)
      printf(
          "# %2d. Source -- Source IP of skitter/scamper monitor performing "
          "the trace.\n"
          "#\n",
          i++);
    ...
    if ((options & OPT_HIDELIST) == 0)
      printf(
          "# %2d. ListId -- ID of the list containing this destination "
          "address.\n"
          "#\n"
    ...
    if ((options & OPT_HIDECYCLE) == 0)
      printf(
          "# %2d. CycleId -- ID of current probing cycle (a cycle is a single "
          "run\n"
          "#                through a given list).  For skitter traces, cycle "
          "IDs\n"
          "#                will be equal to or slightly earlier than the "
          "timestamp\n"
          "#                of the first trace in each cycle. There is no "
          "standard\n"
    ...
    if ((options & OPT_SHOWUSERID) != 0)
      printf(
          ...
          "#\n"
          "#        This value will be zero if no user ID was provided.  "
          "(uint32_t)\n"
          "#\n",
          i++);
    ...
    if ((options & OPT_HIDETIME) == 0)
      printf(
          "# %2d. Timestamp -- Timestamp when trace began to this "
          "destination.\n"
          "#\n",
          i++);
    ...
    if ((options & OPT_HIDEREPLY) == 0) {
      printf(
          "# -------------------- Reply Fields ------------------\n"
          "#\n"
          "# %2d. DestReplied -- Whether a response from the destination was "
          "received.\n"
          "#\n"
          "#        R - Replied, reply was received\n"
          "#        N - Not-replied, no reply was received;\n"
          "#            Since skitter sends a packet with a TTL of 255 when it "
          "halts\n"
          "#            probing, it is still possible for the final "
          "destination to\n"
          "#            send a reply and for the HaltReasonData (see below) to "
          "not\n"
          "#            equal no_halt.  Note: scamper does not perform "
          "last-ditch\n"
      ...
      printf(
          "# %2d. DestRTT -- RTT (ms) of first response packet from "
          "destination.\n"
      ...
      printf(
          "# %2d. RequestTTL -- TTL set in request packet which elicited a "
          "response\n"
    ...
    if ((options & OPT_HIDEHALT) == 0) {
      printf(
          "# -------------------- Halt Fields ------------------\n"
          "#\n"
          "# %2d. HaltReason -- The reason, if any, why incremental probing "
          "stopped.\n"
          "#\n",
          i++);
  ...
  if ((options & OPT_HIDEPATH) == 0) {
    ...
    printf(
        ...
        "#\n"
        "#       If multiple IP addresses respond at the same hop, response "
        "data\n"
    ...
    snprintf(buf2, sizeof(buf2), "#       %%-%ds %%s\n", (int)((off * 2) + 5));
    ...
    if ((options & OPT_HIDEIRTT) == 0) {
      printf(
          ...
          "#       nTries -- number of tries before response received from "
          "hop\n");
    }
    if ((options & OPT_SHOWQTTL) != 0) {
      printf(
          "#       qTTL -- the IP-TTL in the quoted packet ('-' if not "
          "present)\n");
    }
    if ((options & OPT_SHOWMPLS) != 0) {
      printf(
    ...
    printf(
        "#\n"
        "#       This field will have the value 'q' if there was no response "
        "at\n"
        "#       this hop.\n"
        "#\n");

    printf(
        "# %2d. PerHopData -- Response data for the second hop in the same "
        "format\n"
        "#       as field %d.\n",
        i, i - 1);
...
static void print_header_fields(const scamper_trace_t *trace) {
  ...
  if ((options & OPT_SHOWUSERID) != 0) fprintf(out, "\t%d", trace->userid);
...
static void print_reply_fields(const scamper_trace_hop_t *dst) {
  ...
  if (dst != NULL) {
    ...
    fprintf(out, "\tR\t%s\t%d\t%d", rtt, dst->hop_probe_ttl,
            dst->hop_reply_ttl);
  } else {
    fprintf(out, "\tN\t0\t0\t0");
  }
    ...
    for (ie = hop->hop_icmpext; ie != NULL; ie = ie->ie_next) {
      if (SCAMPER_ICMPEXT_IS_MPLS(ie)) {
        for (i = 0; i < SCAMPER_ICMPEXT_MPLS_COUNT(ie); i++) {
          string_concat(
              buf, len, &off, ",M|%d|%d|%d|%d", SCAMPER_ICMPEXT_MPLS_TTL(ie, i),
              SCAMPER_ICMPEXT_MPLS_LABEL(ie, i),
              SCAMPER_ICMPEXT_MPLS_EXP(ie, i), SCAMPER_ICMPEXT_MPLS_S(ie, i));
        }
      }
    }
  }
...
static int print_geo_info(scamper_addr_t *addr) {
  ...
  if (na_query_geo(buf, &answer)) {
    fprintf(out, "G\t%s\t%s=%d\t%s=%d\t%s=%d\t%s\t%d\t%.3f\t%.3f\n", buf,
            answer.country, answer.country_c, answer.region, answer.region_c,
            answer.city, answer.city_c, answer.speed, answer.metro_code,
  ...
  for (res = res0; res != NULL; res = res->ai_next) {
    if (res->ai_family == PF_INET) {
      inet_ntop(res->ai_family,
                &((struct sockaddr_in *)ai_list->ai_addr)->sin_addr, buf,
                sizeof(buf));
