...
static int check_options(int argc, char *argv[]) {
  ...
  if ((options & (OPT_INFILE | OPT_OUTFILE)) != (OPT_INFILE | OPT_OUTFILE) ||
    ...
    if (options == 0)
      ...
      usage(OPT_INFILE | OPT_OUTFILE | OPT_PORT | OPT_REMOTE | OPT_UNIX);
    return -1;
  }
  ...
  if (options & OPT_STDOUT) {
    ...
    if (fcntl_set(stdout_fd, O_NONBLOCK) == -1) return -1;
    if ((stdout_wb = scamper_writebuf_alloc()) == NULL) return -1;
...
static int command_new(char *line, void *param) {
  ...
  if ((tmp = memdup(buf, off + 1)) == NULL ||
      slist_tail_push(commands, tmp) == NULL) {
...
static int do_method(void) {
  ...
  if ((flags & FLAG_IMPATIENT) == 0) more = 0;
  free(command);
...
static int do_scamperread_line(void *param, uint8_t *buf, size_t linelen) {
  ...
  if (data_left > 0) {
    ...
    if (uus != 0) {
      if (outfile_fd != -1) write_wrap(outfile_fd, uu, NULL, uus);
      if (stdout_fd != -1) scamper_writebuf_send(stdout_wb, uu, uus);
    }
...
static int do_stdoutwrite(void) {
  ...
  if (scamper_writebuf_len(stdout_wb) == 0 && scamper_fd == -1) stdout_fd = -1;
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  while (error == 0) {
    ...
    if (stdin_fd != -1) {
      ...
      rfdsp = &rfds;
      if (nfds < stdin_fd) nfds = stdin_fd;
    }

    /* might send output to stdout */
    if (stdout_fd != -1 && scamper_writebuf_len(stdout_wb) > 0) {
      FD_SET(stdout_fd, &wfds);
      wfdsp = &wfds;
      if (nfds < stdout_fd) nfds = stdout_fd;
    }
