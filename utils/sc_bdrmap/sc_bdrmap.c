...
static const char *owner_reasonstr[] = {
    "none",     "first",      "traceset",    "provider",    "peer",
    "customer", "ip2as",      "hidden-peer", "count",       "graph",
    "noip2as",  "thirdparty", "ixp",         "onenet",      "missing",
    "first2",   "count2",     "count3",      "thirdparty2", "onenet2",
    "silent",   "icmp",
};
...
static const sc_dump_t dump_funcs[] = {
    ...
    {"infer border routers", "routers", init_1, process_1_trace, process_1_ping,
     process_1_dealias, finish_1},
    {"dump annotated traceroutes", "traces", NULL, process_2_trace, NULL, NULL,
     NULL},
};
...
static void usage(uint32_t opts) {
  ...
  if (opts & OPT_FLOWID)
    fprintf(
        stderr,
        "       -C: flowid (ICMP csum or UDP sport) for Paris traceroute\n");
  if (opts & OPT_DUMP) {
  ...
  if (opts & OPT_DELEGATED) fprintf(stderr, "       -g: delegated file\n");
  ...
  if (opts & OPT_IPMAP) fprintf(stderr, "       -M: static IP mapping file\n");
  if (opts & OPT_NAMEFILE) fprintf(stderr, "       -n: IP to name file\n");
  if (opts & OPT_OUTFILE)
  ...
  if (opts & OPT_IXPFILE) fprintf(stderr, "       -x: ixp prefix file\n");
...
static int uint32_add(uint32_t **ptr, int *c, uint32_t x) {
  ...
  if (uint32_find(a, *c, x) >= 0) return 0;

  if (realloc_wrap((void **)&a, sizeof(uint32_t) * (*c + 1)) != 0) return -1;
...
static int check_options(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, opts)) != -1) {
    ...
    switch (ch) {
      ...
      case '6':
        if (check_options_once(ch, OPT_IPV6) != 0) goto done;
        options |= OPT_IPV6;
        af = AF_INET6;
        break;

      case 'a':
        if (check_options_once(ch, OPT_IP2AS) != 0) goto done;
        options |= OPT_IP2AS;
        ip2as_fn = optarg;
        break;

      case 'C':
        if (check_options_once(ch, OPT_FLOWID) != 0) goto done;
        options |= OPT_FLOWID;
        opt_flowid = optarg;
        break;
      ...
      case 'A':
        ...
        if (opt_targetases == NULL && (opt_targetases = slist_alloc()) == NULL)
          goto done;
        if (slist_tail_push(opt_targetases, optarg) == NULL) goto done;
        break;

      case 'c':
        if (check_options_once(ch, OPT_ALLYCONF) != 0) goto done;
        options |= OPT_ALLYCONF;
        opt_allyconf = optarg;
        break;

      case 'd':
        if (check_options_once(ch, OPT_DUMP) != 0) goto done;
        options |= OPT_DUMP;
        opt_dumpid = optarg;
        break;

      case 'D':
        if (check_options_once(ch, OPT_DAEMON) != 0) goto done;
        options |= OPT_DAEMON;
        break;

      case 'f':
        if (check_options_once(ch, OPT_FIRSTHOP) != 0) goto done;
        options |= OPT_FIRSTHOP;
        opt_firsthop = optarg;
        break;

      case 'g':
        if (check_options_once(ch, OPT_DELEGATED) != 0) goto done;
        options |= OPT_DELEGATED;
        delegated_fn = optarg;
        break;

      case 'i':
        if (check_options_once(ch, OPT_TARGETIPS) != 0) goto done;
        options |= OPT_TARGETIPS;
        break;

      case 'l':
        if (check_options_once(ch, OPT_LOGFILE) != 0) goto done;
        options |= OPT_LOGFILE;
        logfile_fn = optarg;
        break;

      case 'M':
        if (check_options_once(ch, OPT_IPMAP) != 0) goto done;
        options |= OPT_IPMAP;
        ipmap_fn = optarg;
        break;

      case 'o':
        if (check_options_once(ch, OPT_OUTFILE) != 0) goto done;
        options |= OPT_OUTFILE;
        outfile_fn = optarg;
        break;
      ...
      case 'n':
        if (check_options_once(ch, OPT_NAMEFILE) != 0) goto done;
        options |= OPT_NAMEFILE;
        ip2name_fn = optarg;
        break;

      case 'p':
        if (check_options_once(ch, OPT_PORT) != 0) goto done;
        options |= OPT_PORT;
        opt_port = optarg;
        break;

      case 'r':
        if (check_options_once(ch, OPT_RELFILE) != 0) goto done;
        options |= OPT_RELFILE;
        relfile_fn = optarg;
        break;

      case 'R':
        if (check_options_once(ch, OPT_REMOTE) != 0) goto done;
        options |= OPT_REMOTE;
        opt_unix = optarg;
        break;

      case 'S':
        if (check_options_once(ch, OPT_SRCADDR) != 0) goto done;
        options |= OPT_SRCADDR;
        opt_srcaddr = optarg;
        break;

      case 'U':
        if (check_options_once(ch, OPT_UNIX) != 0) goto done;
        options |= OPT_UNIX;
        opt_unix = optarg;
        break;

      case 'x':
        if (check_options_once(ch, OPT_IXPFILE) != 0) goto done;
        options |= OPT_IXPFILE;
        ixp_fn = optarg;
        break;

      case 'v':
        if (check_options_once(ch, OPT_VPASES) != 0) goto done;
        options |= OPT_VPASES;
        opt_vpases = optarg;
        break;
    }
  ...
  if (check_options_vpas(opt_vpases) != 0) goto done;

  if ((options & (OPT_TARGETIPS | OPT_TARGETASES)) ==
      (OPT_TARGETIPS | OPT_TARGETASES)) {
...
static void logerr(char *format, ...) {
  ...
  if ((options & OPT_DAEMON) && logfile == NULL) return;

  va_start(ap, format);
  vsnprintf(msg, sizeof(msg), format, ap);
  va_end(ap);

  if ((options & OPT_DAEMON) == 0) fprintf(stderr, "%s", msg);
...
static void logprint(char *format, ...) {
  ...
  if ((options & OPT_DAEMON) == 0) printf("%s", msg);
...
static void *pt_find(prefixtree_t *pt, scamper_addr_t *addr) {
  ...
  if (addr->type == SCAMPER_ADDR_TYPE_IPV4) {
    if ((p4 = prefixtree_find_ip4(pt, addr->addr)) == NULL) return NULL;
    return p4->ptr;
  } else if (addr->type == SCAMPER_ADDR_TYPE_IPV6) {
    if ((p6 = prefixtree_find_ip6(pt, addr->addr)) == NULL) return NULL;
    return p6->ptr;
  }
  return NULL;
}
...
static int is_ixp(scamper_addr_t *addr) {
  if (ipmap_pt != NULL && pt_find(ipmap_pt, addr) != NULL) return 0;
  if (ixp_pt != NULL && pt_find(ixp_pt, addr) != NULL) return 1;
  return 0;
}
...
static int sc_ally_cmp(const sc_ally_t *a, const sc_ally_t *b) {
  int rc;
  if ((rc = scamper_addr_cmp(a->a, b->a)) != 0) return rc;
...
static sc_ally_t *sc_ally_get(scamper_addr_t *a, scamper_addr_t *b) {
  ...
  if ((ally = sc_ally_find(a, b)) != NULL) return ally;
  if ((ally = malloc_zero(sizeof(sc_ally_t))) == NULL) return NULL;
...
static sc_link_t *sc_link_alloc(scamper_addr_t *a, scamper_addr_t *b) {
  sc_link_t *link;
  if ((link = malloc_zero(sizeof(sc_link_t))) == NULL) return NULL;
...
static sc_link_t *sc_link_get(scamper_addr_t *a, scamper_addr_t *b) {
  sc_link_t *link;
  if ((link = sc_link_find(a, b)) != NULL) return link;
...
static int sc_stree_add(sc_stree_t *set, void *item) {
  ...
  if (slist_head_push(set->list, item) == NULL) return -1;
...
static void sc_asmap_free(sc_asmap_t *asmap) {
  ...
  if (asmap->ases != NULL) free(asmap->ases);
  free(asmap);
  return;
}
...
static sc_asmap_t *sc_asmap_get(uint32_t *ases, int asc) {
  ...
  if ((map = sc_asmap_find(ases, asc)) != NULL) return map;
  if ((map = malloc_zero(sizeof(sc_asmap_t))) == NULL ||
      (map->ases = memdup(ases, sizeof(uint32_t) * asc)) == NULL)
    goto err;
  map->asc = asc;
  if (splaytree_insert(asmaptree, map) == NULL) goto err;
...
static int sc_asmap_isvp(sc_asmap_t *asmap) {
  ...
  for (i = 0; i < asmap->asc; i++)
    if (is_vpas(asmap->ases[i])) vppref++;
  if (vppref == asmap->asc) return 1;
  return 0;
}
...
static int sc_asrel_r(uint32_t a, uint32_t b, int *r) {
  sc_asrel_t *asr;
  if ((asr = sc_asrel_find(a, b)) == NULL) return -1;
  if (a < b)
    *r = asr->r;
  else
    *r = asr->r * -1;
...
static int sc_asrel_add(uint32_t a, uint32_t b, int r) {
  ...
  if ((asr = malloc_zero(sizeof(sc_asrel_t))) == NULL) return -1;
  if (a < b) {
    asr->a = a;
    asr->b = b;
    asr->r = r;
  ...
  if (splaytree_insert(reltree, asr) == NULL) return -1;
  return 0;
}
...
static sc_prov_t *sc_prov_get(uint32_t as) {
  sc_prov_t *p;
  if ((p = sc_prov_find(as)) != NULL) return p;
  if ((p = malloc_zero(sizeof(sc_prov_t))) == NULL) return NULL;
  p->as = as;
...
static int sc_prov_add(uint32_t a, uint32_t b) {
  sc_prov_t *p;
  if ((p = sc_prov_get(a)) == NULL) return -1;
  if (uint32_add(&p->provs, &p->provc, b) != 0) return -1;
  return 0;
}
...
static int vp_r(uint32_t peer, uint32_t *sib_as, int *r) {
  ...
  for (i = 0; i < vpasc; i++) {
    if (sc_asrel_r(vpas[i], peer, &rl) != 0) continue;
    if (rl == -1) {
...
static int asmap_r(sc_asmap_t *peer, uint32_t *sib, uint32_t *neigh, int *r) {
  ...
  if (peer->asc == 1) {
    if (is_vpas(peer->ases[0])) return -1;
    for (i = 0; i < vpasc; i++) {
    ...
    for (i = 0; i < vpasc; i++) {
      for (j = 0; j < peer->asc; j++) {
        if (is_vpas(peer->ases[j])) continue;
        if (sc_asrel_r(vpas[i], peer->ases[j], &rl) != 0) continue;
        if (rl == -1) {
          if (custc == 0) {
            cust_sib = vpas[i];
...
static int sc_waittest_sec(sc_test_t *test, int sec) {
  sc_waittest_t *wt;
  if ((wt = malloc_zero(sizeof(sc_waittest_t))) == NULL) return -1;
...
static void sc_target_detach(sc_target_t *tg) {
  ...
  if (tg->blocked != NULL) {
    while ((test = slist_head_pop(tg->blocked)) != NULL) sc_waittest(test);
...
static void sc_target_free(sc_target_t *tg) {
  if (tg == NULL) return;
  sc_target_detach(tg);
  if (tg->addr != NULL) scamper_addr_free(tg->addr);
  free(tg);
  return;
}
...
static int sc_prefix_cmp(const sc_prefix_t *a, const sc_prefix_t *b) {
  if (a == b) return 0;
  if (af == AF_INET) return prefix4_cmp(a->pfx.v4, b->pfx.v4);
...
static void sc_prefix_free(sc_prefix_t *p) {
  if (p == NULL) return;
  if (af == AF_INET) {
    if (p->pfx.v4 != NULL) prefix4_free(p->pfx.v4);
  } else {
    if (p->pfx.v6 != NULL) prefix6_free(p->pfx.v6);
  }
  free(p);
  return;
}
...
static sc_prefix_t *sc_prefix_alloc(void *net, int len) {
  ...
  if ((p = malloc_zero(sizeof(sc_prefix_t))) == NULL) goto err;
  if (af == AF_INET) {
    if ((p->pfx.v4 = prefix4_alloc(net, len, p)) == NULL) goto err;
  } else {
    if ((p->pfx.v6 = prefix6_alloc(net, len, p)) == NULL) goto err;
  }
  return p;
...
static sc_prefix_t *sc_prefix_find_in(void *addr) {
  ...
  prefix6_t *p6;
  if (af == AF_INET) {
    if ((p4 = prefixtree_find_ip4(ip2as_pt, addr)) != NULL) return p4->ptr;
  } else if (af == AF_INET6) {
    if ((p6 = prefixtree_find_ip6(ip2as_pt, addr)) != NULL) return p6->ptr;
  }
  return NULL;
}
...
static sc_prefix_t *sc_prefix_find(scamper_addr_t *addr) {
  sc_prefix_t *pfx;
  if (ipmap_pt != NULL && (pfx = pt_find(ipmap_pt, addr)) != NULL) return pfx;
  if (is_reserved(addr) != 0) return NULL;
  if (ixp_pt != NULL && pt_find(ixp_pt, addr) != NULL) return NULL;
...
static sc_prefix_nest_t *sc_prefix_nest_alloc(sc_prefix_t *pfx) {
  ...
  if ((nest = malloc_zero(sizeof(sc_prefix_nest_t))) == NULL) return NULL;
  nest->pfx = pfx;
  return nest;
}
...
static sc_ixpc_t *sc_ixpc_get(sc_stree_t *set, scamper_addr_t *addr) {
  sc_ixpc_t *pfxc;
  if (ixp_pt == NULL) return NULL;
  if ((pfxc = sc_ixpc_find(set, addr)) != NULL) return pfxc;
  if ((pfxc = malloc_zero(sizeof(sc_ixpc_t))) == NULL) return NULL;
...
static int is_vp(scamper_addr_t *addr) {
  sc_prefix_t *pfx;
  if (is_ixp(addr) != 0 || (pfx = sc_prefix_find(addr)) == NULL) return 2;
  return sc_asmap_isvp(pfx->asmap);
}

static void sc_tracetest_free(sc_tracetest_t *tt) {
  if (tt == NULL) return;
  if (tt->target != NULL) sc_target_free(tt->target);
  free(tt);
  return;
}
...
static void sc_linktest_free(sc_linktest_t *lt) {
  if (lt == NULL) return;
  if (lt->ta != NULL) sc_target_free(lt->ta);
  if (lt->tb != NULL) sc_target_free(lt->tb);
  if (lt->ab != NULL) scamper_addr_free(lt->ab);
...
static int sc_linktest_alloc(scamper_addr_t *a, scamper_addr_t *b) {
  ...
  if (sc_link_find(a, b) != NULL) return 0;
...
static sc_ping_t *sc_ping_alloc(scamper_addr_t *addr) {
  sc_ping_t *ping;
  if ((ping = malloc_zero(sizeof(sc_ping_t))) == NULL) return NULL;
...
static void sc_pingtest_free(sc_pingtest_t *pt) {
  ...
  if (pt->t2 != NULL) sc_target_free(pt->t2);
  free(pt);
  return;
}
...
static sc_test_t *sc_pingtest_alloc(scamper_addr_t *addr) {
  ...
  if ((ping = sc_ping_find(addr)) == NULL) {
    if ((ping = sc_ping_alloc(addr)) == NULL) return NULL;
...
#else
#define sc_asmapc_get(set, asmap) \
  sc_asmapc_get_dm((set), (asmap), __FILE__, __LINE__)
static sc_asmapc_t *sc_asmapc_get_dm(sc_stree_t *set, sc_asmap_t *asmap,
                                     const char *file, const int line)
#endif
{
  sc_asmapc_t *asmapc;
  if ((asmapc = sc_asmapc_find(set, asmap)) != NULL) return asmapc;
#ifndef DMALLOC
  if ((asmapc = malloc_zero(sizeof(sc_asmapc_t))) == NULL) return NULL;
#else
...
static sc_asc_t *sc_asc_get(sc_stree_t *set, uint32_t as) {
  sc_asc_t *asc;
  if ((asc = sc_asc_find(set, as)) != NULL) return asc;
  if ((asc = malloc_zero(sizeof(sc_asc_t))) == NULL) return NULL;
  asc->as = as;
...
static sc_asmapc_t *sc_router_dstases_get(sc_router_t *rtr, sc_asmap_t *asmap) {
  if (rtr->dstases == NULL && (rtr->dstases = sc_stree_alloc(
                                   (splaytree_cmp_t)sc_asmapc_as_cmp)) == NULL)
    return NULL;
...
static int sc_router_adj_add(sc_router_t *rtr, sc_router_t *adj) {
  dlist_node_t *dn;
  if (rtr->adj == NULL && (rtr->adj = dlist_alloc()) == NULL) return -1;
  for (dn = dlist_head_node(rtr->adj); dn != NULL; dn = dlist_node_next(dn))
    if (dlist_node_item(dn) == adj) return 0;
  if (dlist_tail_push(rtr->adj, adj) == NULL) return -1;
  return 0;
}
...
static int sc_router_prev_add(sc_router_t *rtr, sc_router_t *prev) {
  dlist_node_t *dn;
  if (rtr->prev == NULL && (rtr->prev = dlist_alloc()) == NULL) return -1;
  for (dn = dlist_head_node(rtr->prev); dn != NULL; dn = dlist_node_next(dn))
    if (dlist_node_item(dn) == prev) return 0;
  if (dlist_tail_push(rtr->prev, prev) == NULL) return -1;
  return 0;
}
...
static sc_addr2router_t *sc_routerset_a2r_add(sc_routerset_t *set,
  ...
  if ((a2r = malloc_zero(sizeof(sc_addr2router_t))) == NULL) return NULL;
  a2r->router = rtr;
  ...
  if (slist_tail_push(rtr->addrs, a2r) == NULL) return NULL;
  return a2r;
}
...
static sc_addr2router_t *sc_routerset_a2r_get(sc_routerset_t *set,
  ...
  sc_addr2router_t *a2r;
  if ((a2r = sc_routerset_a2r_find(set, addr)) != NULL) return a2r;
  if ((rtr = sc_router_alloc()) == NULL ||
      (rtr->node = dlist_tail_push(set->list, rtr)) == NULL) {
    if (rtr != NULL) sc_router_free(rtr);
    return NULL;
  }
  if ((a2r = sc_routerset_a2r_add(set, rtr, addr)) == NULL) return NULL;
  return a2r;
}

static sc_router_t *sc_routerset_find(sc_routerset_t *set, scamper_addr_t *a) {
  sc_addr2router_t *a2r;
  if ((a2r = sc_routerset_a2r_find(set, a)) == NULL) return NULL;
  return a2r->router;
}

static sc_router_t *sc_routerset_get(sc_routerset_t *set, scamper_addr_t *a) {
  sc_addr2router_t *a2r;
  if ((a2r = sc_routerset_a2r_find(set, a)) != NULL) return a2r->router;
  if ((a2r = sc_routerset_a2r_get(set, a)) != NULL) return a2r->router;
  return NULL;
}
...
static void sc_routerset_free(sc_routerset_t *set) {
  if (set == NULL) return;
  if (set->list != NULL) dlist_free_cb(set->list, (dlist_free_t)sc_router_free);
  if (set->tree != NULL)
...
static sc_routerset_t *sc_routerset_alloc(void) {
  ...
  if ((set = malloc_zero(sizeof(sc_routerset_t))) == NULL ||
      ...
      (set->tree = splaytree_alloc((splaytree_cmp_t)sc_addr2router_addr_cmp)) ==
          NULL) {
 ...
 */
static int sc_routerset_merge(sc_routerset_t *set, sc_router_t *a,
                              sc_router_t *b) {
  sc_addr2router_t *a2r;
  sc_router_t *x;
  dlist_node_t *dn, *dn2;
   ...
   * on the router.
   */
  if (b->ttl != 0 && (b->ttl < a->ttl || a->ttl == 0)) a->ttl = b->ttl;
  a->flags |= b->flags;
  ...
  while ((a2r = slist_head_pop(b->addrs)) != NULL) {
    a2r->router = a;
    if (slist_tail_push(a->addrs, a2r) == NULL) return -1;
  }
  ...
  while ((x = dlist_head_pop(b->adj)) != NULL) {
    for (dn = dlist_head_node(a->adj); dn != NULL; dn = dlist_node_next(dn))
      if (dlist_node_item(dn) == x) break;
    if (dn == NULL && dlist_tail_push(a->adj, x) == NULL) return -1;
  }
  if (b->prev != NULL) {
    while ((x = dlist_head_pop(b->prev)) != NULL) {
      if (sc_router_prev_add(a, x) != 0) return -1;
    }
  }
...
static sc_router_t *sc_routerset_getpair(sc_routerset_t *set, scamper_addr_t *a,
                                         scamper_addr_t *b) {
  ...
  if (a2r_a == NULL && a2r_b == NULL) {
    ...
    if (a2r_a->router != a2r_b->router) {
      while ((a2r_a = slist_head_pop(rtr->addrs)) != NULL) {
        ...
        if (slist_tail_push(a2r_b->router->addrs, a2r_a) == NULL) return NULL;
      }
    ...
    rtr = a2r_a->router;
    if (sc_routerset_a2r_add(set, rtr, b) == NULL) return NULL;
  } else {
    rtr = a2r_b->router;
    if (sc_routerset_a2r_add(set, rtr, a) == NULL) return NULL;
  }
...
static int sc_router_isborder(const sc_router_t *rtr) {
  ...
  for (dn = dlist_head_node(rtr->adj); dn != NULL; dn = dlist_node_next(dn)) {
    ...
    if (adj->owner_as == 0 || adj->owner_as != vpas[0]) return 1;
  }
  return 0;
}
...
static int sc_router_owner_cmp(const sc_router_t *a, const sc_router_t *b) {
  ...
  if (a2ra != NULL && a2rb == NULL) return -1;
  if (a2ra == NULL && a2rb != NULL) return 1;
...
static void sc_router_setowner(sc_router_t *r, uint32_t as, uint8_t reason) {
  assert(as != 0);
  assert(reason != SC_ROUTER_OWNER_NONE);
  r->owner_as = as;
...
static sc_allyconftest_t *sc_allyconftest_alloc(sc_ally_t *ally, int method) {
  ...
  if ((act = malloc_zero(sizeof(sc_allyconftest_t))) == NULL) return NULL;
  act->ally = ally;
  act->count = allyconf;
  act->method = method;
...
static void sc_allytest_free(sc_allytest_t *at) {
  ...
  if (at->routers != NULL) sc_routerset_free(at->routers);
...
static void sc_allytest_next(sc_allytest_t *at) {
  ...
  for (;;) {
    ...
    if (sc_ally_find(a->addr, b->addr) != NULL) continue;
...
static sc_addr2name_t *sc_addr2name_find(scamper_addr_t *addr) {
  sc_addr2name_t fm;
  if (ip2name_tree == NULL) return NULL;
  fm.addr = addr;
...
static sc_addr2adj_t *sc_addr2adj_get(sc_stree_t *set, scamper_addr_t *addr) {
  sc_addr2adj_t *a2a;
  if ((a2a = sc_addr2adj_find(set, addr)) != NULL) return a2a;
  if ((a2a = sc_addr2adj_alloc(addr)) == NULL) return NULL;
...
static int sc_farrouter_addnear(sc_farrouter_t *fr, sc_router_t *near) {
  if (slist_tail_push(fr->nears, near) == NULL) return -1;
  return 0;
}

static void sc_farrouter_free(sc_farrouter_t *fr) {
  if (fr == NULL) return;
  if (fr->nears != NULL) slist_free(fr->nears);
  free(fr);
  return;
}
...
static sc_farrouter_t *sc_farrouter_get(sc_stree_t *set, sc_router_t *far) {
  sc_farrouter_t *fr;
  if ((fr = sc_farrouter_find(set, far)) != NULL) return fr;
  if ((fr = sc_farrouter_alloc(far)) == NULL) return NULL;
...
static int sc_astraces_plus1(sc_astraces_t *traces, scamper_addr_t *addr) {
  ...
  if (af == AF_INET) {
    ...
    x = ntohl(in.s_addr);
    if (random_dst != 0) {
      if ((x & 0xC0) == 0xC0) return 0;
      random_u8(&r);
      x = (x & 0xFFFFFF00) | ((((x & 0xC0) >> 6) + 1) << 6) | (r & 0x3F);
    } else {
      if ((x & 0xff) >= 5) return 0;
      x++;
    }
    in.s_addr = htonl(x);
...
static int sc_astraces_aliases_procset(slist_t *list) {
  ...
  if (list == NULL || slist_count(list) < 2 || slist_count(list) > 10) return 0;
  ...
  for (sn = slist_head_node(at->addr_list); sn != NULL;
...
static int sc_astraces_aliases(sc_astraces_t *traces) {
  ...
  if (traces->links == NULL) return 0;

  if ((set = sc_stree_alloc((splaytree_cmp_t)sc_addr2adj_cmp)) == NULL)
    return -1;

  for (sn = slist_head_node(traces->links->list); sn != NULL;
       sn = slist_node_next(sn)) {
    ...
    if (is_vp(link->a) != 1) {
      ...
      if (slist_tail_push(a2a->list[0], b) == NULL) goto err;
    }
    ...
    if (slist_tail_push(a2a->list[1], a) == NULL) goto err;
  }

  if ((rs = sc_routerset_alloc()) == NULL) goto err;

  for (sn = slist_head_node(set->list); sn != NULL; sn = slist_node_next(sn)) {
    a2a = slist_node_item(sn);
    for (i = 0; i < 2; i++) {
      if (a2a->list[i] == NULL || slist_count(a2a->list[i]) < 2) continue;
      ...
      if ((a2r = sc_routerset_a2r_find(rs, a->addr)) != NULL) {
        ...
        if ((a2r = sc_routerset_a2r_get(rs, a->addr)) == NULL) goto err;
        a2r->data = a;
      }
      ra = a2r->router;
      ...
      while ((b = slist_head_pop(a2a->list[i])) != NULL) {
        if ((a2r = sc_routerset_a2r_find(rs, b->addr)) == NULL) {
          if ((a2r = sc_routerset_a2r_add(rs, ra, b->addr)) == NULL) goto err;
          a2r->data = b;
          continue;
        }
        ...
        if ((rb = a2r->router) != ra) {
          while ((a2r = slist_head_pop(rb->addrs)) != NULL) {
            a2r->router = ra;
            if (slist_tail_push(ra->addrs, a2r) == NULL) goto err;
          }
...
err:
  ...
  if (set != NULL) sc_stree_free(set, (slist_free_t)sc_addr2adj_free);
  if (rs != NULL) sc_routerset_free(rs);
  return -1;
}

static void sc_astraces_link_free(sc_link_t *link) {
  if (link == NULL) return;
  if (link->data != NULL) sc_indir_free(link->data);
  sc_link_free(link);
  return;
}
...
static int sc_astraces_link_add(sc_astraces_t *traces, scamper_addr_t *a,
  ...
  fm.a = a;
  fm.b = b;
  if (sc_stree_find(traces->links, &fm) != NULL) return 0;
...
static int sc_astraces_gss_add(sc_astraces_t *traces, scamper_addr_t *addr) {
  if (no_gss != 0) return 0;
  if (traces->gss == NULL &&
      (traces->gss = sc_stree_alloc((splaytree_cmp_t)scamper_addr_cmp)) == NULL)
    return -1;
  if (sc_stree_find(traces->gss, addr) != NULL) return 0;
  if (sc_stree_add(traces->gss, addr) != 0) return -1;
...
static void sc_astraces_free(sc_astraces_t *traces) {
  ...
  if (traces->dsts != NULL) slist_free(traces->dsts);
  if (traces->gss != NULL)
 ...
 * used by -O impatient.
 */
static int sc_astraces_count_cmp(const sc_astraces_t *a,
                                 const sc_astraces_t *b) {
...
static sc_astraces_t *sc_astraces_get(splaytree_t *tree, sc_asmap_t *asmap) {
  ...
  fm.asmap = asmap;
  if ((traces = splaytree_find(tree, &fm)) != NULL) return traces;
  if ((traces = sc_astraces_alloc()) == NULL) return NULL;
  traces->asmap = asmap;
  if (splaytree_insert(tree, traces) == NULL) goto err;
  return traces;
err:
  if (traces != NULL) sc_astraces_free(traces);
  return NULL;
...
static sc_traceset_t *sc_traceset_get(uint32_t asn) {
  sc_traceset_t *ts = NULL;
  if ((ts = sc_traceset_find(asn)) != NULL) return ts;
...
static void trace_dump(const scamper_trace_t *trace, sc_routerset_t *rtrset) {
  ...
  for (i = trace->firsthop - 1; i < trace->hop_count; i++) {
    ...
    printf("%s <%u,%u> %s %d",
           ...
           rtt_tostr(&hop->hop_rtt, rtt, sizeof(rtt)), is_vp(hop->hop_addr));
...
static int do_method_trace(sc_test_t *test, char *cmd, size_t len) {
  ...
  if ((found = sc_target_find(tt->target)) != NULL && found->test != test) {
    if (sc_target_block(found, test) != 0) return -1;
    return 0;
  } else if (found == NULL) {
    /* add the test to the blocked list */
    if (sc_target_add(tt->target) != 0) return -1;
  }
  ...
  if (firsthop > 1) string_concat(cmd, len, &off, " -f %u", firsthop);
  if (no_gss == 0 && tt->astraces->gss != NULL) {
    if (firsthop > 1) string_concat(cmd, len, &off, " -O dtree-noback");
...
static int do_method_ping(sc_test_t *test, char *cmd, size_t len) {
  ...
  string_concat(cmd, len, &off, "ping -i %s -c %u -o %u", wait[pt->method],
                attempts + 2, attempts);
...
static int do_method_ipidseq_addr(sc_test_t *test, scamper_addr_t *addr,
  ...
  if ((tt = sc_pingtest_alloc(addr)) == NULL) return -1;
  pt = tt->data;
  if (sc_target_block(pt->target, test) != 0) return -1;
 ...
 */
static int do_method_ipidseq_ping(sc_test_t *test, sc_ping_t *ping, char *cmd,
                                  size_t len) {
  sc_pingtest_t *pt;
  sc_target_t *found;
  sc_test_t *tt;
  ...
  if ((tt = sc_pingtest_alloc(ping->addr)) == NULL) return -1;
  pt = tt->data;
  if (sc_target_block(pt->target, test) != 0) return -1;
  if (sc_target_add(pt->target) != 0) {
    logerr("%s: could not add target\n", __func__);
  ...
  if (scamper_addr_cmp(ping->indir.dst, ping->addr) != 0) {
    if ((pt->t2 = sc_target_alloc(ping->indir.dst)) == NULL) return -1;
    pt->t2->test = tt;
   ...
   */
  if (pt->ping->methods[0] != IPID_NONE) pt->method = METHOD_INDIR;
...
static int do_method_link(sc_test_t *test, char *cmd, size_t len) {
  ...
  if (lt->step == TEST_LINK_PREFIXSCAN) {
    ...
    if (ping->methods[lt->method] == IPID_INCR ||
        (lt->method == METHOD_UDP &&
         ping->methods[lt->method] != IPID_UNRESP)) {
...
static int do_method_ally(sc_test_t *test, char *cmd, size_t len) {
  ...
  string_concat(cmd, len, &off, "dealias -m ally -W %u -q %u", wait[at->method],
                attempts);
  if (fudge == 0)
    string_concat(cmd, len, &off, " -O inseq");
  else
  ...
  if (at->method != METHOD_INDIR) {
    string_concat(cmd, len, &off, " -p '-P %s' %s %s", method[at->method],
...
static int do_method_allyconf(sc_test_t *test, char *cmd, size_t len) {
  ...
  if (act->method != METHOD_INDIR) {
    string_concat(cmd, len, &off, " -p '-P %s' %s %s", method[act->method],
...
static int virgin_pop(sc_test_t **out) {
  ...
  if ((tt = sc_tracetest_alloc(addr)) == NULL) goto err;
  scamper_addr_free(addr);
  addr = NULL;
  tt->astraces = traces;
  *out = tt->target->test;
...
static int do_method(void) {
  static int (*const func[])(sc_test_t *, char *, size_t) = {
      do_method_trace, do_method_link,     do_method_ping,
      do_method_ally,  do_method_allyconf,
  };
  sc_waittest_t *wt;
  sc_test_t *test;
  int off;
        ...
        if (sc_ally_find(pfs->a, pfs->ab) == NULL) {
          if ((ar = sc_ally_get(pfs->a, pfs->ab)) == NULL) {
            logerr("%s: could not get ally %s:%s: %s\n", __func__, a,
                   scamper_addr_tostr(pfs->ab, b, sizeof(b)), strerror(errno));
            return -1;
          }
          if (allyconf > 0 && sc_allyconftest_alloc(ar, lt->method) == NULL) {
            logerr("%s: could not add allyconftest: %s", __func__,
                   strerror(errno));
            return -1;
          }
        }
...
static int do_decoderead_dealias_ally(scamper_dealias_t *dealias) {
  ...
  if (scamper_addr_cmp(p[0], ally->probedefs[0].dst) != 0) {
    ...
    if (dealias->result == SCAMPER_DEALIAS_RESULT_ALIASES) {
      for (i = 0; i < dealias->probec; i++) {
        ...
        if (scamper_addr_cmp(reply->src, p[probe->def->id]) != 0) break;
      }
      if (i != dealias->probec) dealias->result = SCAMPER_DEALIAS_RESULT_NONE;
    }
  }
  ...
  if (at != NULL) {
    ...
    if (act->count > 0) {
      sc_target_free(act->a);
      act->a = NULL;
      sc_target_free(act->b);
      act->b = NULL;
...
static int do_decoderead_ping_link(sc_test_t *test, scamper_ping_t *ping) {
  ...
  if (lt->step == TEST_LINK_PSTS) {
    if (lt->method == 0 && lt->ab != NULL) {
      lt->method++;
      if (sc_waittest(test) != 0) return -1;
      return 0;
    }
    sc_linktest_free(lt);
    lt = NULL;
...
static int ping_classify(const scamper_addr_t *dst,
  ...
  for (i = 0; i < ping->ping_sent; i++) {
    if ((rx = ping->ping_replies[i]) != NULL &&
      ...
      if (slist_tail_push(list, rx) == NULL) goto done;
    }
  }
  ...
  while (ln1 != NULL) {
    rx = slist_node_item(ln0);
    n0 = rx->reply_ipid;
    rx = slist_node_item(ln1);
    n1 = rx->reply_ipid;
...
static int trace_lasthop(scamper_trace_t *trace) {
   ...
   */
  for (i = trace->firsthop - 1; i < trace->hop_count; i++) {
    if ((hop = trace->hops[i]) == NULL) continue;
...
static int do_decoderead_trace(scamper_trace_t *trace) {
  ...
  if ((target = sc_target_findaddr(trace->dst)) == NULL) goto err;
  ...
  if ((lh = trace_lasthop(trace)) == 0) goto done;

  /* make a note of the last hop mapped to the VP */
  lv = -1;
  for (i = trace->firsthop - 1; i < lh; i++)
    if ((hop = trace->hops[i]) != NULL && is_vp(hop->hop_addr) == 1) lv = i;
  ...
  while (i < lh) {
    if ((hop = trace->hops[i]) != NULL && is_vp(hop->hop_addr) == 0) {
      if (scamper_addr_cmp(trace->dst, hop->hop_addr) != 0) p1 = 0;
      break;
    }
    i++;
  }
  if (p1 != 0 && sc_astraces_plus1(astraces, trace->dst) != 0) goto err;
  ...
  for (i = trace->firsthop - 1; i <= lv; i++) {
    ...
    if (i - 1 >= trace->firsthop - 1 && (x = trace->hops[i - 1]) != NULL &&
      ...
      if (sc_astraces_link_add(astraces, x->hop_addr, hop->hop_addr, &indir) !=
          0)
        goto err;
    }
  }
  ...
  if (lv + 1 < lh && (x = trace->hops[lv + 1]) != NULL) {
    ...
    if (y == NULL || scamper_addr_cmp(x->hop_addr, y->hop_addr) != 0) {
      ...
      if (sc_astraces_link_add(astraces, trace->hops[lv]->hop_addr, x->hop_addr,
                               &indir) != 0)
        goto err;
      ...
      if (y != NULL) {
        indir.ttl = x->hop_probe_ttl;
        if (sc_astraces_link_add(astraces, x->hop_addr, y->hop_addr, &indir) !=
            0)
          goto err;
      }
    }
    ...
    if (y != NULL && SCAMPER_TRACE_HOP_IS_ICMP_TTL_EXP(y) &&
        ...
        slist_count(astraces->dsts) > 0 && is_reserved(y->hop_addr) == 0 &&
        is_vp(y->hop_addr) == 0 &&
  ...
  if ((addr = slist_head_pop(astraces->dsts)) == NULL) {
    ...
    if (no_alias == 0 && sc_astraces_aliases(astraces) != 0) goto err;
  ...
  if (sc_waittest(tt->target->test) != 0) goto err;
  ...
  for (i = 0; i < readbuf_len; i++) {
    if (readbuf[i] == '\n') {
      ...
      if (data_left > 0) {
        ...
        if (uus != 0) write_wrap(decode_out_fd, uu, NULL, uus);
...
static int delegated_line(char *line, void *param) {
  ...
  if (strncasecmp(ptr, "ipv4|", 5) != 0) return 0;
  ptr += 5;
  ...
  *ptr = '\0';
  ptr++;
  if (string_tolong(size, &lo) != 0) return -1;
  ...
  if (strncasecmp(ptr, "allocated", 9) != 0 &&
      strncasecmp(ptr, "assigned", 8) != 0)
    return 0;

  if ((del = malloc(sizeof(sc_delegated_t))) == NULL) return -1;

  if (inet_pton(AF_INET, net, &del->x) != 1) goto err;
  del->y.s_addr = htonl(ntohl(del->x.s_addr) + lo);
  if (slist_tail_push(delegated, del) == NULL) goto err;
...
static int ip2name_line(char *line, void *param) {
  ...
  *name = '\0';
  ...
  if (af == AF_INET) {
    ...
    sa.addr = &in;
    if (scamper_addr_isreserved(&sa)) return 0;
  } else {
  ...
  if (sc_addr2name_find(&sa) != NULL) return 0;
...
static int ip2as_line(char *line, void *param) {
  ...
  m = line;
  while (isspace(*m) == 0 && *m != '\0') m++;
  if (*m == '\0') return -1;
  *m = '\0';
  m++;
  while (isspace(*m) != 0) m++;
  if (string_tolong(m, &lo) != 0) return -1;

  a = m;
  while (isspace(*a) == 0 && *a != '\0') a++;
  if (*a == '\0') return -1;
  *a = '\0';
  a++;
  while (isspace(*a) != 0) a++;
  ...
  if (af == AF_INET) {
    if (lo < IPV4_PREFIX_MIN || lo > IPV4_PREFIX_MAX) return 0;
    if (inet_pton(AF_INET, n, &in) != 1) return -1;
    sa.type = SCAMPER_ADDR_TYPE_IPV4;
    sa.addr = &in;
    if (scamper_addr_isreserved(&sa)) return 0;
    if ((p = sc_prefix_alloc(&in, lo)) == NULL) goto err;
  } else {
    if (lo < IPV6_PREFIX_MIN || lo > IPV6_PREFIX_MAX) return 0;
    if (inet_pton(AF_INET6, n, &in6) != 1) return -1;
    sa.type = SCAMPER_ADDR_TYPE_IPV6;
    sa.addr = &in6;
    if (scamper_addr_isreserved(&sa)) return 0;
    if ((p = sc_prefix_alloc(&in6, lo)) == NULL) goto err;
  }

  for (at = a; last == 0; at++) {
    if (*at != '_' && *at != ',' && *at != ' ' && *at != '\0') continue;
    if (*at == ' ' || *at == '\0') last = 1;
    *at = '\0';
    u32 = atoi(a);
    /* skip over private / reserved ASNs */
    if (u32 == 0 || u32 == 23456 || (u32 >= 64512 && u32 <= 65535) ||
        u32 >= 4200000000UL)
      continue;
    if (uint32_add(&ases, &asc, u32) != 0) goto err;
    a = at + 1;
  }
  ...
  if ((p->asmap = sc_asmap_get(ases, asc)) == NULL) goto err;
...
static int ipmap_line(char *line, void *param) {
  ...
  *a = '\0';
  a++;
  while (isspace(*a) != 0) a++;
  if (string_isnumber(a) == 0 || string_tolong(a, &lo) != 0) goto err;
  ...
  ases[0] = lo;
  if ((p->asmap = sc_asmap_get(ases, 1)) == NULL) goto err;
...
static int relfile_line(char *line, void *param) {
  ...
  if (string_isnumber(as) == 0 || string_isnumber(bs) == 0 ||
      string_isnumber(rs) == 0 || string_tolong(as, &a) != 0 || a < 1 ||
      string_tolong(bs, &b) != 0 || b < 1 || string_tolong(rs, &r) != 0 ||
      r < -1 || r > 1) {
    printf("line %d %s %s %s\n", linec, as, bs, rs);
    return -1;
  }
  ...
  if (r == -1) {
    if (sc_prov_add(b, a) != 0) return -1;
  } else if (r == 1) {
    if (sc_prov_add(a, b) != 0) return -1;
  }
...
static int do_targetips(void) {
  ...
  for (i = 0; i < opt_argc; i++) {
    ...
    if ((traces = sc_astraces_get(astraces, pfx->asmap)) == NULL) goto err;
...
static int do_targets_rec(slist_t *list, splaytree_t *astraces) {
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    ptr = NULL;
    if (af == AF_INET) {
      if (rec_target_4(nest, &in) == 1) ptr = &in;
    } else {
      if (rec_target_6(nest, &in6) == 1) ptr = &in6;
    }
      ...
      if (targetasc > 0) {
        for (i = 0; i < targetasc && probe == 0; i++)
          for (j = 0; j < nest->pfx->asmap->asc && probe == 0; j++)
            if (targetas[i] == nest->pfx->asmap->ases[j]) probe = 1;
      } else {
    ...
    if (nest->list != NULL) do_targets_rec(nest->list, astraces);
  }
...
static int do_targets(void) {
  ...
  for (sn = slist_head_node(prefixes); sn != NULL; sn = slist_node_next(sn)) {
    ...
    while (nest != NULL) {
      ...
      if (af == AF_INET) {
        if ((p4 = prefixtree_find_best4(nest->pt, pfx->pfx.v4)) == NULL) break;
        nest = p4->ptr;
      } else {
        if ((p6 = prefixtree_find_best6(nest->pt, pfx->pfx.v6)) == NULL) break;
        nest = p6->ptr;
      }
    }
  ...
  if ((astracestree = splaytree_alloc((splaytree_cmp_t)sc_astraces_cmp)) ==
      NULL)
    goto err;

  do_targets_rec(root, astracestree);

  while ((nest = slist_head_pop(root)) != NULL) sc_prefix_nest_free(nest);
  slist_free(root);
  root = NULL;
  ...
  splaytree_free(astracestree, NULL);
  astracestree = NULL;
...
static int do_ip2as(void) {
  ...
  if ((lists = malloc_zero(sizeof(slist_t *) * j)) == NULL) goto err;
  for (i = 0; i < j; i++)
    if ((lists[i] = slist_alloc()) == NULL) goto err;
  ...
  if ((prefixes = slist_alloc()) == NULL) goto err;
  for (i = 0; i < j; i++) {
  ...
  if ((ip2as_pt = prefixtree_alloc(af)) == NULL) goto err;
...
static int ixp_line(char *line, void *param) {
  ...
  if (string_tolong(pf, &lo) != 0 || lo < 0) return -1;
  ...
  if (getaddrinfo(line, NULL, &hints, &res0) != 0 || res0 == NULL) return -1;

  for (res = res0; res != NULL; res = res->ai_next) {
    if (res->ai_family == PF_INET) {
      if (af != AF_INET) break;
      va = &((struct sockaddr_in *)res->ai_addr)->sin_addr;
      if (prefixtree_find_exact4(ixp_pt, va, lo) != NULL) break;
      if ((p4 = prefix4_alloc(va, lo, NULL)) == NULL) return -1;
      p4->ptr = p4;
      if (prefixtree_insert4(ixp_pt, p4) == NULL) return -1;
      break;
    } else if (res->ai_family == PF_INET6) {
      if (af != AF_INET6) break;
      va = &((struct sockaddr_in6 *)res->ai_addr)->sin6_addr;
      if (prefixtree_find_exact6(ixp_pt, va, lo) != NULL) break;
      if ((p6 = prefix6_alloc(va, lo, NULL)) == NULL) return -1;
      p6->ptr = p6;
      if (prefixtree_insert6(ixp_pt, p6) == NULL) return -1;
      break;
    }
  }
  freeaddrinfo(res0);
...
static int do_ixp(void) {
  if ((ixp_pt = prefixtree_alloc(af)) == NULL) return -1;
...
static int bdrmap_data(void) {
  ...
  if (options & OPT_TARGETIPS) {
    if (do_targetips() != 0) goto done;
  } else {
    if (do_targets() != 0) goto done;
  }

  if (do_ipmap() != 0) return -1;
  ...
  if (decode_in == NULL) goto done;
  if (fcntl_set(decode_in_fd, O_NONBLOCK) == -1) goto done;
...
static int sc_link4_addadj(sc_link4_t *l4, sc_prefix_t *pfx) {
  ...
  if (l4->adjases == NULL &&
    ...
    return -1;
  if ((asmapc = sc_asmapc_get(l4->adjases, pfx->asmap)) == NULL) return -1;
  asmapc->c++;
  return 0;
}
...
static int sc_link4_addgap(sc_link4_t *l4, sc_prefix_t *pfx) {
  ...
  if (l4->gapases == NULL &&
    ...
    return -1;
  if ((asmapc = sc_asmapc_get(l4->gapases, pfx->asmap)) == NULL) return -1;
  asmapc->c++;
  return 0;
}
...
static int process_1_dealias(scamper_dealias_t *dealias) {
  ...
  if (SCAMPER_DEALIAS_METHOD_IS_PREFIXSCAN(dealias)) {
    pfs = dealias->data;
    if (pfs->ab != NULL && scamper_addr_prefixhosts(pfs->b, pfs->ab) >= 30) {
      if ((link = sc_link_get(pfs->a, pfs->b)) == NULL) goto done;
      if ((pfs->flags & SCAMPER_DEALIAS_PREFIXSCAN_FLAG_CSA) != 0) {
        if (sc_routerset_getpair(rtrset, pfs->a, pfs->ab) == NULL) goto done;
      } else {
        if ((ar = sc_ally_get(pfs->a, pfs->ab)) == NULL) goto done;
    ...
    if (ok && (ar = sc_ally_get(a, b)) == NULL) goto done;
    if (ar != NULL && (ar->result == SCAMPER_DEALIAS_RESULT_NONE ||
...
static sc_link_t *process_1_link(scamper_trace_hop_t *x, scamper_trace_hop_t *y,
                                 sc_asmap_t *dst) {
  sc_link_t *link;
  sc_link4_t *link4;
  sc_asmapc_t *asmapc;
  ...
  if ((link = sc_link_get(x->hop_addr, y->hop_addr)) == NULL) return NULL;
  if ((link4 = link->data) == NULL) {
    if ((link4 = sc_link4_alloc()) == NULL) return NULL;
    link->data = link4;
   ...
   */
  if (link4->ttl > x->hop_probe_ttl) link4->ttl = x->hop_probe_ttl;

  /*
   * keep a track of which networks were seen when tracing through
   * this link
   */
  if ((asmapc = sc_asmapc_get(link4->dstases, dst)) == NULL) return NULL;
  asmapc->c++;
...
static int process_1_trace_unrouted(scamper_trace_t *trace, int lh) {
  ...
  for (i = trace->firsthop - 1; i < lh; i++) {
    ...
    if ((x = trace->hops[i]) == NULL) continue;
    /* if we have come to an external hop, stop */
    if ((vp = is_vp(x->hop_addr)) == 0) break;
    ...
    if (vp == 1 && slist_count(list) > 0) {
      while ((addr = slist_head_pop(list)) != NULL) {
        if (sc_prefix_find_in(addr->addr) != NULL) continue;
        ...
        if (pfx == NULL) goto done;
        if ((pfx->asmap = sc_asmap_get(&as, 1)) == NULL) goto done;
        if (af == AF_INET) {
          if (prefixtree_insert4(ip2as_pt, pfx->pfx.v4) == NULL) goto done;
        } else {
          if (prefixtree_insert6(ip2as_pt, pfx->pfx.v6) == NULL) goto done;
        }
        if (slist_tail_push(prefixes, pfx) == NULL) goto done;
        modified = 1;
      }
      slist_empty(list);
    } else {
...
static int process_1_trace_work(scamper_trace_t *trace, int lh) {
  ...
  if ((dst = sc_prefix_find(trace->dst)) == NULL) return 0;
  ...
  for (i = trace->firsthop - 1; i < lh - 1; i++) {
    ...
    if ((link = process_1_link(x, y, dst->asmap)) == NULL) return -1;
    link4 = link->data;
    ...
    if (vp == 0 || (vp == 2 && is_ixp(y->hop_addr) == 0)) {
      for (j = i + 2; j < lh; j++) {
        if ((z = trace->hops[j]) == NULL ||
          ...
          continue;
        if (sc_link4_addadj(link4, pfx) != 0) return -1;
        break;
      }
      if (vp == 0) {
        ttlnn = 1;
  ...
  if (ttlnn == 0 && asmap_r(dst->asmap, &sib, &asn, &r) == 0 && r != 1) {
    if ((ts = sc_traceset_get(asn)) == NULL) return -1;
    if (slist_tail_push(ts->list, trace) == NULL) return -1;
    return 1;
  }
...
static int process_1_trace(scamper_trace_t *trace) {
  ...
  if (rc != 0) {
    ...
    if (rc == 1) return 0;
    if ((tmp = slist_alloc()) == NULL) return -1;
    ...
    while ((trace = slist_head_pop(held)) != NULL) {
      ...
      if (rc == 0) {
        ...
        if (rc == 0) scamper_trace_free(trace);
        continue;
      }

      slist_tail_push(tmp, trace);
      if (rc == 2) slist_concat(held, tmp);
    }
...
static int process_1_ping(scamper_ping_t *ping) {
  ...
  if (SCAMPER_PING_METHOD_IS_UDP(ping) == 0) goto done;
...
static int sc_asrel_to_vpasc(sc_stree_t *neighs, sc_asrel_t *r) {
  ...
  if (sc_asc_get(neighs, as) == NULL) return -1;
...
static sc_stree_t *asmap_to_asset(sc_stree_t *set) {
  ...
  for (sn = slist_head_node(set->list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    for (i = 0; i < asmapc->asmap->asc; i++) {
      if (is_vpas(asmapc->asmap->ases[i])) continue;
...
static int owner_1_graph(sc_stree_t *set, uint32_t *as) {
  ...
  if (slist_count(set->list) == 1 &&
    ...
    for (i = 0; i < 2; i++) {
      if (is_vpas(asmap->ases[i])) continue;
  ...
  if ((asset = asmap_to_asset(set)) == NULL) return -1;
  ...
  for (sn = slist_head_node(asset->list); sn != NULL;
       sn = slist_node_next(sn)) {
  ...
  *asmap = NULL;
  ...
  for (sn = slist_head_node(y->addrs); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((pfx = sc_prefix_find(a2r->addr)) != NULL) {
      if ((asmapc = sc_asmapc_get(yas_set, pfx->asmap)) == NULL) goto done;
      asmapc->c++;
    }
  }
...
static int owner_1_thirdparty(sc_router_t *y, uint32_t *owner, uint32_t *tp) {
  ...
  if (y->dstases == NULL || slist_count(y->dstases->list) != 1) return 0;
...
static int owner_1_traceset(sc_routerset_t *set, sc_traceset_t *ts) {
  ...
  if ((fars = slist_alloc()) == NULL) return -1;
  ...
  for (sn = slist_head_node(ts->list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    for (i = 0; i < trace->hop_count; i++) {
      ...
      if ((rtr = sc_routerset_find(set, hop->hop_addr)) == NULL) continue;
      if (is_vpas(rtr->owner_as)) last_rtr = rtr;
    }
  ...
  if (sn != NULL || far_hop == NULL) goto done;
  if ((rtr = sc_routerset_find(set, far_hop->hop_addr)) != NULL) {
    if (rtr->owner_as != 0) goto done;
  } else {
    if ((rtr = sc_routerset_get(set, far_hop->hop_addr)) == NULL) goto err;
    rtr->ttl = near_rtr->ttl + 1;
  }
...
static int owner_1(sc_router_t *y, uint32_t *owner_as, uint8_t *owner_reason,
  ...
  *owner_reason = SC_ROUTER_OWNER_NONE;
  ...
  for (sn = slist_head_node(y->addrs); sn != NULL; sn = slist_node_next(sn)) {
    a2r = slist_node_item(sn);
    if (a2r->ttlexp == 0) continue;
    if (slist_tail_push(a2r_list, a2r) == NULL) goto done;
    if (is_vp(a2r->addr) == 1)
      vpc++;
   ...
   */
  for (dn = (y->adj != NULL) ? dlist_head_node(y->adj) : NULL; dn != NULL;
       dn = dlist_node_next(dn)) {
    z = dlist_node_item(dn);
    for (sn2 = slist_head_node(z->addrs); sn2 != NULL;
         sn2 = slist_node_next(sn2)) {
      a2r = slist_node_item(sn2);
      if (a2r->ttlexp == 0) continue;
      if (is_ixp(a2r->addr)) {
        if ((ixppfxc = sc_ixpc_get(ixp_set, a2r->addr)) == NULL) goto done;
        ixppfxc->c++;
      } else if ((pfx = sc_prefix_find(a2r->addr)) != NULL &&
                 sc_asmap_isvp(pfx->asmap) == 0) {
        if ((asmapc = sc_asmapc_get(zas_set, pfx->asmap)) == NULL) goto done;
        asmapc->c++;
      }
    }
  }
  ...
  if (y->flags & SC_ROUTER_FLAG_FIRST) {
    if (y->adj != NULL && slist_count(zas_set->list) > 0) {
      if ((zas2_set = sc_stree_alloc((splaytree_cmp_t)sc_asmapc_as_cmp)) ==
              NULL ||
      ...
      for (dn = dlist_head_node(y->adj); dn != NULL; dn = dlist_node_next(dn)) {
        ...
        if (owner_1_thirdparty(z, &dst_asn, &y_asn) == 1) {
          if ((asc = sc_asc_get(tp_set, y_asn)) == NULL) goto done;
          asc->c++;
        } else if (owner_1_asmap(z, &zas) == 1) {
          if ((asmapc = sc_asmapc_get(zas2_set, zas)) == NULL) goto done;
          asmapc->c++;
        }
      }
      ...
      if (slist_count(tp_set->list) > 0 && slist_count(zas2_set->list) > 0 &&
      ...
      zas2_set = NULL;
      sc_stree_free(tp_set, (sc_stree_free_t)sc_asc_free);
      tp_set = NULL;
    }
    ...
    if (zas_count > 0 && y_dstases_count > 0 && y_dstases_count < 100) {
       ...
       */
      if (owner_1_graph(y->dstases, &y_dstases_owner) != 0) goto done;
      zas = ((sc_asmapc_t *)slist_head_item(zas_set->list))->asmap;
      for (i = 0; i < zas->asc; i++)
        if (zas->ases[i] == y_dstases_owner) break;
   ...
   * close to an IXP
   */
  if (vpc == a2r_count && (ixppfxc = slist_head_item(ixp_set->list)) != NULL &&
      ixppfxc->c >= 2) {
    /*
  ...
  if (vpc + ixpc == a2r_count) {
     ...
     */
    if (ixpc == a2r_count) {
      for (sn = slist_head_node(y->addrs); sn != NULL;
           sn = slist_node_next(sn)) {
        a2r = slist_node_item(sn);
        if (a2r->ttlexp == 0) continue;
        if ((ixppfxc = sc_ixpc_find(ixp_set, a2r->addr)) != NULL) break;
      }
      if (sn != NULL) {
        *owner_reason = SC_ROUTER_OWNER_IXP;
        *owner_as = vpas[0];
  ...
  if (zas_count == 1) {
    ...
    for (dn = dlist_head_node(y->adj); dn != NULL; dn = dlist_node_next(dn)) {
      z = dlist_node_item(dn);
      if (z->adjases == NULL) continue;
      if ((adj_set = asmap_to_asset(z->adjases)) == NULL) goto done;
      for (i = 0; i < zas->asc; i++) {
        z_asn = zas->ases[i];
        if (is_vpas(z_asn)) continue;
  ...
  if (zas_count == 1) {
    ...
    *owner_as = zas->ases[0];
    ...
    sn = slist_head_node(zas_set->list);
    asmapc = slist_node_item(sn);
    sn2 = slist_node_next(sn);
    asmapc2 = slist_node_item(sn2);
    if (asmapc->c == asmapc2->c) {
      asmapc3 = NULL;
      while (sn != NULL) {
        asmapc2 = slist_node_item(sn);
        if (asmapc->c != asmapc2->c) break;
      ...
      if (asmap_r(asmapc->asmap, &sib, &neigh, &r) != 0) {
        if (is_vpas(asmapc->asmap->ases[0]) == 0) {
          ...
          *owner_reason = SC_ROUTER_OWNER_COUNT2;
          ...
          if (owner_1_graph(zas_set, &zas_set_owner) == 0) goto done;
          if (zas_set_owner != 0) neigh = zas_set_owner;
          *owner_as = neigh;
  ...
  if (vpc > 0 && a2r_count != vpc) {
    for (sn = slist_head_node(yas_set->list); sn != NULL;
         sn = slist_node_next(sn)) {
      yas = ((sc_asmapc_t *)slist_node_item(sn))->asmap;
      for (i = 0; i < yas->asc; i++) {
        if (is_vpas(yas->ases[i])) continue;
        *owner_as = yas->ases[i];
  ...
  if (yas_count == 1) {
    yas = ((sc_asmapc_t *)slist_head_item(yas_set->list))->asmap;
    for (i = 0; i < yas->asc; i++) {
      if (is_vpas(yas->ases[i]) && vp_ipasn == 0) continue;
      *owner_as = yas->ases[i];
      *owner_reason = SC_ROUTER_OWNER_IP2AS;
      rc = 1;
      goto done;
...
static void finish_1(void) {
  ...
  while ((link = slist_head_pop(list)) != NULL) {
    ...
    if ((link4 = link->data) == NULL) continue;
     ...
     * observed
     */
    if (y->ttl == 0 || y->ttl > link4->ttl) y->ttl = link4->ttl;
    if (z->ttl == 0 || z->ttl > link4->ttl + 1) z->ttl = link4->ttl + 1;
    ...
    if ((a2r = sc_routerset_a2r_find(rtrset, link->a)) == NULL) goto done;
    a2r->ttlexp = 1;
    if ((a2r = sc_routerset_a2r_find(rtrset, link->b)) == NULL) goto done;
    a2r->ttlexp = 1;

    /* begin to assign ownership */
    if (is_vp(link->b) == 1) y->flags |= SC_ROUTER_FLAG_FIRST;
  }
  ...
  /* assign owners! */
  do {
    for (dn = dlist_head_node(rtrset->list); dn != NULL;
         dn = dlist_node_next(dn)) {
      x = dlist_node_item(dn);
    ...
    for (dn = dlist_head_node(rtrset->list); dn != NULL;
       ...
       */
      x = dlist_node_item(dn);
      if (x->owner_as != vpas[0]) continue;
      if (x->adj == NULL) continue;
      for (dn2 = dlist_head_node(x->adj); dn2 != NULL;
           dn2 = dlist_node_next(dn2)) {
        /* skip over routers that have been assigned an owner */
        y = dlist_node_item(dn2);
        if (y->owner_reason != SC_ROUTER_OWNER_NONE) continue;
        if (y->flags & SC_ROUTER_FLAG_VISITED) continue;
        y->flags |= SC_ROUTER_FLAG_VISITED;

        /* if we have an error, break out */
        if ((rc = owner_1(y, &owner_as, &owner_reason, 0)) < 0) goto done;
    ...
    for (dn = dlist_head_node(rtrset->list); dn != NULL;
      ...
      if (x->owner_reason != SC_ROUTER_OWNER_NONE || sc_router_isvp(x) == 0)
        continue;
      if (x->flags & SC_ROUTER_FLAG_VISITED) continue;
      x->flags |= SC_ROUTER_FLAG_VISITED;
      if ((rc = owner_1(x, &owner_as, &owner_reason, 1)) < 0) goto done;
      if (rc > 0) {
  ...
  do {
    ...
    for (dn = dlist_head_node(rtrset->list); dn != NULL;
      ...
      if (x->owner_as == 0 || x->owner_as != vpas[0] ||
        ...
        continue;
      for (dn2 = (x->adj != NULL) ? dlist_head_node(x->adj) : NULL; dn2 != NULL;
           dn2 = dlist_node_next(dn2)) {
        /* we want the far routers owned by other networks */
        y = dlist_node_item(dn2);
        if (y->owner_as == vpas[0] || y->owner_as == 0) continue;
        if ((fr = sc_farrouter_get(fars, y)) == NULL) goto done;
        if (sc_farrouter_addnear(fr, x) != 0) goto done;
      }
    }
    if (slist_count(fars->list) == 0) break;
    slist_qsort(fars->list, (slist_cmp_t)sc_farrouter_nears_cmp);
    for (sn = slist_head_node(fars->list); sn != NULL;
         sn = slist_node_next(sn)) {
      fr = slist_node_item(sn);
      if (slist_count(fr->nears) <= 1) break;
      rc = 1;
      slist_qsort(fr->nears, (slist_cmp_t)sc_router_p_cmp);
      sn2 = slist_head_node(fr->nears);
      while (sn2 != NULL) {
        y = slist_node_item(sn2);
        sn2 = slist_node_next(sn2);
  ...
  for (dn = dlist_head_node(rtrset->list); dn != NULL;
       dn = dlist_node_next(dn)) {
    x = dlist_node_item(dn);
    if (is_vpas(x->owner_as) == 0) continue;

    for (dn2 = (x->adj != NULL) ? dlist_head_node(x->adj) : NULL; dn2 != NULL;
         dn2 = dlist_node_next(dn2)) {
      y = dlist_node_item(dn2);
      if (y->owner_as == 0 || is_vpas(y->owner_as) != 0) continue;
      if ((asc = sc_asc_find(neighs, y->owner_as)) != NULL) asc->c++;
    }
  }
  splaytree_inorder(tracesets, tree_to_slist, list);
  splaytree_free(tracesets, NULL);
  tracesets = NULL;
  while ((ts = slist_head_pop(list)) != NULL) {
    if ((asc = sc_asc_find(neighs, ts->asn)) != NULL && asc->c == 0) {
      if ((rc = owner_1_traceset(rtrset, ts)) < 0) return;
      if (rc == 0 && dump_tracesets) traceset_dump(ts, rtrset);
    }
    sc_traceset_free(ts);
  }
  ...
  for (dn = dlist_head_node(rtrset->list); dn != NULL;
       dn = dlist_node_next(dn)) {
    x = dlist_node_item(dn);
    if (x->adj != NULL) dlist_qsort(x->adj, (dlist_cmp_t)sc_router_owner_cmp);
  }
  ...
  for (dn = dlist_head_node(rtrset->list); dn != NULL;
       dn = dlist_node_next(dn)) {
    x = dlist_node_item(dn);
    if (x->owner_as != 0) owned++;
    if (x->owner_as != 0 && x->owner_as != vpas[0]) continue;
    if (dump_borders != 0 && sc_router_isborder(x) == 0) continue;

    printf("owner %u (%s", x->owner_as, owner_reasonstr[x->owner_reason]);
    if (x->flags & SC_ROUTER_FLAG_MERGED) printf(",merged");
    printf(")\n");
    for (sn = slist_head_node(x->addrs); sn != NULL; sn = slist_node_next(sn)) {
      a2r = slist_node_item(sn);
      printf("%s", scamper_addr_tostr(a2r->addr, buf, sizeof(buf)));
      if (a2r->ttlexp != 0) printf("*");
    ...
    for (dn2 = (x->adj != NULL) ? dlist_head_node(x->adj) : NULL; dn2 != NULL;
         dn2 = dlist_node_next(dn2)) {
      y = dlist_node_item(dn2);
      if (y->owner_as == vpas[0] && no_self != 0) continue;
      printf(" %u", y->owner_as);
      if (dump_onedsts && y->dstases != NULL &&
        ...
        asmap = asmapc->asmap;
        for (i = 0; i < asmap->asc; i++)
          if (asmap->ases[i] == y->owner_as) break;
        if (i == asmap->asc)
      ...
      for (sn2 = slist_head_node(y->addrs); sn2 != NULL;
        ...
        if (a2r->ttlexp != 0) {
          printf("*");
          if (x->owner_as == 0 && (pfx = sc_prefix_find(a2r->addr)) != NULL)
            printf(" %s", sc_asmap_tostr(pfx->asmap, buf, sizeof(buf)));
        }
        if ((a2n = sc_addr2name_find(a2r->addr)) != NULL)
          printf(" %s", a2n->name);
...
static int process_2_trace(scamper_trace_t *trace) {
   ...
   */
  if (targetasc > 0) {
    if ((pfx = sc_prefix_find(trace->dst)) == NULL) goto done;
    for (i = 0; i < pfx->asmap->asc; i++) {
      for (j = 0; j < targetasc; j++)
        if (pfx->asmap->ases[i] == targetas[j]) break;
      if (j != targetasc) break;
    }
    if (i == pfx->asmap->asc) goto done;
  }
...
static int bdrmap_dump(void) {
  ...
  if (ip2name_fn != NULL) {
    ...
    if (ip2name_tree == NULL) return -1;
    if (file_lines(ip2name_fn, ip2name_line, NULL) != 0) return -1;
  }

  if (delegated_fn != NULL) {
    if ((delegated = slist_alloc()) == NULL) return -1;
    if (file_lines(delegated_fn, delegated_line, NULL) != 0) return -1;
    if (slist_qsort(delegated, (slist_cmp_t)sc_delegated_cmp) != 0) return -1;
  }

  if (do_ipmap() != 0) return -1;

  if (dump_funcs[dump_id].init != NULL) dump_funcs[dump_id].init();
  ...
  if (dump_funcs[dump_id].finish != NULL) dump_funcs[dump_id].finish();
...
static int bdrmap_init(void) {
  ...
  if (ip2as_fn != NULL && do_ip2as() != 0) return -1;
  ...
  if (relfile_fn != NULL) {
    if ((reltree = splaytree_alloc((splaytree_cmp_t)sc_asrel_cmp)) == NULL ||
      ...
      return -1;
    if (file_lines(relfile_fn, relfile_line, NULL) != 0) return -1;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (options & OPT_HELP) return 0;

  /* start a daemon if asked to */
  if ((options & OPT_DAEMON) != 0 && daemon(1, 0) != 0) return -1;

  if (bdrmap_init() != 0) return -1;

  if (options & OPT_DUMP) return bdrmap_dump();
