...
static void usage(uint32_t opt_mask) {
  fprintf(
      stderr,
      "usage: sc_erosprober [-?] [-a addrfile] [-c cmd] [-o outfile] [-p "
      "port]\n"
  ...
  if (opt_mask & OPT_COMMAND) fprintf(stderr, "   -c scamper command to use\n");
...
static int check_options(int argc, char *argv[]) {
  ...
  /* XXX: 60 */
  if (opt_interval == NULL || string_tolong(opt_interval, &lo) != 0 ||
      lo < 10) {
    usage(OPT_INTERVAL);
    return -1;
  }
  ...
  if (opt_rotation == NULL || string_tolong(opt_rotation, &lo) != 0 ||
      lo < 10) {
    usage(OPT_ROTATION);
    return -1;
  }
...
static int ep_tree_to_heap(scamper_addr_t *addr) {
  ...
  fm.addr = addr;
  if ((ep = patricia_find(pt, &fm)) == NULL) return -1;

  if (ep->flags & EP_FLAG_REMOVE) {
    ep_del(ep);
  } else {
    ep->flags &= (~EP_FLAG_PROBING);
    if ((ep->hn = heap_insert(waiting, ep)) == NULL) return -1;
  }
...
static int do_addrfile(void) {
  ...
  if (file_lines(addrfile_name, addrfile_line, list) != 0) goto done;
  if (shuffle != 0) slist_shuffle(list);
...
static int do_outfile(void) {
  ...
  gettimeofday_wrap(&now);
  snprintf(buf, sizeof(buf), "%s.%ld.warts", outfile_name,
           (long int)now.tv_sec);
  logprint("%s\n", buf);
...
static int do_ctrlsock_readline(void *param, uint8_t *buf, size_t len) {
  ...
  if (op == '+' || op == '-') {
    ...
    if ((sa = scamper_addr_resolve(AF_UNSPEC, line)) == NULL) return 0;
...
static int do_ctrlsock(void) {
  ...
  if (ctrlsock_name == NULL) return 0;
...
static int do_method(void) {
  ...
  gettimeofday_wrap(&now);
  if (timeval_cmp(&now, &ep->tv) < 0) return 0;

  if (ep->type == EP_TYPE_ROTATE) {
    ep = heap_remove(waiting);
    free(ep);
    if (do_outfile() != 0) return -1;
    ep = heap_head_item(waiting);
    assert(ep != NULL);
    if (timeval_cmp(&now, &ep->tv) < 0) return 0;
  }

  if (ep->type != EP_TYPE_PROBE || more < 1) return 0;
  ep = heap_remove(waiting);
...
int main(int argc, char *argv[]) {
  ...
  if ((waiting = heap_alloc((heap_cmp_t)sc_ep_tv_cmp)) == NULL) return -1;
  ...
  if ((probing4 = patricia_alloc((patricia_bit_t)sc_ep_addr_bit,
      ...
      (scamper_lp = scamper_linepoll_alloc(do_scamperread_line, NULL)) ==
          NULL ||
      (decode_wb = scamper_writebuf_alloc()) == NULL || do_addrfile() != 0 ||
      (nooutfile == 0 && do_outfile() != 0) || do_scamperconnect() != 0 ||
      do_ctrlsock() != 0 ||
  ...
  while (ep_stop == 0) {
    ...
    if (ctrlsock_fd >= 0) {
      if (FD_ISSET(ctrlsock_fd, &rfds) && do_ctrlsock_accept() != 0) return -1;
      dn = dlist_head_node(ctrlsock_fds);
      while (dn != NULL) {
        cs = dlist_node_item(dn);
        dn = dlist_node_next(dn);
        if (FD_ISSET(cs->fd, &rfds) && do_ctrlsock_read(cs) != 0) return -1;
        if (cs->fd == -1) {
          do_ctrlsock_free(cs);
          continue;
        }
