...
static sc_policytest_t methods[] = {
    {"ICMP", 4, PT_METHOD_ICMP, NULL, 0,
     PT_FLAG_ICMP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"NetBIOS", 7, PT_METHOD_NETBIOS, NULL, 139, PT_FLAG_TCP},
    {"MSSQL", 5, PT_METHOD_MSSQL, NULL, 1433, PT_FLAG_TCP},
    {"FTP", 3, PT_METHOD_FTP, NULL, 21, PT_FLAG_TCP | PT_FLAG_SERVER},
    {"SSH", 3, PT_METHOD_SSH, NULL, 22,
     PT_FLAG_TCP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"Telnet", 6, PT_METHOD_TELNET, NULL, 23,
     PT_FLAG_TCP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"MySQL", 5, PT_METHOD_MYSQL, NULL, 3306, PT_FLAG_TCP | PT_FLAG_SERVER},
    {"RDP", 3, PT_METHOD_RDP, NULL, 3389, PT_FLAG_TCP | PT_FLAG_SERVER},
    {"HTTPS", 5, PT_METHOD_HTTPS, NULL, 443,
     PT_FLAG_TCP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"SMB", 3, PT_METHOD_SMB, NULL, 445, PT_FLAG_TCP | PT_FLAG_SERVER},
    {"VNC", 3, PT_METHOD_VNC, NULL, 5900, PT_FLAG_TCP},
    {"HTTP", 4, PT_METHOD_HTTP, NULL, 80,
     PT_FLAG_TCP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"BGP", 3, PT_METHOD_BGP, NULL, 179, PT_FLAG_TCP | PT_FLAG_ROUTER},
    {"NTP", 3, PT_METHOD_NTP, "160200010000000000000000", 123,
     PT_FLAG_UDP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
    {"DNS", 3, PT_METHOD_DNS,
     "a980010000010000000000000377777706676f6f676c6503636f6d0000010001", 53,
     PT_FLAG_UDP | PT_FLAG_SERVER | PT_FLAG_ROUTER},
...
static void usage(uint32_t opt_mask) {
  ...
  fprintf(stderr,
          "usage: sc_filterpolicy [-D] [-a infile] [-o outfile] [-p port] [-U "
          "unix]\n"
  ...
  if (opt_mask & OPT_OPTIONS)
    fprintf(stderr,
            "   -O options [impatient | incongruent | trace | tuples]\n");
...
static int check_options(int argc, char *argv[]) {
  ...
  if (u32 != OPT_READ && u32 != (OPT_INFILE | OPT_OUTFILE)) {
    usage(0);
    goto err;
  }
  ...
  if (options & (OPT_INFILE | OPT_OUTFILE)) {
    ...
    if (opt_type != NULL) {
      if (strcasecmp(opt_type, "router") == 0) {
        ...
        for (i = 0; i < methodc; i++) methods[i].flags |= PT_FLAG_USE;
      }
    }
    ...
    if (test_list != NULL) {
      while ((test = slist_head_pop(test_list)) != NULL) {
        ...
        for (i = 0; i < methodc; i++)
          if (strcasecmp(methods[i].name, test + 1) == 0) break;
        if (i == methodc) {
          usage(OPT_TEST);
    ...
    for (i = 0; i < methodc; i++)
      if (methods[i].flags & PT_FLAG_USE) break;
    if (i == methodc) {
    ...
    if ((options & (OPT_PORT | OPT_UNIX | OPT_LOG | OPT_TYPE | OPT_TEST |
                    OPT_DAEMON)) != 0 ||
  ...
  for (i = 0; i < ping->ping_sent; i++) {
    for (r = ping->ping_replies[i]; r != NULL; r = r->next) {
      if (method->flags & PT_FLAG_TCP) {
        ...
        if (SCAMPER_PING_REPLY_IS_UDP(r)) return 1;
      } else if (method->flags & PT_FLAG_ICMP) {
        if (SCAMPER_PING_REPLY_IS_ICMP_ECHO_REPLY(r)) return 1;
      } else
        return -1;
    }
  }
...
static sc_iditem_t *sc_iditem_get(sc_idset_t *set, scamper_addr_t *addr) {
  ...
  if ((item = malloc_zero(sizeof(sc_iditem_t))) == NULL) return NULL;
...
static void sc_idset_free(sc_idset_t *set) {
  ...
  if (set->items != NULL) {
    for (i = 0; i < set->itemc; i++) sc_iditem_free(set->items[i]);
    free(set->items);
  }
  free(set);
  return;
...
static sc_idset_t *sc_idset_get(splaytree_t *tree, uint32_t id) {
  ...
  if ((set = splaytree_find(tree, &fm)) != NULL) return set;

  /* no collection, alloc a new one */
  if ((set = malloc_zero(sizeof(sc_idset_t))) == NULL) return NULL;
  set->userid = id;
...
static int sc_idset_incongruent(const sc_idset_t *set) {
  ...
  for (i = 1; i < set->itemc; i++) {
    item = set->items[i];
    if (first->results != item->results) return 1;
  }
...
static void sc_idset_print(const sc_idset_t *set) {
  ...
  if ((flags & FLAG_INCONGRUENT) != 0 && sc_idset_incongruent(set) == 0) return;
  ...
  for (i = 0; i < set->itemc; i++) {
    ...
    if ((len = strlen(buf)) > maxaddr) maxaddr = len;
    o |= item->results;
  }
  ...
  for (j = 0; j < PT_METHOD_MAX; j++) {
    if ((set->tests & (1 << j)) == 0) continue;
    pt = &methods[j];
    if (pt->namelen > max) max = pt->namelen;
  }
  ...
  for (len = 0; len < maxaddr; len++) printf("=");
  printf("==");
  for (j = 0; j < PT_METHOD_MAX; j++) {
    if ((set->tests & (1 << j)) == 0) continue;
    printf("===");
...
static sc_ip2n2i_t *sc_ip2n2i_get(scamper_addr_t *ip, sc_name2ips_t *n2i) {
  ...
  if ((ip2n2i = malloc_zero(sizeof(sc_ip2n2i_t))) == NULL) return NULL;
...
static int sc_name2ips_name_cmp(const sc_name2ips_t *a,
                                const sc_name2ips_t *b) {
  return strcmp(a->name, b->name);
}

static int sc_name2ips_addr_cmp(const sc_name2ips_t *a,
                                const sc_name2ips_t *b) {
...
static void sc_name2ips_free(sc_name2ips_t *n2i) {
  scamper_addr_t *addr;
  if (n2i == NULL) return;
  if (n2i->tree_node != NULL) splaytree_remove_node(n2i_tree, n2i->tree_node);
  if (n2i->addrs != NULL) {
    while ((addr = slist_head_pop(n2i->addrs)) != NULL) scamper_addr_free(addr);
    slist_free(n2i->addrs);
  }
  if (n2i->methods != NULL) slist_free(n2i->methods);
  if (n2i->name != NULL) free(n2i->name);
  if (n2i->set != NULL) sc_idset_free(n2i->set);
  free(n2i);
  return;
}
...
static sc_name2ips_t *sc_name2ips_get(char *name) {
  ...
  fm.name = name;
  if ((n2i = splaytree_find(n2i_tree, &fm)) != NULL) return n2i;
...
static int do_method(void) {
  ...
  if (flags & FLAG_TRACEROUTE) {
    ...
    if (pt->flags & PT_FLAG_ICMP)
      ...
      string_concat(cmd, sizeof(cmd), &off, " -P udp -O dl -d %u -B %s",
                    pt->port, pt->payload);
    string_concat(cmd, sizeof(cmd), &off, " %s\n", buf);
  }
...
static int do_infile(void) {
  ...
  if (flags & FLAG_TUPLES) {
    if (file_lines(infile, infile_tuples_line, addrtree) != 0) goto err;
  } else {
    if (file_lines(infile, infile_rows_line, addrtree) != 0) goto err;
  }
  splaytree_free(addrtree, (splaytree_free_t)scamper_addr_free);
  addrtree = NULL;

  for (node = slist_head_node(n2i_list); node != NULL;
...
static sc_name2ips_t *sc_name2ips_find(scamper_addr_t *dst) {
  sc_ip2n2i_t *ip2n2i;
  if ((ip2n2i = sc_ip2n2i_find(dst)) == NULL) return NULL;
  return ip2n2i->n2i;
}
...
static int do_decoderead_addr(scamper_addr_t *dst) {
  ...
  if (slist_count(n2i->methods) == 0) {
    ...
    if (slist_count(n2i->addrs) == 0) {
      if (n2i->set != NULL) sc_idset_print(n2i->set);
...
static int do_decoderead_ping(scamper_ping_t *ping) {
  ...
  if ((options & OPT_DAEMON) == 0) {
    ...
    if (code == 1) item->results |= (1 << (method->id - 1));
  }
...
static int do_scamperread_line(void *param, uint8_t *buf, size_t linelen) {
  ...
  if (linelen >= 3 && strncasecmp(head, "ERR", 3) == 0) {
    more++;
    if (do_n2i_next() != 0 || do_method() != 0) return -1;
    return 0;
  }
...
static int fp_data(void) {
  ...
  if ((n2i_tree = splaytree_alloc((splaytree_cmp_t)sc_name2ips_name_cmp)) ==
          NULL ||
      ...
      (scamper_lp = scamper_linepoll_alloc(do_scamperread_line, NULL)) ==
          NULL ||
      (decode_wb = scamper_writebuf_alloc()) == NULL || do_infile() != 0 ||
      do_scamperconnect() != 0 ||
...
static int fp_read(void) {
  ...
  if ((tree = splaytree_alloc((splaytree_cmp_t)sc_idset_cmp)) == NULL) goto err;

  while (scamper_file_read(in, ffilter, &type, &data) == 0) {
    if (data == NULL) break;

    ping = data;
    if ((method = ping_to_method(ping)) == NULL) goto err;
    code = ping_r(method, ping);

    if ((set = sc_idset_get(tree, ping->userid)) == NULL) goto err;
    set->tests |= (1 << (method->id - 1));

    if ((item = sc_iditem_get(set, ping->dst)) == NULL) goto err;
    item->tests |= (1 << (method->id - 1));
    if (code == 1) item->results |= (1 << (method->id - 1));
...
static int fp_init(void) {
  ...
  if ((ffilter = scamper_file_filter_alloc(&type, 1)) == NULL) return -1;
...
int main(int argc, char *argv[]) {
  ...
  if (options & OPT_READ) return fp_read();
