...
typedef struct sc_domain {
  ...
  slist_t *routers; /* of sc_routerdom_t: training routers with an iface in the
                       domain */
...
#define REFINE_ALL                                                    \
  (REFINE_TP | REFINE_FNE | REFINE_CLASS | REFINE_FNU | REFINE_SETS | \
   REFINE_IP | REFINE_FP | REFINE_MERGE | REFINE_DICT)
...
static void usage(uint32_t opts) {
  ...
  if (opts & OPT_RTTS) fprintf(stderr, "       -R: RTT file\n");
  if (opts & OPT_STOPID)
    fprintf(stderr, "       -s: stop at specified step #\n");
  if (opts & OPT_SIBLINGS) fprintf(stderr, "       -S: siblings file\n");
  if (opts & OPT_THREADC)
...
static int check_options(int argc, char *argv[]) {
  ...
  if (opt_dumpid != NULL) {
    if (string_isnumber(opt_dumpid) != 0) {
      ...
      for (x = 1; x < dump_funcc; x++) {
        if (dump_funcs[x].label == NULL) continue;
        if (strcasecmp(dump_funcs[x].label, opt_dumpid) == 0) break;
      }
      if (x == dump_funcc) {
        usage(OPT_DUMPID);
        return -1;
  ...
  if (opt_stopid != NULL) {
    ...
    if (do_learnasn != 0)
      stop_id = STOP_ASN_MAX;
    else if (do_learnasnames != 0)
      stop_id = STOP_ASNAMES_MAX;
    else if (do_learnalias != 0)
      stop_id = STOP_ALIAS_MAX;
    else if (do_learngeo != 0)
      stop_id = STOP_GEO_MAX;
  }
  ...
  if (do_learngeo != 0) {
    ...
    if (opt_lightspeed != NULL) {
      if (string_isdigit(opt_lightspeed) == 0 ||
          string_tolong(opt_lightspeed, &lo) != 0 || lo < 0 ||
          lo > LIGHT_SPEED_MAX) {
...
static uint32_t mask_cnt(uint32_t *mask, uint32_t len) {
  uint32_t i, c = 0;
  for (i = 0; i < len; i++) c += countbits32(mask[i]);
  return c;
}
...
#ifdef HAVE_PTHREAD
  if (pthread_mutex_lock(&dom->mutex) != 0) return -1;
#endif
  return 0;
}
...
static sc_uint32c_t *sc_uint32c_get(splaytree_t *tree, uint32_t num) {
  sc_uint32c_t *ptr, fm;
  fm.num = num;
  if ((ptr = splaytree_find(tree, &fm)) != NULL) return ptr;
  if ((ptr = malloc(sizeof(sc_uint32c_t))) == NULL) return NULL;
  ptr->num = num;
  ptr->c = 0;
...
static int slist_to_dlist(void *entry, void *ptr) {
  if (dlist_tail_push((dlist_t *)ptr, entry) != NULL) return 0;
  return -1;
}

static int dlist_to_slist(void *entry, void *ptr) {
  if (slist_tail_push((slist_t *)ptr, entry) != NULL) return 0;
  return -1;
}
...
static int dled(const char *a, const char *b) {
  ...
  if ((d = malloc(sizeof(int) * (al + 2) * (bl + 2))) == NULL) return -1;
...
static int str_tojson(const char *str, char *buf, size_t len) {
  ...
  while (*str != '\0') {
    if (*str == '\\' || *str == '#') {
      ...
      if (isprint(*str) == 0 || len - off < 2) return -1;
    }
    buf[off++] = *str;
    str++;
  }
...
static sc_ptrc_t *sc_ptrc_get(splaytree_t *tree, void *ptr) {
  sc_ptrc_t *ptrc;
  if ((ptrc = sc_ptrc_find(tree, ptr)) != NULL) return ptrc;
  if ((ptrc = malloc(sizeof(sc_ptrc_t))) == NULL) goto err;
  ptrc->ptr = ptr;
  ptrc->c = 0;
  if (splaytree_insert(tree, ptrc) == NULL) goto err;
  return ptrc;
...
static sc_strlist_t *sc_strlist_get(splaytree_t *tree, char *str) {
  sc_strlist_t *sl;
  if ((sl = sc_strlist_find(tree, str)) != NULL) return sl;
...
static sc_as2org_t *sc_as2org_find(uint32_t asn) {
  ...
  fm.asn = asn;
  return array_find((void **)siblings, siblingc, &fm,
                    (array_cmp_t)sc_as2org_cmp);
}
...
static int sc_as2tag_cmp(sc_as2tag_t *a, sc_as2tag_t *b) {
  int i;
  if ((i = strcmp(a->tag, b->tag)) != 0) return i;
  if (a->asn < b->asn) return -1;
  if (a->asn > b->asn) return 1;
  return 0;
}
...
#else
#define sc_as2tag_get(tree, asn, tag) \
  sc_as2tag_get_dm((tree), (asn), (tag), __FILE__, __LINE__)
...
#endif
{
  ...
  fm.asn = asn;
  fm.tag = tag;
  if ((a2t = splaytree_find(tree, &fm)) != NULL) return a2t;

#ifndef DMALLOC
  a2t = malloc(sizeof(sc_as2tag_t));
#else
  a2t = dmalloc_malloc(file, line, sizeof(sc_as2tag_t), DMALLOC_FUNC_MALLOC, 0,
                       0);
#endif
  if (a2t == NULL) goto err;
  a2t->asn = asn;
  a2t->tag = NULL;
  a2t->sxes = NULL;
...
static char *re_escape_str(char *buf, size_t len, char *in) {
  ...
  while (*in != '\0') {
    if ((r = re_escape(tmp, sizeof(tmp), *in)) == 0) return NULL;
    /* string_concat(buf, len, &off, "%s", tmp); */
    if (len - off < r + 1) return NULL;
    memcpy(buf + off, tmp, r + 1);
    off += r;
    in++;
  }
...
static uint8_t bits_to_geohint_type(int k) {
  switch (k) {
    case BIT_TYPE_GEO_IATA:
      return GEOHINT_TYPE_IATA;
    case BIT_TYPE_GEO_ICAO:
      return GEOHINT_TYPE_ICAO;
    case BIT_TYPE_GEO_CLLI:
      return GEOHINT_TYPE_CLLI;
    case BIT_TYPE_GEO_PLACE:
      return GEOHINT_TYPE_PLACE;
    case BIT_TYPE_GEO_LOCODE:
      return GEOHINT_TYPE_LOCODE;
    case BIT_TYPE_GEO_FACILITY:
      return GEOHINT_TYPE_FACILITY;
    case BIT_TYPE_GEO_CC:
      return GEOHINT_TYPE_CC;
    case BIT_TYPE_GEO_ST:
      return GEOHINT_TYPE_ST;
  }
  return 0;
}
...
static int geohint_to_bits_type(uint8_t k) {
  switch (k) {
    case GEOHINT_TYPE_IATA:
      return BIT_TYPE_GEO_IATA;
    case GEOHINT_TYPE_ICAO:
      return BIT_TYPE_GEO_ICAO;
    case GEOHINT_TYPE_CLLI:
      return BIT_TYPE_GEO_CLLI;
    case GEOHINT_TYPE_PLACE:
      return BIT_TYPE_GEO_PLACE;
    case GEOHINT_TYPE_LOCODE:
      return BIT_TYPE_GEO_LOCODE;
    case GEOHINT_TYPE_FACILITY:
      return BIT_TYPE_GEO_FACILITY;
    case GEOHINT_TYPE_CC:
      return BIT_TYPE_GEO_CC;
    case GEOHINT_TYPE_ST:
      return BIT_TYPE_GEO_ST;
  }
  return -1;
}
...
static int pt_xor(size_t len, int *m, int mc, int **outn, int *outc) {
  ...
  for (i = 0; i < mc - 2; i += 2)
    if (m[i + 1] + 1 < m[i + 2]) nc += 2;

  if (nc == 0) goto done;

  if ((n = malloc(sizeof(int) * nc)) == NULL) return -1;
 ...
 */
static int pt_to_bits_ip(const sc_ifacedom_t *ifd, int *l, int lc, int **bits,
                         int *bitc) {
  slist_t *list = NULL;
  int i, j, k, x;
  ...
  if ((list = slist_alloc()) == NULL) goto done;
  ...
  if (ip_s != 0 && lc != 0 && l[0] != 0) {
    k = (ip_s < l[0] ? ip_s : l[0]);
    if (pt_to_bits_trip(list, BIT_TYPE_SKIP, 0, k - 1) != 0) goto done;
    j = k;
  }
  ...
  while (j < ifd->len) {
    /* IP match */
    if (j == ip_s) {
      if (pt_to_bits_trip(list, type, ip_s, ip_e) != 0) goto done;
      j = ip_e + 1;
      continue;
    }
    ...
    while (isalnum(ifd->label[j]) == 0 && j < k) j++;
    x = k;
    while (isalnum(ifd->label[x]) == 0 && x >= j) x--;
    if (x >= j && pt_to_bits_trip(list, BIT_TYPE_SKIP, j, x) != 0) goto done;
    j = k + 1;
  }

  if (pt_list_to_bits(list, bits, bitc) != 0) goto done;

  if (do_debug != 0 && threadc <= 1) {
    printf("%s %d | %d %d", ifd->label, (int)ifd->len, ip_s, ip_e);
    if (lc > 0) {
      printf(" |");
      for (i = 0; i < lc; i++) printf(" %d", l[i]);
    }
    printf(" |");
    for (i = 0; i < *bitc; i++) printf(" %d", (*bits)[i]);
    printf("\n");
  }
...
static int pt_overlap(const int *X, int Xc, const int *L, int Lc) {
  ...
  assert(Xc % 2 == 0);
  assert(Lc % 2 == 0);
  for (x = 0; x < Xc - 1; x++) assert(X[x + 1] >= X[x]);
  for (l = 0; l < Lc - 1; l++) assert(L[l + 1] >= L[l]);

  for (x = 0; x < Xc; x += 2) {
    for (l = 0; l < Lc; l += 2) {
      if (overlap(X[x], X[x + 1], L[l], L[l + 1]) != 0) return 1;
    }
  }
 ...
 */
static int pt_to_bits_ctype(const sc_ifacedom_t *ifd, int *ctypes, int16_t *c,
                            int cc, int **bits, int *bitc) {
  ...
  if (c[0] != 0) {
    ...
    if (isdigit(s[k]) != 0) {
      d = e = k;
      while (d > 0 && isdigit(s[d - 1]) != 0) d--;
      k = d - 1;
    }

    /* calculate boundaries of skip, skipping over punctuation */
    if (k >= 0) {
      while (k > 0 && isalnum(s[k]) == 0) k--;
      if (pt_to_bits_trip(list, BIT_TYPE_SKIP, 0, k) != 0) goto done;
    }
  ...
  for (i = 0; i < cc; i += 2) {
    ...
    if (isdigit(s[j]) != 0) {
      k = j;
      while (isdigit(s[k + 1]) != 0) k++;
      if (pt_to_bits_trip(list, BIT_TYPE_SKIP_DIGIT, j, k) != 0) goto done;
      j = k + 1;
    }

    /* open the next skip portion, skipping over dashes and dots */
    while (s[j] != '\0' && isalnum(s[j]) == 0) j++;

    /* skip portion */
    if (s[j] != '\0' && (i + 2 == cc || c[i + 2] != j)) {
      if (i + 2 < cc) {
        k = c[i + 2] - 1;
        while (isalnum(s[k]) == 0 && k > c[i + 1]) k--;
      } else {
        k = c[i + 1];
        while (s[k + 1] != '\0') k++;
      }

      if (pt_to_bits_trip(list, BIT_TYPE_SKIP, j, k) != 0) goto done;
    }
  }
 ...
 *
 */
static int pt_to_bits(const char *str, size_t len, int *c, int cc, int *l,
                      int lc, int *d, int dc, int **bits, int *bitc) {
  ...
  for (i = 0; i < cc; i += 2)
    for (j = c[i]; j <= c[i + 1]; j++) plan[j] |= cap;
  for (i = 0; i < lc; i += 2)
    for (j = l[i]; j <= l[i + 1]; j++) plan[j] |= lit;
  for (i = 0; i < dc; i += 2)
    for (j = d[i]; j <= d[i + 1]; j++) plan[j] |= dig;

  i = j = 0;
  while (i < len) {
    j = i + 1;
    while (j < len && plan[i] == plan[j]) j++;
    j--;
    ...
    if ((plan[i] & cap) != 0) {
      if ((plan[i] & dig) != 0)
        ...
        m = BIT_TYPE_CAPTURE;
      if (pt_to_bits_trip(list, m, i, j) != 0) goto done;
    } else if (plan[i] != 0) {
      if ((plan[i] & dig) != 0)
      ...
      while (i <= j && isalnum(str[i]) == 0) i++;
      k = j;
      while (i <= k && isalnum(str[k]) == 0) k--;

      if (i <= k && isalnum(str[i]) != 0) {
        m = BIT_TYPE_SKIP;
        if (pt_to_bits_trip(list, m, i, k) != 0) goto done;
      }
    }
 ...
 */
static int pt_to_bits_noip(const sc_ifacedom_t *ifd, int *c, int cc, int **out,
                           int *bitc) {
  int ip_s = ifd->iface->ip_s;
  int ip_e = ifd->iface->ip_e;
  int *d = NULL;
   ...
   */
  if ((d = malloc(sizeof(int) * (cc + 2))) == NULL) return -1;
  ...
  for (i = 0; i < cc; i += 2) {
     ...
     */
    if (ip_s <= c[i] && c[i + 1] <= ip_e) continue;
    ...
    if (ip_s > c[i]) {
      x = ip_s - 1;
      while (isalpha(ifd->label[x]) == 0 && x > c[i]) x--;
      assert(dc < cc + 2);
      d[dc++] = c[i];
      d[dc++] = x;
    }
    ...
    if (ip_e < c[i + 1]) {
      x = ip_e + 1;
      while (isalpha(ifd->label[x]) == 0 && x < c[i + 1]) x++;
      assert(dc < cc + 2);
      d[dc++] = x;
      d[dc++] = c[i + 1];
    }
...
static sc_clligp_t *sc_clligp_get(splaytree_t *tree, const char *gp,
  ...
  if ((clligp = splaytree_find(tree, &fm)) != NULL) return clligp;
...
static int sc_geomap_code(sc_geomap_t *map, sc_ptrc_t *x, int xc) {
  ...
  if (xc == 2) {
    if (x[0].c + x[1].c > sizeof(map->code)) return -1;
    ...
    if (x[0].c > sizeof(map->code)) return -1;
    j = 0;
    cptr = x[0].ptr;
    for (i = 0; i < x[0].c; i++) {
      if (isalnum(cptr[i]) == 0) continue;
      map->code[j++] = cptr[i];
    }
    map->code[j] = '\0';
    map->codelen = j;
...
static char *sc_geomap_tostr(const sc_geomap_t *map, char *buf, size_t len) {
  ...
  if (map->st[0] != '\0') string_concat(buf, len, &off, "|%s", map->st);
  if (map->cc[0] != '\0') string_concat(buf, len, &off, "|%s", map->cc);
  return buf;
}
...
static sc_georef_t *sc_georef_get(splaytree_t *tree, const sc_geomap_t *map) {
  sc_georef_t *gr;
  if ((gr = sc_georef_find(tree, map)) != NULL) return gr;
  ...
  if (splaytree_insert(tree, gr) == NULL) goto err;
  return gr;
...
static int sc_state_sort_cmp(const sc_state_t *a, const sc_state_t *b) {
  int i;
  if ((i = strcmp(a->st, b->st)) != 0) return i;
...
static sc_geohint_t *sc_geohint_alloc(uint8_t type, const char *code,
                                      const char *place, const char *st,
                                      const char *cc, double lat, double lng,
                                      long popn) {
  sc_geohint_t *hint;
  ...
  if (cc != NULL) memcpy(hint->cc, cc, sizeof(hint->cc));
  if (st != NULL) memcpy(hint->st, st, sizeof(hint->st));
  return hint;
...
static int sc_geoeval_cmp(const sc_geoeval_t *a, const sc_geoeval_t *b) {
  ...
  /* tie breaks */
  if ((x = strcmp(a->hint->code, b->hint->code)) != 0) return x;
  if ((x = strcmp(a->hint->cc, b->hint->cc)) != 0) return x;
  if ((x = strcmp(a->hint->st, b->hint->st)) != 0) return x;

  return 0;
}

static void sc_geoeval_free(sc_geoeval_t *ge) {
  if (ge->alloc != 0 && ge->hint != NULL) sc_geohint_free(ge->hint);
  free(ge);
  return;
}
...
static int sc_geohint_street_tostr2(const char *in, char *out, size_t len) {
  static const char *digit2string[] = {"One", "Two",   "Three", "Four", "Five",
                                       "Six", "Seven", "Eight", "Nine", "Ten"};
  size_t off = 0;
  char *endptr;
  long lo;

  if (isdigit(*in) == 0) return -1;
  ...
  in = endptr;
  while (*in != '\0') {
    if (isalpha(*in) != 0) break;
    if (isspace(*in) == 0) return -1;
    in++;
  }
  if (*in == '\0') return -1;
...
static int sc_geohint_street_tostr(const char *in, char *out, size_t len) {
  static const char *ones[] = {"fir",       "seco",     "thi",     "four",
                               "fif",       "six",      "seven",   "eigh",
                               "nin",       "ten",      "eleven",  "twelf",
  ...
  while (*in != '\0') {
    if (isdigit(*in) != 0) {
      ...
      if (lo < 20)
        ...
        snprintf(tmp, sizeof(tmp), "%sy%s", tens[(lo / 10) - 2],
                 ones[(lo % 10) - 1]);
      tmp[0] = toupper(tmp[0]);

      x = 0;
      while (tmp[x] != '\0') out[off++] = tmp[x++];
      out[off++] = *endptr;
      endptr++;
      out[off++] = *endptr;
      in = endptr;
...
static char *sc_geohint_place_tostr(const sc_geohint_t *hint, char *buf,
                                    size_t len) {
  size_t off = 0;
  char uc[8];

  if (hint->facname != NULL)
    string_concat(buf, len, &off, "%s%s", off != 0 ? ", " : "", hint->facname);
  if (hint->street != NULL)
    string_concat(buf, len, &off, "%s%s", off != 0 ? ", " : "", hint->street);
  if (hint->place != NULL)
    string_concat(buf, len, &off, "%s%s", off != 0 ? ", " : "", hint->place);
  if (hint->st[0] != '\0')
...
static int sc_geohint_place_tojson(const sc_geohint_t *hint, char *buf,
                                   size_t len) {
  size_t off = 0;
  char tmp[512], *comma = "";
  ...
  if (hint->facname != NULL) {
    if (str_tojson(hint->facname, tmp, sizeof(tmp)) != 0) return -1;
    string_concat(buf, len, &off, "\"facname\":\"%s\"", tmp);
    comma = ", ";
  }
  if (hint->street != NULL) {
    if (str_tojson(hint->street, tmp, sizeof(tmp)) != 0) return -1;
    string_concat(buf, len, &off, "%s\"street\":\"%s\"", comma, tmp);
    comma = ", ";
  }
  if (hint->place != NULL) {
    if (str_tojson(hint->place, tmp, sizeof(tmp)) != 0) return -1;
 ...
 */
static int sc_geohint_checkrtt(const sc_geohint_t *hint,
  ...
  for (i = 0; i < rtr->rttc; i++) {
    ...
    if (rtt > sample->rtt && rtt - sample->rtt > rtt_fudge) return 0;
    ok++;
  }
...
static int sc_geohint_checkmap(const sc_geohint_t *hint,
...
static sc_geohint_t *sc_geohint_findx(sc_geohint_t **in_geohints,
  ...
  hint = hint->head;
  while (hint != NULL) {
    if (strcmp(map->code, hint->code) != 0) break;
    if (sc_geohint_checkmap(hint, map) != 0) return hint;
    hint = hint->next;
  }
...
static sc_geohint_t *sc_geohint_find(const sc_regex_t *re,
  ...
  if (re != NULL && re->dom->geohints != NULL &&
      (hint = sc_geohint_findx(re->dom->geohints, re->dom->geohintc, map)) !=
          NULL)
    return hint;
...
static int sc_geohint_fudgepre(const sc_geohint_t *hint,
                               const sc_geomap_t *map) {
  /* make the CC / ST match, if these are part of the map */
  if (map->cc[0] != '\0' && cceq(map->cc, hint) == 0) return 0;
  if (map->st[0] != '\0' && strcmp(map->st, hint->st) != 0) return 0;
  return 1;
}
...
static uint32_t sc_geohint_fudgeeval(const sc_geohint_t *hint,
  ...
  *tp_rtt = 65535;
  tp_c = 0;
  for (sn = slist_head_node(gr->rd_list); sn != NULL;
       sn = slist_node_next(sn)) {
    rd = slist_node_item(sn);
    if (sc_geohint_checkrtt(hint, rd->rtr) != 0) {
      tp_c++;
      if (rd->rtr->rtts[0].rtt < *tp_rtt) *tp_rtt = rd->rtr->rtts[0].rtt;
    }
  }
  if (tp_c <= rt_tp_c) return 0;
   ...
   */
  if (gr->class == '!' && tp_c - rt_tp_c == 1 && *tp_rtt > rtt_close) return 0;
   ...
   */
  tp_c = 0;
  for (sn = slist_head_node(gr->ifi_list); sn != NULL;
       sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if (sc_geohint_checkrtt(hint, ifi->ifd->rd->rtr) != 0) tp_c++;
  }
 ...
 */
static int sc_geohint_abbrv(const char *hint_p, const char *map_c, size_t *offs,
                            size_t len,
  ...
  for (;;) {
    for (;;) {
      ...
      if (hint_p[ho] != '\0' && isalnum(hint_p[ho]) == 0) {
        ho++;
        for (;;) {
          while (isalnum(hint_p[ho]) == 0 && hint_p[ho] != '\0') ho++;
          if (hint_p[ho] == '\0' || tolower(hint_p[ho]) == map_c[mo]) break;
          while (isalnum(hint_p[ho]) != 0) ho++;
        }
      }
    ...
    if (mo == len) {
      if (ok == NULL || ok(hint_p, offs, len) != 0) return 1;
      ho = offs[mo - 1] + 1;
      mo--;
    }
  }
...
static int sc_geohint_fudge_state(const sc_georef_t *gr, const sc_regex_t *re,
  ...
  if (state->hintc < 1) return 0;

  for (i = 0; i < state->hintc; i++) {
    hint = state->hints[i];
    if ((tp_c = sc_geohint_fudgeeval(hint, gr, re, &tp_rtt)) == 0) continue;
    break;
  }
...
static int sc_geohint_fudge_iata_do(const sc_georef_t *gr, sc_geohint_t *hint,
  ...
  if (hint->code[0] != map->code[0]) return 0;
  if (sc_geohint_fudgepre(hint, map) == 0) return 0;

  /* three adjacent letters */
  if (strncmp(hint->code, map->code, 3) == 0) round = 0;
  ...
  if ((tp_c = sc_geohint_fudgeeval(hint, gr, re, &tp_rtt)) == 0) return 0;

  if (mask_cnt(gr->f_mask, re->dom->rtmlen) >= 3 || map->cc[0] != '\0' ||
      map->st[0] != '\0' || tp_rtt <= rtt_close) {
    if (sc_geoeval_add(geoeval_list, hint, tp_c, round) == NULL) return -1;
  }
...
static int sc_geohint_fudge_iata(const sc_georef_t *gr, const sc_regex_t *re,
  ...
  if (map->st[0] == '\0') {
    for (state = sc_state_find(map->code);
         ...
         state = state->next) {
      if (map->cc[0] != '\0' && strcasecmp(map->cc, state->cc) != 0) continue;

      /* evaluate the plausability of the state */
      if (sc_geohint_fudge_state(gr, re, state, geoeval_list) != 0) return -1;
  ...
  for (i = 0; i < geohint_plc[x]; i++) {
    hint = geohint_pls[x][i];
    assert(hint->type == GEOHINT_TYPE_PLACE);
...
static int sc_geohint_fudge_locode_do(const sc_georef_t *gr, sc_geohint_t *hint,
                                      const sc_regex_t *re,
                                      splaytree_t *gh_tree,
                                      slist_t *geoeval_list) {
  const sc_geomap_t *map = &gr->map;
  ...
  if (hint->code[0] != map->code[2]) return 0;
  if (sc_geohint_fudgepre(hint, map) == 0) return 0;
  ...
  cc[1] = map->code[1];
  cc[2] = '\0';
  if (cceq(cc, hint) == 0) return 0;

  /* check if the next three characters match */
  if (strncmp(hint->code, map->code + 2, 3) == 0) round = 0;
  /* three letters in order */
...
static int sc_geohint_fudge_clli_do(const sc_georef_t *gr, sc_geohint_t *hint,
  ...
  if ((strcmp(hint->cc, "us") == 0 || strcmp(hint->cc, "ca") == 0)) {
    if (strcmp(hint->st, map->code + 4) != 0) return 0;
    state = 1;
  }
  ...
  if (mask_cnt(gr->f_mask, re->dom->rtmlen) < 3 && state == 0 &&
...
static int sc_geohint_fudge_place0(const sc_geohint_t *hint,
  ...
  if (strncmp(hint->code, map->code, hint->codelen) == 0 &&
      ...
      ((hint->st[0] != '\0' &&
        strcmp(&map->code[hint->codelen], hint->st) == 0) ||
...
static int sc_geohint_fudge_place1(const sc_georef_t *gr,
  ...
  if (map->codelen < 3) return 0;

  if (splaytree_find(gh_tree, hint) == NULL) return 0;

  if (map->codelen == 3 &&
      sc_geohint_abbrv(hint->place, map->code, gr->offs, 3, NULL) != 0)
    return 1;
  if (map->codelen > 3 && dled(hint->code, map->code) < 2) return 1;
...
static int sc_geohint_fudge_fac_ok(const char *hint_p, const size_t *offs,
                                   size_t codelen) {
  /* if we're dealing with a 3-letter acronym, then we're less picky */
  if (codelen == 3) return 1;
...
static int sc_geohint_fudge_place2_ok(const char *hint_p, const size_t *offs,
                                      size_t codelen) {
  /* if we're dealing with a 3-letter acronym, then we're less picky */
  if (codelen == 3) return 1;
...
static int sc_geohint_fudge_place2(const sc_georef_t *gr,
  ...
  if (map->codelen < 3 || hint->codelen <= map->codelen) return 0;
...
static int sc_geohint_fudge_place_do(const sc_georef_t *gr, sc_geohint_t *hint,
  ...
  if (map->code[0] != hint->code[0]) return 0;
  if (sc_geohint_fudgepre(hint, map) == 0) return 0;
...
static int sc_geohint_fudge_place_fac_ok(const char *hint_p, const size_t *offs,
                                         size_t codelen) {
  return sc_geohint_fudge_runlen(hint_p, offs, codelen, 6);
}

static int sc_geohint_fudge_place_fac(const sc_georef_t *gr, sc_geohint_t *hint,
                                      const sc_regex_t *re,
                                      slist_t *geoeval_list) {
  const sc_geomap_t *map = &gr->map;
  uint32_t tp_c;
  ...
  if (hint->facname != NULL && map->code[0] == tolower(hint->facname[0]) &&
    ...
    if (sc_geoeval_add(geoeval_list, hint, tp_c, 0) == NULL) return -1;
    return 0;
  }
  ...
  if (hint->street != NULL &&
    ...
    if (sc_geoeval_add(geoeval_list, hint, tp_c, 1) == NULL) return -1;
  }

  return 0;
}

static int sc_geohint_fudge_place(const sc_georef_t *gr, const sc_regex_t *re,
                                  splaytree_t *gh_tree, slist_t *geoeval_list) {
  const sc_geomap_t *map = &gr->map;
  sc_country_t *country;
  sc_geohint_t *hint;
...
static int sc_geohint_fudge_facility(const sc_georef_t *gr,
  ...
  if (map->codelen < 5 || (map->cc[0] == '\0' && map->st[0] == '\0')) return 0;

  for (i = 0; i < geohint_facc; i++) {
    hint = geohint_facs[i];
    if (hint->street == NULL || sc_geohint_fudgepre(hint, map) == 0 ||
        (tp_c = sc_geohint_fudgeeval(hint, gr, re, &tp_rtt)) == 0)
      continue;
...
static sc_lcs_pt_t *sc_lcs_pt_alloc(int S_s, int S_e, int T_s, int T_e) {
  sc_lcs_pt_t *pt = NULL;
  if ((pt = malloc(sizeof(sc_lcs_pt_t))) == NULL) goto err;
  pt->S_start = S_s;
  pt->S_end = S_e;
  pt->T_start = T_s;
  pt->T_end = T_e;
...
static int lcs_check(slist_t *X) {
  ...
  for (sn = slist_head_node(X); sn != NULL; sn = slist_node_next(sn)) {
    pt = slist_node_item(sn);
    if (last != NULL && pt->T_start < last->T_start) return 0;
    last = pt;
  }
...
static int lcs_trim_class(slist_t *X, const char *S, const char *T) {
  ...
  if ((Y = slist_alloc()) == NULL) return -1;
  while ((pt = slist_head_pop(X)) != NULL) {
    ...
    if (isalnum(S[pt->S_start]) != 0 &&
      ...
      if (slist_tail_push(Y, pt) == NULL) goto err;
    } else {
      sc_lcs_pt_free(pt);
    }
  }
...
static int lcs_trim(slist_t *X, const char *S, const char *T) {
  ...
  while ((pt = slist_head_pop(X)) != NULL) {
    ...
    if (S[pt->S_end + 1] != '\0' && T[pt->T_end + 1] != '\0') {
      while (pt->S_start != pt->S_end && pt->T_start != pt->T_end) {
        if (isalnum(S[pt->S_end + 1]) != 0 || isalnum(T[pt->T_end + 1]) != 0 ||
    ...
    if ((pt->S_start == 0 || isalnum(S[pt->S_start - 1]) == 0) &&
        (pt->T_start == 0 || isalnum(T[pt->T_start - 1]) == 0) &&
        isalnum(S[pt->S_end + 1]) == 0 && isalnum(T[pt->T_end + 1]) == 0) {
      if (slist_tail_push(Y, pt) == NULL) goto err;
    } else {
      sc_lcs_pt_free(pt);
    }
...
static slist_t *lcs_asn(sc_ifacedom_t *S, sc_ifacedom_t *T) {
  ...
  assert(S->iface->as_s >= 0);
  assert(S->iface->as_e >= 0);
  assert(T->iface->as_s >= 0);
  assert(T->iface->as_e >= 0);

  if ((L = slist_alloc()) == NULL) goto err;
  ...
  if (S->iface->as_s > 0 && T->iface->as_s > 0 &&
    ...
    if (sc_lcs_pt_push(L, s_start, s_end, t_start, t_end) != 0) goto err;
  }
...
static slist_t *lcs(const char *S, int r, const char *T, int n, int min_z) {
  ...
  if ((X = slist_alloc()) == NULL || (bits = slist_alloc()) == NULL ||
  ...
  for (;;) {
    ...
    for (i = pt->S_start; i < pt->S_end; i++) {
      for (j = 0; j < n; j++) L[(i * n) + j] = 0;
    }
    for (j = 0; j < n; j++) {
      if ((z = L[idx(r, n, pt->S_end, j)]) == 0) continue;
      for (i = 0; j + i < n && (pt->S_end + i) < r &&
                  L[idx(r, n, pt->S_end + i, j + i)] > 0;
           i++)
        L[((pt->S_end + i) * n) + (j + i)] -= z;
    }

    for (j = pt->T_start; j < pt->T_end; j++) {
      for (i = 0; i < r; i++) L[(i * n) + j] = 0;
    }
    for (i = 0; i < r; i++) {
      if ((z = L[idx(r, n, i, pt->T_end)]) == 0) continue;
      for (j = 0; pt->T_end + j < n && (i + j) < r &&
                  L[idx(r, n, i + j, pt->T_end + j)] > 0;
           j++)
...
static sc_css_t *sc_css_alloc0(void) {
  sc_css_t *css;
  if ((css = malloc(sizeof(sc_css_t))) == NULL) return NULL;
  css->css = NULL;
  css->cssc = 0;
  css->len = 0;
  css->count = 0;
...
static sc_css_t *sc_css_alloc(size_t len) {
  sc_css_t *css;
  if ((css = malloc(sizeof(sc_css_t))) == NULL) goto err;
  if (len > 0) {
    if ((css->css = malloc(len)) == NULL) goto err;
  } else
    css->css = NULL;
  css->cssc = 0;
  css->len = 0;
...
static char *sc_css_tostr(const sc_css_t *css, char delim, char *out,
                          size_t len) {
  size_t off = 0;
  char *ptr = css->css;
  int i;
...
static int sc_css_morespecific(const sc_css_t *base, const sc_css_t *eval) {
  ...
  if (eval->len <= base->len) return 0;

  while (eval_i < eval->cssc) {
    if (strcmp(base_p, eval_p) == 0) {
      base_i++;
      if (base_i == base->cssc) return 1;
      while (*base_p != '\0') base_p++;
      base_p++;

      eval_i++;
      while (*eval_p != '\0') eval_p++;
      eval_p++;
    } else {
      eval_i++;
      if (eval_i == eval->cssc) break;
      while (*eval_p != '\0') eval_p++;
      eval_p++;
    }
  }
...
static int sc_css_morespecific_ex(const sc_css_t *base, const sc_css_t *eval,
  ...
  *out = NULL;
  ...
  if (sc_css_morespecific(base, eval) == 0) return 0;

  /* alloc a css structure to contain the more specific part */
  if ((tmp = sc_css_alloc(eval->len + 1)) == NULL) return -1;
  tmp_p = tmp->css;
...
static sc_css_t *sc_css_alloc_pt(const char *str, int *m, int mc) {
  ...
  for (i = 0; i < mc; i += 2) off += m[i + 1] - m[i] + 1 + 1;

  if ((css = sc_css_alloc(off)) == NULL) return NULL;
  for (i = 0; i < mc; i += 2) {
...
static sc_css_t *sc_css_alloc_xor(const char *str, size_t len, int *m, int mc) {
  sc_css_t *css = NULL;
  int *n = NULL, nc;
  if (pt_xor(len, m, mc, &n, &nc) == 0) css = sc_css_alloc_pt(str, n, nc);
  if (n != NULL) free(n);
  return css;
}
...
static sc_css_t *sc_css_alloc_lcs(const slist_t *X, const char *S) {
  ...
  if ((css = sc_css_alloc(off)) == NULL) goto err;
  for (sn = slist_head_node(X); sn != NULL; sn = slist_node_next(sn)) {
    pt = slist_node_item(sn);
...
static sc_css_t *sc_css_alloc_tags(const char *str) {
  ...
  if ((css = sc_css_alloc(strlen(str) + 1)) == NULL) return NULL;

  /* skip over any training non-alnum characters */
  while (isalnum(str[i]) == 0 && str[i] != '\0') i++;
  if (str[i] == '\0') return css;
...
static int sc_css_insert(splaytree_t *tree, const sc_css_t *css) {
  assert(css->cssc > 0);
  if (splaytree_insert(tree, css) == NULL) return -1;
  return 0;
}
...
static sc_css_t *sc_css_get(splaytree_t *tree, const sc_css_t *css) {
  sc_css_t *x;
  assert(css->cssc > 0);
  if ((x = sc_css_find(tree, css)) != NULL) return x;
  if ((x = sc_css_dup(css)) == NULL || splaytree_insert(tree, x) == NULL)
    goto err;
  return x;
 ...
 */
static int sc_css_match(const sc_css_t *css, const char *S, int *out,
                        int alnum) {
  int i, c, x, y;
  ...
  while (i < css->cssc) {
    ...
    y = 0;
    while (S[x + y] == css->css[c + y] && css->css[c + y] != '\0') y++;
...
static sc_css_t *sc_css_matchxor(const sc_css_t *css,
  ...
  if (X_array[0] > 0) {
    l = 0;
    r = X_array[0] - 1;
    while (isalnum(ifd->label[l]) == 0 && l < r) l++;
    while (isalnum(ifd->label[r]) == 0 && r > l) r--;
...
static int sc_css_geomap(const sc_css_t *css, const uint8_t *geotypes,
  ...
  for (i = 0; i < css->cssc; i++) {
    ...
    if (geotypes[i] == GEOHINT_TYPE_CC || geotypes[i] == GEOHINT_TYPE_ST) {
      ...
      if (xi >= sizeof(x) / sizeof(sc_ptrc_t)) return -1;
      x[xi].ptr = ptr;
      while (*ptr != '\0') ptr++;
...
static int sc_geomap2hint_tojson(const sc_geomap2hint_t *m2h, char *buf,
                                 size_t len) {
  char place[512];
  sc_geohint_t *gh;
  size_t off = 0;

  string_concat(buf, len, &off, "{\"code\":\"%s\", \"type\":\"%s\"",
  ...
  if (m2h->hint != NULL) {
    gh = m2h->hint;
    if (sc_geohint_place_tojson(gh, place, sizeof(place)) != 0) return -1;
...
static sc_geomap2hint_t *sc_geomap2hint_get(splaytree_t *tree, sc_geomap_t *map,
                                            sc_geohint_t *hint) {
  sc_geomap2hint_t fm, *m2h;
  memcpy(&fm.map, map, sizeof(sc_geomap_t));
  fm.hint = hint;
  if ((m2h = splaytree_find(tree, &fm)) != NULL) return m2h;
  if ((m2h = malloc_zero(sizeof(sc_geomap2hint_t))) == NULL) return NULL;
...
static slist_t *sc_geomap2hint_make(sc_regex_t *re, slist_t *ifi_list) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((m2h = sc_geomap2hint_get(tree, &map, ifi->geohint)) == NULL) goto err;
    if (ifi->class == '+')
      m2h->tp_c++;
    else if (ifi->class == '!')
      m2h->fp_c++;
...
static sc_remerge_t *sc_remerge_get(splaytree_t *tree, sc_css_t *css) {
  ...
  fm.css = css;
  if ((rem = splaytree_find(tree, &fm)) != NULL) return rem;
...
static int sc_regex_show(const sc_regex_t *re) {
  if (do_show == 0 || (do_show & (1 << re->class)) != 0) return 1;
  return 0;
}
...
static char *sc_regex_caprep_css(const char *in, const sc_css_t *css) {
  ...
  if ((out = malloc(len)) == NULL) goto err;
  ...
  out[off++] = '\0';
  if ((dup = memdup(out, off)) == NULL) return out;
  free(out);
  return dup;
 ...
 * in lit.
 */
static char *sc_regex_caprep(const char *in, int rep, int cap,
                             const char *lit) {
  const char *ptr = in;
  char *out, *dup;
  int element = 0;
  ...
  if ((out = malloc(len)) == NULL) return NULL;
...
static char *sc_regex_caponly(const char *in, int only) {
  ...
  if ((out = malloc(strlen(in) + 1)) == NULL) return NULL;
...
static int sc_regex_capget_css_lit2(char *buf, size_t len, size_t *off_in,
  ...
  while (litend >= start && isalnum(*litend) == 0) litend--;
...
static int sc_regex_capget_css(const char *in, sc_css_t **out) {
  ...
  while (*ptr != '\0') {
    if (ptr[0] == '(' && ptr[1] != '?') {
      ...
      tmp = ptr - start + 1;
      if ((dup = memdup(start, tmp)) == NULL) goto done;
      dup[tmp - 1] = '\0';
      if (slist_tail_push(list, dup) == NULL) goto done;
      dup = NULL;
      start = NULL;
      len += tmp;
    }
  ...
  if ((css = sc_css_alloc(len)) == NULL) goto done;
...
static int sc_regex_pt_decons2(slist_t *list, int *c, int cc, int co, int *o,
                               int oo, int use) {
  sc_ptrc_t *ptrc = NULL;
...
static dlist_t *sc_css_reduce_ls(splaytree_t *tree) {
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (css->cssc != 1) continue;

    /* make a copy of the literal on the out list, properly escaped */
    if (re_escape_str(buf, sizeof(buf), css->css) == NULL) goto done;
    fm.css = buf;
    fm.cssc = 1;
    fm.count = 3;
    fm.len = strlen(buf) + 1;
    if (sc_css_get(out, &fm) == NULL) goto done;
    ...
    if (al != 0) {
      for (ptr = css->css; *ptr != '\0'; ptr++) {
        ...
        if ((r = re_escape(tmp, sizeof(tmp), *ptr)) == 0) goto done;
        if (sizeof(buf) - off < r) goto done;
  ...
  if ((out_list = dlist_alloc()) == NULL) goto done;
  splaytree_inorder(out, tree_to_dlist, out_list);
  splaytree_free(out, NULL);
  out = NULL;
...
static int sc_css_reduce_pair(sc_css_t *a, sc_css_t *b, int trim, int min_z,
  ...
  *out = NULL;
  if ((X = lcs(a->css, a->len, b->css, b->len, min_z)) == NULL) goto done;
  if (trim == 1)
  ...
  if ((css = sc_css_alloc_lcs(X, a->css)) == NULL) goto done;
  *out = css;
  rc = 0;
...
static int sc_css_reduce(splaytree_t *tree, int trim, int min_z) {
  ...
  do {
    ...
    while ((css = slist_head_pop(list)) != NULL) {
      ...
      for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
        ...
        if (sc_css_reduce_pair(css, css2, trim, min_z, &cssa) != 0) goto done;
        if (cssa == NULL) continue;
        ...
        if (sc_css_insert(tree, cssa) != 0) goto done;
        cssa = NULL;
        i++;
      }
      css->count = 1;
  ...
  splaytree_inorder(tree, tree_to_slist, list);
  while ((css = slist_head_pop(list)) != NULL) css->count = 0;
...
static const char *label_get(const char *string, int x) {
  ...
  while (ptr != string) {
    if (*ptr == '.') {
      if (off == x) return ptr + 1;
      off++;
    }
    ptr--;
  }

  if (off == x) return string;
 ...
 */
static int sc_rework_match(sc_rework_t *rew, sc_iface_t *iface,
                           sc_css_t **out) {
  sc_css_t *css = NULL;
  size_t off;
  int i, l, k, rc;
  ...
  for (k = 0; k < rew->c; k++) {
#ifdef HAVE_PCRE2
    rc = pcre2_match(rew->pcre[k], (PCRE2_SPTR)iface->name, iface->len, 0, 0,
                     rew->match_data, NULL);
#else
    rc = pcre_exec(rew->pcre[k], rew->study[k], iface->name, iface->len, 0, 0,
                   rew->ovector, rew->n);
#endif
    assert(rc != 0);
    if (rc <= 0) {
...
#ifdef HAVE_PCRE2
  if (rew->match_data != NULL) pcre2_match_data_free(rew->match_data);
#else
  if (rew->study != NULL) {
    for (i = 0; i < rew->c; i++)
      if (rew->study[i] != NULL) pcre_free_study(rew->study[i]);
    free(rew->study);
  }
  if (rew->ovector != NULL) free(rew->ovector);
#endif
...
static int sc_rework_capcount(const sc_rework_t *rew, int i) {
  ...
  n = x;
#else
  if (pcre_fullinfo(rew->pcre[i], rew->study[i], PCRE_INFO_CAPTURECOUNT, &n) !=
      0)
    return -1;
#endif
  return n;
...
static sc_rework_t *sc_rework_alloc(sc_regex_t *re) {
  ...
  if ((rew = malloc_zero(sizeof(sc_rework_t))) == NULL) goto err;
  rew->c = re->regexc;
...
#ifdef HAVE_PCRE2
    if ((rew->pcre[i] = pcre2_compile((PCRE2_SPTR)str, PCRE2_ZERO_TERMINATED, 0,
                                      &errnumber, &erroffset, NULL)) == NULL)
      goto err;
    if (do_jit != 0) pcre2_jit_compile(rew->pcre[i], PCRE2_JIT_COMPLETE);
#else
    if ((rew->pcre[i] = pcre_compile(str, 0, &error, &erroffset, NULL)) == NULL)
      goto err;
#ifdef PCRE_STUDY_JIT_COMPILE
    if (do_jit != 0) options |= PCRE_STUDY_JIT_COMPILE;
#endif
    rew->study[i] = pcre_study(rew->pcre[i], options, &error);
#endif

    /* figure out how large the ovector has to be for this regex */
    if ((n = sc_rework_capcount(rew, i)) < 0) goto err;
    if (n <= k) continue;
    k = n;
  }
...
#else
  n = ((k + 1) * 3);
  if ((rew->ovector = malloc_zero(sizeof(int) * n)) == NULL) goto err;
  rew->n = n;
#endif
...
static void sc_suffix_free(sc_suffix_t *suffix) {
  int i;
  if (suffix->label != NULL) free(suffix->label);
  if (suffix->suffixes != NULL) {
    for (i = 0; i < suffix->suffixc; i++) sc_suffix_free(suffix->suffixes[i]);
...
static int suffix_file_line(char *line, void *param) {
  ...
  if (end_icann != 0) return 0;

  if (line[0] == '/') {
    if (strncmp(line, "// ===END", 9) == 0) end_icann = 1;
    return 0;
  }
  if (line[0] == '!') return 0;
  ...
  if ((ptr = strdup(line)) == NULL || slist_tail_push(list, ptr) == NULL)
    return -1;
...
static const char *sc_suffix_find(const char *domain) {
  ...
  if (m == NULL || m - 2 <= domain) return NULL;
  m = m - 2;
  while (m >= domain) {
    if (*m == '.') return m + 1;
    m--;
...
static sc_segscore_t *sc_segscore_get(splaytree_t *tree, char *seg, int score) {
  ...
  fm.seg = seg;
  fm.score = score;
  if ((ss = splaytree_find(tree, &fm)) != NULL) return ss;
...
static int sc_segscore_switch2(sc_segscore_t *ss, char *alpha) {
  ...
  buf[i] = '\0';
  if (splaytree_find(ss->tree, buf) != NULL) return 0;
...
static uint8_t sc_regexn_geotype(const sc_regexn_t *ren) {
  int i;
  for (i = 0; i < ren->capc; i++) assert(ren->plan[i] != 0);
...
static int sc_regex_findnew(const sc_regex_t *cur, const sc_regex_t *can) {
  ...
  for (i = 0; i < cur->regexc; i++)
    if (strcmp(cur->regexes[i]->str, can->regexes[i]->str) != 0) return i;
...
static char *sc_regex_score_tostr(const sc_regex_t *re, char *buf, size_t len) {
  ...
  if (re->fne_c > 0) string_concat(buf, len, &off, " fne %u", re->fne_c);
  if (re->fnu_c > 0) string_concat(buf, len, &off, " fnu %u", re->fnu_c);
  if (re->unk_c > 0) string_concat(buf, len, &off, " unk %u", re->unk_c);
  if (re->sp_c > 0) string_concat(buf, len, &off, " sp %u", re->sp_c);
  if (re->sn_c > 0) string_concat(buf, len, &off, " sn %u", re->sn_c);
  if (re->ip_c > 0) string_concat(buf, len, &off, " ip %u", re->ip_c);

  string_concat(buf, len, &off, " tpr %.1f atp %d", sc_regex_score_tpr(re),
                sc_regex_score_atp(re));
...
static char *sc_regex_score_tojson(const sc_regex_t *re, char *buf,
                                   size_t len) {
  static const char *class[] = {"poor", "good", "promising", "single"};
  size_t off = 0;
  uint32_t fp;
  ...
  string_concat(buf, len, &off,
                ", \"score\":%u, \"matches\":%u, \"routers\":%u", re->score,
                re->matchc, slist_count(re->dom->routers));
...
static int sc_regex_str_cmp(const sc_regex_t *a, const sc_regex_t *b) {
  ...
  for (i = 0; i < a->regexc; i++)
    if ((x = strcmp(a->regexes[i]->str, b->regexes[i]->str)) != 0) return x;
  for (i = 0; i < a->regexc; i++) {
    if (a->regexes[i]->plan != NULL && b->regexes[i]->plan == NULL) return -1;
    if (a->regexes[i]->plan == NULL && b->regexes[i]->plan != NULL) return 1;
    if (a->regexes[i]->plan == NULL && b->regexes[i]->plan == NULL) continue;
    if (a->regexes[i]->capc < b->regexes[i]->capc) return -1;
    if (a->regexes[i]->capc > b->regexes[i]->capc) return 1;
    for (x = 0; x < a->regexes[i]->capc; x++) {
      if (a->regexes[i]->plan[x] < b->regexes[i]->plan[x]) return -1;
      if (a->regexes[i]->plan[x] > b->regexes[i]->plan[x]) return 1;
    }
  }
  return 0;
...
static int sc_regex_str_len(const sc_regex_t *re) {
  size_t len = 0;
  int i;
  for (i = 0; i < re->regexc; i++) len += strlen(re->regexes[i]->str);
  return len;
}
...
static int sc_regex_score_tie_cmp(const sc_regex_t *a, const sc_regex_t *b) {
  ...
  if (do_learngeo != 0) {
     ...
     */
    ac = 0;
    for (i = 0; i < a->regexc; i++) ac += a->regexes[i]->capc;
    bc = 0;
    for (i = 0; i < b->regexc; i++) bc += b->regexes[i]->capc;
    if (ac < bc) return -1;
    if (ac > bc) return 1;
  }
...
static int sc_regex_score_thin_sort_cmp(const sc_regex_t *a,
                                        const sc_regex_t *b) {
  int x;
  if ((x = sc_regex_score_thin_cmp(a, b)) != 0) return x;
...
static int sc_regex_score_rank_cmp(const sc_regex_t *a, const sc_regex_t *b) {
  int x;
  if ((x = sc_regex_score_atp_cmp(a, b)) != 0) return x;
  if (a->fp_c < b->fp_c) return -1;
  if (a->fp_c > b->fp_c) return 1;
  if (a->tp_c > b->tp_c) return -1;
  if (a->tp_c < b->tp_c) return 1;
...
static int sc_regex_score_rank_asn_cmp(const sc_regex_t *a,
                                       const sc_regex_t *b) {
  int x;
  if ((x = sc_regex_score_atp_cmp(a, b)) != 0) return x;
  if (a->tp_c > b->tp_c) return -1;
...
static int sc_regex_score_rank_asnames_cmp(const sc_regex_t *a,
                                           const sc_regex_t *b) {
...
static void sc_regex_free(sc_regex_t *re) {
  ...
  if (re->regexes != NULL) {
    for (i = 0; i < re->regexc; i++)
      if (re->regexes[i] != NULL) sc_regexn_free(re->regexes[i]);
    free(re->regexes);
  }
  if (re->geohints != NULL) {
    for (i = 0; i < re->geohintc; i++)
      if (re->geohints[i] != NULL) sc_geohint_free(re->geohints[i]);
    free(re->geohints);
  }
...
static sc_regex_t *sc_regex_plus1(sc_regex_t *re, sc_regexn_t *ren, int i) {
  ...
  for (j = 0; j < i; j++)
    if ((out->regexes[j] = sc_regexn_dup(re->regexes[j])) == NULL) goto err;
  if ((out->regexes[i] = sc_regexn_dup(ren)) == NULL) goto err;
  for (j = i; j < re->regexc; j++)
    if ((out->regexes[j + 1] = sc_regexn_dup(re->regexes[j])) == NULL) goto err;
...
static sc_regex_t *sc_regex_alloc_list(slist_t *list) {
  ...
  if ((re = malloc_zero(sizeof(sc_regex_t))) == NULL) goto err;
  re->regexc = slist_count(list);
  if ((re->regexes = malloc_zero(sizeof(sc_regexn_t *) * re->regexc)) == NULL)
    goto err;
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ptr = slist_node_item(sn);
    if ((re->regexes[k] = sc_regexn_alloc(ptr)) == NULL) goto err;
    if ((capc = capcount(ptr)) < 0) goto err;
...
static sc_regex_t *sc_regex_alloc_str(char *str) {
  ...
  if ((list = slist_alloc()) == NULL) goto err;

  do {
    string_nullterm_char(ptr, ' ', &next);
    if (slist_tail_push(list, ptr) == NULL) goto err;
    ptr = next;
  } while (ptr != NULL);

  if ((re = sc_regex_alloc_list(list)) == NULL) goto err;
  slist_free(list);
  list = NULL;
...
static char *sc_regex_capseg(const char *in) {
  ...
  len = strlen(in) * 3;
  if ((buf = malloc(len)) == NULL) goto done;
...
static sc_regex_t *sc_regex_alloc_dm(char *str, const char *file,
                                     const int line)
#endif
{
  sc_regex_t *re = NULL;
  ...
  if ((capc = capcount(str)) < 0) goto err;
...
static sc_regex_t *sc_regex_dup(sc_regex_t *in) {
  ...
  if ((out = memdup(in, sizeof(sc_regex_t))) == NULL) goto err;
  out->regexes = NULL;
  out->tp_mask = NULL;
  len = sizeof(sc_regexn_t *) * in->regexc;
  if ((out->regexes = malloc_zero(len)) == NULL) goto err;
  if (in->tp_mask != NULL) {
    len = sizeof(uint32_t) * in->dom->tpmlen;
    if ((out->tp_mask = memdup(in->tp_mask, len)) == NULL) goto err;
  }

  for (i = 0; i < in->regexc; i++)
    if ((out->regexes[i] = sc_regexn_dup(in->regexes[i])) == NULL) goto err;
...
static sc_regex_t *sc_regex_get_dm(splaytree_t *tree, char *str, int capc,
                                   uint8_t *geotypes, const char *file,
                                   const int line)
#endif
{
  sc_regex_t *re = NULL;
  ...
  if ((re = sc_regex_find(tree, str, capc, geotypes)) != NULL) return re;
  if ((dup = strdup(str)) == NULL) goto err;
#ifndef DMALLOC
  ...
  if (geotypes != NULL &&
    ...
    goto err;
  re->regexes[0]->capc = capc;
  if (splaytree_insert(tree, re) == NULL) goto err;
  return re;
...
static int sc_regex_capset_css(dlist_t *list, splaytree_t *tree,
                               sc_css_t *cap) {
  ...
  for (dn = dlist_head_node(list); dn != NULL; dn = dlist_node_next(dn)) {
    ...
    if ((ptr = sc_regex_caprep_css(re_eval->regexes[0]->str, cap)) == NULL)
      continue;
    if ((re_new = sc_regex_alloc(ptr)) == NULL) goto err;
    ptr = NULL;
    if ((re = splaytree_find(tree, re_new)) != NULL) {
      if (re->score < re_eval->score) re->score = re_eval->score;
    ...
    if (splaytree_insert(tree, re_new) == NULL) goto err;
  }
...
static void sc_iface_free(sc_iface_t *iface) {
  uint8_t i;
  if (iface->addr != NULL) scamper_addr_free(iface->addr);
  if (iface->name != NULL) free(iface->name);
  if (iface->asnames != NULL) free(iface->asnames);
  if (iface->geos != NULL) {
    for (i = 0; i < iface->geoc; i++) free(iface->geos[i].tags);
    free(iface->geos);
  }
  free(iface);
  return;
...
static int sc_ifaceinf_class_cmp(const sc_ifaceinf_t *a,
                                 const sc_ifaceinf_t *b) {
  if (a->class == b->class) return sc_iface_cmp(a->ifd->iface, b->ifd->iface);
  if (a->class == '+') return -1;
  if (b->class == '+') return 1;
  if (a->class == '=') return -1;
  if (b->class == '=') return 1;
...
static int sc_ifaceinf_rtrc_cmp(const sc_ifaceinf_t *a,
                                const sc_ifaceinf_t *b) {
...
#else
#define sc_ifaceinf_get(list, ifd, css, ip, regex) \
  sc_ifaceinf_get_dm((list), (ifd), (css), (ip), (regex), __FILE__, __LINE__)
...
#endif
{
...
#else
  ifi = dmalloc_malloc(file, line, sizeof(sc_ifaceinf_t), DMALLOC_FUNC_MALLOC,
                       0, 0);
#endif
  if (ifi == NULL) goto err;
  ifi->ifd = ifd;
  ifi->css = css;
  ifi->ri = NULL;
  ifi->geohint = NULL;
  ...
  ifi->class = '\0';
  ifi->ipm = ip;
  if (slist_tail_push(list, ifi) == NULL) goto err;
  return ifi;
...
static void sc_regex_css_free(sc_regex_css_t *recss) {
  if (recss == NULL) return;
  if (recss->work != NULL) sc_regex_free(recss->work);
  if (recss->regex != NULL) sc_regex_free(recss->regex);
  if (recss->css != NULL) sc_css_free(recss->css);
  free(recss);
  return;
}
...
static int sc_iface_ip_find_4(sc_iface_t *iface, const char *suffix) {
  ...
  while (bo < sizeof(sb) / sizeof(long)) {
    while (isdigit(*ptr) == 0 && ptr != suffix) ptr++;
    if (ptr >= suffix) break;
      ...
      if (c == l) {
        ...
        if (do_debug != 0 && threadc <= 1) {
          printf("found %s in %s bytes %d - %d\n",
                 scamper_addr_tostr(iface->addr, buf, sizeof(buf)), iface->name,
                 iface->ip_s, iface->ip_e);
        }
        return 1;
      }
    }
...
static void sc_charpos_score(sc_charpos_t *cp, int max) {
  ...
  for (i = 0; i < max; i++) {
    if (cp->pos[i] == -1) continue;
    cp->digits++;
    if (cp->pos[i] < cp->left || cp->left == -1) cp->left = cp->pos[i];
    if (cp->pos[i] > cp->right || cp->right == -1) cp->right = cp->pos[i];
  }
...
static int sc_iface_ip_isok(const sc_iface_t *iface, const sc_charpos_t *cp) {
  ...
  for (i = 0; i < max; i++) {
    if (cp->pos[i] == -1) continue;
    if (cp->pos[i] < left || left == -1) left = cp->pos[i];
    if (cp->pos[i] > right || right == -1) right = cp->pos[i];
  }
  for (i = left + 1; i < right; i++) {
    if (isalnum(iface->name[i]) == 0 || iface->name[i] == '0') continue;
    if (ishex(iface->name[i]) == 0) return 0;
    for (j = 0; j < max; j++)
      if (cp->pos[j] == i) break;
    if (j == max) return 0;
  }
...
static void sc_iface_ip_fill_zero6(sc_iface_t *iface, sc_charpos_t *cp) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (asc > desc && asc > 0) {
      ...
      for (j = pos - 1; j >= 0; j--) {
        if (iface->name[cp->pos[x + pos] - (pos - j)] != '0') break;
        cp->pos[x + j] = cp->pos[x + pos] - (pos - j);
      }

      /* fill zeros to the right of the first set character */
      for (j = pos + 1; j < 4; j++) {
        if (cp->pos[x + j] != -1) continue;
        if (iface->name[cp->pos[x + pos] + (j - pos)] != '0') break;
...
static int sc_iface_ip_find_hex_rec(sc_iface_t *iface, const char *suffix,
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV6(iface->addr) && (x % 4) == 0 && x != 0) {
    ...
    c = 0;
    for (j = 0; j < 4; j++)
      if (cp->pos[i + j] != -1) a[c++] = cp->pos[i + j];
    for (j = 2; j < c; j++)
    ...
    if (c > 0 && ((cp->pos[i + 0] == -1 && cp->c[i + 0] != '0') ||
    ...
    for (j = 0; j < 4; j++) {
      if (cp->pos[i + j] == -1) continue;
      for (k = j + 1; k < 4; k++) {
        if (cp->pos[i + k] == -1) continue;
        if (pos_diff(cp->pos[i + j], cp->pos[i + k]) != k - j) return 0;
      }
    }
  }
...
static int sc_iface_ip_find_hex_enough(sc_iface_t *iface, sc_charpos_t *cp) {
  int i;
  if (SCAMPER_ADDR_TYPE_IS_IPV4(iface->addr)) {
    if (cp->digits < 6) return 0;
    for (i = 0; i < 8; i++)
      if (cp->c[i] != '0' && cp->pos[i] == -1) return 0;
    return 1;
  } else if (SCAMPER_ADDR_TYPE_IS_IPV6(iface->addr)) {
    if (cp->digits < 4) return 0;
    return 1;
  }
  return 0;
}
...
static int sc_iface_ip_find_hex(sc_iface_t *iface, const char *suffix) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(iface->addr))
    ...
    return -1;
  for (i = 0; i < max; i++) {
    u = scamper_addr_bit(iface->addr, (i * 4) + 1) << 3 |
  ...
  for (i = 1; i < 16; i++) {
    ...
    if ((posl[i].pos = malloc(sizeof(int) * j)) == NULL) goto done;
    posl[i].posc = j;
    j = 0;
    ptr = iface->name;
    while (ptr != suffix) {
  ...
  for (i = 0; i < 16; i++)
    if (posl[i].pos != NULL) free(posl[i].pos);
  return rc;
}
...
static void sc_iface_ip_find_thread(sc_iface_t *iface) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(iface->addr)) {
    ...
    if (iface->ip_s == -1) sc_iface_ip_find_hex(iface, suffix);
  } else if (SCAMPER_ADDR_TYPE_IS_IPV6(iface->addr)) {
...
static int sc_iface_ip_matched(sc_iface_t *iface, sc_rework_t *rew) {
  ...
  for (i = 1; i < rew->m; i++) {
    if (iface->ip_e < rew->ovector[2 * i]) continue;
    if (iface->ip_s > rew->ovector[(2 * i) + 1]) continue;
    return 1;
  }
...
static void sc_iface_asn_find_thread(sc_ifacedom_t *ifd) {
  ...
  len = strlen(buf);
  if ((dup = strdup(ifd->label)) == NULL) return;

  i = 0;
  stop = 0;
  for (;;) {
    while (isdigit(dup[i]) == 0 && dup[i] != '\0') i++;
    if (dup[i] == '\0') break;
  ...
  if (as_s != -1) {
    iface->as_s = as_s;
    iface->as_e = as_e;
    if (as_dist > 0) iface->flags |= SC_IFACE_FLAG_AS_ED1;
    if (do_debug != 0 && threadc <= 1) {
      printf("found %u in %s %s bytes %d - %d\n", iface->rtr->asn, iface->name,
             scamper_addr_tostr(iface->addr, buf, sizeof(buf)), iface->as_s,
             iface->as_e);
    }
  }
...
static void sc_iface_asname_find_thread(sc_ifacedom_t *ifd) {
  ...
  while (isalnum(str[i]) == 0 && str[i] != '\0') i++;
  if (str[i] == '\0') goto done;

  j = i;
  for (;;) {
    while (isalnum(str[j]) != 0) j++;
    if (str[j] == '\0') stop = 1;

    /* see if we can find this exact tag */
    if ((x = sc_iface_asname_find_check(list, iface, str, i, j)) < 0) goto done;
...
static int sc_router_ooridx(const sc_router_t *rtr, sc_geohint_t *hint) {
  ...
  for (i = 0; i < rtr->rttc; i++) {
    ...
    distance = sc_geohint_dist(hint, vp);
    rtt = dist2rtt(distance);
    if (rtt > sample->rtt && rtt - sample->rtt > rtt_fudge) return i;
  }
...
static sc_geohint_t *sc_router_checkgeo(const sc_regex_t *re,
  ...
  if (rtr->rttc == 0) return NULL;

  for (hint = sc_geohint_find(re, map); hint != NULL; hint = hint->next) {
    if (sc_geohint_checkmap(hint, map) == 0) continue;
    if (sc_geohint_checkrtt(hint, rtr) > 0) return hint;
  }
...
static void sc_iface_geo_find_print(const sc_ifacedom_t *ifd, const char *code,
  ...
  off = 0;
  string_concat(buf, sizeof(buf), &off, "found %s %s",
  ...
  string_concat(buf, sizeof(buf), &off, " \"%s\" in %s bytes",
                ...
                ifd->iface->name);
  for (i = 0; i < tag->tagc; i++)
    string_concat(buf, sizeof(buf), &off, " %d %d", tag->tags[i].start,
                  tag->tags[i].end);
  printf("%s\n", buf);
...
static int sc_iface_geo_find_fac(slist_t *list, const sc_ifacedom_t *ifd,
  ...
  if (string_tolong(str, &in_no) != 0) return 0;
  ptr = str;
  while (isdigit(*ptr) != 0) ptr++;
  while (*ptr != '\0' && isalnum(*ptr) == 0) ptr++;
  if (*ptr == '\0') return 0;
  street = ptr;
  if (strlen(street) < 3) return 0;

  if (iface->ip_s != -1 && (iface->flags & SC_IFACE_FLAG_IP_HEX) != 0) {
    ip[0] = iface->ip_s;
    ip[1] = iface->ip_e;
    if (pt_overlap(code, 2, ip, 2) != 0) return 0;
  } else
    ip[0] = ip[1] = -1;
  ...
  for (i = 0; i < geohint_facc; i++) {
    ...
    if (sc_geohint_street_tostr(ptr, buf, sizeof(buf)) != 0) continue;
  ...
  if ((tag = sc_iface_geo_alloctag(ptrc_list, ifd, hint, code, 1, ip)) == NULL)
    goto done;
  if (slist_tail_push(list, tag) == NULL) goto done;

  if (do_debug != 0 && threadc <= 1) sc_iface_geo_find_print(ifd, str, tag);
...
static int sc_iface_geo_find_check(slist_t *list, const sc_ifacedom_t *ifd,
  ...
  if (iface->ip_s != -1 && (iface->flags & SC_IFACE_FLAG_IP_HEX) != 0) {
    ip[0] = iface->ip_s;
    ip[1] = iface->ip_e;
    if (pt_overlap(code, codec * 2, ip, 2) != 0) return 0;
  } else
    ip[0] = ip[1] = -1;
  ...
  for (hint = hint->head; hint != NULL; hint = hint->next) {
    if (strcmp(str, hint->code) != 0) break;
    if ((type != 0 && hint->type != type) ||
        sc_geohint_checkrtt(hint, iface->rtr) == 0)
      continue;

    if ((tag = sc_iface_geo_alloctag(ptrc_list, ifd, hint, code, codec, ip)) ==
        NULL)
      goto done;
  ...
  if (place != NULL) {
    if (do_debug != 0 && threadc <= 1) sc_iface_geo_find_print(ifd, str, place);
    if (slist_tail_push(list, place) == NULL) goto done;
    place = NULL;
  }
...
static void sc_iface_geo_find_thread(sc_ifacedom_t *ifd) {
  ...
  /* skip over any non-alnum characters */
  while (isalpha(ifd->label[i]) == 0 && ifd->label[i] != '\0') i++;
  if (ifd->label[i] == '\0') goto done;

  start = j = i;
  for (;;) {
    while (isalpha(ifd->label[j]) != 0) j++;
    if (ifd->label[j] == '\0') stop = 1;

    if ((ptrc = malloc(sizeof(sc_ptrc_t))) == NULL) goto done;
    ptrc->ptr = ifd->label + i;
    ptrc->c = j - i;
    if (slist_tail_push(ptrc_list, ptrc) == NULL) goto done;
    ptrc = NULL;

    if (stop != 0) break;
    j++;
    while (ifd->label[j] != '\0') {
      if (isalpha(ifd->label[j]) != 0) break;
      j++;
    }
    if (ifd->label[j] == '\0') break;
    i = j;
  }

  for (sn = slist_head_node(ptrc_list); sn != NULL; sn = slist_node_next(sn)) {
    a = slist_node_item(sn);
    if (a->c + 1 >= sizeof(buf) || a->c < 3) continue;
    ...
    if (a->c == 4 && b != NULL && b->c == 2) {
      ...
      if (sc_iface_geo_find_check(list, ifd, buf, gc, 2, geotype, ptrc_list) <
          0)
        goto done;
    }
    ...
    if (a->c > 6) {
      ...
      gc[1] = gc[0] + 5;
      geotype = GEOHINT_TYPE_CLLI;
      if (sc_iface_geo_find_check(list, ifd, buf, gc, 1, geotype, ptrc_list) <
          0)
        goto done;
    }
    ...
    if (do_splitlocode != 0 && a->c == 3) {
      memcpy(buf + 2, a->ptr, a->c);
      buf[5] = '\0';
      for (s2 = slist_head_node(ptrc_list); s2 != NULL;
  ...
  for (;;) {
    ...
    if (isdigit(ifd->label[j]) == 0) goto next;
    j++;
    while (isdigit(ifd->label[j]) != 0) j++;
    if (isalpha(ifd->label[j]) == 0) goto next;
    j++;
    while (isalpha(ifd->label[j]) != 0) j++;

    assert(j <= ifd->len);
    if (j - i >= sizeof(buf)) goto next;
  ...
  next:
    while (isalnum(ifd->label[j]) != 0) j++;
    if (ifd->label[j] == '\0') break;
    while (isalnum(ifd->label[j]) == 0 && ifd->label[j] != '\0') j++;
    if (ifd->label[j] == '\0') break;
    i = j;
  }
  ...
  while ((tag = slist_head_pop(list)) != NULL) {
    iface->geos[i].hint = tag->hint;
    tag->hint = NULL;
    iface->geos[i].tags = tag->tags;
    tag->tags = NULL;
    iface->geos[i].tagc = tag->tagc;
    tag->tagc = 0;
    sc_geotag_free(tag);
    tag = NULL;
    i++;
...
static sc_ifdptr_t *sc_ifdptr_get(splaytree_t *tree, sc_ifacedom_t *ifd) {
  sc_ifdptr_t *ifp;
  if ((ifp = sc_ifdptr_find(tree, ifd)) != NULL) return ifp;
  if ((ifp = malloc_zero(sizeof(sc_ifdptr_t))) == NULL) return NULL;
  ifp->ifd = ifd;
  ifp->ptr = NULL;
...
static splaytree_t *sc_ifdptr_tree(const slist_t *routers) {
  ...
  for (sn = slist_head_node(routers); sn != NULL; sn = slist_node_next(sn)) {
    ...
    for (i = 0; i < rd->ifacec; i++) {
      if ((ifp = sc_ifdptr_get(ifp_tree, rd->ifaces[i])) == NULL) goto err;
    }
  }
...
static int sc_ifdptr_tree_ifi(splaytree_t *ifp_tree, const slist_t *ifi_list) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if ((ifp = sc_ifdptr_find(ifp_tree, ifi->ifd)) == NULL) return -1;
    ifp->ptr = ifi;
  }
...
static int sc_routercss_cmp(const sc_routercss_t *a, const sc_routercss_t *b) {
  ...
  if (a->css != NULL && b->css != NULL) {
    if ((rc = sc_css_css_cmp(a->css, b->css)) != 0) return rc;
...
static int sc_routercss_rd_cmp(const sc_routercss_t *a,
                               const sc_routercss_t *b) {
  return ptrcmp(a->rd, b->rd);
}

static sc_routercss_t *sc_routercss_rd_find(splaytree_t *tr,
                                            sc_routerdom_t *rd) {
  sc_routercss_t fm;
  fm.rd = rd;
...
static int sc_routercss_geo_rd(splaytree_t *rcss_tree, sc_routerdom_t *rd,
  ...
  if (splaytree_count(css_tree) == 0) return 0;
  ...
  if (splaytree_insert(rcss_tree, rcss) == NULL) goto done;
  rcss = NULL;
  rc = 0;
...
static splaytree_t *sc_routercss_geo(slist_t *ifi_list) {
  ...
  if ((css_tree = splaytree_alloc((splaytree_cmp_t)sc_css_css_cmp)) == NULL ||
      (rcss_tree = splaytree_alloc((splaytree_cmp_t)sc_routercss_rd_cmp)) ==
          NULL)
    goto err;

  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if (ifi->class != '+') continue;

    if (ifi->ifd->rd != rd) {
      if (sc_routercss_geo_rd(rcss_tree, rd, css_tree) != 0) goto err;
      splaytree_empty(css_tree, (splaytree_free_t)sc_css_free);
      rd = ifi->ifd->rd;
    }

    if ((fm = sc_css_get(css_tree, ifi->css)) == NULL) goto err;
    fm->count++;
  }
 ...
 */
static sc_routercss_t *sc_routercss_alias(sc_routerdom_t *rd,
                                          sc_rework_t *rew) {
  ...
  if ((rcss = malloc_zero(sizeof(sc_routercss_t))) == NULL) goto done;
  rcss->rd = rd;
  ...
  for (i = 0; i < rd->ifacec; i++) {
    ...
    if ((x = sc_rework_match(rew, iface, &css)) < 0) goto done;
    if (x == 0 || css == NULL) continue;
    if (sc_iface_ip_matched(iface, rew) != 0) {
      ip++;
    } else {
      if ((fm = sc_css_get(tree, css)) == NULL) goto done;
      fm->count++;
    }
    sc_css_free(css);
    css = NULL;
...
static void sc_routercsss_free(sc_routercss_t **rcsss, int rcssc) {
  int i;
  for (i = 0; i < rcssc; i++)
    if (rcsss[i] != NULL) sc_routercss_free(rcsss[i]);
  free(rcsss);
  return;
}

static sc_routercss_t **sc_routercss_alias_alloc(slist_t *rs,
                                                 sc_rework_t *rew) {
  ...
  for (sn = slist_head_node(rs); sn != NULL; sn = slist_node_next(sn)) {
    rd = slist_node_item(sn);
    if ((rn = sc_routercss_alias(rd, rew)) == NULL) goto err;
    rnames[i++] = rn;
  }
...
static int sc_ifacedom_css(const sc_ifacedom_t *ifd, sc_css_t **out, int trim) {
  ...
  *out = NULL;
  ...
  if (ifd->iface->ip_s == -1) {
    if ((css = sc_css_alloc(len + 1)) == NULL) goto err;
  ...
  if ((css = sc_css_alloc0()) == NULL) goto err;
  ...
  ip_e = ifd->iface->ip_e;
  if (trim != 0) {
    while (ip_s > 0 && isalnum(ifd->label[ip_s - 1]) == 0) ip_s--;
  ...
  if ((css->css = malloc(css->len)) == NULL) goto err;
...
static void sc_routerinf_free(sc_routerinf_t *ri) {
  ...
  if (ri->ifaces != NULL) {
    for (i = 0; i < ri->ifacec; i++)
      if (ri->ifaces[i] != NULL) sc_ifaceinf_free(ri->ifaces[i]);
    free(ri->ifaces);
  }
  free(ri);
  return;
...
static int sc_routerinf_finish(sc_routerinf_t *ri) {
  ...
  x = 0;
  rd = ri->ifaces[x]->ifd->rd;
  for (i = 1; i < ri->ifacec; i++) {
    if (ri->ifaces[i]->ifd->rd != rd) {
      c = i - x;
      while (x != i) ri->ifaces[x++]->rtrc = c;
      x = i;
      rd = ri->ifaces[x]->ifd->rd;
    }
  }
  c = ri->ifacec - x;
  while (x != ri->ifacec) ri->ifaces[x++]->rtrc = c;
...
static int sc_regex_alias_ri_build(slist_t *ifi_list_in, slist_t *ri_list_out) {
  ...
  if ((ifi_list = slist_dup(ifi_list_in, NULL, NULL)) == NULL) goto done;
  if ((ri_list = slist_alloc()) == NULL) goto done;

  slist_qsort(ifi_list, (slist_cmp_t)sc_ifaceinf_inf_cmp);
  while ((ifi = slist_head_pop(ifi_list)) != NULL) {
    i = 1;
    if (ifi->css != NULL) {
      for (sn = slist_head_node(ifi_list); sn != NULL;
           sn = slist_node_next(sn)) {
        if (sc_ifaceinf_inf_cmp(ifi, slist_node_item(sn)) != 0) break;
        i++;
      }
    }
    ...
    if (sc_routerinf_finish(ri) != 0) goto done;

    /* add router to the list of inferred routers */
    if (slist_tail_push(ri_list, ri) == NULL) goto done;
    ri = NULL;
  }
  rc = 0;
 ...
 * ifi2.
 */
static int sc_regex_ifi_build2(slist_t *ifi1_list, slist_t *ifi2_list, int p,
                               slist_t *ifi_list_out) {
  ...
  if ((ifi_list = slist_alloc()) == NULL) goto done;
  ...
  while (sn1 != NULL) {
    ...
    if (ifi2->regex != -1 && (ifi1->regex >= p || ifi1->regex == -1)) {
      ...
      if (sc_ifaceinf_get(ifi_list, ifi1->ifd, NULL, 0, -1) == NULL) goto done;
    }
  }
  assert(sn2 == NULL);
  rc = 0;
...
static int sc_regex_ifi_build(sc_regex_t *re, slist_t *ifi_list_out) {
  ...
  if ((ifi_list = slist_alloc()) == NULL || (rew = sc_rework_alloc(re)) == NULL)
    goto done;

  for (i = 0; i < re->regexc; i++) {
    if ((x = sc_rework_capcount(rew, i)) < 0) goto done;
    re->regexes[i]->capc = (uint32_t)x;
  }

  /* go through all the interfaces and determine router assignments */
  for (sn = slist_head_node(re->dom->routers); sn != NULL;
       sn = slist_node_next(sn)) {
    rd = slist_node_item(sn);
    for (i = 0; i < rd->ifacec; i++) {
      ifd = rd->ifaces[i];
      if ((x = sc_rework_match(rew, ifd->iface, &css)) < 0) goto done;

      ip = 0;
      if (x == 1) {
        if (css != NULL) ip = sc_iface_ip_matched(ifd->iface, rew);
        if (sc_ifaceinf_get(ifi_list, ifd, css, ip, rew->k) == NULL) goto done;
        css = NULL;
      } else {
        if (sc_ifaceinf_get(ifi_list, ifd, NULL, ip, -1) == NULL) goto done;
      }
    }
  }
...
static int sc_regex_ifi_thin(slist_t *ifi_work_l, slist_t *ifi_cand_l) {
  ...
  while (sn_cand != NULL) {
    ...
    if (ifi_cand->css != NULL && ifi_work->css != NULL &&
      ...
      sc_css_free(ifi_cand->css);
      ifi_cand->css = NULL;
      ifi_cand->ipm = 0;
      ifi_cand->regex = -1;
    }

    if (ifi_cand->css != NULL) c++;
...
static int sc_regex_alias_ri_sp(sc_ifaceinf_t **ifimap, sc_routerdom_t *rd) {
  ...
  for (i = 0; i < rd->ifacec; i++) {
     ...
     */
    if (ifi->css == NULL) continue;
    if (ifi->ipm != 0) continue;
    matchc++;
  }

  if (matchc == 1) return 1;
  return 0;
}
...
static int sc_regex_alias_ri_score(sc_regex_t *re, slist_t *ri_list) {
  ...
  for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (x == 1) {
      ...
      /* true positives! */
      tp = 0;
      for (i = 0; i < re->regexc; i++) remap[i] = 0;
      for (i = 0; i < ri->ifacec; i++) {
      ...
      if (tp >= 2) {
        ...
        for (i = 0; i < re->regexc; i++)
          if (remap[i] > 0) re->regexes[i]->rt_c++;
      }
      continue;
    }
    ...
    for (i = ri->maxrtrc + 1; i < ri->ifacec; i++) {
      if (ri->ifaces[i]->ifd->rd != rd) break;
      x++;
    }
    if (ri->maxrtrc > x) {
      /* mark the router as counted */
...
static int sc_regex_thin(sc_regex_t *re) {
  ...
  for (i = 0; i < re->regexc; i++)
    if (re->regexes[i]->matchc > 0) mc++;
  if (mc == 0 || mc == re->regexc) return 0;

  if ((regexes = malloc_zero(sizeof(sc_regexn_t *) * mc)) == NULL) return -1;
  j = 0;
  for (i = 0; i < re->regexc; i++) {
 ...
 */
static int sc_regex_issame(sc_regex_t *re1, slist_t *ifi1_list,
                           sc_regex_t *re2) {
  ...
  while (sn1 != NULL && sn2 != NULL) {
    ifi1 = slist_node_item(sn1);
    ifi2 = slist_node_item(sn2);
    assert(scamper_addr_cmp(ifi1->ifd->iface->addr, ifi2->ifd->iface->addr) ==
           0);
...
static int sc_regex_alias_eval(sc_regex_t *re, slist_t *out) {
  ...
  if ((ri_list = slist_alloc()) == NULL) goto done;
  if (sc_regex_alias_ri_build(ifi_list, ri_list) != 0) goto done;
  slist_free(ifi_list);
  ifi_list = NULL;

  /* score the router inferences */
  if (sc_regex_alias_ri_score(re, ri_list) != 0) goto done;

  if (out != NULL) slist_concat(out, ri_list);
  rc = 0;
...
static int sc_regex_eval_cb(sc_regex_t *re, slist_t *out,
  ...
  if (score(re, ifi_list) != 0) goto done;

  if (out != NULL) slist_concat(out, ifi_list);
  rc = 0;
...
static void sc_reasn_free(sc_reasn_t *ts, int regexc) {
  ...
  if (ts->exts != NULL) {
    for (i = 0; i < regexc; i++)
      if (ts->exts[i] != NULL) splaytree_free(ts->exts[i], free);
    free(ts->exts);
  }
  if (ts->infs != NULL) {
    for (i = 0; i < regexc; i++)
      if (ts->infs[i] != NULL) splaytree_free(ts->infs[i], free);
    free(ts->infs);
  }
  free(ts);
...
static sc_reasn_t *sc_reasn_alloc(int regexc) {
  ...
  if ((ts = malloc_zero(sizeof(sc_reasn_t))) == NULL ||
      ...
      (ts->inf = splaytree_alloc((splaytree_cmp_t)sc_uint32c_num_cmp)) ==
          NULL ||
...
static int sc_reasn_org(sc_reasn_t *ts, uint32_t a, uint32_t *o) {
  ...
  if ((a2o = malloc(sizeof(sc_as2org_t))) == NULL) return -1;
  a2o->asn = a;
  a2o->org = ts->sib;
  if (splaytree_insert(ts->org, a2o) == NULL) return -1;
...
static int sc_reasn_tp(sc_reasn_t *ts, sc_regex_t *re, sc_ifaceinf_t *ifi,
  ...
  if ((c = sc_uint32c_get(ts->inf, inf_org)) == NULL) return -1;
  c->c++;
  if ((c = sc_uint32c_get(ts->ext, ext_org)) == NULL) return -1;
  c->c++;

  if (re->regexc > 1) {
    if ((c = sc_uint32c_get(ts->infs[ifi->regex], inf_org)) == NULL) return -1;
    c->c++;
    if ((c = sc_uint32c_get(ts->exts[ifi->regex], ext_org)) == NULL) return -1;
    c->c++;
  }
...
static int sc_regex_asn_ifi_score(sc_regex_t *re, slist_t *ifi_list) {
  ...
  if ((ts = sc_reasn_alloc(re->regexc)) == NULL) goto done;

  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    iface = ifi->ifd->iface;

    if (ifi->regex != -1) re->regexes[ifi->regex]->matchc++;

    if (ifi->css != NULL) {
      if (string_isnumber(ifi->css->css) == 0 ||
          string_tollong(ifi->css->css, &ll) != 0 || ll < 0 ||
          ll > 4294967295) {
        ifi->class = '!';
        re->fp_c++;
        ...
        if (do_ed1 != 0 && x == 1 && len >= 3 && css_len >= 3 &&
          ...
          if (sc_reasn_tp(ts, re, ifi, ll) != 0) goto done;
          ifi->class = '=';
          ed1++;
        } else {
          ifi->class = '!';
...
static int sc_regex_asnames_ifi_score(sc_regex_t *re, slist_t *ifi_list) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (ifi->css != NULL) {
      if (sc_rework_match(rew, iface, NULL) < 0) goto done;
      if (sc_iface_ip_matched(iface, rew) != 0) {
        ifi->class = 'x';
        ifi->ipm = 1;
        re->ip_c++;
      ...
      if (sc_as2org_siblings(a2t->asn, iface->rtr->asn) != 0) {
        if (sc_reasn_tp(ts, re, ifi, a2t->asn) != 0) goto done;
...
static int sc_regex_geo_ifi_score_do(sc_regex_t *re, slist_t *ifi_list) {
  ...
  if ((rew = sc_rework_alloc(re)) == NULL) goto done;
  ...
  if (geo_trees != NULL) {
    for (i = 0; i < re->regexc; i++)
      if (geo_trees[i] != NULL) splaytree_free(geo_trees[i], NULL);
    free(geo_trees);
  }
  return rc;
}
...
static int sc_regex_geo_ifi_score(sc_regex_t *re, slist_t *ifi_list) {
  ...
  if (sc_regex_geo_ifi_score_do(re, ifi_list) != 0) goto done;
   ...
   */
  if ((m2h_list = sc_geomap2hint_make(re, ifi_list)) == NULL) goto done;
  m2h_last = NULL;
  ...
  for (;;) {
    ...
    if (geoeval_list != NULL && slist_count(geoeval_list) > 0) {
      ...
      if ((hint =
               sc_geohint_alloc(ge->hint->type, ge->hint->code, ge->hint->place,
                                ge->hint->st, ge->hint->cc, ge->hint->lat,
                                ge->hint->lng, ge->hint->popn)) == NULL ||
          array_insert((void ***)&re->geohints, &re->geohintc, hint, NULL) != 0)
        goto done;
      hint = NULL;
...
static int sc_regex_permute(sc_regex_t *work, slist_t *work_ifi,
  ...
  for (i = 0; i < work->regexc; i++)
    if (strcmp(work->regexes[i]->str, cand->regexes[0]->str) == 0) return 0;

  if ((ifi = slist_alloc()) == NULL || (ri_list = slist_alloc()) == NULL ||
      (cand_ifi = slist_alloc()) == NULL)
    goto done;

  if (sc_regex_ifi_build(cand, cand_ifi) != 0) goto done;
  ...
  for (i = 0; i <= work->regexc; i++) {
    if ((re = sc_regex_plus1(work, cand->regexes[0], i)) == NULL) goto done;
    re->score = work->score + cand->score;

    /* build a new set of inferences */
    if (sc_regex_ifi_build2(work_ifi, cand_ifi, i, ifi) != 0) goto done;

    if (do_learnalias != 0) {
      if (sc_regex_alias_ri_build(ifi, ri_list) != 0) goto done;
      if (sc_regex_alias_ri_score(re, ri_list) != 0) goto done;
      ...
      if (do_learnasn != 0) {
        if (sc_regex_asn_ifi_score(re, ifi) != 0) goto done;
      } else if (do_learnasnames != 0) {
        if (sc_regex_asnames_ifi_score(re, ifi) != 0) goto done;
      } else if (do_learngeo != 0) {
        if (sc_regex_geo_ifi_score(re, ifi) != 0) goto done;
      }
      slist_foreach(ifi, (slist_foreach_t)sc_ifaceinf_css_null, NULL);
    ...
    /* keep regex around */
    if (slist_tail_push(set, re) == NULL) goto done;
    re = NULL;
  }
  ...
  if (cand_ifi != NULL) slist_free_cb(cand_ifi, (slist_free_t)sc_ifaceinf_free);
  if (ri_list != NULL) slist_free_cb(ri_list, (slist_free_t)sc_routerinf_free);
  if (re != NULL) sc_regex_free(re);
  return rc;
}
...
#ifdef HAVE_PTHREAD
  if (pthread_mutex_lock(&sn->mutex) != 0) return -1;
#endif
  return 0;
}
...
#ifdef HAVE_PTHREAD
  if (sn->mutex_o != 0) pthread_mutex_destroy(&sn->mutex);
#endif
...
static splaytree_t *sc_routerdom_css_tree(const slist_t *routers) {
  ...
  for (sn = slist_head_node(routers); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((css = sc_css_dup(rd->css)) == NULL) goto err;
    css->count = 1;
    if (splaytree_insert(rd_css_tree, css) == NULL) goto err;
  }
...
err:
  if (css != NULL) sc_css_free(css);
  if (rd_css_tree != NULL)
    splaytree_free(rd_css_tree, (splaytree_free_t)sc_css_free);
  return NULL;
}
...
static int sc_routerdom_lcs(sc_routerdom_t *rd) {
  ...
  for (i = 0; i < rd->ifacec; i++) {
    ...
    if (sc_ifacedom_css(ifd, &css, 1) != 0) goto done;
    if (css == NULL) continue;
    if (sc_css_get(tree, css) == NULL) goto done;
    sc_css_free(css);
    css = NULL;
  }
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    css = slist_node_item(sn);
    for (i = 0; i < rd->ifacec; i++)
      if (sc_css_match(css, rd->ifaces[i]->label, NULL, 1) == 1) css->count++;
  }

  if (slist_count(list) > 0) {
    slist_qsort(list, (slist_cmp_t)sc_css_count_cmp);
    css = slist_head_item(list);
    if (css->count > 1 && (rd->css = sc_css_dup(css)) == NULL) goto done;
  }
...
static void sc_routerdom_free(sc_routerdom_t *rd) {
  ...
  if (rd->ifaces != NULL) {
    for (i = 0; i < rd->ifacec; i++)
      if (rd->ifaces[i] != NULL) sc_ifacedom_free(rd->ifaces[i]);
    free(rd->ifaces);
  }
...
static int sc_regex_del_ppv_ok(const sc_regex_t *cur, const sc_regex_t *can) {
  ...
  if (cur->tp_c >= can->tp_c && cur->fp_c <= can->fp_c) return 0;
  if (cur->tp_c <= can->tp_c && cur->fp_c >= can->fp_c) return 1;
  ...
  if (del_ppv < cur_ppv && cur_ppv - del_ppv > 100 && del_fp != 1) return 0;
...
static sc_regex_t *sc_domain_bestre_asn(sc_domain_t *dom) {
  ...
  for (sn = slist_head_node(dom->regexes); sn != NULL;
       sn = slist_node_next(sn)) {
    ...
    if (re->regexc < best->regexc && re->matchc >= best->matchc &&
        re->tp_c >= best->tp_c && re_fp > best_fp && re_fp - best_fp <= 1)
      goto new_best;
    ...
    if (re->rt_c > best->rt_c && re->tp_c > 0 &&
        ((best->rt_c == 1 && re->tp_c * 100 / (re->tp_c + re_fp) > 50) ||
...
static sc_regex_t *sc_domain_bestre_geo(sc_domain_t *dom) {
  ...
  for (sn = slist_head_node(dom->regexes); sn != NULL;
     ...
     * then replace
     */
    if (best->regexc >= re->regexc && best->class == RE_CLASS_POOR &&
        class_cmp(re->class, best->class) < 0)
      goto new_best;
...
static sc_regex_t *sc_domain_bestre_alias(sc_domain_t *dom) {
  ...
  for (sn = slist_head_node(dom->regexes); sn != NULL;
    ...
    re = slist_node_item(sn);
    if (re == best) continue;
    if (re->tp_c == 0 && re->fp_c == 0) continue;
    if ((re_atp = sc_regex_score_atp(re)) < 1) break;
    ...
    if ((best->regexc > re->regexc || best_len > re_len * 4) &&
        (diff_atp_r <= 40 || (diff_atp == 2 && best->tp_c > re->tp_c &&
                              best->tp_c - re->tp_c == 1)))
      goto new_best;
...
static sc_regex_t *sc_domain_bestre(sc_domain_t *dom) {
  if (slist_count(dom->regexes) < 1) return NULL;
  if (do_learnalias != 0)
...
static void sc_domain_free(sc_domain_t *dom) {
  ...
  if (dom->geohints != NULL) {
    for (i = 0; i < dom->geohintc; i++)
      if (dom->geohints[i] != NULL) sc_geohint_free(dom->geohints[i]);
    free(dom->geohints);
  }
  if (dom->domain != NULL) free(dom->domain);
  if (dom->escape != NULL) free(dom->escape);
  if (dom->routers != NULL)
...
#ifdef HAVE_PTHREAD
  if (dom->mutex_o != 0) pthread_mutex_destroy(&dom->mutex);
#endif
  free(dom);
  return;
}
...
static sc_domain_t *sc_domain_alloc(const char *domain) {
  ...
  off++;
  if ((dom->escape = malloc(off)) == NULL) goto err;
  off = 0;
  for (x = 0; domain[x] != '\0'; x++) {
    if (domain[x] == '.') dom->escape[off++] = '\\';
...
#ifdef HAVE_PTHREAD
  if (pthread_mutex_init(&dom->mutex, NULL) != 0) goto err;
  dom->mutex_o = 1;
#endif
...
static sc_domain_t *sc_domain_get(const char *domain) {
  ...
  if ((dom = sc_domain_find(domain)) != NULL) return dom;
  if ((dom = sc_domain_alloc(domain)) == NULL) return NULL;
...
static int label_cmp(const char *ap, const char *bp) {
  ...
  for (;;) {
    if ((ap[j] == '.' || ap[j] == '\0') && (bp[j] == '.' || bp[j] == '\0'))
      break;
    if (ap[j] == '.' || ap[j] == '\0') return -1;
    if (bp[j] == '.' || bp[j] == '\0') return 1;
    if (ap[j] < bp[j]) return -1;
    if (ap[j] > bp[j]) return 1;
    j++;
  }
  return 0;
...
static int suffix_file_line_cmp(const char *a, const char *b) {
  ...
  for (i = 0; i <= bc; i++) {
    ap = label_get(a, i);
    bp = label_get(b, i);
    if ((rc = label_cmp(ap, bp)) != 0) return rc;
  }
...
static int process_suffix(slist_t *list) {
  ...
  if ((tmp = slist_alloc()) == NULL) goto err;
  if ((suffix_root = malloc_zero(sizeof(sc_suffix_t))) == NULL) goto err;
  for (;;) {
    if ((sn = slist_head_node(list)) == NULL) break;
    suffix = slist_node_item(sn);
    if (dotcount(suffix) != 0) break;
    slist_head_pop(list);
    slist_tail_push(tmp, suffix);
  }
  if ((x = slist_count(tmp)) > 0) {
    if ((suffix_root->suffixes = malloc_zero(x * sizeof(sc_suffix_t *))) ==
        NULL)
      goto err;
    x = 0;
    while ((suffix = slist_head_pop(tmp)) != NULL) {
      if ((se = malloc_zero(sizeof(sc_suffix_t))) == NULL) goto err;
  ...
  while ((suffix = slist_head_pop(list)) != NULL) {
    if (sc_suffix_get(suffix) == NULL) goto err;
    free(suffix);
  }

  if ((se = sc_suffix_get("arpa")) != NULL) se->end = -1;
...
static int isgeo(int x) {
  ...
  if (x == BIT_TYPE_GEO_IATA || x == BIT_TYPE_GEO_ICAO ||
      x == BIT_TYPE_GEO_CLLI || x == BIT_TYPE_GEO_PLACE ||
      x == BIT_TYPE_GEO_LOCODE || x == BIT_TYPE_GEO_FACILITY ||
      x == BIT_TYPE_GEO_CC || x == BIT_TYPE_GEO_ST)
    return 1;
  return 0;
}
...
static int iscapture(int x) {
  assert(x >= BIT_TYPE_MIN && x <= BIT_TYPE_MAX);
  if (x == BIT_TYPE_CAPTURE || x == BIT_TYPE_CAPTURE_LIT ||
      x == BIT_TYPE_CAPTURE_DIGIT || isgeo(x) != 0)
    return 1;
  return 0;
}
...
static int sc_regex_build_skip(const char *name, int j, char *buf, size_t len,
  ...
  while (isalnum(name[j]) == 0 && name[j] != '\0') {
    if ((r = re_escape(tmp, sizeof(tmp), name[j])) == 0) return -1;

    /* string_concat(buf, len, to, "%s", tmp); */
    if (len - *to < r + 1) return -1;
...
static size_t sc_regex_build_0(const char *name, const sc_rebuild_p_t *p,
  ...
  assert(bits[x] >= BIT_TYPE_MIN);
  assert(bits[x] <= BIT_TYPE_MAX);
  if (bits[x] != BIT_TYPE_SKIP && bits[x] != BIT_TYPE_CAPTURE) return 0;

  /* does not apply if there is no dot or dash separator at the end */
  sep = name[bits[x + 2] + 1];
  if (sep == '\0') sep = '.';
  if (isalnum(sep) != 0) return 0;
  ...
  if (char_within(name, j, bits[x + 2], sep) != 0) return 0;
  if ((r = re_escape(tmp, sizeof(tmp), sep)) == 0) return 0;

  /* string_concat(buf, len, &to, "[^%s]+", tmp); */
  if (len < 4 + r + 1) return 0;
  buf[to++] = '[';
  buf[to++] = '^';
  memcpy(buf + to, tmp, r);
  to += r;
    ...
    if (name[j] == sep) {
      ...
      *score += 1;
      while (name[j] == sep && j <= bits[x + 2]) {
        if ((r = re_escape(tmp, sizeof(tmp), name[j])) == 0) return 0;

        /* string_concat(buf, len, &to, "%s", tmp);*/
        if (len - to < r + 1) return 0;
        memcpy(buf + to, tmp, r);
        to += r;
...
static size_t sc_regex_build_3(const char *name, const sc_rebuild_p_t *p,
  ...
  if (last_sep != bits[x + 2] + 1) return 0;
...
static size_t sc_regex_build_4(const char *name, const sc_rebuild_p_t *p,
  ...
  if (bits[x] != BIT_TYPE_SKIP_LIT && bits[x] != BIT_TYPE_CAPTURE_LIT) return 0;
...
static size_t sc_regex_build_5_dec(const char *name, const sc_rebuild_p_t *p,
  ...
  while (ptr < name + bits[x + 2] + 1) {
    ...
    while (isdigit(*ptr) == 0 && ptr < name + bits[x + 2]) {
      if ((r = re_escape(tmp, sizeof(tmp), *ptr)) == 0) return 0;
...
static size_t sc_regex_build_9(const char *name, const sc_rebuild_p_t *p,
  ...
  /* do not allow partial starts for now */
  if (rb->o != bits[x + 1] && p->dom != NULL) return 0;

  /* Cannot be the first addition to the regex. */
  if (rb->off == 0) return 0;

  /* only applies to geo components */
  if (isgeo(bits[x]) == 0) return 0;
  ...
  if (bits[x] == BIT_TYPE_GEO_PLACE) {
    ...
    buf[to++] = 'z';
    if (punc != '\0') {
      if (re_escape_c(punc) != 0 || punc == '-') buf[to++] = '\\';
      buf[to++] = punc;
    }
    buf[to++] = ']';
    buf[to++] = '+';
...
static sc_rebuild_t *sc_rebuild_push(slist_t *list, char *buf, size_t off,
                                     int score, int f, int x, int o, int any,
                                     int capc) {
  sc_rebuild_t *rb;
  ...
  if (off > 0) memcpy(rb->buf, buf, off);
  rb->off = off;
  rb->score = score;
  rb->f = f;
  rb->x = x;
...
#define RB_BASE (RB_SEG_PUNC_START | RB_SEG_PUNC_END | RB_SEG_ANY | RB_SEG_PUNC)
...
static int sc_regex_build(splaytree_t *tree, const char *name, sc_domain_t *dom,
  ...
  if ((stack = slist_alloc()) == NULL) goto done;
  ...
  if (dom != NULL) {
    ...
    if (sc_rebuild_push(stack, "^", 1, 0, 0, 0, 0, 0, 0) == NULL) goto done;
    if (bits[0] == BIT_TYPE_SKIP) {
      assert(bitc > 3);
      assert(bits[3] != BIT_TYPE_SKIP);
      if (sc_rebuild_push(stack, "", 0, 0, 0, 3, bits[3 + 1], 0, 0) == NULL)
        goto done;
    }
  } else {
    if (sc_rebuild_push(stack, "", 0, 0, 0, 0, 0, 0, 0) == NULL) goto done;
  }
  ...
  for (;;) {
    ...
    while (rb != NULL) {
      if (rb->f < sizeof(func) / sizeof(sc_regex_build_t)) break;
    ...
    if ((rb->o == bits[x + 1] && iscapture(bits[x]) == 1 &&
      ...
      if (sizeof(buf) - off < 1) goto done;
      buf[off++] = '(';
    }

    /* string_concat(buf, sizeof(buf), &off, "%s", tmp); */
    if (sizeof(buf) - off < to) goto done;
    memcpy(buf + off, tmp, to);
    off += to;
    ...
    while (isalnum(name[o]) == 0 && name[o] != '\0' &&
      ...
      if ((r = re_escape(tmp, sizeof(tmp), name[o])) == 0) goto done;
      /* string_concat(buf, sizeof(buf), &off, "%s", tmp); */
      if (sizeof(buf) - off < r) goto done;
      memcpy(buf + off, tmp, r);
      off += r;
    ...
    if (x + 3 == bitc && name[o] == '\0') {
      if (dom != NULL) {
        ...
        if (sizeof(buf) - off < 4 + dom->escapel) goto done;
        buf[off++] = '\\';
        buf[off++] = '.';
        memcpy(buf + off, dom->escape, dom->escapel);
        off += dom->escapel;
        buf[off++] = '$';
        buf[off++] = '\0';
      } else {
      ...
      if (do_learngeo != 0) {
        if ((geotypes = malloc_zero(sizeof(uint8_t) * capc)) == NULL) return -1;
        r = 0;
        for (k = 0; k < bitc; k += 3)
          if (isgeo(bits[k]) != 0)
      ...
      if ((re = sc_regex_get(tree, buf, capc, geotypes)) == NULL) goto done;
      re->score = score;
      re->dom = dom;
...
static int sc_regex_asn_lcs(splaytree_t *tree, sc_domain_t *dom,
   ...
   */
  if ((L = lcs_asn(S, T)) == NULL) goto done;

  Lc = slist_count(L) * 2;
  if (Lc > 0 && (L_array = malloc(sizeof(int) * Lc)) == NULL) goto done;
  ...
  for (i = 0; i < 2; i++) {
    ...
    cc[0] = R->iface->as_s;
    cc[1] = R->iface->as_e;
    if (pt_to_bits(R->label, R->len, cc, 2, L_array, Lc, cc, 2, &bits, &bitc) ==
        0) {
      if (sc_regex_build(tree, R->label, dom, mask, bits, bitc) != 0) goto done;
    }
    free(bits);
    bits = NULL;
  }
...
static int sc_regex_lcs2(splaytree_t *tree, sc_domain_t *dom,
  ...
  if (pt_to_bits(R->label, R->len, X_array, Xc, NULL, 0, NULL, 0, &bits,
                 &bitc) == 0) {
    if (sc_regex_build(tree, R->label, dom, mask, bits, bitc) != 0) goto done;
  }
  if (bits != NULL) {
  ...
  if (R->iface->ip_s == -1) return 0;
  ip[0] = R->iface->ip_s;
  ip[1] = R->iface->ip_e;
  if (pt_overlap(X_array, Xc, ip, 2) == 0) return 0;
...
static int sc_regex_lcs(splaytree_t *tree, sc_domain_t *dom,
  ...
  if ((X = lcs(S->label, 0, T->label, 0, 2)) == NULL) goto done;
  ...
  if ((X_css = sc_css_alloc_lcs(X, S->label)) == NULL) goto done;
  Xc = X_css->cssc * 2;
  if ((X_array = malloc(sizeof(int) * Xc)) == NULL) goto done;
  if ((X_list = slist_alloc()) == NULL) goto done;
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (sc_css_match(X_css, R->label, X_array, 1) == 0) continue;

    if (sc_regex_lcs2(tree, dom, R, X_array, Xc) != 0) goto done;

    if (Xc > 2) {
      if (sc_regex_pt_decons(X_list, X_array, Xc) != 0) goto done;
      while ((ptrc = slist_head_pop(X_list)) != NULL) {
        if (sc_regex_lcs2(tree, dom, R, ptrc->ptr, ptrc->c) != 0) goto done;
        sc_ptrc_free2(ptrc);
        ptrc = NULL;
      }
    }
...
static int sc_router_istraining(sc_router_t *rtr) {
  if (do_learnalias != 0) {
    if (rtr->ifacec > 1) return 1;
  } else if (do_learnasn != 0 || do_learnasnames != 0) {
    if (rtr->asn != 0) return 1;
  } else if (do_learngeo != 0) {
    if (rtr->rtts != NULL) return 1;
  }
  return 0;
}
...
static void sc_router_free(sc_router_t *rtr) {
  ...
  if (rtr->ifaces != NULL) {
    for (i = 0; i < rtr->ifacec; i++) sc_iface_free(rtr->ifaces[i]);
    free(rtr->ifaces);
  }

  if (rtr->rtts != NULL) free(rtr->rtts);
...
static int sc_router_finish(sc_routerload_t *rl) {
  ...
  if (slist_tail_push(router_list, rtr) == NULL) goto err;
  free_rtr = 0;
  ...
  for (i = 0; i < rtr->ifacec; i++) {
    ...
    iface = rtr->ifaces[i];
    if ((suffix = sc_suffix_find(iface->name)) == NULL) continue;
    ...
    if ((dc = sc_css_get_str(dctree, suffix)) == NULL) goto err;
    dc->count++;
  }
  ...
  while ((dc = slist_head_pop(tmp)) != NULL) {
    if ((dom = sc_domain_get(dc->css)) == NULL) goto err;
    ...
    if (slist_tail_push(dom->routers, rd) == NULL) goto err;
    rd = NULL;
...
static int router_file_line(char *line, void *param) {
  ...
  if (line[0] == '\0') {
    if (sc_router_finish(rl) != 0) return -1;
    return 0;
  }

  ip = line;
  ptr = line;
  while (*ptr != '\0' && isspace(*ptr) == 0) ptr++;
  if (*ptr != '\0') {
    *ptr = '\0';
    ptr++;
...
static int sibling_file_line(char *line, void *param) {
  ...
  for (;;) {
    ...
    *ptr = '\0';
    ...
    if (string_tollong(asn, &ll) != 0) goto err;
    if ((a2o = malloc(sizeof(sc_as2org_t))) == NULL) goto err;
    a2o->asn = ll;
    a2o->org = sibling_id;
    if (slist_tail_push(list, a2o) == NULL) goto err;
    a2o = NULL;
...
static void dump_0_regex_geomap(slist_t *m2h_list) {
  ...
  for (sn = slist_head_node(m2h_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (m2h->tp_c == 0 && m2h->fp_c > 0) continue;
...
static int dump_1(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    if (slist_count(dom->regexes) < 1) continue;
    slist_qsort(dom->regexes, (slist_cmp_t)sc_regex_score_rank_cmp);
    hdr = 0;

    for (s2 = slist_head_node(dom->regexes); s2 != NULL;
         s2 = slist_node_next(s2)) {
      re = slist_node_item(s2);
      if (sc_regex_show(re) == 0) continue;
...
static void dump_2_regex_iface(const sc_regex_t *re, sc_iface_t *iface,
  ...
  if (class != '\0') printf(", \"code\":\"%c\"", class);
  printf(", \"hostname\":\"");
  ...
  if (rew != NULL) {
    ...
    x = iface->asnames[1] - iface->asnames[0] + 1;
    assert(x < sizeof(buf));
    memcpy(buf, iface->name + iface->asnames[0], x);
    buf[x] = '\0';
  ...
  if (ifi != NULL && ifi->regex >= 0) printf(", \"regex\":%d", ifi->regex);

  if (class == '!' && do_learngeo != 0) {
    assert(ifi != NULL && re != NULL && ifi->geohint != NULL);
    x = sc_router_ooridx(iface->rtr, ifi->geohint);
    assert(x >= 0);
...
static int dump_2_regex_alias(sc_domain_t *dom, sc_regex_t *re) {
  ...
  if ((rnames = sc_routercss_alias_alloc(dom->routers, rew)) == NULL) goto done;

  /* take another pass, getting all the interfaces within the suffix */
  if ((ifp_tree = sc_ifdptr_tree(dom->routers)) == NULL) goto done;

  /* take a pass through the inferred routers, pairing inference with iface */
  if ((ri_list = slist_alloc()) == NULL) goto done;
  if (sc_regex_alias_eval(re, ri_list) != 0) goto done;
  if (sc_ifdptr_tree_ri(ifp_tree, ri_list) != 0) goto done;

  if (do_ri != 0) {
    /* compute the Rand index if requested */
    if ((ifp_list = slist_alloc()) == NULL) goto done;
  ...
  if (do_json == 0) {
    ...
    for (i = 0; i < re->regexc; i++) printf(" %s", re->regexes[i]->str);
    printf(" %s", sc_regex_score_tostr(re, buf, sizeof(buf)));
    if (do_ri != 0) printf(", ri %.2f", randi);
    printf("\n");
  } else {
    printf("{\"domain\":\"%s\", \"routerc\":%d", dom->domain,
  ...
  for (r = 0; r < rnamec; r++) {
    ...
    if (do_json == 0) {
      if (rn->css != NULL) {
        ...
        printf("unnamed %d", rn->matchc);
      if (rd->css != NULL)
        printf(" %s %d", sc_css_tostr(rd->css, '|', buf, sizeof(buf)),
               rd->css->count);
      printf("\n");
    } else {
      if (r > 0) printf(", ");
    ...
    if (do_json != 0) printf(", \"ifaces\":[");

    for (i = 0; i < rd->ifacec; i++) {
      ifd = rd->ifaces[i];
      iface = ifd->iface;
      ifp = sc_ifdptr_find(ifp_tree, ifd);
      assert(ifp != NULL);
      ifi = ifp->ptr;
      assert(ifi != NULL);
      code = ifi->class;
      ...
      if (do_json == 0) {
        if (rd->css != NULL && ifi->css != NULL && code == '-' &&
          ...
          code = 'M';
                 ...
                 (ifi->css == NULL || sc_css_css_cmp(rn->css, ifi->css) != 0) &&
        ...
        if (ifi->css != NULL) {
          ...
          dump_2_regex_iface(re, iface, code, NULL, NULL, NULL);
      }
    }
  ...
  if (do_appl != 0 && slist_count(dom->appl) > 0) {
    ...
    for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
      ri = slist_node_item(sn);
      if (ri->ifaces[0]->css == NULL) continue;
      if (splaytree_insert(ri_tree, ri->ifaces[0]->css) == NULL) goto done;
    }

    if ((appl_list = slist_alloc()) == NULL) goto done;

    for (sn = slist_head_node(dom->appl); sn != NULL;
         sn = slist_node_next(sn)) {
      rd = slist_node_item(sn);
      assert(rd->ifacec == 1);
      ifd = rd->ifaces[0];
    ...
    while (sn != NULL) {
      ...
      if ((sn2 = slist_node_next(sn)) != NULL) {
        ...
        while (ifi2->css != NULL && sc_css_css_cmp(ifi->css, ifi2->css) == 0) {
          x++;
          if ((sn2 = slist_node_next(sn2)) == NULL) break;
          ifi2 = slist_node_item(sn2);
        }
      }
      ...
      while (sn != sn2) {
        ...
        if (do_json == 0) {
          printf("%16s %c %s\n",
                 scamper_addr_tostr(iface->addr, buf, sizeof(buf)), ifi->class,
                 iface->name);
        } else {
          if (i > 0) printf(", ");
          if (sc_rework_match(rew, iface, NULL) < 0) goto done;
...
static void dump_2_regex_router_geo(const sc_routerdom_t *rd, sc_css_t *css) {
  ...
  if (do_json != 0) {
    printf("{");
    if (rd->rtr->id != 0) printf("\"id\":%u, ", rd->rtr->id);
    if (rd->rtr->rttc > 0) {
      rtt = &rd->rtr->rtts[0];
...
static int dump_2_regex_router(sc_domain_t *dom, sc_regex_t *re,
  ...
  if ((rd_tree = splaytree_alloc((splaytree_cmp_t)ptrcmp)) == NULL ||
      (rd_list = slist_alloc()) == NULL || (ifi_list = slist_alloc()) == NULL ||
  ...
  if (do_json == 0) {
    ...
    if (do_learngeo != 0) dump_0_regex_geomap(m2h_list);
  } else {
    ...
    if (do_learngeo != 0) {
      ...
      printf(", \"geohints\":[");
      i = 0;
      for (sn = slist_head_node(m2h_list); sn != NULL;
           sn = slist_node_next(sn)) {
        m2h = slist_node_item(sn);
        if (sc_geomap2hint_tojson(m2h, buf, sizeof(buf)) != 0) goto done;
        if (i > 0) printf(",");
        printf("%s", buf);
        i++;
      }
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if (ifi->class == ' ') continue;
    if (splaytree_find(rd_tree, ifi->ifd->rd) != NULL) continue;
  ...
  for (z = 0; z < 2; z++) {
    ...
    for (sn = slist_head_node(z_list); sn != NULL; sn = slist_node_next(sn)) {
      ...
      if ((ptr == NULL && z == 0) || (ptr != NULL && z == 1)) continue;
  ...
  if (do_appl != 0 && slist_count(dom->appl) > 0) {
    ...
    slist_empty(rd_list);
    if ((rd0_list = slist_alloc()) == NULL) goto done;
    ...
    if (slist_count(rd_list) > 0) {
      ...
      for (sn = slist_head_node(rd_list); sn != NULL;
           sn = slist_node_next(sn)) {
        rd = slist_node_item(sn);
        if (do_json != 0) {
          if (r > 0) printf(",");
    ...
    if (slist_count(rd0_list) > 0) {
      ...
      for (sn = slist_head_node(rd0_list); sn != NULL;
           sn = slist_node_next(sn)) {
        rd = slist_node_item(sn);
        if (do_json != 0) {
          if (r > 0) printf(",");
...
static int dump_2(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    if (do_learnalias != 0) {
      if (dump_2_regex_alias(dom, re) != 0) return -1;
    } else if (do_learnasn != 0 || do_learnasnames != 0) {
...
static int dump_3_json(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    if (do_learngeo != 0) {
      ...
      printf(", \"geohints\":[");
      i = 0;
      for (s2 = slist_head_node(m2h_list); s2 != NULL;
           s2 = slist_node_next(s2)) {
        m2h = slist_node_item(s2);

        /* skip over hints with no true positives */
        if (m2h->tp_c == 0) continue;
...
static int dump_3_text(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    if ((re = sc_domain_bestre(dom)) == NULL || sc_regex_show(re) == 0)
      continue;
    printf("%s:", dom->domain);
    for (k = 0; k < re->regexc; k++) printf(" %s", re->regexes[k]->str);
...
static int dump_3(void) {
  if (do_json != 0) return dump_3_json();
  return dump_3_text();
}
...
static int dump_4_regex_asn(sc_domain_t *dom, sc_regex_t *re) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if (ifi->class == ' ' && do_showclass != 0) continue;
    iface = ifi->ifd->iface;
    scamper_addr_tostr(iface->addr, buf, sizeof(buf));
    tmp[0] = '\0';
    restr[0] = '\0';
    ...
    printf("%16s %c %6d %6s %3s %s\n", buf, ifi->class, iface->rtr->asn, tmp,
           restr, iface->name);
  }
...
static int dump_4_regex_asnames(sc_domain_t *dom, sc_regex_t *re) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    printf("%16s %c %6d %12s %6s [%d] %s\n", buf, ifi->class, iface->rtr->asn,
           ifi->css != NULL ? ifi->css->css : "", tmp, ifi->regex + 1,
           iface->name);
  }
...
static int dump_4_regex_geo(sc_domain_t *dom, sc_regex_t *re) {
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (ifi->class == '+') {
      ...
      assert(ifi->geohint != NULL);
      s = sc_router_ooridx(rtr, ifi->geohint);
      assert(s >= 0);
  ...
  if (do_appl != 0 && slist_count(dom->appl) > 0) {
    ...
    for (z = 0; z < 2; z++) {
      for (sn = slist_head_node(dom->appl); sn != NULL;
        ...
        for (i = 0; i < rd->ifacec; i++) {
          iface = rd->ifaces[i]->iface;
          if (sc_rework_match(rew, iface, &css) < 0) goto done;
          ...
          if ((render[0] != '\0' && z == 0) || (render[0] == '\0' && z == 1)) {
            if ((d4 = malloc_zero(sizeof(sc_dump4_t))) == NULL ||
                (d4->render = strdup(render)) == NULL ||
                slist_tail_push(out_list, d4) == NULL)
              goto done;
  ...
  while ((d4 = slist_head_pop(out_list)) != NULL) {
    ...
    printf(fstr, buf, d4->class, d4->render, d4->vp != NULL ? d4->vp->code : "",
           d4->rtt != NULL ? d4->rtt : "", restr, d4->iface->name);
    sc_dump4_free(d4);
  }
  rc = 0;
...
static int dump_4(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    if (do_learnasn != 0) {
      if (dump_4_regex_asn(dom, re) != 0) return -1;
    } else if (do_learnasnames != 0) {
      if (dump_4_regex_asnames(dom, re) != 0) return -1;
    } else if (do_learngeo != 0) {
      if (dump_4_regex_geo(dom, re) != 0) return -1;
    }
  }
...
static char *duration_tostr(char *buf, size_t len, const struct timeval *start,
  ...
  if (tv.tv_sec < 60)
    snprintf(buf, len, "%d.%d seconds", (int)tv.tv_sec,
             (int)(tv.tv_usec / 100000));
  else if (tv.tv_sec < 3600)
    snprintf(buf, len, "%d.%d minutes", (int)tv.tv_sec / 60,
             (((int)tv.tv_sec % 60) * 10) / 60);
  else
    snprintf(buf, len, "%d.%d hours", (int)tv.tv_sec / 3600,
             (((int)tv.tv_sec % 3600) * 10) / 3600);
  return buf;
}
...
static int thin_regexes_domain_same_set(dlist_t *set, slist_t *kept,
  ...
  while ((re = dlist_head_pop(set)) != NULL) {
    if (slist_tail_push(kept, re) == NULL) goto done;

    dn = dlist_head_node(set);
    while (dn != NULL) {
      re2 = dlist_node_item(dn);
      dn_this = dn;
      dn = dlist_node_next(dn);
      if (sc_regex_issame(re, ifi_list, re2) == 1) {
        dlist_node_pop(set, dn_this);
        if (slist_tail_push(same, re2) == NULL) goto done;
      }
    }
...
static int thin_regexes_domain_same(slist_t *regexes) {
  ...
  if ((kept = slist_alloc()) == NULL || (same = slist_alloc()) == NULL ||
      (set = dlist_alloc()) == NULL)
    goto done;

  for (sn = slist_head_node(regexes); sn != NULL; sn = slist_node_next(sn)) {
    if (re == NULL) {
      re = slist_node_item(sn);
      if (dlist_tail_push(set, re) == NULL) goto done;
      continue;
    }
    ...
    if (sc_regex_score_thin_cmp(re, re2) != 0) {
      if (thin_regexes_domain_same_set(set, kept, same) != 0) goto done;
      dlist_empty(set);
      re = re2;
    }

    if (dlist_tail_push(set, re2) == NULL) goto done;
  }
...
static int thin_regexes_domain_matchc(slist_t *regexes) {
  ...
  for (sn = slist_head_node(regexes); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (do_learnalias != 0 && re->matchc >= 3 && re->rt_c > 0)
      ...
      list = del;
    if (slist_tail_push(list, re) == NULL) goto done;
  }
...
static int thin_regexes_domain_mask(slist_t *regexes) {
  ...
  if (slist_foreach(regexes, slist_to_dlist, kept) != 0) goto done;
  ...
  for (dn1 = dlist_head_node(kept); dn1 != NULL; dn1 = dlist_node_next(dn1)) {
    ...
    while (dn2 != NULL) {
      ...
      if (c2 == 0) {
        if (slist_tail_push(cont, re2) == NULL) goto done;
...
static int thin_regexes(int mode) {
  ...
  if (thin_same == 0 && thin_matchc == 0 && thin_mask == 0) return 0;
...
static int sc_regex_tp_isbetter(sc_regex_t *re, sc_regex_t *can) {
   ...
   */
  if ((can->rt_c >= 3 || re->rt_c == can->rt_c) && can->tp_c >= re->tp_c &&
      can->fp_c <= re->fp_c)
    return 1;
...
static int sc_regex_refine_tp(sc_regex_t *re) {
  static const uint16_t mask =
      RB_BASE | RB_FIRST_PUNC_END | RB_SEG_LITERAL | RB_SEG_DIGIT;
  ...
  if ((rew = sc_rework_alloc(re)) == NULL) goto done;
  if ((capc = sc_rework_capcount(rew, 0)) < 0) goto done;
  if (capc == 0) {
    rc = 0;
    goto done;
  }
  ...
  for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
    ri = slist_node_item(sn);
    if (ri->ifacec == 1 || ri->ip != 0) continue;
    css = ri->ifaces[0]->css;
    css_fm.css = css->css;
    css_fm.cssc = 1;
    ...
    for (i = 0; i < capc; i++) {
      ...
      if (sc_css_get(css_trees[i], &css_fm) == NULL) goto done;
      if (i + 1 != capc) {
        while (css_fm.css[0] != '\0') css_fm.css++;
        css_fm.css++;
      }
    }
  }
  ...
  if ((css_list = slist_alloc()) == NULL ||
      ...
      (re_list = slist_alloc()) == NULL || (re2_list = slist_alloc()) == NULL)
    goto done;

  for (i = 0; i < capc; i++) {
    /* trim on non-alnum, min length 1 */
    if (sc_css_reduce(css_trees[i], 1, 1) != 0) goto done;
    /* trim on char class, min length 1 */
    if (sc_css_reduce(css_trees[i], 2, 1) != 0) goto done;

    /* get all the substrings out for this capture element */
    splaytree_inorder(css_trees[i], tree_to_slist, css_list);
    splaytree_free(css_trees[i], NULL);
    css_trees[i] = NULL;
    ...
    for (sn = slist_head_node(css_list); sn != NULL; sn = slist_node_next(sn)) {
      ...
      for (s2 = slist_head_node(ri_list); s2 != NULL;
           s2 = slist_node_next(s2)) {
    ...
    for (sn = slist_head_node(re_list); sn != NULL; sn = slist_node_next(sn)) {
      ...
      if (str == NULL || (re_new = sc_regex_alloc(str)) == NULL) goto done;
      str = NULL;
      re_new->dom = re->dom;

      if (sc_regex_alias_eval(re_new, NULL) != 0) goto done;
      ...
      if (slist_tail_push(re2_list, re_new) == NULL) goto done;
      re_new = NULL;
    }
    ...
    if (slist_count(re2_list) > 0) {
      ...
      if (sc_domain_lock(re->dom) != 0) goto done;
...
static int sc_regex_ip_ri_eval(sc_regex_t *re, slist_t *ri_list) {
  ...
  if ((rew = sc_rework_alloc(re)) == NULL) goto done;
  if ((tree = splaytree_alloc((splaytree_cmp_t)ptrcmp)) == NULL) goto done;
  sc_regex_score_reset(re);
  ...
  for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    for (i = 0; i < ri->ifacec; i++) {
      ...
      if ((x = sc_rework_match(rew, iface, NULL)) < 0) goto done;
      ...
      if (x == 1) {
        ...
        if (ri->ifaces[i]->class == 'x') re->fnu_c++;
      }
    }
  }
 ...
 */
static int sc_regex_fne_isbetter(sc_regex_t *cur, sc_regex_t *can1,
                                 sc_regex_t *can2) {
  ...
  if (can2->rt_c < can1->rt_c + 3 &&
      can2->regexes[can2->regexc - 1]->rt_c < 3) {
#if 0
  ...
  if (can2_fp > can1_fp) {
    ...
    if ((del_ppv < cur_ppv && del_ppv < can1_ppv && can1_ppv - del_ppv > 20 &&
         del_fp > 1) ||
...
static int sc_regex_fne_isbetter2(sc_regex_t *cur, sc_regex_t *can) {
  ...
  if (cur_ppv > can_ppv && cur_ppv - can_ppv > 5) return 0;
...
static int sc_regex_css_thin(const sc_regex_t *re, int cap, dlist_t *css_list) {
  ...
  if (dlist_count(css_list) < 2) return 0;

  if ((out = dlist_alloc()) == NULL) goto err;
  ...
  if (dlist_head_push(out, css) == NULL) goto err;
  ...
  while ((css = dlist_tail_pop(css_list)) != NULL) {
    ...
    free(str);
    str = NULL;
    if (sc_regex_eval(re_work, NULL) != 0) goto err;

    if (re_work->regexes[re_work->regexc - 1]->matchc != 0) {
      if (dlist_tail_push(out, css) == NULL) goto err;
    } else {
      sc_css_free(css);
      css = NULL;
...
static int sc_regex_refine_fne(sc_regex_t *re) {
  ...
  if ((css_tree = sc_routerdom_css_tree(re->dom->routers)) == NULL) goto done;
  ...
  if ((caps = malloc_zero(sizeof(int) * (capc + 1))) == NULL) goto done;

  for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
    /* skip over interfaces where the regex did not match */
    ri = slist_node_item(sn);
    ri_css = ri->ifaces[0]->css;
    if (ri->ifacec == 1 && ri_css == NULL) continue;
    ...
    for (i = 0; i < ri->ifacec; i++) {
       ...
       * skip
       */
      if ((css = splaytree_find(css_tree, rd_css)) == NULL) goto done;
      if (css->count > 1) continue;
      ...
      if (sc_css_morespecific_ex(rd_css, ri_css, &css) != 0 || css == NULL ||
          css->cssc != 1) {
        if (css != NULL) {
          sc_css_free(css);
          css = NULL;
    ...
    while ((ptrc = slist_head_pop(rd_ri_list)) != NULL) {
      if ((ptrc = sc_ptrc_get(rd_tree, ptrc->ptr)) == NULL) goto done;
      ptrc->c++;
    }
  ...
  cap = 0;
  for (i = 1; i <= capc; i++)
    if (caps[cap] < caps[i]) cap = i;
  if (cap == 0) goto done;
  free(caps);
  caps = NULL;

  /* build a regex that only matches this element */
  if ((str = sc_regex_caponly(re->regexes[0]->str, cap)) == NULL) goto done;
  if ((re_eval = sc_regex_alloc(str)) == NULL) goto done;
  str = NULL;
  re_eval->dom = re->dom;
  if ((rew_eval = sc_rework_alloc(re_eval)) == NULL) goto done;
  ...
  for (sn = slist_head_node(re->dom->routers); sn != NULL;
    ...
    for (i = 0; i < rd->ifacec; i++) {
      if ((x = sc_rework_match(rew_eval, rd->ifaces[i]->iface, &css)) < 0)
        goto done;
      if (x != 1 || css == NULL) continue;
      if (sc_css_get(css_tree, css) == NULL) goto done;
      sc_css_free(css);
    }
  }
  sc_rework_free(rew_eval);
  rew_eval = NULL;
  sc_regex_free(re_eval);
  re_eval = NULL;

  if ((css_list = sc_css_reduce_ls(css_tree)) == NULL) goto done;
  splaytree_free(css_tree, (splaytree_free_t)sc_css_free);
  css_tree = NULL;
  ...
  while ((css = dlist_head_pop(css_list)) != NULL) {
    ...
    if (sc_regex_find(re_tree, str, 0, NULL) == NULL) {
      ...
      if ((re_eval = sc_regex_get(re_tree, str, 0, NULL)) == NULL) goto done;
      re_eval->score = re->score + css->count;
      re_eval->dom = re->dom;
      if (sc_regex_alias_eval(re_eval, NULL) != 0) goto done;

      /* tag the regex with the css */
      if ((recss = malloc_zero(sizeof(sc_regex_css_t))) == NULL) goto done;
      recss->regex = re_eval;
      re_eval = NULL;
      recss->css = css;
      css = NULL;
  ...
  splaytree_free(re_tree, NULL);
  re_tree = NULL;
  dlist_qsort(recss_list, (dlist_cmp_t)sc_regex_css_regex_score_cmp);

  if (do_debug != 0 && threadc <= 1) {
    for (dn = dlist_head_node(recss_list); dn != NULL;
         dn = dlist_node_next(dn)) {
  ...
  if (dlist_tail_push(css_list, recss->css) == NULL) goto done;
  recss->css = NULL;
  re_work = recss->regex;
  recss->regex = NULL;
  sc_regex_css_free(recss);
  ...
  for (;;) {
    slist_empty_cb(ifi_list, (slist_free_t)sc_ifaceinf_free);
    if (sc_regex_ifi_build(re_work, ifi_list) != 0) goto done;
    ...
    while (dn != NULL) {
      ...
      if ((re_eval = sc_regex_tail_push(re_work, ren)) == NULL) goto done;
      ...
      if (sc_regex_ifi_build(recss->regex, ifi2_list) != 0 ||
          sc_regex_ifi_build2(ifi_list, ifi2_list, recss->work->regexc - 1,
                              ifi3_list) != 0 ||
    ...
    for (dn = dlist_head_node(recss_list); dn != NULL;
      ...
      if (sc_regex_fne_isbetter(re, re_work, recss->work) > 0) {
        ...
        re_work = recss->work;
        recss->work = NULL;
        dlist_tail_push(css_list, recss->css);
        recss->css = NULL;
        sc_regex_css_free(recss);
        recss = NULL;
        break;
  ...
  if (sc_regex_css_thin(re, cap, css_list) != 0) goto done;
  sc_regex_free(re_work);
  re_work = NULL;
  str = sc_regex_caprep_list(re->regexes[0]->str, cap, css_list);
  if (str == NULL) goto done;
  if ((re_work = sc_regex_alloc(str)) == NULL) goto done;
  str = NULL;
  re_work->dom = re->dom;
  ...
  if (sc_regex_alias_eval(re_work, NULL) != 0) goto done;

  if (sc_regex_fne_isbetter2(re, re_work) > 0) {
    if (sc_domain_lock(re->dom) != 0) goto done;
    if (slist_tail_push(re->dom->regexes, re_work) == NULL) goto done;
    sc_domain_unlock(re->dom);
    re_work = NULL;
...
static int sc_regex_refine_init(const sc_regex_t *re, const sc_rework_t *rew,
  ...
  *Xc = sc_rework_capcount(rew, 0);
  if ((*Xa = malloc(sizeof(int) * *Xc * 2)) == NULL) goto done;

  /* extract the capture portion of the regex */
  if (sc_regex_capget_css(re->regexes[0]->str, capcss) != 0) goto done;
  if ((*capcss)->cssc != *Xc) goto done;
   ...
   */
  if ((*capre = sc_regex_alloc_css(*capcss)) == NULL) goto done;
  *LXc = 0;
  for (i = 0; i < (*capre)->regexc; i++) *LXc += (*capre)->regexes[i]->capc;
  if (*LXc == 0) {
    sc_regex_free(*capre);
    *capre = NULL;
  } else {
...
static int sc_regex_refine_class_seg(slist_t *list, sc_regex_t *re, int ro_in,
  ...
  if (c == cc) {
    while (res[ro] != '\0') buf[off++] = res[ro++];
    buf[off++] = '\0';
    if ((dup = strdup(buf)) == NULL) return -1;
  ...
  while (res[ro] != '\0') {
    ...
    if ((res[ro] == '.' && res[ro + 1] == '+') ||
        (res[ro] == '[' && res[ro + 1] == '^')) {
      if (res[ro] == '[') {
        while (res[ro] != '\0' && res[ro] != ']') ro++;
        if (res[ro] != ']' || res[ro + 1] != '+') return -1;
        adj -= 1;
      ...
      off_x = off;
      for (sn = slist_head_node(segs[c]); sn != NULL;
           sn = slist_node_next(sn)) {
        ...
        segl = strlen(ss->seg);
        if (len - off < segl + 1) return -1;
        memcpy(buf + off, ss->seg, segl + 1);
        off += segl;

        if (sc_regex_refine_class_seg(list, re, ro, buf, len, off, segs, c + 1,
                                      cc, adj + ss->score) != 0)
          return -1;
      }
      return 0;
...
static size_t slist_str_len(const slist_t *list, int (*filter)(const char *)) {
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ptr = slist_node_item(sn);
    if (filter != NULL && filter(ptr) == 0) continue;
    len = strlen(ptr);
    if (seglen != len) {
      if (seglen == 0) {
        seglen = len;
...
static int sc_regex_refine_class_do(sc_regex_t *re, slist_t *ifd_list,
  ...
  if ((str = sc_regex_capseg(re->regexes[0]->str)) == NULL) goto done;
  if ((capre = sc_regex_alloc(str)) == NULL) goto done;
  str = NULL;
  if ((caprew = sc_rework_alloc(capre)) == NULL) goto done;
  ...
  for (sn = slist_head_node(ifd_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((x = sc_rework_match(caprew, ifd->iface, &css)) < 0) goto done;
    if (x == 0) continue;
    ptr = css->css;
  ...
  if ((seg_tree = splaytree_alloc((splaytree_cmp_t)sc_segscore_cmp)) == NULL)
    goto done;
  if ((segs = malloc_zero(sizeof(slist_t *) * cc)) == NULL) goto done;
  ...
  for (i = 0; i < cc; i++) {
     ...
     */
    if ((segs[i] = slist_alloc()) == NULL) goto done;
    ...
    for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
       ...
       */
      if (*ptr != '\0' || switchc == 0) {
        if (sc_regex_capget(capre->regexes[0]->str, i + 1, buf, sizeof(buf)) !=
            0)
          goto done;
        if (strcmp(buf, ".+") == 0 || strcmp(buf, ".+?") == 0)
          ...
          score = 1;
        if (sc_segscore_get(seg_tree, buf, score) == NULL) goto done;
        continue;
      }
      ...
      if (switchc <= 2) {
        ...
        if ((ss = sc_segscore_get(seg_tree, buf, score)) == NULL) goto done;

        /* keep track of the alpha literals -- foo in foo1, foo2 */
        if (switchc == 2 && sc_segscore_switch2(ss, alpha_ptr) != 0) goto done;
      }

      if (switchc > 0) {
        snprintf(buf, sizeof(buf), "[a-z\\d]%s",
                 class_seglen(tmp, sizeof(tmp), seglen));
        if (sc_segscore_get(seg_tree, buf, 2) == NULL) goto done;
      }
    }
    ...
    for (sn = slist_head_node(segs[i]); sn != NULL; sn = slist_node_next(sn)) {
      ...
      /* XXX: set score */
      if ((str = strdup(buf)) == NULL) goto done;
      free(ss->seg);
      ss->seg = str;
      str = NULL;
    }
  ...
  if (do_debug != 0 && threadc <= 1) {
    for (i = 0; i < cc; i++) {
      for (sn = slist_head_node(segs[i]); sn != NULL;
           sn = slist_node_next(sn)) {
        ss = slist_node_item(sn);
        printf("%d %s %d\n", i, ss->seg, ss->score);
      }
  ...
  if ((re_list = slist_alloc()) == NULL) goto done;
  len = strlen(re->regexes[0]->str) * 3;
  if ((str = malloc(len)) == NULL) goto done;
  ...
  for (sn = slist_head_node(re_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (re->regexes[0]->plan != NULL) {
      ...
      if (re2->regexes[0]->plan == NULL) goto done;
    }
  }
  ...
  if (trees != NULL) {
    for (i = 0; i < cc; i++) splaytree_free(trees[i], free);
    free(trees);
  }
  if (seg_tree != NULL)
...
static int sc_regex_refine_class_tree(splaytree_t *re_tree, slist_t *ifd_list) {
  ...
  for (sn = slist_head_node(re_tree_list); sn != NULL;
       sn = slist_node_next(sn)) {
    re = slist_node_item(sn);
    if (sc_regex_refine_class_do(re, ifd_list, re_list) != 0) goto done;
    while ((re_new = slist_head_pop(re_list)) != NULL) {
      ...
      if (splaytree_insert(re_tree, re_new) == NULL) goto done;
    }
    if (sn == sn_tail) break;
  }
...
static int sc_regex_refine_class(sc_regex_t *re) {
  ...
  if (do_learnalias != 0) {
    ...
    for (sn = slist_head_node(ri_list); sn != NULL; sn = slist_node_next(sn)) {
      ...
      for (i = 0; i < ri->ifacec; i++) {
        ifi = ri->ifaces[i];
        if (ifi->class != '+') continue;
        if (slist_tail_push(ifd_list, ifi->ifd) == NULL) goto done;
      }
    }
    slist_free_cb(ri_list, (slist_free_t)sc_routerinf_free);
    ri_list = NULL;
  ...
  if (slist_count(ifd_list) == 0) goto done;
  ...
  for (sn = slist_head_node(re_list); sn != NULL; sn = slist_node_next(sn)) {
    re_eval = slist_node_item(sn);
    if (sc_regex_eval(re_eval, NULL) != 0) goto done;
  }
...
static int sc_regex_fnu_isbetter(sc_regex_t *cur, sc_regex_t *can, int i) {
  ...
  if (can_fp > cur_fp) {
     ...
     * regex, and worse by more than 0.5%, then it is not better
     */
    if (cur_ppv > del_ppv && cur_ppv - del_ppv > 5) return 0;
  }
...
static int sc_regex_refine_fnu(sc_regex_t *re) {
  ...
  tp = re->tp_c;
  fp = re->fp_c + re->ip_c;
  if (tp + fp == 0 || tp * 100 / (tp + fp) < 90) return 0;
  ...
  if ((rew = sc_rework_alloc(re)) == NULL ||
      ...
      (re_tree = splaytree_alloc((splaytree_cmp_t)sc_regex_str_cmp)) == NULL ||
      (re_list = dlist_alloc()) == NULL || (re2_list = slist_alloc()) == NULL ||
      (css_list = slist_alloc()) == NULL || (ri_list = slist_alloc()) == NULL ||
      sc_regex_alias_eval(re, ri_list) != 0 ||
      sc_ifdptr_tree_ri(ifp_tree, ri_list) != 0)
    goto done;

  if (sc_regex_refine_init(re, rew, &Xc, &Xa, &LXc, &LXa, &capcss, &capre,
                           &caprew) != 0)
    goto done;
  ...
  for (r = 0; r < rnamec; r++) {
    ...
    for (i = 0; i < rd->ifacec; i++) {
      ...
      if (ifi->css != NULL && sc_css_css_cmp(ifi->css, rn->css) == 0) continue;
      ...
      if ((css = sc_css_matchxor(rn->css, ifd)) != NULL) {
        if (css->cssc > 0) {
          ...
          if (sc_regex_capget(re->regexes[0]->str, 1, buf, sizeof(buf)) != 0)
            continue;
          if (snprintf(rebuf, sizeof(rebuf), "^(%s)\\.%s$", buf,
                       re->dom->escape) >= sizeof(rebuf))
            goto done;
  ...
  for (sn = slist_head_node(css_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (sc_css_hasalpha(css) == 0) continue;
     ...
     */
    Lc = css->cssc;
    if ((La = malloc(sizeof(int) * Lc * 2)) == NULL) goto done;
    if ((LAa = malloc(sizeof(int) * (Lc + LXc) * 2)) == NULL) goto done;
    LAc = Lc + LXc;
    ...
    for (r = 0; r < rnamec; r++) {
      ...
      for (i = 0; i < rd->ifacec; i++) {
        ...
        if (sc_css_match(rn->css, ifd->label, Xa, 1) != 1) continue;

        /*
         * if the literal is not found in this interface, skip
         * XXX: need to xor the name out.
         */
        if (sc_css_match(css, ifd->label, La, 0) == 0) continue;
        ...
        if (threadc <= 1 && do_debug != 0)
          printf("%s %s\n", ifd->label,
                 sc_css_tostr(css, '|', buf, sizeof(buf)));
        if (pt_overlap(Xa, Xc * 2, La, Lc * 2) != 0) continue;
        ...
        if (pt_to_bits(ifd->label, ifd->len, Xa, Xc * 2, LAa, LAc * 2, NULL, 0,
                       &bits, &bitc) == 0) {
          /* 0xff, no char classes */
          if (sc_regex_build(re_tree, ifd->label, re->dom, mask, bits, bitc) !=
              0)
            goto done;
        }
        if (bits != NULL) {
  ...
  splaytree_empty(re_tree, NULL);
  if (sc_regex_capset_css(re_list, re_tree, capcss) != 0) goto done;
  ...
  if ((re_set = slist_alloc()) == NULL || (fnu_list = slist_alloc()) == NULL)
    goto done;
  ...
  for (;;) {
    ...
    slist_empty_cb(ifi_list, (slist_free_t)sc_ifaceinf_free);
    if (sc_regex_ifi_build(re_cur, ifi_list) != 0) goto done;
    ...
    while (dn != NULL) {
      ...
      if (sc_regex_permute(re_cur, ifi_list, re_eval, re_set) != 0) goto done;
    ...
    while ((re_new = slist_head_pop(re2_list)) != NULL) {
      ...
      if ((ptr = strdup(re_new->regexes[i]->str)) == NULL ||
        ...
        goto done;
      ptr = NULL;
      if (slist_tail_push(fnu_list, re_fnu) == NULL) goto done;
...
static int sc_regex_refine_ip(sc_regex_t *re) {
  ...
  static const uint16_t mask_nolit = RB_BASE | RB_SEG_LITERAL_IP | RB_SEG_DIGIT;
  ...
  if (do_learnalias != 0) {
    ...
    for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
      ifi = slist_node_item(sn);
      if (ifi->class == 'x' && slist_tail_push(ifd_list, ifi->ifd) == NULL)
        goto done;
    }
  }
  ...
  for (sn = slist_head_node(ifd_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (sc_ifacedom_css(ifd, &css, 0) != 0) goto done;
    if (css == NULL) continue;
  ...
  if ((css_list = slist_alloc()) == NULL) goto done;
  splaytree_inorder(css_tree, tree_to_slist, css_list);

  for (sn = slist_head_node(css_list); sn != NULL; sn = slist_node_next(sn)) {
    css = slist_node_item(sn);
    I_array = malloc(sizeof(int) * 2 * css->cssc);
    for (sn2 = slist_head_node(ifd_list); sn2 != NULL;
         sn2 = slist_node_next(sn2)) {
  ...
  if (sc_regex_refine_class_tree(re_tree, ifd_list) != 0) goto done;

  if ((re_list = dlist_alloc()) == NULL) goto done;
  splaytree_inorder(re_tree, tree_to_dlist, re_list);
  splaytree_free(re_tree, NULL);
  re_tree = NULL;
  ...
  for (;;) {
     ...
     */
    if (do_learnasn != 0) {
      for (dn = dlist_head_node(re_list); dn != NULL;
           dn = dlist_node_next(dn)) {
        re_ip = dlist_node_item(dn);
        if (sc_regex_ip_ri_eval(re_ip, ri_list) != 0) goto done;
      }
      slist_empty_cb(ri_list, (slist_free_t)sc_routerinf_free);
    } else if (do_learnasn != 0 || do_learnasnames != 0 || do_learngeo != 0) {
      for (dn = dlist_head_node(re_list); dn != NULL;
           dn = dlist_node_next(dn)) {
        re_ip = dlist_node_item(dn);
        if (sc_regex_ip_ifi_eval(re_ip, ifi_list) != 0) goto done;
      }
      slist_empty_cb(ifi_list, (slist_free_t)sc_ifaceinf_free);
    }
    ...
    if (re_ip != NULL && re_ip->fp_c == 0 && re_ip->rt_c >= 3) {
       ...
       */
      if ((do_learnalias != 0 && sc_regex_alias_eval(re_new, ri_list) != 0) ||
          (do_learnasn != 0 && sc_regex_asn_eval(re_new, ifi_list) != 0) ||
          (do_learnasnames != 0 &&
           sc_regex_asnames_eval(re_new, ifi_list) != 0) ||
          (do_learngeo != 0 && sc_regex_geo_eval(re_new, ifi_list) != 0))
        goto done;

      /* put a copy on the regex list */
      if (sc_domain_lock(dom) != 0) goto done;
...
static int sc_regex_refine_fp_best(sc_regex_t *re, int x, dlist_t *re_list,
  ...
  *re_out = NULL;
  ...
  for (dn = dlist_head_node(re_list); dn != NULL; dn = dlist_node_next(dn)) {
    ...
    if ((i = sc_regex_fp_isbetter(re, re_fp, x)) == -1) goto done;
    if (i == 1) break;
  }
  if (dn == NULL) {
    rc = 0;
  ...
  if ((class_list = slist_alloc()) == NULL) goto done;
  if (sc_regex_refine_class_do(re_fp, ifd_list, class_list) != 0) goto done;
  if (slist_count(class_list) > 0) {
    for (sn = slist_head_node(class_list); sn != NULL;
         sn = slist_node_next(sn)) {
      re_fp = slist_node_item(sn);
      if (sc_regex_alias_fp_eval(ri_list, re_fp) != 0) goto done;
    }
    slist_qsort(class_list, (slist_cmp_t)sc_regex_score_fp_cmp);

    re_fp = dlist_node_item(dn);
    re_fp2 = slist_head_item(class_list);
    if (sc_regex_score_fp_cmp(re_fp, re_fp2) > 0) re_fp = re_fp2;
  }

  if ((re_tmp = sc_regex_plus1(re, re_fp->regexes[0], x)) == NULL) goto done;
  ...
  *re_out = re_tmp;
  ...
  if (class_list != NULL)
    slist_free_cb(class_list, (slist_free_t)sc_regex_free);
  return rc;
}
...
static int sc_regex_refine_fp(sc_regex_t *re) {
  ...
  if (do_learnalias != 0) {
    ...
    for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
      ifi = slist_node_item(sn);
      if (ifi->class != '!' && slist_tail_push(ifd_list, ifi->ifd) == NULL)
        goto done;
    }
  }
  ...
  for (sn = slist_head_node(css_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    for (sn2 = slist_head_node(ifd_list); sn2 != NULL;
      ...
      if (sc_css_match(css, str, La, 0) == 1) {
        len = strlen(str);
        if (pt_to_bits(str, len, NULL, 0, La, css->cssc * 2, NULL, 0, &bits,
                       &bitc) == 0) {
  ...
  for (;;) {
    ...
    if (sc_regex_refine_fp_best(re, x, re_list, ifd_list, ri_list, &re_tmp) !=
        0)
      goto done;

    if (re_tmp == NULL) break;
...
static int sc_regex_sets_stop(sc_regex_t *cur, sc_regex_t *can) {
  ...
  if (can->fne_c + can->fnu_c + can->unk_c + can->fp_c < 2) return 1;
  ...
  if (do_learnalias != 0) {
     ...
     * with, then no better
     */
    if (can->regexes[i]->rt_c * 100 / cur->rt_c < 4) return 1;
  }
...
static int sc_regex_sets_isbetter(sc_regex_t *cur, sc_regex_t *can) {
  ...
  /* if we don't gain any true positives, then no better */
  if (can->tp_c <= cur->tp_c) return 0;

  /* if the atp score does not increase, then no better */
  if (sc_regex_score_atp(cur) >= sc_regex_score_atp(can)) return 0;
  ...
  if (do_learnalias != 0) {
     ...
     */
    i = sc_regex_findnew(cur, can);
    if (can->regexes[i]->rt_c < 3) return 0;

    /* make sure the PPV rate is acceptable, otherwise no better */
    if (sc_regex_del_ppv_ok(cur, can) == 0) return 0;
  } else if (do_learngeo != 0) {
    /* figure out the maximum number of TPs contributed */
    for (i = 0; i < can->regexc; i++)
      if (can->regexes[i]->tp_c > tp_max) tp_max = can->regexes[i]->tp_c;
    ...
    for (i = 0; i < can->regexc; i++) {
      if (can->regexes[i]->rt_c < 3) return 0;
      if (can->regexes[i]->tp_c * 1000 / tp_max < 10) return 0;
    }
     ...
     */
    for (i = 0; i < can->regexc; i++)
      if (can->regexes[i]->rt_c < 2) return 0;
  }
...
static int sc_regex_refine_sets_permute(sc_regex_t *work, slist_t *work_ifi,
  ...
  if ((set = slist_alloc()) == NULL) goto done;
  if (sc_regex_permute(work, work_ifi, cand, set) != 0) goto done;
  ...
  *best = slist_head_pop(set);
  ...
  if (set != NULL) slist_free_cb(set, (slist_free_t)sc_regex_free);
  return rc;
}
...
static int sc_regex_refine_sets(sc_regex_sni_t *sin) {
  ...
  if ((ifi_list = slist_alloc()) == NULL ||
      ...
      sc_regex_refine_sets_permute(work->re, ifi_list, sin->re, &sin->out) !=
          0) {
    work->done = 1;
    goto done;
  }

  if (sc_regex_sn_lock(work) != 0) goto done;
  work->snic--;
  snic = work->snic;
  ...
  if (sn == NULL) goto done;
  if (sc_regex_sets_stop(work->re, best) != 0) work->done = 1;
  work->re = best;
  rc = 0;
...
static sc_regex_t *sc_regex_refine_merge_do(sc_remerge_t *rem) {
  ...
  if (slist_count(rem->list) == 1 &&
    ...
    if ((re = sc_regex_alloc(str)) != NULL) return re;
    goto done;
  }
  ...
  off = 0;
  if (rem->css->cssc == 2) string_concat(str, len, &off, "%s", rem->css->css);
...
static int sc_regex_refine_merge_skip(const sc_remerge_t *rem) {
  ...
  if (slist_count(rem->list) < 1) {
    ...
    assert(ptr != NULL);
    if (strcmp(ptr, "\\d*") == 0) return 1;
  } else {
    /* if all are digits or \d+ or \d* then skip */
    x = 0;
    for (sn = slist_head_node(rem->list); sn != NULL;
         sn = slist_node_next(sn)) {
      ptr = slist_node_item(sn);
...
static int sc_regex_refine_merge_setup(char *S, char *T, sc_css_t **css_o,
  ...
  *T_css_o = NULL;
  ...
  if ((X = lcs(S, S_len, T, T_len, 1)) == NULL) goto done;
  ...
  if ((css = sc_css_alloc_lcs(X, S)) == NULL) goto done;
  pt_s_flatten(X, flat, &flat_len);
  if ((S_css = sc_css_alloc_xor(S, S_len, flat, flat_len)) == NULL) goto done;
  pt_t_flatten(X, flat, &flat_len);
  if ((T_css = sc_css_alloc_xor(T, T_len, flat, flat_len)) == NULL) goto done;
...
static int sc_regex_refine_merge(sc_regex_t *re, slist_node_t *sn) {
  ...
  while (sn != NULL) {
    ...
    if (sc_regex_refine_merge_setup(S, T, &css, &S_css, &T_css) != 0) goto done;
    if (css == NULL) continue;
     ...
     */
    if ((rem = sc_remerge_get(tree, css)) == NULL) goto done;
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (sc_regex_refine_merge_skip(rem) != 0) continue;
    if ((re_new = sc_regex_refine_merge_do(rem)) == NULL) goto done;
    ...
    if (re->regexes[0]->plan != NULL) {
      ...
      if (re_new->regexes[0]->plan == NULL) goto done;
    }

    if (sc_regex_eval(re_new, NULL) != 0) goto done;
    ...
    if (slist_tail_push(re_list, re_new) == NULL) goto done;
    re_new = NULL;
  }
...
static int generate_regexes_domain_alias(sc_domain_t *dom) {
  ...
  for (sn = slist_head_node(dom->routers); sn != NULL;
       sn = slist_node_next(sn)) {
    rd = slist_node_item(sn);
    for (i = 0; i < rd->ifacec; i++) {
...
static int generate_regexes_domain_asn(sc_domain_t *dom) {
   ...
   */
  for (sn1 = slist_head_node(dom->routers); sn1 != NULL;
       sn1 = slist_node_next(sn1)) {
...
static int regex_file_line_d3(char *line) {
  ...
  if ((dup = strdup(line)) == NULL) return -1;
...
static int regex_file_line(char *line, void *param) {
  ...
  if ((rc = regex_file_line_d3(line)) == -1) return -1;
  if (rc == 1) return 0;
  ...
  *ptr = '\0';
  ...
  if (do_learngeo != 0 &&
      (plan = (char *)string_findlc(ptr, ", plan:")) == NULL)
    return -1;
...
static int generate_regexes(void) {
  ...
  if (regex_eval != NULL) return generate_regexes_supplied();

  gettimeofday_wrap(&start);
  if ((tp = threadpool_alloc(threadc)) == NULL) return -1;
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
...
static void refine_regexes_tp_thread(sc_regex_t *re) {
  if (do_learnalias != 0) sc_regex_refine_tp(re);
  return;
}
...
static void refine_regexes_sets_onion_thread(sc_regex_sn_t *work) {
  ...
  if (work->snis == NULL && (work->snis = slist_alloc()) == NULL) return;

#ifdef HAVE_PTHREAD
  if (work->mutex_o == 0) {
    if (pthread_mutex_init(&work->mutex, NULL) != 0) return;
    work->mutex_o = 1;
  }
#endif
  ...
  while (sn != NULL) {
    ...
    threadpool_head_push_nolock(
        tp, (threadpool_func_t)refine_regexes_sets_work_thread, sni);
    sn = sn_next;
  }
...
static void refine_regexes_sets_domain_check(sc_domain_fn_t *domfn) {
  ...
  work = slist_head_item(domfn->work);
  assert(work != NULL);
  head = work->re;
  ...
  all_done = 1;
  for (sn = slist_head_node(domfn->work); sn != NULL;
       sn = slist_node_next(sn)) {
    work = slist_node_item(sn);
    if (work->done != 0) continue;
    ...
    for (s2 = slist_head_node(domfn->work); s2 != sn;
         s2 = slist_node_next(s2)) {
...
static int refine_regexes_sets_domain_init(dlist_t *out, sc_domain_t *dom) {
   ...
   * could be merged.
   */
  if (slist_count(dom->regexes) < 2) return 0;
   ...
   * two
   */
  if (head->fne_c + head->fnu_c + head->unk_c + head->fp_c < 2) return 0;
  ...
  for (sn = slist_head_node(dom->regexes); sn != NULL;
    ...
    if (head != re && re->tp_c <= head->tp_c &&
      ...
      continue;
    for (s2 = slist_head_node(domfn->work); s2 != NULL;
         s2 = slist_node_next(s2)) {
  ...
  if (dlist_tail_push(out, domfn) == NULL) goto done;
  domfn = NULL;
  rc = 0;
...
static int refine_regexes_sets(void) {
  ...
  if ((refine_mask & REFINE_SETS) == 0) return 0;
  ...
  if ((domfn_list = dlist_alloc()) == NULL) goto done;
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    if (refine_regexes_sets_domain_init(domfn_list, dom) != 0) goto done;
  }

  while (dlist_count(domfn_list) > 0) {
    if (do_debug != 0 && threadc <= 1) printf("\n###\n");

    /*
     * do the main bit of work which figures out which regex in the set
     * below makes the most sense to merge in
     */
    if ((tp = threadpool_alloc(threadc)) == NULL) goto done;
    for (dn = dlist_head_node(domfn_list); dn != NULL;
         dn = dlist_node_next(dn)) {
      ...
      for (sn = slist_head_node(domfn->work); sn != NULL;
        ...
        if (work->done != 0) continue;
        threadpool_tail_push_onion(
            tp, (threadpool_func_t)refine_regexes_sets_onion_thread, work);
      }
    }
    threadpool_join(tp);
    tp = NULL;
    ...
    for (dn = dlist_head_node(domfn_list); dn != NULL;
         dn = dlist_node_next(dn)) {
      domfn = dlist_node_item(dn);
      threadpool_tail_push(
          tp, (threadpool_func_t)refine_regexes_sets_domain_check, domfn);
    }
    threadpool_join(tp);
    tp = NULL;
...
static int refine_regexes(const char *type, threadpool_func_t func,
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->regexes); s2 != NULL;
         s2 = slist_node_next(s2)) {
      re = slist_node_item(s2);
      if (skip != NULL && skip(re, param) != 0) continue;
      if (prep == NULL) {
        threadpool_tail_push(tp, func, re);
        if (s2 == sn_tail) break;
      } else {
        if (s2 == sn_tail) break;
        if ((ptr = prep(re, slist_node_next(s2))) == NULL) goto done;
...
static int refine_regexes_ip_skip(sc_regex_t *re, void *param) {
  if (re->ip_c == 0 || re->rt_c == 0 || re->tp_c == 0) return 1;
  return 0;
}
...
static int refine_regexes_ip(void) {
  if ((refine_mask & REFINE_IP) == 0) return 0;
...
static int refine_regexes_fp(void) {
  ...
  if ((refine_mask & REFINE_FP) == 0) return 0;
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    if ((best = sc_domain_bestre(dom)) == NULL) continue;
    for (s2 = slist_head_node(dom->regexes); s2 != NULL;
         s2 = slist_node_next(s2)) {
      re = slist_node_item(s2);
      if (re->tp_c < best->tp_c || re->rt_c < 3 || re->fp_c < 2) continue;
      threadpool_tail_push(tp, (threadpool_func_t)refine_regexes_fp_thread, re);
    }
  }
...
static int refine_regexes_fnu_skip(sc_regex_t *re, void *param) {
  if (re->fnu_c == 0) return 1;
  return 0;
}

static int refine_regexes_fnu(void) {
  if ((refine_mask & REFINE_FNU) == 0) return 0;
...
static void *refine_regexes_merge_prep(sc_regex_t *re, slist_node_t *sn) {
  sc_regex_mn_t *rmn;
  if ((rmn = malloc(sizeof(sc_regex_mn_t))) == NULL) return NULL;
  rmn->re = re;
  rmn->sn = sn;
  return rmn;
}
...
static int refine_regexes_merge(void) {
  if ((refine_mask & REFINE_MERGE) == 0) return 0;

  if (refine_regexes("merge", (threadpool_func_t)refine_regexes_merge_thread,
...
static int refine_regexes_class(void) {
  if ((refine_mask & REFINE_CLASS) == 0) return 0;

  if (refine_regexes("class", (threadpool_func_t)refine_regexes_class_thread,
                     NULL, NULL, NULL) != 0)
    return -1;
...
static int refine_regexes_fne_skip(sc_regex_t *re, void *param) {
  if (re->fne_c == 0) return 1;
  return 0;
}

static int refine_regexes_fne(void) {
  if ((refine_mask & REFINE_FNE) == 0) return 0;
...
static int refine_regexes_tp_skip(sc_regex_t *re, void *param) {
  if (re->tp_c == 0) return 1;
  return 0;
}

static int refine_regexes_tp(void) {
  if ((refine_mask & REFINE_TP) == 0) return 0;
...
static int refine_dict_asnames_name_dig(slist_t *thin, const sc_as2tag_t *a2t) {
  ...
  for (sn = slist_head_node(thin); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (a2t->asn != a2tb->asn) continue;
    S = a2tb->tag;
    S_len = strlen(S);
    if ((S_len < T_len && strncmp(S, T, S_len) == 0 &&
         string_isdigit(T + S_len) != 0) ||
        (S_len > T_len && strncmp(S, T, T_len) == 0 &&
         string_isdigit(S + T_len) != 0))
      return 1;
  }
...
static int refine_dict_asnames_unk_want(slist_t *thin, const sc_as2tag_t *a2t,
                                        const sc_as2tag_t *a2tb) {
  static const char *digit2string[] = {"zero",  "one",  "two", "three",
                                       "four",  "five", "six", "seven",
                                       "eight", "nine", "ten"};
  ...
  if (refine_dict_asnames_name_dig(thin, a2t) != 0) return 0;
   ...
   */
  if (a2tb != NULL && strcmp(a2t->tag, a2tb->tag) == 0 &&
      splaytree_count(a2t->sxes) >= 3 && splaytree_count(a2tb->sxes) <= 1)
    return 1;
  ...
  for (sn = slist_head_node(thin); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (sc_as2org_siblings(a2tn->asn, a2t->asn) == 0) continue;
    S = a2tn->tag;
    S_len = strlen(S);
    S_i = T_i = 0;
    ...
    if (S_i == S_len && T_i == T_len) return 1;
  }
...
static int refine_dict_asnames_thin(slist_t *list, slist_t *unk_list) {
  ...
  if ((thin = slist_alloc()) == NULL) goto done;
  ...
  for (;;) {
    if ((a2t = slist_head_pop(unk_list)) == NULL) break;
    ...
    while ((a2tb = slist_head_item(unk_list)) != NULL) {
      if (strcmp(a2t->tag, a2tb->tag) != 0) break;
...
static int refine_dict_asnames(void) {
  ...
  if ((refine_mask & REFINE_DICT) == 0 || domain_eval != NULL) return 0;
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    if ((re = sc_domain_bestre(dom)) == NULL || re->rt_c < 3) continue;
    if (sc_regex_asnames_eval(re, ifi_list) != 0) goto done;
  ...
  splaytree_free(unk_tree, NULL);
  unk_tree = NULL;
  if (refine_dict_asnames_thin(list, unk_list) != 0) goto done;

  /* clean out the old asnames dictionary */
  if (tag2ass != NULL) {
    for (i = 0; i < tag2asc; i++)
      if (tag2ass[i] != NULL) sc_as2tag_free(tag2ass[i]);
    free(tag2ass);
    tag2ass = NULL;
  }
  ...
  if ((tag2ass = malloc_zero(sizeof(sc_as2tag_t *) * tag2asc)) == NULL)
    goto done;
  i = 0;
  while ((a2t = slist_head_pop(list)) != NULL) tag2ass[i++] = a2t;
  slist_free(list);
  list = NULL;
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
         s2 = slist_node_next(s2)) {
      rd = slist_node_item(s2);
      for (i = 0; i < rd->ifacec; i++) {
        ifd = rd->ifaces[i];
        threadpool_tail_push(tp, (threadpool_func_t)sc_iface_asname_find_thread,
                             ifd);
      }
    }
  }
  ...
  if (generate_regexes() != 0) goto done;
  if (eval_regexes() != 0) goto done;
  if (refine_regexes_merge() != 0) goto done;
...
static void refine_dict_geo_thread(sc_regex_t *re) {
  ...
  if (re->rt_c < 3 || re->tp_c * 100 / (re->tp_c + re->fp_c) < 40 ||
      (ifi_list = slist_alloc()) == NULL ||
      sc_regex_geo_eval(re, ifi_list) != 0 ||
  ...
  for (sn = slist_head_node(ifi_list); sn != NULL; sn = slist_node_next(sn)) {
    ifi = slist_node_item(sn);
    if (ifi->class != '!' && ifi->class != '?') continue;
    ...
    if (geotype != GEOHINT_TYPE_IATA && geotype != GEOHINT_TYPE_LOCODE &&
      ...
      continue;
    sc_css_geomap(ifi->css, re->regexes[ifi->regex]->plan, &map);
    if (map.codelen == 0) continue;
    if ((gr = sc_georef_get(gr_tree, &map)) == NULL ||
        slist_tail_push(gr->ifi_list, ifi) == NULL ||
        (gr->f_mask == NULL &&
         (gr->f_mask = malloc_zero(sizeof(uint32_t) * re->dom->rtmlen)) ==
             NULL))
      goto done;
    assert(gr->class == '\0' || gr->class == ifi->class);
    if (gr->class == '\0') gr->class = ifi->class;
    if (mask_isset(gr->f_mask, re->dom->rtmlen, ifi->ifd->rd->id) != 0)
      continue;
    if (slist_tail_push(gr->rd_list, ifi->ifd->rd) == NULL) goto done;
  ...
  if ((gr_list = slist_alloc()) == NULL) goto done;
  splaytree_inorder(gr_tree, tree_to_slist, gr_list);
  splaytree_free(gr_tree, NULL);
  gr_tree = NULL;
  ...
  for (sn = slist_head_node(m2h_list); sn != NULL; sn = slist_node_next(sn)) {
    m2h = slist_node_item(sn);
    if (m2h->tp_c == 0 || splaytree_find(gh_tree, m2h->hint) != NULL) continue;
    if (splaytree_insert(gh_tree, m2h->hint) == NULL) goto done;
  }

  if ((geoeval_list = slist_alloc()) == NULL) goto done;
  for (sn = slist_head_node(gr_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (do_debug != 0 && threadc <= 1)
      printf("%s rt %d/%d ifi %d/%d\n",
             sc_geomap_tostr(&gr->map, buf, sizeof(buf)), rt_tp_c, rt_c,
             gr->ifi_tp_c, ifi_c);
    ...
    if (slist_count(geoeval_list) < 1) continue;
    ...
    if (ge->alloc == 0) {
      ...
      if (gr->map.type == GEOHINT_TYPE_PLACE &&
        ...
        if (ge->round == 0) {
          if ((gh->facname = strdup(ge->hint->facname)) == NULL) goto done;
        } else if (ge->round == 1) {
          if ((gh->street = strdup(ge->hint->street)) == NULL) goto done;
        }
      } else if (gr->map.type == GEOHINT_TYPE_FACILITY) {
        if ((gh->street = strdup(ge->hint->street)) == NULL) goto done;
      }
...
static int refine_dict_geo(void) {
  ...
  if ((refine_mask & REFINE_DICT) == 0) return 0;
...
static int assert_domains(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
      ...
      rd = slist_node_item(s2);
      for (i = 0; i < rd->ifacec; i++) {
        if (id + 1 != rd->ifaces[i]->id) return -1;
        id++;
      }
    }
    if (id != dom->ifacec) return -1;
  }
...
static void load_routers_asn(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
      ...
      rd = slist_node_item(s2);
      for (i = 0; i < rd->ifacec; i++)
        threadpool_tail_push(tp, (threadpool_func_t)sc_iface_asn_find_thread,
                             rd->ifaces[i]);
    }
  }
  threadpool_join(tp);
...
static int load_routers_asnames_dict(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
      ...
      rd = slist_node_item(s2);
      for (i = 0; i < rd->ifacec; i++) {
        if ((css = sc_css_alloc_tags(rd->ifaces[i]->label)) == NULL) goto done;
        if (css->cssc == 0) {
          sc_css_free(css);
          css = NULL;
          continue;
  ...
  for (;;) {
    if ((a2t_head = slist_head_pop(list)) == NULL) break;
    a2t = NULL;
    for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    while ((a2t = slist_head_item(list)) != NULL) {
      if (strcmp(a2t_head->tag, a2t->tag) != 0) break;
      slist_head_pop(list);
      sc_as2tag_free(a2t);
    }
...
static int load_routers_asnames(void) {
  ...
  if (dicts == NULL && load_routers_asnames_dict() != 0) return -1;
...
static void load_routers_geo(void) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
      ...
      rd = slist_node_item(s2);
      for (i = 0; i < rd->ifacec; i++)
        threadpool_tail_push(tp, (threadpool_func_t)sc_iface_geo_find_thread,
                             rd->ifaces[i]);
    }
  }
  threadpool_join(tp);
...
static int load_routers_1(void) {
  ...
  if (slist_count(rl.ifaces) > 0 && sc_router_finish(&rl) != 0) goto done;
  if ((domain_list = slist_alloc()) == NULL) goto done;
...
static int load_routers_2(void) {
  ...
  if ((tmp = slist_alloc()) == NULL) goto done;
  for (sn = slist_head_node(domain_list); sn != NULL;
       sn = slist_node_next(sn)) {
    dom = slist_node_item(sn);
    ...
    while ((rd = slist_head_pop(tmp)) != NULL) {
      if (sc_router_istraining(rd->rtr) != 0) {
        if (slist_tail_push(dom->routers, rd) == NULL) goto done;
      } else {
        if (slist_tail_push(dom->appl, rd) == NULL) goto done;
      }
    }
  }
  ...
  if (do_learnalias != 0)
    ...
    if (load_routers_asnames() != 0) goto done;
    if (domain_eval != 0) {
  ...
  for (sn = slist_head_node(domain_list); sn != NULL;
    ...
    for (s2 = slist_head_node(dom->routers); s2 != NULL;
      ...
      rd = slist_node_item(s2);
      rd->id = rtr_id++;
      for (i = 0; i < rd->ifacec; i++) rd->ifaces[i]->id = ifd_id++;
...
static int load_siblings(void) {
  ...
  while ((a2o = slist_head_pop(list)) != NULL) siblings[x++] = a2o;
  assert(x == siblingc);
...
static int geohints_file_line_iso3166(const char *line, char **ptr_in, char *cc,
                                      char *st) {
  char *ptr = *ptr_in;
  char iso3166[8];
  int i = 0;
...
static sc_geohint_t *geohints_file_line_make(slist_t *list, uint8_t type,
                                             char *code, char *place, char *cc,
                                             char *st, double lat, double lng,
                                             long popn) {
  sc_geohint_t *hint;
...
static int geohints_file_line(char *line, void *param) {
  ...
  if (type == GEOHINT_TYPE_IATA || type == GEOHINT_TYPE_ICAO ||
    ...
    i = j = 0;
    while (place[i] != '\0') {
      if (isalpha(place[i]) != 0) code[j++] = tolower(place[i]);
      i++;
    }
    code[j] = '\0';
    ...
    /* iso3166 code */
    if (geohints_file_line_iso3166(line, &ptr, cc, st) != 0) goto err;
    ...
    /* iso3166 3-letter CC */
    if (isalpha(ptr[0]) != 0 && isalpha(ptr[1]) != 0 && isalpha(ptr[2]) != 0 &&
        ptr[3] == ' ') {
      st[0] = tolower(ptr[0]);
      st[1] = tolower(ptr[1]);
      st[2] = tolower(ptr[2]);
    ...
    if ((hint = geohints_file_line_make(list, type, code, place, cc, st, lat,
                                        lng, popn)) == NULL)
      goto err;
    ...
    if (string_isalpha(st) == 0 || strlen(st) != 3) return 0;
  ...
  if (got_lat == 0 || got_lng == 0) return 0;
  ...
  if ((hint = geohints_file_line_make(list, type, code, place, cc, st, lat, lng,
                                      popn)) == NULL)
    goto err;
  ...
  if (type == GEOHINT_TYPE_FACILITY) {
    ...
    if ((code[0] == 'g' && code[1] == 'b') ||
      ...
      if (geohints_file_line_make(list, type, code, place, cc, st, lat, lng,
                                  popn) == NULL)
        goto err;
    }
  }
...
static int load_dict_geohints(void) {
  ...
  if ((geo_list = slist_alloc()) == NULL ||
      ...
      (clligp_tree = splaytree_alloc((splaytree_cmp_t)sc_clligp_3cmp)) ==
          NULL ||
      (strlist_tree = splaytree_alloc((splaytree_cmp_t)sc_strlist_cmp)) ==
          NULL ||
  ...
  if ((geohint_cous = malloc_zero(sizeof(sc_country_t *) * geohint_couc)) ==
      NULL)
    goto done;
  i = 0;
  while ((hint = slist_head_pop(country_list)) != NULL) {
    ...
    geohint_cous[i]->name = hint->place;
    hint->place = NULL;
    sc_geohint_free(hint);
    i++;
  }
  ...
  for (sn = slist_head_node(place_list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((i = sc_country_findpos(hint->cc)) >= 0) cou_hintc[i]++;
    if ((i = sc_state_sort_findpos(hint->st, hint->cc)) >= 0) sta_hintc[i]++;
  }

  /* put some landmarks in for each country and state */
  for (i = 0; i < geohint_couc; i++) {
    if (cou_hintc[i] > 0 &&
        (geohint_cous[i]->hints =
             malloc_zero(sizeof(sc_geohint_t *) * cou_hintc[i])) == NULL)
      goto done;
  }
  for (i = 0; i < geohint_stac; i++) {
    if (sta_hintc[i] > 0 &&
        (geohint_stas[i]->hints =
             malloc_zero(sizeof(sc_geohint_t *) * sta_hintc[i])) == NULL)
      goto done;
  }
  for (sn = slist_head_node(place_list); sn != NULL; sn = slist_node_next(sn)) {
    hint = slist_node_item(sn);
  ...
  for (i = 0; i < geohint_facc; i++) {
    ...
    sl = sc_strlist_get(strlist_tree, fac->code);
    x = 0;
    ...
    if ((hint = sc_geohint_alloc(GEOHINT_TYPE_PLACE, fac->code, fac->place,
                                 fac->st, fac->cc, fac->lat, fac->lng, 0)) ==
            NULL ||
        slist_tail_push(sl->list, hint) == NULL ||
  ...
  for (i = 0; i < 26; i++) assert(first[i] == geohint_plc[i]);
  ...
  i = 0;
  while ((hint = slist_head_pop(geo_list)) != NULL) geohints[i++] = hint;
  sc_geohint_sort(geohints, geohintc);
  for (i = 0; i < geohintc; i++) geohints[i]->index = i;
...
static int asnames_file_line(char *line, void *param) {
  ...
  *ptr = '\0';
  ...
  if (string_isdigit(asn) == 0 || string_tollong(asn, &ll) != 0 || ll < 0 ||
      ll > 4294967295) {
...
static int load_dict_asnames(void) {
  ...
  x = 0;
  while ((a2t = slist_head_pop(list)) != NULL) tag2ass[x++] = a2t;
...
static int load_dict(void) {
  ...
  if (do_learngeo != 0) {
    if (load_dict_geohints() != 0) return -1;
  } else if (do_learnasnames != 0) {
    if (load_dict_asnames() != 0) return -1;
  } else {
...
static int rttload_router_finish(sc_rttload_t *rtl) {
  ...
  assert(rtl->rttc >= 0);
  if (rtl->rttc == 0) return 0;
  if ((rtl->rtr->rtts = memdup(rtl->rtts, sizeof(sc_rtt_t) * rtl->rttc)) ==
      NULL) {
...
static int load_rtt(void) {
  ...
  if (rtt_file == NULL) return 0;

  gettimeofday_wrap(&start);

  memset(&rtl, 0, sizeof(rtl));
  for (sn = slist_head_node(router_list); sn != NULL;
       sn = slist_node_next(sn)) {
    rtr = slist_node_item(sn);
    if ((rtr->flags & SC_ROUTER_FLAG_ID) != 0) rtl.routerc++;
  }
  if (rtl.routerc == 0) {
  ...
  if ((rtl.routers = malloc_zero(sizeof(sc_router_t *) * rtl.routerc)) ==
      NULL) {
  ...
  for (sn = slist_head_node(router_list); sn != NULL;
       sn = slist_node_next(sn)) {
    rtr = slist_node_item(sn);
    if ((rtr->flags & SC_ROUTER_FLAG_ID) != 0) rtl.routers[i++] = rtr;
  }
  ...
  if (rtl.rtr != NULL) rttload_router_finish(&rtl);

  tp = threadpool_alloc(threadc);
  for (i = 0; i < rtl.routerc; i++) {
    if (rtl.routers[i]->rttc == 0) continue;
    rtc++;
  ...
  if (rtl.unknown != NULL) {
    for (i = 0; i < rtl.unknownc; i++) free(rtl.unknown[i]);
    free(rtl.unknown);
  }
  return rc;
}
...
static void cleanup(void) {
  ...
  if (siblings != NULL) {
    for (i = 0; i < siblingc; i++)
      if (siblings[i] != NULL) free(siblings[i]);
    free(siblings);
    siblings = NULL;
  }
  ...
  if (geohints != NULL) {
    for (i = 0; i < geohintc; i++)
      if (geohints[i] != NULL) sc_geohint_free(geohints[i]);
    free(geohints);
    geohints = NULL;
  }
  ...
  if (geohint_facs != NULL) {
    for (i = 0; i < geohint_facc; i++)
      if (geohint_facs[i] != NULL) sc_geohint_free(geohint_facs[i]);
    free(geohint_facs);
    geohint_facs = NULL;
  }
  ...
  if (geohint_cous != NULL) {
    for (i = 0; i < geohint_couc; i++)
      if (geohint_cous[i] != NULL) sc_country_free(geohint_cous[i]);
    free(geohint_cous);
    geohint_cous = NULL;
  }
  ...
  if (geohint_stas != NULL) {
    for (i = 0; i < geohint_stac; i++)
      if (geohint_stas[i] != NULL) sc_state_free(geohint_stas[i]);
    free(geohint_stas);
    geohint_stas = NULL;
  }

  for (i = 0; i < 26; i++)
    if (geohint_pls[i] != NULL) free(geohint_pls[i]);

  if (clligps != NULL) {
    for (i = 0; i < clligpc; i++)
      if (clligps[i] != NULL) free(clligps[i]);
    free(clligps);
    clligps = NULL;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (threadc == -1) {
    ...
    if ((i = sysconf(_SC_NPROCESSORS_ONLN)) > 1) threadc = i;
#endif
  }
  ...
  if (load_suffix() != 0) return -1;

  /* load the siblings */
  if (load_siblings() != 0) return -1;

  /* load the routers, part 1 */
  if (load_routers_1() != 0) return -1;

  /* load any dictionaries */
  if (load_dict() != 0) return -1;

  /* load the RTT samples */
  if (load_rtt() != 0) return -1;

  /* load the routers, part 2 */
  if (load_routers_2() != 0) return -1;

  /* stop if we were told to stop after loading the data */
  if (do_loadonly != 0) return 0;

  /* generate regular expressions */
  if (generate_regexes() != 0) return -1;

  /* evaluate regular expressions */
  if (eval_regexes() != 0) return -1;

  if (do_learnasn != 0) {
    for (j = 1; j <= stop_id; j++) {
      if (j == 1) {
        if (refine_regexes_merge() != 0) return -1;
      } else if (j == 2) {
        if (refine_regexes_class() != 0) return -1;
      } else if (j == 3) {
        if (refine_regexes_merge() != 0) return -1;
      } else if (j == 4) {
        if (refine_regexes_sets() != 0) return -1;
      }
    }
  } else if (do_learnasnames != 0) {
    for (j = 1; j <= stop_id; j++) {
      if (j == 1) {
        ...
        if (refine_dict_asnames() != 0) return -1;
      } else if (j == 3) {
        if (refine_regexes_class() != 0) return -1;
      } else if (j == 4) {
        if (refine_regexes_merge() != 0) return -1;
        ...
        if (refine_regexes_ip() != 0) return -1;
      }
    }
  } else if (do_learngeo != 0) {
    for (j = 1; j <= stop_id; j++) {
      if (j == 1) {
        ...
        if (refine_dict_geo() != 0) return -1;
      } else if (j == 6) {
        if (refine_regexes_fp() != 0) return -1;
      }
    }
  } else if (do_learnalias != 0) {
    for (j = 1; j <= stop_id; j++) {
      if (j == 1) {
        if (regex_eval == NULL && thin_regexes(0) != 0) return -1;
      } else if (j == 2) {
        if (refine_regexes_tp() != 0) return -1;
      } else if (j == 3) {
        if (refine_regexes_fne() != 0) return -1;
      } else if (j == 4) {
        if (refine_regexes_class() != 0) return -1;
      } else if (j == 5) {
        if (refine_regexes_fnu() != 0) return -1;
      } else if (j == 6) {
        if (refine_regexes_sets() != 0) return -1;
      } else if (j == 7) {
        if (refine_regexes_ip() != 0) return -1;
