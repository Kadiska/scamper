...
static int ip_find(scamper_addr_t **set, size_t len, scamper_addr_t *a) {
  if (bsearch(&a, set, len, sizeof(scamper_addr_t *), ip_cmp) != NULL) return 1;
  return 0;
}
...
static int check_options(int argc, char *argv[]) {
  ...
  if ((str = opt_userid) != NULL) {
    ...
    if ((userids = malloc(sizeof(uint32_t) * x)) == NULL) goto err;
    for (i = 0; i < x; i++) userids[i] = u32_a[i];
    useridc = x;
  ...
  if ((str = opt_ips) != NULL) {
    ...
    if ((ips = malloc(sizeof(scamper_addr_t *) * x)) == NULL) goto err;
    for (i = 0; i < x; i++) ips[i] = addr_a[i];
    ipc = x;
...
static char *ipid_sample_ipid(const ipid_sample_t *sample, char *buf,
                              size_t len) {
...
static void ipid_sample_free(ipid_sample_t *sample) {
  ...
  if (sample->probe_src != NULL) scamper_addr_free(sample->probe_src);
  free(sample);
  return;
}
...
static int process_dealias(scamper_dealias_t *dealias) {
  ...
  for (i = 0; i < dealias->probec; i++) {
    ...
    for (j = 0; j < probe->replyc; j++) {
      ...
      if (ipc > 0 && ip_find(ips, ipc, reply->src) == 0) continue;
      ...
      if ((sample = malloc_zero(sizeof(ipid_sample_t))) == NULL) goto err;
      ...
      if (slist_tail_push(list, sample) == NULL) goto err;
    }
  }
...
static int process_ping(scamper_ping_t *ping) {
  ...
  for (i = 0; i < ping->ping_sent; i++) {
    for (reply = ping->ping_replies[i]; reply != NULL; reply = reply->next) {
      if (reply->tx.tv_sec == 0) continue;
      if (ipc > 0 && ip_find(ips, ipc, reply->addr) == 0) continue;
...
static int process_trace(scamper_trace_t *trace) {
  ...
  if (trace->dst->type != SCAMPER_ADDR_TYPE_IPV4) goto done;
  ...
  for (u16 = trace->firsthop - 1; u16 < trace->hop_count; u16++) {
    for (hop = trace->hops[u16]; hop != NULL; hop = hop->hop_next) {
      ...
      if (ipc > 0 && ip_find(ips, ipc, hop->hop_addr) == 0) continue;
...
static void cleanup(void) {
  ...
  if (ips != NULL) {
    for (i = 0; i < ipc; i++)
      if (ips[i] != NULL) scamper_addr_free(ips[i]);
    free(ips);
    ips = NULL;
  }
...
int main(int argc, char *argv[]) {
  ...
  if ((flags & FLAG_NOTRACE) == 0) types[typec++] = SCAMPER_FILE_OBJ_TRACE;

  if ((filter = scamper_file_filter_alloc(types, typec)) == NULL) return -1;
  ...
  while ((sample = slist_head_pop(list)) != NULL) {
    printf("%d.%06d %d.%06d %s %s %s\n", (int)sample->tx.tv_sec,
           (int)sample->tx.tv_usec, (int)sample->rx.tv_sec,
           (int)sample->rx.tv_usec,
