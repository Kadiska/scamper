...
static void usage(uint32_t opt_mask) {
  ...
  if (opt_mask & OPT_COUNT) fprintf(stderr, "     -c [replyc]/probec\n");
...
static int check_options(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, opts)) != -1) {
    switch (ch) {
      ...
      case 'c':
        if ((opt_count = strdup(optarg)) == NULL) goto done;
        break;
    ...
    if (*ptr == '/') {
      *ptr = '\0';
      ...
      if (string_isdigit(ptr) == 0 || string_isdigit(opt_count) == 0 ||
          ...
          string_tolong(ptr, &lo_pc) != 0 || lo_rc > lo_pc || lo_pc > 30 ||
          lo_rc < 1 || lo_pc < 1) {
        usage(OPT_COUNT);
        goto done;
      }
  ...
  if ((methodc = slist_count(list)) > 0) {
    if ((methods = malloc_zero(sizeof(char *) * methodc)) == NULL) goto done;
    i = 0;
    while ((ptr = slist_head_pop(list)) != NULL) methods[i++] = ptr;
  } else {
    methodc = 3;
...
static void print(char *format, ...) {
  ...
  if (logfile_fd == NULL && (options & OPT_DAEMON) != 0) return;
  ...
  if ((options & OPT_DAEMON) == 0) printf("%ld: %s", (long int)tv.tv_sec, msg);
...
static int do_addrfile(void) {
  ...
  while (off <= end) {
    ...
    if (addrfile_buf[off] == '\n' || (off == end && start < off)) {
      addrfile_buf[off] = '\0';
      if (do_addrfile_line(addrfile_buf + start) != 0) goto err;
      start = ++off;
    } else {
      ++off;
    }
...
static int do_method(void) {
  ...
  if ((pinger = slist_head_pop(waiting)) == NULL &&
    ...
    if (addrfile_fd == -1) return 0;
    if (do_addrfile() != 0) return -1;
    if ((pinger = slist_head_pop(virgin)) == NULL) return 0;
  }
...
static int pinger_data(void) {
  ...
  if ((logfile_name != NULL &&
       (logfile_fd = fopen(logfile_name, "w")) == NULL) ||
  ...
  while (error == 0) {
     ...
     */
    tv_ptr = NULL;
    if (more > 0 && (slist_count(waiting) > 0 || slist_count(virgin) > 0 ||
                     addrfile_fd != -1)) {
...
static void cleanup(void) {
  ...
  if (methods != NULL) {
    for (i = 0; i < methodc; i++)
      if (methods[i] != NULL) free(methods[i]);
    free(methods);
    methods = NULL;
  }
