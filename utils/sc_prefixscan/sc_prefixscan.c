...
static int check_options(int argc, char *argv[]) {
  ...
  if ((options & (OPT_INFILE | OPT_OUTFILE | OPT_PORT | OPT_UNIX | OPT_READ)) ==
      0) {
...
static void sc_pingtest_free(sc_pingtest_t *pt) {
  ...
  if (pt->tg != NULL) sc_target_free(pt->tg);
  free(pt);
  return;
}
...
static sc_test_t *sc_pingtest_new(scamper_addr_t *addr) {
  ...
  if ((test = sc_test_alloc(TEST_PING, pt)) == NULL) goto err;
...
static int sc_prefixscan_cmp(const sc_prefixscan_t *a,
...
static int process_ping(sc_test_t *test, scamper_ping_t *ping) {
  ...
  for (j = 0; j < ping->ping_sent && rc < 4; j++) {
    if ((rx = ping->ping_replies[j]) == NULL) continue;
    if (SCAMPER_PING_REPLY_FROM_TARGET(ping, rx)) r[rc++] = rx;
  }
  ...
  u32 = 0;
  for (i = 0; i < 4; i++) {
    if (r[i]->probe_ipid == r[i]->reply_ipid) u32++;
  }
  if (u32 > 1) {
    class = IPID_ECHO;
    goto done;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (samples[r[i]->reply_ipid] > 1) u32++;
  }
  if (u32 > 1) {
    class = IPID_CONST;
    goto done;
...
static int sc_test_ping(sc_test_t *test, char *cmd, size_t len) {
  ...
  if ((tg = sc_target_findaddr(dst)) != NULL && tg->test != test) {
    if (sc_target_block(tg, test) != 0) return -1;
    return 0;
  } else if (tg == NULL) {
    if ((pt->tg = sc_target_add(dst, test)) == NULL) return -1;
  }
...
static int sc_test_scan(sc_test_t *test, char *cmd, size_t len) {
  ...
  if ((seq = sc_ipidseq_get(ps->a)) == NULL) {
    if ((test2 = sc_pingtest_new(ps->a)) == NULL) return -1;
    pt = test2->data;
    if ((pt->tg = sc_target_add(ps->a, test2)) == NULL) return -1;
    if (sc_target_block(pt->tg, test) != 0) return -1;
    return sc_test_ping(test2, cmd, len);
  }

  /* add a pointer to the test in the target tree */
  if (tg == NULL && (ps->tg = sc_target_add(ps->a, test)) == NULL) return -1;
  ...
  string_concat(cmd, len, &off,
                ...
                scamper_addr_tostr(ps->b, b, sizeof(b)), ps->pfx);
...
static int pf_data(void) {
  ...
  if ((targets = splaytree_alloc(sc_target_cmp)) == NULL) return -1;
  if ((ipidseqs = splaytree_alloc(sc_ipidseq_cmp)) == NULL) return -1;
  if ((virgin = slist_alloc()) == NULL) return -1;
  if ((waiting = heap_alloc(sc_waittest_cmp)) == NULL) return -1;
...
static int pf_read(void) {
  ...
  if ((ffilter = scamper_file_filter_alloc(&type, 1)) == NULL) goto err;
  ...
  while (scamper_file_read(in, ffilter, &type, &data) == 0) {
    ...
    if (SCAMPER_DEALIAS_METHOD_IS_PREFIXSCAN(dealias)) {
      ps = dealias->data;
      if (ps->ab != NULL && scamper_addr_prefixhosts(ps->b, ps->ab) >=
...
int main(int argc, char *argv[]) {
  ...
  if ((options & OPT_READ) != 0) return pf_read();
