...
static const char *ipid_classes[] = {"none", "incr",  "rand",
                                     "echo", "const", "unresp"};
...
static void usage(uint32_t opt_mask) {
  ...
  if (opt_mask & OPT_PPS) fprintf(stderr, "     -P: pps\n");

  if (opt_mask & OPT_ATTEMPTS) fprintf(stderr, "     -q: attempts\n");

  if (opt_mask & OPT_WAITROUND)
    fprintf(stderr, "     -r: time between rounds (seconds)\n");

  if (opt_mask & OPT_ROUNDCOUNT) fprintf(stderr, "     -R: round count\n");

  if (opt_mask & OPT_LOGFILE) fprintf(stderr, "     -t: log file\n");
...
static int check_options(int argc, char *argv[]) {
  ...
  if (options & OPT_FUDGE) {
    ...
    if ((options & OPT_DUMP) == 0) fudge = 5000;
  }
...
static void status(char *format, ...) {
  ...
  snprintf(pref, sizeof(pref), "p %d, w %d, r %d", probing, heap_count(waiting),
           clist_count(rglist));
...
static sc_test_t *sc_test_alloc(int type, void *data) {
  sc_test_t *test;
  if ((test = malloc_zero(sizeof(sc_test_t))) == NULL) return NULL;
  test->type = type;
  test->data = data;
  return test;
}
...
static sc_test_t *sc_pingtest_alloc(scamper_addr_t *addr) {
  ...
  if (sc_target_add(pt->target) != 0) goto err;
...
static void sc_radargun_free(sc_radargun_t *rg) {
  ...
  if (rg->test != NULL) sc_test_free(rg->test);
  if (rg->addrs != NULL)
...
static int do_method_ping(sc_test_t *test, char **cmd_out, size_t *len_out) {
  ...
  string_concat(cmd, sizeof(cmd), &off, " -c %d -o %d %s\n", attempts + 2,
                attempts, scamper_addr_tostr(pt->ping->addr, buf, sizeof(buf)));
...
static int do_method_radargun(sc_test_t *test, char **cmd_out,
                              size_t *len_out) {
  ...
  *len_out = 0;
  ...
  if (rg->rglist_cn != NULL) {
    if (rglist_cn == rg->rglist_cn) {
      ...
      if (rglist_cn == rg->rglist_cn) rglist_cn = NULL;
    }
  ...
  string_concat(header, sizeof(header), &len,
                "dealias -m radargun -O shuffle -q %d -W %d -r %d", round_count,
                rg_waitprobe, wait_round);
  ...
  for (i = 0; i < incrc; i++) string_concat(cmd, len, &off, "%s", defs[i]);
...
static int process_ping(scamper_ping_t *ping) {
  ...
  if (c != IPID_INCR) {
    ...
    pt->ping->method++;
    if (sc_waittest(tg->test) != 0) goto err;
    return 0;
  }

  status("%s %s incr", scamper_addr_tostr(pt->ping->addr, buf, sizeof(buf)),
         method_tostr(pt->ping->method));

done:
  if (splaytree_insert(pingtree, pt->ping) == NULL) goto err;
  pt->ping = NULL;
  sc_pingtest_free(pt);
  return 0;
...
static int process_dealias(scamper_dealias_t *dealias) {
  ...
  if ((tg = sc_target_findaddr(srg->probedefs[0].dst)) == NULL) goto err;
  rg = tg->test->data;
  sc_radargun_free(rg);
...
static int addrfile_resolve(const char *str, splaytree_t *tree) {
  ...
  if ((addr = scamper_addr_resolve(AF_INET, str)) == NULL) goto err;
  ...
  if (splaytree_find(tree, addr) == NULL) {
    if (splaytree_insert(tree, addr) == NULL) goto err;
  } else {
    scamper_addr_free(addr);
  }
...
static int addrfile_line(char *line, void *param) {
  ...
  if (flags & FLAG_ROWS) {
    ...
    while (last == 0) {
      if (*ptr == '\0' || isspace(*ptr) != 0) {
        ...
        *ptr = '\0';
        ...
        if (addrfile_resolve(start, tree) != 0) goto err;
    ...
    if ((rg = sc_radargun_alloc()) == NULL) goto err;
    ...
    if (addrfile_resolve(line, tree) != 0) goto err;
  }
  return 0;
...
static int do_files(void) {
  ...
  if (fcntl_set(decode_in_fd, O_NONBLOCK) == -1) return -1;
...
static int inseq(scamper_dealias_radargun_t *rg, sc_ipid_t *pts, int ptc,
                 uint32_t limit) {
  int i;

  for (i = 0; i < ptc - 1; i++) {
    if (pts[i + 1].seq > limit) break;

    /* adjacent samples are for the same probedef. */
    if (pts[i].def == pts[i + 1].def) continue;

    /* in sequence */
    if (pts[i].ipid < pts[i + 1].ipid) continue;
...
static int points_add(scamper_dealias_t *dealias, sc_ipid_t *points,
  ...
  for (k = 0; k < rg->attempts; k++) {
    ...
    if (probe->replyc != 1) continue;
    reply = probe->replies[0];
    if (SCAMPER_DEALIAS_REPLY_FROM_TARGET(probe, reply) == 0) continue;
...
static void initialwrap(sc_ipid_t *pts, int ptc) {
  ...
  for (i = 0; i < ptc - 1; i++)
    if (pts[i].def != pts[i + 1].def) break;

  if (i == ptc - 1 || pts[i].ipid <= pts[i + 1].ipid) return;

  i++;
  def = pts[i].def;

  while (i < ptc) {
    if (pts[i].def == def) pts[i].ipid += 0x10000;
    i++;
  }
...
static sc_addr2set_t *sc_addr2set_find(splaytree_t *tree,
                                       scamper_addr_t *addr) {
  sc_addr2set_t fm;
  fm.addr = addr;
...
static sc_addr2set_t *sc_addr2set_get(splaytree_t *tree, scamper_addr_t *addr) {
  sc_addr2set_t *a2s;
  if ((a2s = sc_addr2set_find(tree, addr)) != NULL) return a2s;
  if ((a2s = malloc_zero(sizeof(sc_addr2set_t))) == NULL) goto err;
  a2s->addr = scamper_addr_use(addr);
  if (splaytree_insert(tree, a2s) == NULL) goto err;
  return a2s;
...
static int process_dealias_1(scamper_dealias_t *dealias) {
  ...
  if (!SCAMPER_DEALIAS_METHOD_IS_RADARGUN(dealias)) return 0;
  rg = dealias->data;

  if ((points = malloc(sizeof(sc_ipid_t) * rg->attempts * 2)) == NULL) goto err;
  ...
  for (i = 0; i < rg->probedefc; i++) {
    for (j = i + 1; j < rg->probedefc; j++) {
      p = 0;
      if (points_add(dealias, points, &p, &rg->probedefs[i]) == 0) break;
      if (points_add(dealias, points, &p, &rg->probedefs[j]) == 0) continue;
      ...
      if (inseq(rg, points, p, rg->attempts) != 0) {
        if ((flags & FLAG_TC) == 0) {
          ...
          if (a2s_a->set == NULL && a2s_b->set == NULL) {
            if ((addrset = sc_addrset_alloc(list)) == NULL) goto err;
            a2s_a->set = addrset;
            a2s_b->set = addrset;
            ...
            a2s_a->set = a2s_b->set;
            if (slist_tail_push(a2s_b->set->addrs, a2s_a) == NULL) goto err;
          } else if (a2s_b->set == NULL) {
            a2s_b->set = a2s_a->set;
            if (slist_tail_push(a2s_a->set->addrs, a2s_b) == NULL) goto err;
...
static int process_ping_2(scamper_ping_t *ping) {
  ...
  if ((scp = sc_ping_find(ping->dst)) == NULL) {
    if ((scp = malloc_zero(sizeof(sc_ping_t))) == NULL) goto err;
    scp->addr = scamper_addr_use(ping->dst);
    scp->class = ping_classify(ping);
    if (splaytree_insert(pingtree, scp) == NULL) goto err;
  } else {
    if ((class = ping_classify(ping)) < 0) return -1;
    if (class < scp->class) scp->class = class;
  }
...
static int do_probing(void) {
  ...
  if ((ffilter = scamper_file_filter_alloc(types, typec)) == NULL ||
      ...
      do_scamperconnect() != 0)
    return -1;
  if ((options & OPT_ADDRFILE) && do_addrfile() != 0) return -1;
  ...
  for (;;) {
    ...
    if (more > 0 && do_method() != 0) return -1;

    if (scamper_fd >= 0 && FD_ISSET(scamper_fd, &rfds) && do_scamperread() != 0)
      return -1;
...
int main(int argc, char *argv[]) {
  ...
  if (options & OPT_DUMP) return do_dump();
