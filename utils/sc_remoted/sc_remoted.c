...
static void remote_debug(const char *func, const char *format, ...) {
  ...
  if (options & OPT_DAEMON) return;

  if ((flags & FLAG_DEBUG) == 0) return;
  ...
  t = now.tv_sec;
  if ((tm = localtime(&t)) == NULL) return;
  ms = now.tv_usec / 1000;
  snprintf(ts, sizeof(ts), "[%02d:%02d:%02d:%03d]", tm->tm_hour, tm->tm_min,
           tm->tm_sec, ms);
...
static int sc_fd_read_add(sc_fd_t *fd) {
  ...
  if ((fd->flags & FD_FLAG_READ) != 0) return 0;
  ...
  if ((fd->flags & FD_FLAG_WRITE) == 0) {
    ev.events = EPOLLIN;
    if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd->fd, &ev) != 0) return -1;
  } else {
    ev.events = EPOLLIN | EPOLLOUT;
    if (epoll_ctl(epfd, EPOLL_CTL_MOD, fd->fd, &ev) != 0) return -1;
  }
#elif defined(HAVE_KQUEUE)
  EV_SET(&kev, fd->fd, EVFILT_READ, EV_ADD, 0, 0, fd);
  if (kevent(kqfd, &kev, 1, NULL, 0, NULL) != 0) return -1;
#endif
  return 0;
}
...
static int sc_fd_read_del(sc_fd_t *fd) {
  ...
  if ((fd->flags & FD_FLAG_WRITE) == 0) {
    ev.events = 0;
    if (epoll_ctl(epfd, EPOLL_CTL_DEL, fd->fd, &ev) != 0) return -1;
  } else {
    ev.events = EPOLLOUT;
    if (epoll_ctl(epfd, EPOLL_CTL_MOD, fd->fd, &ev) != 0) return -1;
  }
...
static int sc_fd_write_add(sc_fd_t *fd) {
  ...
  if ((fd->flags & FD_FLAG_WRITE) != 0) return 0;
...
static sc_fd_t *sc_fd_alloc(int fd, uint8_t type, sc_unit_t *unit) {
  sc_fd_t *sfd;
  if ((sfd = malloc_zero(sizeof(sc_fd_t))) == NULL) return NULL;
  sfd->fd = fd;
  sfd->type = type;
  sfd->unit = unit;
  return sfd;
...
static void sc_unit_free(sc_unit_t *scu) {
  if (scu == NULL) return;
  if (scu->node != NULL) dlist_node_pop(scu->list, scu->node);
  free(scu);
  return;
}
...
static sc_channel_t *sc_master_channel_find(sc_master_t *ms, uint32_t id) {
  dlist_node_t *dn;
  sc_channel_t *cn;
  for (dn = dlist_head_node(ms->channels); dn != NULL;
...
static int sc_master_inet_send(sc_master_t *ms, void *ptr, uint16_t len,
  ...
  if (ack == 0 && ms->mode == MASTER_MODE_CONNECT) return 0;
  ...
  if (sc_master_inet_write(ms, ptr, len, sequence, channel) != 0) goto err;
...
static void sc_master_inet_free(sc_master_t *ms) {
  ...
  if (ms->inet_ssl != NULL) {
    ...
    if (ms->inet_wbio != NULL) BIO_free(ms->inet_wbio);
    if (ms->inet_rbio != NULL) BIO_free(ms->inet_rbio);
  }
  ms->inet_ssl = NULL;
  ms->inet_wbio = NULL;
  ms->inet_rbio = NULL;
...
static int sc_master_unix_create(sc_master_t *ms) {
  ...
  if (sc_fd_peername(&ms->inet_fd, sab, sizeof(sab)) != 0) goto err;
  ...
  if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
    remote_debug(__func__, "could not create unix socket: %s", strerror(errno));
    goto err;
  }
...
static void sc_master_inet_write_do(sc_master_t *ms) {
  if (scamper_writebuf_write(ms->inet_fd.fd, ms->inet_wb) != 0) goto zombie;
  ...
  if (scamper_writebuf_len(ms->inet_wb) == 0) {
    ...
    if (sc_fd_write_del(&ms->inet_fd) != 0) goto zombie;
  }
...
static int sc_master_tx_rej(sc_master_t *ms) {
  ...
  if (sc_master_inet_send(ms, buf, 1, 0, 0) != 0) return -1;
  return 0;
}
...
static int sc_master_control_master(sc_master_t *ms, uint8_t *buf, size_t len) {
  ...
  if (len - off < magic_len) {
    remote_debug(__func__, "len %u - off %u < magic_len %u", len, off,
                 magic_len);
    goto err;
  }
  off += magic_len;
  ...
  if (off < len && (monitorname_len = buf[off++]) > 0) {
    if (off + monitorname_len > len) {
      remote_debug(__func__,
                   "malformed monitorname length variable: %u + %u > %u", off,
                   monitorname_len, len);
      goto err;
    }
    monitorname = (char *)(buf + off);
    for (u8 = 0; u8 < monitorname_len - 1; u8++) {
      if (isalnum(monitorname[u8]) == 0 && monitorname[u8] != '.' &&
          monitorname[u8] != '-')
        goto err;
    }
    if (monitorname[monitorname_len - 1] != '\0') goto err;
  ...
  if (sc_master_unix_create(ms) != 0) goto err;
 ...
 *
 */
static int sc_master_control_channel_fin(sc_master_t *ms, uint8_t *buf,
                                         size_t len) {
  sc_channel_t *cn;
  uint32_t id;
...
static int sc_master_control_keepalive(sc_master_t *ms, uint8_t *buf,
                                       size_t len) {
  if (len != 0) {
...
static int sc_master_control_resume(sc_master_t *ms, uint8_t *buf, size_t len) {
  ...
  rcv_nxt = bytes_ntohl(buf + off);
  off += 4;
  snd_una = bytes_ntohl(buf + off);
  off += 4;
  snd_nxt = bytes_ntohl(buf + off);
  off += 4;
  assert(off <= len);
  ...
  if ((ms2 = splaytree_find(mstree, &fm)) == NULL) {
    ...
    if (sc_master_tx_rej(ms) != 0) goto err;
    goto done;
  }
  ...
  if (ms2->snd_nxt != rcv_nxt) {
    for (sn = slist_head_node(ms2->messages); sn != NULL;
         sn = slist_node_next(sn)) {
      msg = slist_node_item(sn);
      if (msg->sequence == rcv_nxt) break;
    }
    if (sn == NULL) {
      remote_debug(__func__, "rcv_nxt value %u expected %u", rcv_nxt,
                   ms2->snd_nxt);
      if (sc_master_tx_rej(ms) != 0) goto err;
      goto done;
    }
  ...
  if (SEQ_GT(snd_una, ms2->rcv_nxt) || SEQ_GT(ms2->rcv_nxt, snd_nxt)) {
    remote_debug(__func__, "scamper's send window %u:%u not expected %u",
  ...
  ms->inet_fd.fd = -1;
  ms2->inet_wb = ms->inet_wb;
  ms->inet_wb = NULL;
#ifdef HAVE_OPENSSL
  ms2->inet_mode = ms->inet_mode;
  ms2->inet_ssl = ms->inet_ssl;
  ms->inet_ssl = NULL;
  ms2->inet_rbio = ms->inet_rbio;
  ms->inet_rbio = NULL;
  ms2->inet_wbio = ms->inet_wbio;
  ms->inet_wbio = NULL;
#endif
  ...
  if (sc_master_unix_create(ms2) != 0) goto err;
  ...
  if (sc_master_inet_write(ms2, ok, 5, 0, 0) != 0) goto err;
...
static int sc_master_control(sc_master_t *ms) {
  ...
  if (ms->mode == MASTER_MODE_CONNECT) {
    ...
    if (type == CONTROL_CHANNEL_FIN) {
      if (sc_master_tx_ack(ms, seq) != 0) return -1;
      ms->rcv_nxt++;
    }
...
static void sc_master_inet_read_cb(sc_master_t *ms, uint8_t *buf, size_t len) {
  ...
  while (off < len) {
    ...
    if (id == 0) {
      if (sc_master_control(ms) != 0) goto err;
      continue;
    }

    if (sc_master_tx_ack(ms, seq) != 0) goto err;
    ms->rcv_nxt++;
  ...
  if (ms->inet_ssl != NULL) {
    ...
    if ((rc = SSL_get_error(ms->inet_ssl, rc)) == SSL_ERROR_WANT_READ) {
      ...
      remote_debug(__func__, "mode %s rc %d",
                   ms->inet_mode == SSL_MODE_ACCEPT ? "accept" : "estab", rc);
      goto err;
    }
...
static void sc_master_unix_accept_do(sc_master_t *ms) {
  ...
  if ((cn = malloc_zero(sizeof(sc_channel_t))) == NULL) goto err;
  cn->id = ms->next_channel++;
  if (ms->next_channel == 0) ms->next_channel++;
  ...
  if ((cn->unix_wb = scamper_writebuf_alloc()) == NULL) goto err;
  if ((cn->node = dlist_tail_push(ms->channels, cn)) == NULL) goto err;
  cn->master = ms;

  /* send a new channel message to scamper. expect an acknowledgement */
  msg[0] = CONTROL_CHANNEL_NEW;
  bytes_htonl(msg + 1, cn->id);
  if (sc_master_inet_send(ms, msg, 1 + 4, 0, 1) != 0) goto err;
...
static sc_master_t *sc_master_alloc(int fd) {
  ...
  if ((ms = malloc_zero(sizeof(sc_master_t))) == NULL) return NULL;
  ms->inet_fd.fd = fd;
  fd = -1;
  ...
  if (tls_certfile != NULL) {
    ...
    if (ssl_want_read(ms) < 0) goto err;
  }
#endif
...
static void sc_channel_unix_write_do(sc_channel_t *cn) {
  ...
  if (cn->unix_fd == NULL) return;

  if (scamper_writebuf_write(cn->unix_fd->fd, cn->unix_wb) != 0) {
    remote_debug(__func__, "write to %s channel %u failed", cn->master->name,
                 cn->id);
    goto err;
  }
   ...
   * write again
   */
  if ((gtzero = scamper_writebuf_gtzero(cn->unix_wb)) != 0) return;
  ...
  if ((cn->flags & CHANNEL_FLAG_EOF_RX) != 0) {
    remote_debug(__func__, "received EOF for %s channel %u", cn->master->name,
                 cn->id);
    sc_unit_gc(cn->unit);
    return;
  }
...
err:
  ...
  sc_fd_free(cn->unix_fd);
  cn->unix_fd = NULL;
  scamper_writebuf_free(cn->unix_wb);
  cn->unix_wb = NULL;
...
static void sc_channel_unix_read_do(sc_channel_t *cn) {
  ...
  if ((rc = read(cn->unix_fd->fd, buf, sizeof(buf))) <= 0) {
    if (rc == -1 && (errno == EAGAIN || errno == EINTR)) return;
...
static int serversocket_init_sa(const struct sockaddr *sa) {
  ...
  if (sa->sa_family == PF_INET6) {
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&opt, sizeof(opt)) !=
        0) {
      remote_debug(__func__, "could not set IPV6_V6ONLY: %s", strerror(errno));
      goto err;
    }
  }
#endif
...
static int serversocket_init(void) {
  ...
  if (ss_addr != NULL) {
    ...
    if ((fd = serversocket_init_sa((struct sockaddr *)&sas)) == -1) return -1;
    serversockets[0] = fd;
    return 0;
  }
...
static int unixdomain_direxists(void) {
  ...
  if ((sb.st_mode & S_IFDIR) != 0) return 0;
  usage(OPT_UNIX);
...
static void cleanup(void) {
  ...
  for (i = 0; i < 2; i++) close(serversockets[i]);

  if (ss_addr != NULL) free(ss_addr);
  if (mslist != NULL) dlist_free_cb(mslist, (dlist_free_t)sc_master_free);
  if (mstree != NULL) splaytree_free(mstree, NULL);
...
static int remoted_tlsctx(void) {
  if ((tls_ctx = SSL_CTX_new(SSLv23_method())) == NULL) return -1;
  ...
  if (SSL_CTX_use_PrivateKey_file(tls_ctx, tls_privfile, SSL_FILETYPE_PEM) !=
      1) {
...
#endif
{
  ...
  for (i = 0; i < 2; i++) {
    if (serversockets[i] == -1) continue;
    scfd_ss[i].type = FD_TYPE_SERVER;
    scfd_ss[i].fd = serversockets[i];
    if (sc_fd_read_add(&scfd_ss[i]) != 0) return -1;
  }
  ...
  while (stop == 0) {
    if (reload != 0) {
      reload = 0;
#ifdef HAVE_OPENSSL
      if (remoted_tlsctx_reload() != 0) return -1;
#endif
    }
    ...
    if ((dn = dlist_head_node(mslist)) != NULL) {
      ...
      set_timeout:
        if (rc == 0) {
          ...
          if (timeval_cmp(&tv, &to) < 0) timeval_cpy(&to, &tv);
        }
      }
    }
    ...
    if (rc != 0) {
      ...
      if (timeout == 0 && to.tv_usec != 0) timeout++;
    }
...
static int select_loop(void) {
  ...
  while (stop == 0) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (serversockets[i] > nfds) nfds = serversockets[i];
    }
    ...
    if ((dn = dlist_head_node(mslist)) != NULL) {
      ...
      while (dn != NULL) {
        ...
        if (ms->inet_fd.fd != -1) {
          ...
          if (ms->inet_fd.fd > nfds) nfds = ms->inet_fd.fd;
          if (scamper_writebuf_len(ms->inet_wb) > 0) {
    ...
    if (count > 0) {
      for (i = 0; i < 2; i++) {
        if (serversockets[i] != -1 && FD_ISSET(serversockets[i], &rfds) &&
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 2; i++) serversockets[i] = -1;

  atexit(cleanup);

  if (check_options(argc, argv) != 0) return -1;

  if (pidfile != NULL && remoted_pidfile() != 0) return -1;

#ifdef HAVE_OPENSSL
  if (tls_certfile != NULL) {
    SSL_library_init();
    SSL_load_error_strings();
    if (remoted_tlsctx() != 0) return -1;
  }
#endif
  ...
  if (unixdomain_direxists() != 0 || serversocket_init() != 0) return -1;
...
#if defined(HAVE_EPOLL)
  if ((flags & FLAG_SELECT) == 0) return epoll_loop();
#elif defined(HAVE_KQUEUE)
  if ((flags & FLAG_SELECT) == 0) return kqueue_loop();
#endif
