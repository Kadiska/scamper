...
static const char *step_names[] = {"classify", "descend", "overlap", "descend2",
                                   "ally"};
...
static const sc_dump_t dump_funcs[] = {
    {NULL, NULL, NULL, NULL},
    {"dump transitive closure from ally probes", process_1_ping, process_1_ally,
     finish_1},
    {"dump interface classification", process_2_ping, NULL, NULL},
    {"summary table of per-stage statistics", process_3_ping, process_3_ally,
     finish_3},
    {"dump transitive closure from descend and overlap stages", process_4_ping,
     NULL, finish_4},
};
...
static void usage(uint32_t opt_mask) {
  ...
  fprintf(stderr,
          "usage: sc_speedtrap [-a addr-file] [-o outfile] [-p [ip:]port] [-U "
          "unix]\n"
          "                    [-I] [-l log] [-s stop]\n"
#ifdef HAVE_PTHREAD
          "                    [-t thread-count]\n"
  ...
  if (opt_mask & OPT_STOP) {
    fprintf(stderr,
            ...
            "        [%s",
            step_names[0]);
    for (i = 1; i < step_namec; i++) fprintf(stderr, "|%s", step_names[i]);
    fprintf(stderr, "]\n");
  }
...
static int check_options(int argc, char *argv[]) {
  ...
  if ((options & (OPT_ADDRFILE | OPT_OUTFILE | OPT_DUMP)) !=
          (OPT_ADDRFILE | OPT_OUTFILE) &&
...
static int mode_ok(int m) {
  if (stop_stepname == NULL) return 1;
  if (m > stop_stepid) return 0;
  return 1;
}
...
static void logprint(int st, char *format, ...) {
  ...
  if (logfile != NULL) {
    if (st != 0) {
      ...
      fprintf(logfile, "%ld: p %d, w %d, l %d, b %d : ", (long int)now.tv_sec,
...
static int uint32_find(uint32_t *a, size_t len, uint32_t u32) {
  if (bsearch(&u32, a, len, sizeof(uint32_t), uint32_cmp) != NULL) return 1;
  return 0;
}

static int ipid_inseq3(uint64_t a, uint64_t b, uint64_t c) {
  if (a == b || b == c || a == c) return 0;
  if (a > b) b += 0x100000000ULL;
  if (a > c) c += 0x100000000ULL;
  if (a > b || b > c) return 0;
...
static int ipid_incr(uint32_t *ipids, int ipidc) {
  ...
  if (ipidc < 3) return 0;
  for (i = 2; i < ipidc; i++)
    if (ipid_inseq3(ipids[i - 2], ipids[i - 1], ipids[i]) == 0) return 0;
  return 1;
}
...
static sc_addr2router_t *sc_addr2router_alloc(scamper_addr_t *a,
                                              sc_router_t *r) {
  sc_addr2router_t *a2r = NULL;
  if ((a2r = malloc_zero(sizeof(sc_addr2router_t))) == NULL) goto err;
  a2r->addr = scamper_addr_use(a);
  a2r->router = r;
  if (splaytree_insert(addr2routers, a2r) == NULL) goto err;
  return a2r;
...
static sc_notaliases_t *sc_notaliases_alloc(sc_router_t *r) {
  ...
  if ((na = malloc_zero(sizeof(sc_notaliases_t))) == NULL ||
    ...
    goto err;
  na->r = r;
  if ((na->tree_node = splaytree_insert(notaliases, na)) == NULL) goto err;
  return na;
err:
 ...
 */
static int sc_notaliases_merge_r(sc_notaliases_t *na, sc_router_t *b,
                                 sc_router_t *c) {
  sc_notaliases_t *nc;
...
static int sc_notaliases_merge(sc_router_t *a, sc_router_t *b) {
  ...
  if ((nb = sc_notaliases_find(b)) == NULL) return 0;

  if ((list = slist_alloc()) == NULL || ((na = sc_notaliases_find(a)) == NULL &&
                                         (na = sc_notaliases_alloc(a)) == NULL))
    goto done;

  splaytree_inorder(nb->tree, tree_to_slist, list);
  while ((c = slist_head_pop(list)) != NULL) {
    if (sc_notaliases_merge_r(na, b, c) != 0) goto done;
  }
...
static int sc_wait_target(struct timeval *tv, sc_target_t *target) {
  sc_wait_t *w;
  if ((w = malloc_zero(sizeof(sc_wait_t))) == NULL) return -1;
  timeval_cpy(&w->tv, tv);
  w->un.target = target;
  if (heap_insert(waiting, w) == NULL) return -1;
  return 0;
}
...
static sc_target_t *sc_target_free(sc_target_t *tg) {
  ...
  if (tg->samples != NULL) slist_free_cb(tg->samples, free);
  if (tg->tree_node != NULL) splaytree_remove_node(targets, tg->tree_node);
...
static void sc_targetset_logprint(const sc_targetset_t *ts) {
  ...
  for (ss = slist_head_node(ts->targets); ss != NULL;
       ss = slist_node_next(ss)) {
...
static int pairwise_test(sc_targetipid_t **tis, int tc,
  ...
  for (i = 0; i < tc; i++) {
    ...
    ti = tis[i];
    if (uint32_find(pairwise_u32, ipidc, ti->ipid) != 0) return 0;
    ...
    if (st[si][1] == NULL || st[sj][1] == NULL) goto next;
...
static int pairwise(sc_target_t *ta, sc_target_t *tb) {
  ...
  if (tc > pairwise_max) {
    ...
    if (realloc_wrap((void **)&pairwise_tipid, len) != 0) return -1;
    len = tc * sizeof(uint32_t);
    if (realloc_wrap((void **)&pairwise_uint32, len) != 0) return -1;
    pairwise_max = tc;
  }
...
static void pairwise_all_thread(void *param) {
  ...
  for (sb = pw->sb; sb != NULL; sb = slist_node_next(sb)) {
    ...
    if (slist_count(tb->samples) == 0) continue;

    tc = slist_count(ta->samples) + slist_count(tb->samples);
    if (tc > pairwise_max) {
      len = tc * sizeof(sc_targetipid_t *);
      if (realloc_wrap((void **)&pairwise_tipid, len) != 0) goto done;
      len = tc * sizeof(uint32_t);
      if (realloc_wrap((void **)&pairwise_uint32, len) != 0) goto done;
      pairwise_max = tc;
    }

    tc = 0;
    for (ss = slist_head_node(ta->samples); ss != NULL;
         ss = slist_node_next(ss))
      pairwise_tipid[tc++] = slist_node_item(ss);
    for (ss = slist_head_node(tb->samples); ss != NULL;
         ss = slist_node_next(ss))
      pairwise_tipid[tc++] = slist_node_item(ss);
    array_qsort((void **)pairwise_tipid, tc, (array_cmp_t)sc_targetipid_tx_cmp);
...
#endif
    if (slist_tail_push(candidates, ts) == NULL) goto done;
    ts = NULL;
#if defined(HAVE_PTHREAD)
...
#if defined(HAVE_PTHREAD)
  if (candidates_mutex_held != 0) pthread_mutex_unlock(&candidates_mutex);
#endif
  if (pairwise_tipid != NULL) free(pairwise_tipid);
  if (pairwise_uint32 != NULL) free(pairwise_uint32);
  if (list != NULL) slist_free(list);
  if (ts != NULL) sc_targetset_free(ts);
  free(pw);
  return;
}
...
#if defined(HAVE_PTHREAD)
  if (pthread_mutex_init(&candidates_mutex, NULL) != 0) goto done;
  candidates_mutex_ok = 1;
#endif

  for (sa = slist_head_node(targets); sa != NULL; sa = slist_node_next(sa)) {
    ta = slist_node_item(sa);
    if (slist_count(ta->samples) <= 0) continue;
    if ((pw = malloc(sizeof(sc_pairwise_t))) == NULL) goto done;
    pw->sb = slist_node_next(sa);
    pw->ta = ta;
    if (threadpool_tail_push(tp, pairwise_all_thread, pw) != 0) goto done;
    pw = NULL;
  }
...
#if defined(HAVE_PTHREAD)
  if (candidates_mutex_ok != 0) pthread_mutex_destroy(&candidates_mutex);
#endif
...
static int test_ping6(sc_target_t *target, char *cmd, size_t len) {
  size_t off = 0;
  char buf[64];
  string_concat(cmd, len, &off, "ping -O dl -U %d -c 6 -s 1300 -M 1280 %s\n",
...
static sc_target_t *target_classify(void) { return slist_head_pop(probelist); }

static sc_target_t *target_descend(void) { return slist_head_pop(probelist); }
...
static sc_target_t *target_overlap(void) {
  ...
  for (;;) {
    if ((ts = slist_head_pop(probelist)) == NULL) return NULL;
    for (dn = dlist_head_node(overlap_act); dn != NULL;
         dn = dlist_node_next(dn)) {
      tt = dlist_node_item(dn);
      if (timeval_overlap(&ts->min, &ts->max, &tt->min, &tt->max) != 0) break;
    }
    if (dn == NULL) break;
    if (slist_tail_push(tt->blocked, ts) == NULL) return NULL;
    blocked++;
  }

  if ((ts->node = dlist_tail_push(overlap_act, ts)) == NULL) return NULL;
...
static sc_target_t *target_descend2(void) { return slist_head_pop(probelist); }
...
static sc_targetset_t *targetset_ally(void) {
  ...
  while ((ts = heap_remove(probeheap)) != NULL) {
     ...
     * these addresses
     */
    for (sn = slist_head_node(ts->targets); sn != NULL;
         sn = slist_node_next(sn)) {
      tg = slist_node_item(sn);
      if (splaytree_find(targets, tg) != NULL) break;
    }
    ...
    a2r_a = sc_addr2router_find(tg->addr);
    assert(a2r_a != NULL);
    sn = slist_node_next(slist_head_node(ts->targets));
    assert(sn != NULL);
    while (sn != NULL) {
      tg = slist_node_item(sn);
      a2r_b = sc_addr2router_find(tg->addr);
      assert(a2r_b != NULL);
...
static int do_method_ally(void) {
  ...
  string_concat(cmd, sizeof(cmd), &off,
                "dealias -U %d -m ally -f %u -w 2 -W 1000 -p '%s' %s", mode,
                fudge, "-P icmp-echo -s 1300 -M 1280",
                scamper_addr_tostr(tg->addr, addr, sizeof(addr)));
...
static int finish_classify(void) {
   ...
   */
  if (mode_ok(MODE_DESCEND) == 0) return 0;
  mode = MODE_DESCEND;
  ...
  if ((descend = slist_alloc()) == NULL) return -1;
  return 0;
}
...
static int finish_descend(void) {
   ...
   * after descend stage
   */
  if (mode_ok(MODE_OVERLAP) == 0) return 0;
  mode = MODE_OVERLAP;
  ...
  while ((ti = slist_head_pop(descend)) != NULL) {
    if (ts == NULL || timeval_cmp(&tv, &ti->tx) <= 0) {
      ...
      if (timeval_cmp(&ts->max, &ti->rx) < 0) timeval_cpy(&ts->max, &ti->rx);
    }
...
static int reply_descend(sc_target_t *target, sc_targetipid_t *ipids,
  ...
  if (ipidc == 0) {
    if (target->attempt >= 2) goto done;
    target->attempt++;
    timeval_add_s(&tv, &now, 1);
    if (sc_wait_target(&tv, target) != 0) return -1;
    return 0;
  }

  if ((ti = sc_target_sample(target, &ipids[0])) == NULL) return -1;
  if (slist_tail_push(descend, ti) == NULL) return -1;
...
static int finish_overlap(void) {
   ...
   * after overlap stage
   */
  if (mode_ok(MODE_DESCEND2) == 0) return 0;
  mode = MODE_DESCEND2;
  for (sn = slist_head_node(incr); sn != NULL; sn = slist_node_next(sn)) {
    target = slist_node_item(sn);
    target->attempt = 0;
...
static int finish_descend2(void) {
  ...
  if (pairwise_all(incr) != 0) return -1;
  ...
  for (sa = slist_head_node(candidates); sa != NULL; sa = slist_node_next(sa)) {
    ...
    for (sb = slist_head_node(ts->targets); sb != NULL;
         sb = slist_node_next(sb)) {
      tg = slist_node_item(sb);
      if ((a2r = sc_addr2router_find(tg->addr)) != NULL) continue;
  ...
  if ((probeheap = heap_alloc((heap_cmp_t)sc_targetset_targetc_desc_cmp)) ==
      NULL)
    return -1;
  while ((ts = slist_head_pop(candidates)) != NULL) {
    if (heap_insert(probeheap, ts) == NULL) return -1;
  }
...
static int reply_descend2(sc_target_t *target, sc_targetipid_t *ipids,
  ...
  if (ipidc > 0 && sc_target_sample(target, &ipids[0]) == NULL) return -1;

  return finish_descend2();
}

static int do_decoderead_ping(scamper_ping_t *ping) {
  static int (*const func[])(sc_target_t *, sc_targetipid_t *, uint16_t,
                             uint16_t) = {
      reply_classify,
      reply_descend,
      reply_overlap,
...
static int do_decoderead_dealias(scamper_dealias_t *dealias) {
  ...
  assert(a2r_a != NULL);
  assert(a2r_a->router != NULL);
  a2r_b = sc_addr2router_find(b);
  assert(a2r_b != NULL);
  assert(a2r_b->router != NULL);
  ...
  if (dealias->result == SCAMPER_DEALIAS_RESULT_ALIASES) {
    ...
    if (a2r_a->router != a2r_b->router) {
      ...
      if (sc_notaliases_merge(a2r_a->router, a2r_b->router) != 0) goto done;

      /* copy across b's aliases into a */
      list = a2r_b->router->addrs;
      a2r_b->router->addrs = NULL;
      sc_router_free(a2r_b->router);
      a2r_b->router = NULL;
    ...
    if (a2r_a->router != a2r_b->router) {
      if (sc_notaliases_add(a2r_a->router, a2r_b->router) != 0) goto done;
    }
  } else if (dealias->result == SCAMPER_DEALIAS_RESULT_NONE &&
             ts->attempt < 2) {
    ts->attempt++;
    timeval_add_s(&tv, &now, 1);
    if (sc_wait_targetset(&tv, ts) == 0) rc = 0;
    goto done;
  }
  ...
  for (;;) {
    ...
    if ((ts->next = slist_node_next(ts->next)) == NULL) {
      ...
      while ((tg = slist_head_pop(ts->blocked)) != NULL) {
        if (heap_insert(probeheap, tg) == NULL) goto done;
        blocked--;
      }
    ...
    tg = slist_node_item(ts->next);
    a2r_c = sc_addr2router_find(tg->addr);
    assert(a2r_c != NULL);
...
static int do_scamperread_line(void *param, uint8_t *buf, size_t linelen) {
  ...
  if (data_left > 0) {
    ...
    if (uus != 0) scamper_writebuf_send(decode_wb, uu, uus);
    data_left -= (linelen + 1);
    return 0;
  }

  /* feedback letting us know that the command was accepted */
  if (linelen >= 2 && strncasecmp(head, "OK", 2) == 0) return 0;
...
static int do_addressfile(void) {
  ...
  while ((addr = slist_head_pop(list)) != NULL) {
    if ((target = sc_target_alloc(addr)) == NULL) goto done;
...
static int speedtrap_data_select(void) {
  ...
  for (;;) {
    ...
    FD_ZERO(&wfds);
    wfdsp = NULL;
    if (scamper_fd < 0 && decode_in_fd < 0) break;
    ...
    if (scamper_fd >= 0) {
      if (FD_ISSET(scamper_fd, &rfds) && do_scamperread() != 0) return -1;
    ...
    if (decode_in_fd >= 0) {
      if (FD_ISSET(decode_in_fd, &rfds) && do_decoderead() != 0) return -1;
    }
...
static int speedtrap_data(void) {
  ...
  if ((targets = splaytree_alloc((splaytree_cmp_t)sc_target_addr_cmp)) ==
          NULL ||
      (waiting = heap_alloc((heap_cmp_t)sc_wait_cmp)) == NULL ||
      (probelist = slist_alloc()) == NULL || (incr = slist_alloc()) == NULL ||
      (candidates = slist_alloc()) == NULL ||
      (notaliases = splaytree_alloc((splaytree_cmp_t)sc_notaliases_cmp)) ==
          NULL ||
      (overlap_act = dlist_alloc()) == NULL || do_addressfile() != 0 ||
      do_scamperconnect() != 0 ||
      (scamper_lp = scamper_linepoll_alloc(do_scamperread_line, NULL)) ==
          NULL ||
...
static int ping_read(const scamper_ping_t *ping, uint32_t *ipids, int *ipidc,
                     int *replyc) {
  scamper_ping_reply_t *reply;
  ...
  *replyc = 0;
  ...
  for (i = 0; i < ping->ping_sent; i++) {
    if ((reply = ping->ping_replies[i]) == NULL) continue;
    if (SCAMPER_PING_REPLY_IS_ICMP_ECHO_REPLY(reply) == 0) continue;
    (*replyc)++;
    if (reply->flags & SCAMPER_PING_REPLY_FLAG_REPLY_IPID) {
      if (*ipidc == maxipidc) return -1;
...
static int process_1_ping(const scamper_ping_t *ping) {
  ...
  if (ping->userid != 0) return 0;

  ipidc = sizeof(ipids) / sizeof(uint32_t);
  if (ping_read(ping, ipids, &ipidc, &replyc) != 0) return -1;
  if (ipid_incr(ipids, ipidc) == 0) return 0;

  if (sc_addr2router_find(ping->dst) != NULL) return 0;
...
static int process_1_ally(const scamper_dealias_t *dealias) {
  ...
  if (dealias->result != SCAMPER_DEALIAS_RESULT_ALIASES) return 0;
...
static void finish_3(void) {
  ...
  for (i = 0; i < 6; i++) {
    if (d3_states_probec[i] == 0) continue;
...
static int process_4_ping(const scamper_ping_t *ping) {
  ...
  if (ping->userid != MODE_DESCEND && ping->userid != MODE_OVERLAP &&
  ...
  for (u16 = 0; u16 < ping->ping_sent; u16++) {
    ...
    if (sc_target_sample(target, &ti) == NULL) return -1;
  }
...
static int speedtrap_init(void) {
  ...
  if ((ffilter = scamper_file_filter_alloc(types, typec)) == NULL) return -1;
  ...
  if ((addr2routers = splaytree_alloc((splaytree_cmp_t)sc_addr2router_cmp)) ==
          NULL ||
...
int main(int argc, char *argv[]) {
  ...
  if (speedtrap_init() != 0) return -1;

  if (options & OPT_DUMP) return speedtrap_read();
