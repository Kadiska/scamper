...
static char *methods[] = {
    "-t blind-rst",  "-t blind-rst -o -70000",
    "-t blind-syn",  "-t blind-syn -o -70000",
    "-t blind-data", "-t blind-data -o 70000",
};
...
static void usage(uint32_t opt_mask) {
  fprintf(stderr,
          ...
          "                    [-A app] [-c comp] [-l limit] [-t logfile] [-T "
          "ttl]\n"
          "                    [-O options] [-w wait]\n");
  return;
}
...
static void target_free(target_t *target) {
  ...
  if (target->methods != NULL) slist_free(target->methods);
...
static int do_method(void) {
  ...
  if ((target = heap_head_item(heap)) != NULL &&
    ...
    if ((target->methods = slist_alloc()) == NULL) return -1;
    for (i = 0; i < methodc; i++)
      if (slist_tail_push(target->methods, methods[i]) == NULL) return -1;
    if (noshuffle == 0) slist_shuffle(target->methods);
  } else {
    return 0;
  }
  ...
  if (tbit_app == SCAMPER_TBIT_APP_HTTP) {
    if (string_firstof_char(target->un.url, '\'') == NULL)
      ...
      string_concat(cmd, sizeof(cmd), &off, "tbit -u \"%s\"", target->un.url);
    if (ttl != 0) string_concat(cmd, sizeof(cmd), &off, " -T %d", ttl);
  } else if (tbit_app == SCAMPER_TBIT_APP_BGP) {
...
static int parse_list(char *str, void *param) {
  ...
  if (tbit_app == SCAMPER_TBIT_APP_HTTP) {
    ...
    *ptr = '\0';
    ...
    if (string_tolong(asn, &lo) != 0 || lo < 1 || lo > 65535) return -1;
...
static int process_tbit(target_t *target, scamper_tbit_t *tbit) {
  ...
  if (target->mode == MODE_BLIND) {
    if (slist_count(target->methods) == 0) target->mode++;
  } else {
    target->mode++;
  }

  if (target->mode > MODE_LAST) goto completed;
...
int main(int argc, char *argv[]) {
  ...
  if ((options & OPT_RANDOM) != 0) {
    random_seed();
    if (noshuffle == 0) slist_shuffle(list);
  }
