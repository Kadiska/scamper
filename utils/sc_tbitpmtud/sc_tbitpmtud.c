...
static const sc_dump_t dump_funcs[] = {
    ...
    {"dump results by MSS", "mssresults", init_1, process_1_tbit, NULL,
     finish_1},
    {"dump results by ASN", "asnresults", init_2, process_2_tbit, NULL,
     finish_2},
};
...
static void usage(uint32_t opts) {
  ...
  if (opts & OPT_TEXT) fprintf(stderr, "       -t: output log file\n");
  if (opts & OPT_WINDOW)
    ...
    switch (ch) {
      ...
      case 'A':
        ...
        if (ip2as_files == NULL && (ip2as_files = slist_alloc()) == NULL)
          return -1;
        if (slist_tail_push(ip2as_files, optarg) == NULL) return -1;
        break;
...
static void target_free(target_t *target) {
  ...
  if (target->urls != NULL) {
    while ((url = heap_remove(target->urls)) != NULL) target_url_free(url);
    heap_free(target->urls, NULL);
  }

  if (target->hn != NULL) heap_delete(heap, target->hn);

  if (target->tn != NULL) splaytree_remove_node(tree, target->tn);
...
static int do_method(void) {
  ...
  if (target->tn == NULL &&
      (target->tn = splaytree_insert(tree, target)) == NULL) {
    return -1;
  }
  ...
  if (target->mode == MODE_PING) {
    ...
    string_concat(command_buf, sizeof(command_buf), &off, "tbit -t pmtud -U %d",
                  target->pos);
  ...
  print("p %d/%d, v %d : %s", probing, splaytree_count(tree), slist_count(list),
        command_buf);
...
static int parse_list(char *str, void *param) {
  ...
  *ptr = '\0';
  ...
  if (strlen(url) > 300) return 0;

  if (string_firstof_char(url, '\'') != NULL &&
      string_firstof_char(url, '\"') != NULL)
    return 0;

  if ((i = atoi(pos)) < 0) return -1;

  if ((tf.addr = scamper_addr_resolve(AF_UNSPEC, ip)) == NULL) return -1;
  ...
  if ((target = splaytree_find(tree, &tf)) == NULL) {
    if ((target = malloc_zero(sizeof(target_t))) == NULL) return -1;
    if ((target->urls = heap_alloc(target_url_size_cmp)) == NULL) return -1;
    ...
    if (splaytree_insert(tree, target) == NULL) return -1;

    if (slist_tail_push(list, target) == NULL) return -1;
  }

  scamper_addr_free(tf.addr);

  if ((uint32_t)i < target->pos) target->pos = i;

  if ((tu = malloc_zero(sizeof(target_url_t))) == NULL ||
      (tu->url = strdup(url)) == NULL)
    return -1;
  tu->size = atoi(size);
  if (heap_insert(target->urls, tu) == NULL) return -1;
...
static int do_files(void) {
  ...
  if ((decode_in = scamper_file_openfd(decode_in_fd, NULL, 'r', "warts")) ==
      NULL) {
    return -1;
  }
...
static int process_tbit(target_t *target, scamper_tbit_t *tbit) {
  ...
  if (target->addr->type == SCAMPER_ADDR_TYPE_IPV6) ipv6 = 1;
  ...
  if (target->mtu == 1480) {
    if (ipv6 == 1 && mtu == 0) target->mss = 1380;
    target->mtu = 1280;
  } else if (target->mtu == 1280) {
    if (ipv6 == 1 && mtu == 0) target->mss = 1220;
    target->mtu = 576;
...
static int do_decoderead(void) {
  ...
  if (outfile_obj >= limit) {
    ...
    if ((outfile = scamper_file_open(buf, 'w', "warts")) == NULL) return -1;
  }

  if (type == SCAMPER_FILE_OBJ_PING) {
    ping = (scamper_ping_t *)data;
    findme.addr = ping->dst;
    if (scamper_file_write_ping(outfile, ping) != 0) return -1;
    outfile_obj++;
  } else if (type == SCAMPER_FILE_OBJ_TBIT) {
    tbit = (scamper_tbit_t *)data;
    findme.addr = tbit->dst;
    if (scamper_file_write_tbit(outfile, tbit) != 0) return -1;
    outfile_obj++;
  } else
    return -1;
...
static int pmtud_data(void) {
  ...
  if ((tree = splaytree_alloc(target_addr_cmp)) == NULL) return -1;
  if ((heap = heap_alloc(target_next_cmp)) == NULL) return -1;
  heap_onremove(heap, target_onremove);
  if ((list = slist_alloc()) == NULL) return -1;
  if (file_lines(addressfile, parse_list, NULL) != 0) return -1;
  ...
  if (do_scamperconnect() != 0) return -1;
  if (do_files() != 0) return -1;
  ...
  for (;;) {
    ...
    if (splaytree_count(tree) == 0 && slist_count(list) == 0) break;
    ...
    if (FD_ISSET(decode_in_fd, &rfds)) {
      if (do_decoderead() != 0) return -1;
    }

    if (FD_ISSET(scamper_fd, &rfds)) {
      if (do_scamperread() != 0) return -1;
    }
...
static int ip2as_line(char *line, void *param) {
  ...
  *a = '\0';
  ...
  if ((p->af == AF_INET && prefixtree_insert4(ip2as_pt_4, p->pfx.v4) == NULL) ||
      (p->af == AF_INET6 && prefixtree_insert6(ip2as_pt_6, p->pfx.v6) == NULL))
    goto err;
...
static sc_prefix_t *sc_prefix_find(scamper_addr_t *addr) {
  ...
  if (addr->type == SCAMPER_ADDR_TYPE_IPV4) {
    if ((p4 = prefixtree_find_ip4(ip2as_pt_4, addr->addr)) == NULL) return NULL;
    return p4->ptr;
  } else if (addr->type == SCAMPER_ADDR_TYPE_IPV6) {
    if ((p6 = prefixtree_find_ip6(ip2as_pt_6, addr->addr)) == NULL) return NULL;
    return p6->ptr;
  }
...
static int do_ip2as(void) {
  ...
  if (ip2as_files == NULL) return -1;
  if ((ip2as_pt_4 = prefixtree_alloc(AF_INET)) == NULL) return -1;
  if ((ip2as_pt_6 = prefixtree_alloc(AF_INET6)) == NULL) return -1;
  if ((asmaptree = splaytree_alloc((splaytree_cmp_t)sc_asmap_cmp)) == NULL)
    return -1;

  while ((name = slist_head_pop(ip2as_files)) != NULL) {
    if (file_lines(name, ip2as_line, NULL) != 0) return -1;
  }
...
static char *percentage(char *buf, size_t len, uint32_t a, uint32_t x) {
  ...
  if (x == 0)
    ...
    string_concat(buf, len, &off, "100%%");
  else
    string_concat(buf, len, &off, "%.1f%%", (float)(a * 100) / x);
  return buf;
}
...
static int init_1(void) {
  if ((table_1_4 = malloc_zero(sizeof(sc_mssresult_t *) * 65536)) == NULL)
    return -1;
  if ((total_1_4 = malloc_zero(sizeof(sc_mssresult_t))) == NULL) return -1;
  if ((table_1_6 = malloc_zero(sizeof(sc_mssresult_t *) * 65536)) == NULL)
    return -1;
  if ((total_1_6 = malloc_zero(sizeof(sc_mssresult_t))) == NULL) return -1;
  return 0;
}
...
static int process_1_tbit(scamper_tbit_t *tbit) {
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(tbit->dst)) {
    if ((mr = table_1_4[tbit->server_mss]) == NULL) {
      if ((mr = malloc_zero(sizeof(sc_mssresult_t))) == NULL) goto done;
...
static int finish_1(void) {
  ...
  for (v = 0; v < 2; v++) {
    ...
    printf(
        "    success   |     fail     |   toosmall   |    other    |    total");
    printf("\n");
    printf("total |");
    for (i = 0; i < 3; i++)
  ...
  for (i = 0; i < 65536; i++)
    if (table_1_4[i] != NULL) free(table_1_4[i]);
  free(table_1_4);
  for (i = 0; i < 65536; i++)
    if (table_1_6[i] != NULL) free(table_1_6[i]);
  free(table_1_6);
  free(total_1_4);
  free(total_1_6);
...
static int sc_asnresult_as_cmp(const sc_asnresult_t *a,
                               const sc_asnresult_t *b) {
  return sc_asmap_cmp(a->asmap, b->asmap);
}

static int sc_asnresult_c_cmp(const sc_asnresult_t *a,
                              const sc_asnresult_t *b) {
  if (a->count > b->count) return -1;
  if (a->count < b->count) return 1;
  return sc_asmap_cmp(a->asmap, b->asmap);
...
static sc_asnresult_t *sc_asnresult_get(splaytree_t *tree, sc_asmap_t *asmap) {
  ...
  fm.asmap = asmap;
  if ((asr = splaytree_find(tree, &fm)) != NULL) return asr;

  if ((asr = malloc_zero(sizeof(sc_asnresult_t))) == NULL) return NULL;
  asr->asmap = asmap;
...
static int process_2_tbit(scamper_tbit_t *tbit) {
  ...
  if ((asr = sc_asnresult_get(tree, pfx->asmap)) == NULL) goto done;
...
static int finish_2(void) {
  ...
  for (v = 0; v < 2; v++) {
    ...
    if (slist_count(list) == 0) continue;
...
static int pmtud_dump(void) {
  ...
  if (dump_funcs[dump_id].finish != NULL) rc = dump_funcs[dump_id].finish();
...
int pmtud_init(void) {
  ...
  if ((ffilter = scamper_file_filter_alloc(types, 2)) == NULL) return -1;
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  if (pmtud_init() != 0) return -1;

  if (options & OPT_DUMP) return pmtud_dump();
