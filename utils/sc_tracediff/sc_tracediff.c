...
static char *addr_toname(const scamper_addr_t *addr, char *buf, size_t len) {
  ...
  if (getnameinfo(sa, sl, buf, len, NULL, 0, NI_NAMEREQD) != 0) return NULL;

  return buf;
}

static char *hop_tostr(const scamper_trace_t *trace, int i, char *buf,
                       size_t *len_out) {
  ...
  if (hop->hop_addr->type == SCAMPER_ADDR_TYPE_IPV4) {
    if (hop->hop_icmp_type == ICMP_UNREACH) {
      ...
      string_concat(buf, len, &off, " !<%d,%d>", hop->hop_icmp_type,
                    hop->hop_icmp_code);
    }
...
static void tracepair_free(tracepair_t *pair) {
  ...
  for (i = 0; i < filec; i++)
    if (pair->traces[i] != NULL) scamper_trace_free(pair->traces[i]);
  free(pair);
...
static void tracepair_dump(const tracepair_t *pair) {
  ...
  if (pair->tracec != 2) return;

  /* print the header of the traceroute */
  trace = pair->traces[0];
  for (i = 1; i < pair->tracec; i++)
    if (scamper_addr_cmp(trace->dst, pair->traces[i]->dst) != 0) break;
  w = 0;
  ...
  for (k = 0; k < pair->tracec; k++) {
    trace = pair->traces[k];
    if (max_ttl < trace->hop_count) max_ttl = trace->hop_count;
    if (min_ttl == 0 || min_ttl > trace->firsthop) min_ttl = trace->firsthop;
  }
  ...
  for (i = min_ttl - 1; i < max_ttl; i++) {
    ws[0] = sizeof(a);
    ws[1] = sizeof(b);
    printf(fs, i + 1, hop_tostr(pair->traces[0], i, a, &ws[0]),
...
static int tracepair_isdiff(const tracepair_t *pair) {
  ...
  for (i = 0; i < hopc; i++) {
    if (a->hops[i] == NULL || b->hops[i] == NULL) continue;
...
static void tracepair_process(const tracepair_t *pair) {
  if ((options & OPT_ALLPAIRS) || tracepair_isdiff(pair)) tracepair_dump(pair);
  return;
}
...
int main(int argc, char *argv[]) {
  ...
  if (check_options(argc, argv) != 0) goto err;
  ...
  while (filec_open != 0) {
    for (i = 0; i < filec; i++) {
      ...
      if ((pair = splaytree_find(pairs, &fm)) == NULL) {
        if ((pair = malloc_zero(sizeof(tracepair_t))) == NULL) goto err;
        pair->traces[i] = trace;
        pair->tracec = 1;
        if ((pair->node = splaytree_insert(pairs, pair)) == NULL) goto err;
      } else {
