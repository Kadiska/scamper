...
static void usage(uint32_t opt_mask) {
  ...
  if (opt_mask & OPT_CREATE) fprintf(stderr, "   -c create db file\n");
...
static int check_options(int argc, char *argv[]) {
  ...
  u32 = OPT_ADDRFILE | OPT_CREATE | OPT_IMPORT | OPT_REBOOTS | OPT_OUTFILE |
        OPT_DONOTPROBE;
  ...
  if ((options & OPT_OUTFILE) == 0 || (options & (OPT_PORT | OPT_UNIX)) == 0 ||
...
static int sc_dst_insert(sc_dst_t *dst) {
  if ((dst->tree_node = splaytree_insert(tree, dst)) == NULL) return -1;
  return 0;
}
...
static sc_dst_t *sc_dst_alloc(uint32_t id, scamper_addr_t *addr) {
  sc_dst_t *dst;
  if ((dst = malloc_zero(sizeof(sc_dst_t))) == NULL) return NULL;
  dst->addr = addr;
  dst->id = id;
  return dst;
}
...
static int do_method(void) {
  ...
  if (srcaddr != NULL) string_concat(cmd, sizeof(cmd), &off, " -S %s", srcaddr);
...
static int ipid_inseq3(uint64_t a, uint64_t b, uint64_t c) {
  ...
  if (fudge != 0) {
    if (b - a > fudge || c - b > fudge) return 0;
  } else {
    if (a > b || b > c) return 0;
  }
  return 1;
...
static int do_meta_getlong(char *type, long *lo) {
  ...
  if ((x = sqlite3_prepare_v2(db, sql, strlen(sql) + 1, &stmt, NULL)) !=
      SQLITE_OK) {
...
static int do_sqlite_vacuum(void) {
  char *errmsg;
  if (sqlite3_exec(db, "vacuum", NULL, NULL, &errmsg) != SQLITE_OK) return -1;
  return 0;
}
...
static int db_update(sc_dst_t *dst, uint32_t last_tx) {
  ...
  if (dst->class != CLASS_INCR) {
     ...
     * over 2 hours
     */
    if (dst->loss > 10 && now.tv_sec - dst->last_rx > (2 * 60 * 60) &&
...
static int do_decoderead(void) {
  ...
  if (type == SCAMPER_FILE_OBJ_PING) return do_decoderead_ping(data);
...
static int donotprobe_line(char *line, void *param) {
  ...
  if (prefixtree_find_exact6(tree, &in6, netlen) != NULL) return 0;
...
static int addrfile_line(char *line, void *param) {
  ...
  if ((dst = sc_dst_find(sa)) == NULL) {
    ...
    if (sqlite3_step(st_addr_i) != SQLITE_DONE) goto done;
...
static int do_sqlite_open(void) {
  ...
  if (options & OPT_CREATE) {
    if (rc == 0 || errno != ENOENT) {
      fprintf(stderr, "%s: will not create db called %s: it already exists\n",
              __func__, dbfile);
      return -1;
    }
  } else {
...
static int do_sqlite_state(void) {
  ...
  if (expire != 0 && do_sqlite_state_expire() != 0) goto done;
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    if (next != 0 && now.tv_sec + interval < next) continue;
  ...
  sql =
      "update state_dsts set class=?,next=?,last_ipid=?,last_rx=?,loss=?,"
      " last_tx=?"
      " where id=?";
  len = strlen(sql);
  if ((x = sqlite3_prepare_v2(db, sql, len + 1, &st_class, NULL)) !=
      SQLITE_OK) {
...
static int up_data(void) {
  ...
  for (;;) {
    ...
    if (more > 0 && do_method() != 0) goto done;

    if (scamper_fd >= 0) {
      if (FD_ISSET(scamper_fd, &rfds) && do_scamperread() != 0) goto done;
    ...
    if (decode_in_fd >= 0) {
      if (FD_ISSET(decode_in_fd, &rfds) && do_decoderead() != 0) goto done;
    }
...
static int up_create(void) {
  if (init_state != 0 && do_sqlite_init_state() != 0) return -1;
  if (init_data != 0 && do_sqlite_init_data() != 0) return -1;
  return 0;
}
...
static int up_addrfile(void) {
  ...
  if ((x = do_meta_getlong(opt_args[0], &lo)) < 0) goto done;
  if (x == 1 && mtime == (time_t)lo) {
    rc = 0;
    goto done;
  }

  if ((tree = splaytree_alloc((splaytree_cmp_t)sc_dst_cmp)) == NULL) goto done;
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    if ((dst = sc_dst_alloc(id, sa)) == NULL) goto done;
    sa = NULL;
    if (sc_dst_insert(dst) != 0) {
  ...
  if (file_lines(opt_args[0], addrfile_line, NULL) != 0) goto done;

  if (do_meta_setlong(opt_args[0], (long)mtime) != 0) goto done;

  if (expire != 0 && do_sqlite_state_expire() != 0) goto done;

  if (vacuum_db != 0 && do_sqlite_vacuum() != 0) goto done;
  ...
  if (st_addr_i != NULL) {
    sqlite3_finalize(st_addr_i);
    st_addr_i = NULL;
  }
  if (st_addr_u != NULL) {
    sqlite3_finalize(st_addr_u);
    st_addr_u = NULL;
  }
...
static int up_donotprobe(void) {
  ...
  if (file_lines(opt_args[0], donotprobe_line, tree) != 0) goto done;

  sqlite3_exec(db, "begin", NULL, NULL, NULL);
  begun = 1;

  sql = "select id, addr from state_dsts";
  if ((x = sqlite3_prepare_v2(db, sql, strlen(sql) + 1, &st_s, NULL)) !=
      SQLITE_OK) {
  ...
  if ((x = sqlite3_prepare_v2(db, sql, strlen(sql) + 1, &st_d, NULL)) !=
      SQLITE_OK) {
 ...
 *     uint32_t tx_usec
 */
static int up_import_sample(sqlite3_blob *blob, uint8_t *buf, uint32_t *off,
                            uint32_t len) {
  int x;
...
static int up_import(void) {
  ...
  if (safe_db == 0) {
    ...
    if (signal(SIGINT, up_import_sigint) == SIG_ERR) goto done;
  }
  ...
  snprintf(buf, sizeof(buf),
           "insert into data_samples(dst_id, data)"
           " values(?, zeroblob(%d))",
           BLOB_SIZE_MIN);
  ...
  for (i = 0; i < opt_argc && up_import_stop == 0; i++) {
    ...
    fprintf(stderr, "[%02d:%02d:%02d] %s %d of %d\n", tm->tm_hour, tm->tm_min,
            tm->tm_sec, opt_args[i], i, opt_argc);
    ...
    while (scamper_file_read(in, ffilter, &type, &data) == 0) {
      ...
      if ((dst = sc_dst_find(ping->dst)) == NULL) {
        ...
        if (up_import_blob_new(dst, st_sample_ins, st_addr_upd) != 0) goto done;
      }

      if (up_import_blob_get(dst, &blob) != 0) goto done;
      ...
      for (j = 0; j < ping->ping_sent; j++) {
        ...
        if (r != NULL && SCAMPER_PING_REPLY_IS_ICMP_ECHO_REPLY(r)) {
          ...
          c++;
          if (up_import_sample(blob, u8, &blob_off, blob_len) != 0) goto done;
        }
      }
...
static int up_reboots_arerandom(sc_sample_t **samples, int samplec, int l,
                                int r) {
  ...
  for (;;) {
    ...
    if (samples[i + 1]->ipid > samples[i]->ipid) {
      ...
      if (posdiffc == 0 || u32 < posdiff_min) posdiff_min = u32;
      posdiffc++;
    }
    i++;
  }
  ...
  if (posdiff_min < 1000) return 0;
  if (r - l - 1 == posdiffc) return 0;
  if (((double)(r - l - 1)) - posdiffc / 1.0 / posdiffc > 0.5) return 1;
  ...
  *outc = 0;
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if ((s2 = slist_node_next(sn)) != NULL)
      ...
      sample->next = NULL;
    if (sample->type == 1) samplec++;
  }
  if (samplec < 10) return 0;
  ...
  for (sn = slist_head_node(list); sn != NULL; sn = slist_node_next(sn)) {
    sample = slist_node_item(sn);
    if (sample->type == 1) samples[i++] = sample;
  }
...
static int up_reboots_seqs_make(slist_t *seqs, sc_sample_t **samples, int l,
                                int r) {
  ...
  for (i = 0; i < r - l; i++) seq->samples[i] = samples[l + i];
  seq->samplec = r - l;
  seq->velocity = 0;
  ...
  for (i = 1; i < seq->samplec; i++) {
    tx = seq->samples[i]->tx_sec;
    ipid = seq->samples[i]->ipid;
    last_tx = seq->samples[i - 1]->tx_sec;
    last_ipid = seq->samples[i - 1]->ipid;
...
static int up_reboots_seqs_class_reseed(slist_t *seqs) {
  ...
  if ((series = slist_alloc()) == NULL) goto done;
  ...
  for (sn = slist_head_node(seqs); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (seq->type != 0 ||
      ...
      if (slist_count(series) >= 10)
        while ((seq = slist_head_pop(series)) != NULL) seq->type = 2;
      slist_empty(series);
      continue;
    }
    if (seq->type == 0 && slist_tail_push(series, seq) == NULL) goto done;
  }
...
static void up_reboots_seqs_merge_wrap32(slist_t *seqs) {
  ...
  for (sn = slist_head_node(seqs); sn != NULL; sn = slist_node_next(sn)) {
    seq = slist_node_item(sn);
    if (seq->type != 0) continue;
    if ((sn_next = slist_node_next(sn)) == NULL) break;
...
static void up_reboots_seqs_merge_wrap16(slist_t *seqs) {
  ...
  if ((sn_prev = slist_head_node(seqs)) == NULL) return;
  ...
  while (sn != NULL) {
    ...
    if (max_expected_ipid < 65535 || max_expected_ipid - last_ipid > 10000 ||
...
static int up_reboots_seqs_merge_parallel(slist_t *seqs) {
  ...
  if ((ctrs = dlist_alloc()) == NULL) return -1;
  for (sn = slist_head_node(seqs); sn != NULL; sn = slist_node_next(sn)) {
    seq = slist_node_item(sn);
    if (seq->type != 0) continue;
    ...
    for (dn = dlist_head_node(ctrs); dn != NULL; dn = dlist_node_next(dn)) {
      ...
      last_ipid = seq_prev->samples[seq_prev->samplec - 1]->ipid;
      max_expected_ipid =
          last_ipid + ((tx - last_tx) * 10 * seq_prev->velocity);
    ...
    while (dn != NULL) {
      ...
      seq_prev = dlist_node_item(dn);
      if (seq_prev->samples[seq_prev->samplec - 1]->tx_sec + (48 * 60 * 60) <
          tx)
        dlist_node_pop(ctrs, dn);
      dn = dn_next;
    }
...
static slist_t *up_reboots_seqs(sc_sample_t **samples, int samplec) {
  ...
  if (up_reboots_arerandom(samples, samplec, 0, 5) == 1) random = 1;

  l = 0;

  if ((seqs = slist_alloc()) == NULL) goto err;
  ...
  for (i = 1; i <= samplec; i++) {
    ...
    if (i == samplec) {
      if (up_reboots_seqs_make(seqs, samples, l, i) != 0) goto err;
      break;
    }
    ...
    if (ipid < last_ipid || ipid > max_expected_ipid + fudge) {
      if (up_reboots_seqs_make(seqs, samples, l, i) != 0) goto err;
      if (up_reboots_arerandom(samples, samplec, i, i + 5) == 1) random = 1;
      l = i;
    } else {
  ...
  if (up_reboots_seqs_class_reseed(seqs) != 0) goto err;
  up_reboots_seqs_merge_wrap32(seqs);
  up_reboots_seqs_merge_wrap16(seqs);
  if (up_reboots_seqs_merge_parallel(seqs) != 0) goto err;
...
static int up_reboots_doone(sc_dst_t *dst, slist_t *samplist,
                            slist_t *reboots) {
  ...
  if (up_reboots_init(samplist, &samples, &samplec) != 0) goto err;
  if (samples == NULL) return 0;
  if ((tree = splaytree_alloc(ptrcmp)) == NULL) goto err;
  if ((seqs = up_reboots_seqs(samples, samplec)) == NULL) goto err;

  for (sn = slist_head_node(seqs); sn != NULL; sn = slist_node_next(sn)) {
    seq = slist_node_item(sn);

    /* if this sequence ends here, remove it */
    if (splaytree_find(tree, seq) != NULL) splaytree_remove_item(tree, seq);

    /* if sequence is contained within a continuous sequence, no reboot */
    if (splaytree_count(tree) != 0) goto next;

    /* if a sequence ended here, then no reboot */
    if (seq->prev != NULL) goto next;

    /*
     * both the current and previous sequences must be assigned from
     * a counter to infer a reboot
     */
    if (seq->type != 0 || seq_last == NULL || seq_last->type != 0) goto next;
    ...
    tx = seq->samples[0]->tx_sec;
    if (tx - last_tx < 60 || tx - last_tx > (7 * 24 * 60 * 60)) goto next;

    if ((reboot = malloc(sizeof(sc_reboot_t))) == NULL) goto err;
    reboot->left = last_tx;
    reboot->right = tx;
...
static void up_reboots_samples_dumpone(sc_sample_t *sample) {
  ...
  t = sample->tx_sec;
  tm = gmtime(&t);
  printf("%04d%02d%02d %02d:%02d:%02d:%03d", tm->tm_year + 1900, tm->tm_mon + 1,
...
static void up_reboots_samples_dump(slist_t *samplist) {
  ...
  if (up_reboots_init(samplist, &samples, &samplec) != 0) return;
  if (samples == NULL) return;
  if ((seqs = up_reboots_seqs(samples, samplec)) == NULL) return;
  ...
  for (sn = slist_head_node(samplist); sn != NULL; sn = slist_node_next(sn)) {
    ...
    if (seq != NULL && s == seq->samples[0]) break;
...
static int up_reboots(void) {
  ...
  if ((addrs = slist_alloc()) == NULL) goto done;
  ...
  if ((x = sqlite3_prepare_v2(db, sql, strlen(sql) + 1, &st, NULL)) !=
      SQLITE_OK) {
    fprintf(stderr, "%s: could not prepare sql: %s\n", __func__,
            sqlite3_errstr(x));
    goto done;
  ...
  while ((dst = slist_head_pop(addrs)) != NULL) {
    ...
    while ((x = sqlite3_step(st)) == SQLITE_ROW) {
      ...
      if (blob != NULL)
        ...
        x = sqlite3_blob_open(db, "main", "data_samples", "data", sample_id, 0,
                              &blob);
      if (x != SQLITE_OK) {
        fprintf(stderr, "%s: could not open blob %lld for %s: %s\n", __func__,
                sample_id, buf, sqlite3_errstr(x));
        goto done;
      }
    ...
    if (verbose != 0) up_reboots_samples_dump(samples);
    up_reboots_doone(dst, samples, reboots);
    while ((sample = slist_head_pop(samples)) != NULL) free(sample);
...
static int up_init(void) {
  ...
  if (do_sqlite_open() != 0) return -1;
  if (do_sqlite_init_meta() != 0) return -1;
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  if (up_init() != 0) return -1;

  if (options & OPT_CREATE) return up_create();

  if (options & OPT_ADDRFILE) return up_addrfile();

  if (options & OPT_DONOTPROBE) return up_donotprobe();

  if (options & OPT_IMPORT) return up_import();

  if (options & OPT_REBOOTS) return up_reboots();
