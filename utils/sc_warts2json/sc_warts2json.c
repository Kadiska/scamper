...
int main(int argc, char *argv[]) {
  uint16_t types[] = {
      SCAMPER_FILE_OBJ_CYCLE_START, SCAMPER_FILE_OBJ_CYCLE_STOP,
      SCAMPER_FILE_OBJ_PING,        SCAMPER_FILE_OBJ_TRACE,
  ...
  for (i = 0; i <= filec; i++) {
    ...
    while (scamper_file_read(in, filter, &type, (void *)&data) == 0) {
      ...
      if (scamper_file_write_obj(out, type, data) != 0) return -1;
