...
static int check_options(int argc, char *argv[]) {
  ...
  if (outfile_name == NULL) outfile_name = "-";
...
static int pkt_write(uint8_t *data, uint16_t len, struct timeval *tv, void *p) {
  ...
  if (fwrite(&rec, sizeof(rec), 1, outfile_fd) != 1) return -1;
  if (fwrite(data, len, 1, outfile_fd) != 1) return -1;
  return 0;
}
...
static int pkt_push(uint8_t *data, uint16_t len, struct timeval *tv, void *p) {
  ...
  if ((pkt = malloc_zero(sizeof(pkt_t))) == NULL) goto err;
  if ((pkt->data = memdup(data, len)) == NULL) goto err;
  pkt->len = len;
  timeval_cpy(&pkt->tv, tv);

  if (slist_tail_push(list, pkt) == NULL) goto err;
...
static int finish_1(void) {
  pkt_t *pkt;
  if (slist_qsort(list, (slist_cmp_t)pkt_cmp) != 0) return -1;
  while ((pkt = slist_head_pop(list)) != NULL) {
    if (pkt_write(pkt->data, pkt->len, &pkt->tv, NULL) != 0) return -1;
    pkt_free(pkt);
  }
  slist_free(list);
  return 0;
...
static int do_file(scamper_file_t *in) {
  ...
  while ((rc = scamper_file_read(in, filter, &type, &data)) == 0) {
    if (data == NULL) break;
    if (sort[sorti].foreach (type, data) != 0) goto err;
  }
  if (rc != 0) goto err;
...
int main(int argc, char *argv[]) {
  ...
  if (fwrite(&hdr, sizeof(phdr_t), 1, outfile_fd) != 1) goto err;

  if (sort[sorti].init != NULL && sort[sorti].init() != 0) goto err;
  ...
  if (sort[sorti].finish != NULL && sort[sorti].finish() != 0) goto err;
