...
static char *ip2descr_lookup(scamper_addr_t *addr) {
  ...
  fm.addr = addr;
  if ((ip2descr = splaytree_find(tree, &fm)) == NULL) return NULL;
...
static int check_options(int argc, char *argv[]) {
  ...
  if (opt_descr != NULL) {
    if ((tree = splaytree_alloc((splaytree_cmp_t)ip2descr_cmp)) == NULL)
      return -1;
    if (file_lines(opt_descr, ip2descr_line, NULL) != 0) return -1;
  }

  filec = argc - optind;
  if (filec > 0) files = argv + optind;
...
int main(int argc, char *argv[]) {
  funcset_t funcs[] = {
      ...
      {(wf_t)scamper_file_write_tracelb, (ff_t)scamper_tracelb_free,
       tracelb_dst},
  ...
  uint16_t types[] = {
      SCAMPER_FILE_OBJ_TRACE,   SCAMPER_FILE_OBJ_PING, SCAMPER_FILE_OBJ_TRACELB,
      SCAMPER_FILE_OBJ_DEALIAS, SCAMPER_FILE_OBJ_TBIT, SCAMPER_FILE_OBJ_STING,
  };
