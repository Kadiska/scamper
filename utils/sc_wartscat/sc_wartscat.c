...
static int write_obj(uint16_t type, void *data) {
  ...
  switch (type) {
    case SCAMPER_FILE_OBJ_TRACELB:
      if (scamper_file_write_tracelb(outfile, data) != 0) return -1;
      scamper_tracelb_free(data);
      break;

    case SCAMPER_FILE_OBJ_TRACE:
      if (scamper_file_write_trace(outfile, data) != 0) return -1;
      scamper_trace_free(data);
      break;

    case SCAMPER_FILE_OBJ_PING:
      if (scamper_file_write_ping(outfile, data) != 0) return -1;
      scamper_ping_free(data);
      break;

    case SCAMPER_FILE_OBJ_DEALIAS:
      if (scamper_file_write_dealias(outfile, data) != 0) return -1;
      scamper_dealias_free(data);
      break;

    case SCAMPER_FILE_OBJ_CYCLE_START:
      if (scamper_file_write_cycle_start(outfile, data) != 0) return -1;
      scamper_cycle_free(data);
      break;

    case SCAMPER_FILE_OBJ_CYCLE_STOP:
      if (scamper_file_write_cycle_stop(outfile, data) != 0) return -1;
      scamper_cycle_free(data);
      break;
...
static int simple_cat(void) {
  ...
  for (i = 0; i < infile_cnt; i++) {
    while ((rc = scamper_file_read(infiles[i], filter, &type, &data)) == 0) {
      /* EOF */
      if (data == NULL) break;
      if (write_obj(type, data) != 0) return -1;
    }
...
static int sort_struct_cmp(const sort_struct_t *a, const sort_struct_t *b) {
  ...
  if (a->type == SCAMPER_FILE_OBJ_CYCLE_STOP) {
    if (b->type == SCAMPER_FILE_OBJ_CYCLE_STOP)
      return 0;
    else
      return 1;
  }
...
static int sort_cat_fill(heap_t *heap, sort_struct_t *s) {
  ...
  if (scamper_file_read(infiles[i], filter, &s->type, &s->data) == 0) {
    ...
    if (heap_insert(heap, s) == NULL) return -1;
  } else
    return -1;
...
static int sort_cat(void) {
  ...
  if ((heap = heap_alloc((heap_cmp_t)sort_struct_cmp)) == NULL) goto err;

  if ((ss = malloc_zero(sizeof(sort_struct_t) * infile_cnt)) == NULL) goto err;
  ...
  for (i = 0; i < infile_cnt; i++) {
    ss[i].file = i;
    if (sort_cat_fill(heap, &ss[i]) != 0) goto err;
  }
  ...
  while ((s = (sort_struct_t *)heap_remove(heap)) != NULL) {
    if (write_obj(s->type, s->data) != 0) goto err;
    if (sort_cat_fill(heap, s) != 0) goto err;
  }
...
int main(int argc, char *argv[]) {
  uint16_t filter_types[] = {
      SCAMPER_FILE_OBJ_TRACELB,    SCAMPER_FILE_OBJ_TRACE,
      SCAMPER_FILE_OBJ_PING,       SCAMPER_FILE_OBJ_DEALIAS,
      SCAMPER_FILE_OBJ_TBIT,       SCAMPER_FILE_OBJ_NEIGHBOURDISC,
      SCAMPER_FILE_OBJ_STING,      SCAMPER_FILE_OBJ_CYCLE_START,
  ...
  if (check_options(argc, argv) == -1) return -1;
