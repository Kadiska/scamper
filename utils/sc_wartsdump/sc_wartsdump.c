...
static void dump_list_summary(scamper_list_t *list) {
  if (list != NULL) {
    ...
    if (list->name != NULL) printf(", name: %s", list->name);
    if (list->monitor != NULL) printf(", monitor: %s", list->monitor);
    printf("\n");
  }
  return;
}
...
static void dump_cycle_summary(scamper_cycle_t *cycle) {
  if (cycle != NULL) printf(" cycle id: %d\n", cycle->id);
  return;
}

static void dump_tcp_flags(uint8_t flags) {
  if (flags != 0) {
    printf(" (%s%s%s%s%s%s%s%s )", (flags & 0x01) ? " fin" : "",
           (flags & 0x02) ? " syn" : "", (flags & 0x04) ? " rst" : "",
           (flags & 0x08) ? " psh" : "", (flags & 0x10) ? " ack" : "",
           (flags & 0x20) ? " urg" : "", (flags & 0x40) ? " ece" : "",
           (flags & 0x80) ? " cwr" : "");
  }
  return;
}
...
static void dump_trace_hop(const scamper_trace_t *trace,
  ...
  printf("hop %2d  %s", hop->hop_probe_ttl,
         scamper_addr_tostr(hop->hop_addr, addr, sizeof(addr)));
  if (hop->hop_name != NULL) printf(" name %s", hop->hop_name);
  printf("\n");
  ...
  printf(", rtt: %d.%06ds, probe-size: %d\n", (int)hop->hop_rtt.tv_sec,
         (int)hop->hop_rtt.tv_usec, hop->hop_probe_size);
  ...
  if (SCAMPER_TRACE_HOP_IS_ICMP(hop)) {
    printf(" icmp-type: %d, icmp-code: %d", hop->hop_icmp_type,
           hop->hop_icmp_code);
    if (SCAMPER_TRACE_HOP_IS_ICMP_Q(hop)) {
      printf(", q-ttl: %d, q-len: %d", hop->hop_icmp_q_ttl,
             hop->hop_icmp_q_ipl);
      if (hop->hop_addr->type == SCAMPER_ADDR_TYPE_IPV4)
  ...
  if (hop->hop_flags != 0) {
    printf(" (");
    if (hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_TS_SOCK_RX) printf(" sockrxts");
    if (hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_TS_DL_TX) printf(" dltxts");
    if (hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_TS_DL_RX) printf(" dlrxts");
    if (hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_TS_TSC) printf(" tscrtt");
    if (hop->hop_flags & SCAMPER_TRACE_HOP_FLAG_REPLY_TTL) printf(" replyttl");
    printf(" )");
  }
  printf("\n");
    ...
    if (SCAMPER_ICMPEXT_IS_MPLS(ie)) {
      for (i = 0; i < SCAMPER_ICMPEXT_MPLS_COUNT(ie); i++) {
        ...
        printf("%9s ttl: %d, s: %d, exp: %d, label: %d\n",
               (i == 0) ? "mpls ext" : "", SCAMPER_ICMPEXT_MPLS_TTL(ie, i),
               SCAMPER_ICMPEXT_MPLS_S(ie, i), SCAMPER_ICMPEXT_MPLS_EXP(ie, i),
               u32);
      }
    }
  }
...
static void dump_trace(scamper_trace_t *trace) {
  ...
  switch (trace->type) {
    ...
    case SCAMPER_TRACE_TYPE_UDP:
      printf("udp, sport: %d, base dport: %d", trace->sport, trace->dport);
      break;

    case SCAMPER_TRACE_TYPE_UDP_PARIS:
      printf("udp paris, sport: %d, dport: %d", trace->sport, trace->dport);
      break;
    ...
    case SCAMPER_TRACE_TYPE_TCP_ACK:
      printf("tcp-ack, sport: %d, dport: %d", trace->sport, trace->dport);
      break;
  ...
  if (trace->offset != 0) printf(", offset %d", trace->offset);
  printf("\n");

  if (trace->dtree != NULL) {
    printf(" doubletree firsthop: %d", trace->dtree->firsthop);
    if (trace->dtree->lss != NULL) printf(", lss-name: %s", trace->dtree->lss);
  ...
  printf(" squeries: %d, gaplimit: %d, gapaction: ", trace->squeries,
         trace->gaplimit);
  ...
  if (trace->confidence != 0) printf(", confidence: %d%%", trace->confidence);
  printf("\n");

  printf(" flags: 0x%02x", trace->flags);
  if (trace->flags != 0) {
    printf(" (");
    if (trace->flags & SCAMPER_TRACE_FLAG_ALLATTEMPTS) printf(" all-attempts");
    if (trace->flags & SCAMPER_TRACE_FLAG_PMTUD) printf(" pmtud");
    if (trace->flags & SCAMPER_TRACE_FLAG_DL) printf(" dl");
    if (trace->flags & SCAMPER_TRACE_FLAG_IGNORETTLDST) printf(" ignorettldst");
    if (trace->flags & SCAMPER_TRACE_FLAG_DOUBLETREE) printf(" doubletree");
    ...
    if (trace->flags & SCAMPER_TRACE_FLAG_RXERR) printf(" rxerr");
    if (trace->flags & SCAMPER_TRACE_FLAG_PTR) printf(" ptr");
    printf(" )");
  }
  printf("\n");
  ...
  if ((pmtud = trace->pmtud) != NULL) {
    printf("pmtud: ver %d ifmtu %d, pmtu %d", pmtud->ver, pmtud->ifmtu,
           pmtud->pmtu);
    if (pmtud->outmtu != 0) printf(", outmtu %d", pmtud->outmtu);
    if (pmtud->notec != 0) printf(", notec %d", pmtud->notec);
    printf("\n");
...
static void dump_tracelb_reply(scamper_tracelb_probe_t *probe,
  ...
  printf("   reply from: %s, rtt: %d.%06d, ttl: %d", from, (int)rtt.tv_sec,
         (int)rtt.tv_usec, reply->reply_ttl);
  ...
  if (reply->reply_flags & SCAMPER_TRACELB_REPLY_FLAG_TCP) {
    ...
    printf("     icmp: %d/%d, q-tos: 0x%02x", reply->reply_icmp_type,
           reply->reply_icmp_code, reply->reply_icmp_q_tos);
      ...
      if (SCAMPER_ICMPEXT_IS_MPLS(ie)) {
        for (m = 0; m < SCAMPER_ICMPEXT_MPLS_COUNT(ie); m++) {
          ...
          printf("   %9s: label %d exp %d s %d ttl %d\n",
                 ...
                 SCAMPER_ICMPEXT_MPLS_EXP(ie, m), SCAMPER_ICMPEXT_MPLS_S(ie, m),
...
static void dump_tracelb_probe(scamper_tracelb_t *trace,
  ...
  printf("  probe flowid: %d, ttl: %d, attempt: %d, tx: %d.%06d\n",
         probe->flowid, probe->ttl, probe->attempt, (int)probe->tx.tv_sec,
         (int)probe->tx.tv_usec);
...
static void dump_tracelb(scamper_tracelb_t *trace) {
  ...
  printf(" firsthop: %d, attempts: %d, confidence: %d\n", trace->firsthop,
         trace->attempts, trace->confidence);
  ...
  printf(" nodec: %d, linkc: %d, probec: %d, probec_max: %d\n", trace->nodec,
         trace->linkc, trace->probec, trace->probec_max);
  if (trace->flags != 0) {
    printf(" flags:");
    l = 0;
    for (i = 0; i < 1; i++) {
      if ((trace->flags & (0x1 << i)) == 0) continue;
      if (l > 0) printf(",");
      printf(" %s", flags[i]);
      l++;
    }
  ...
  for (i = 0; i < trace->nodec; i++) {
    ...
    if (SCAMPER_TRACELB_NODE_QTTL(node) != 0) printf(", q-ttl %d", node->q_ttl);
    if (node->name != NULL) printf(", name %s", node->name);
    printf("\n");
...
static void dump_ping_reply(const scamper_ping_t *ping,
  ...
  if ((v4rr = reply->v4rr) != NULL) {
    ...
    for (i = 0; i < v4rr->rrc; i++) {
      ...
      printf(" %-15s", scamper_addr_tostr(v4rr->rr[i], buf, sizeof(buf)));
    }
    printf("\n");
  }
...
static void dump_ping(scamper_ping_t *ping) {
  ...
  if (ping->flags & SCAMPER_PING_FLAG_SPOOF) printf(" (spoofed)");
  ...
  if (ping->reply_count > 0) printf(", replies-req: %d", ping->reply_count);
  printf(", size: %d", ping->probe_size);
  if (ping->reply_pmtu > 0) printf(", reply-pmtu: %d", ping->reply_pmtu);
  printf(", wait: %u", ping->probe_wait);
  if (ping->probe_wait_us > 0) {
    u32 = ping->probe_wait_us;
    while ((u32 % 10) == 0) u32 /= 10;
    printf(".%u", u32);
  }
  ...
  if (ping->flags != 0) {
    ...
    u32 = 0;
    for (i = 0; i < 9; i++) {
      if ((ping->flags & (0x1 << i)) == 0) continue;
      if (u32 > 0) printf(",");
      printf(" %s", flags[i]);
      u32++;
    }
  ...
  switch (ping->probe_method) {
    ...
    case SCAMPER_PING_METHOD_TCP_ACK_SPORT:
      printf(", base-sport: %d, dport: %d", ping->probe_sport,
             ping->probe_dport);
      break;

    case SCAMPER_PING_METHOD_UDP_DPORT:
      printf(", sport: %d, base-dport %d", ping->probe_sport,
             ping->probe_dport);
      break;
  }
  ...
  switch (ping->stop_reason) {
    ...
    case SCAMPER_PING_STOP_ERROR:
      printf("sendto errno %d", ping->stop_data);
      break;
    ...
    default:
      printf("reason 0x%02x data 0x%02x", ping->stop_reason, ping->stop_data);
      break;
  }
  printf("\n");
...
static void dump_dealias_probedef(scamper_dealias_probedef_t *def) {
  ...
  printf(" probedef %d: dst: %s, ttl: %d, tos: 0x%02x\n  src: %s", def->id,
         scamper_addr_tostr(def->dst, dst, sizeof(dst)), def->ttl, def->tos,
         scamper_addr_tostr(def->src, src, sizeof(src)));
  if (def->size > 0) printf(", size: %d", def->size);
  if (def->mtu > 0) printf(", mtu: %d", def->mtu);
  printf("\n");
...
static void dump_dealias(scamper_dealias_t *dealias) {
  ...
  if (dealias->method == SCAMPER_DEALIAS_METHOD_MERCATOR) {
    ...
    printf(
        "radargun, wait-probe: %dms, wait-round: %dms\n"
        "  wait-timeout: %ds, attempts: %d, probedefc: %d\n",
        radargun->wait_probe, radargun->wait_round, radargun->wait_timeout,
        radargun->attempts, radargun->probedefc);
    if ((u8 = radargun->flags) != 0) {
      printf("  flags: ");
      for (i = 0; i < 8; i++) {
        if ((u8 & (1 << i)) == 0) continue;
        switch (1 << i) {
    ...
    printf("prefixscan, %s:", scamper_addr_tostr(ps->a, buf, sizeof(buf)));
    printf("%s/%d", scamper_addr_tostr(ps->b, buf, sizeof(buf)), ps->prefix);
    if (ps->ab != NULL)
      printf(", alias: %s/%d", scamper_addr_tostr(ps->ab, buf, sizeof(buf)),
             scamper_addr_prefixhosts(ps->b, ps->ab));
    printf("\n");

    printf(
        "  attempts: %d, replyc: %d, fudge: %d, wait-probe: %dms, "
        "wait-timeout: %ds",
        ps->attempts, ps->replyc, ps->fudge, ps->wait_probe, ps->wait_timeout);
    if (SCAMPER_DEALIAS_PREFIXSCAN_IS_NOBS(dealias)) printf(", nobs");
    printf("\n");
    if (ps->xc > 0) {
      printf("  exclude:");
  ...
  for (i = 0; i < dealias->probec; i++) {
    ...
    printf(" probe: %d, def: %d, seq: %d, tx: %d.%06d", i, probe->def->id,
           probe->seq, (int)probe->tx.tv_sec, (int)probe->tx.tv_usec);
    ...
    for (j = 0; j < probe->replyc; j++) {
      ...
      printf("  reply: %d, src: %s, ttl: %d, rtt: %d.%06d", j,
             scamper_addr_tostr(reply->src, buf, sizeof(buf)), reply->ttl,
      ...
      if (SCAMPER_DEALIAS_REPLY_IS_ICMP(reply)) {
        printf("  icmp-type: %d, icmp-code: %d", reply->icmp_type,
               reply->icmp_code);
...
static void tbit_bits_print(uint32_t flags, int bits, const char **f2s,
                            int f2sc) {
  int i, f = 0;
  uint32_t u32;
...
static void dump_tbit(scamper_tbit_t *tbit) {
  ...
  static const char *null_options[] = {
      "tcpts", "ipts-syn", "iprr-syn", "ipqs-syn", "sack", "fo", "fo-exp"};
  ...
  printf(" client-mss: %d, server-mss: %d, ttl: %u", tbit->client_mss,
         tbit->server_mss, tbit->ttl);
  if (tbit->wscale > 0) printf(", wscale: %u", tbit->wscale);
  printf("\n");
  ...
  if (tbit->type == SCAMPER_TBIT_TYPE_PMTUD && tbit->data != NULL) {
    ...
    if (null->results != 0) {
      ...
      if ((null->results & SCAMPER_TBIT_NULL_RESULT_FO) &&
        ...
        printf(" fo-cookie: ");
        for (i = 0; i < u8; i++) printf("%02x", cookie[i]);
        printf("\n");
      }
    }
  ...
  for (i = 0; i < tbit->pktc; i++) {
    ...
    if ((pkt->data[0] >> 4) == 4) {
      ...
      proto = pkt->data[9];
      ecn = pkt->data[1] & 0x3;
      if (pkt->data[6] & 0x20) mf = 1;
      off = (bytes_ntohs(pkt->data + 6) & 0x1fff) * 8;
      if (mf != 0 || off != 0) frag = 1;
      ...
      for (;;) {
        switch (proto) {
          ...
          case IPPROTO_FRAGMENT:
            if (255 - iphlen <= 8) break;
            if (pkt->data[iphlen + 3] & 0x1) mf = 1;
    ...
    if (off != 0) {
      ...
      while (u8 < tcphlen) {
        ...
        if (tmp[1] == 0 || u8 + tmp[1] > tcphlen) break;
      ...
      if (flags & TH_ACK) string_concat(buf, sizeof(buf), &soff, ":%u", ack);
      if (datalen != 0) string_concat(buf, sizeof(buf), &soff, "(%d)", datalen);
...
static void dump_sting(scamper_sting_t *sting) {
  ...
  for (i = 0; i < sting->pktc; i++) {
    ...
    if (pkt->flags & SCAMPER_STING_PKT_FLAG_TX)
      dir = "TX";
    else if (pkt->flags & SCAMPER_STING_PKT_FLAG_RX)
      dir = "RX";
    else
      dir = "??";
...
static void dump_sniff(scamper_sniff_t *sniff) {
  ...
  printf(" limit-pktc: %d, limit-time: %d, icmp-id %d\n", sniff->limit_pktc,
         sniff->limit_time, sniff->icmpid);
  switch (sniff->stop_reason) {
    case SCAMPER_SNIFF_STOP_NONE:
      str = "none";
      break;
    case SCAMPER_SNIFF_STOP_ERROR:
      str = "error";
      break;
    case SCAMPER_SNIFF_STOP_LIMIT_TIME:
      str = "limit-time";
      break;
    case SCAMPER_SNIFF_STOP_LIMIT_PKTC:
      str = "limit-pktc";
      break;
    case SCAMPER_SNIFF_STOP_HALTED:
      str = "halted";
      break;
    default:
  ...
  for (i = 0; i < sniff->pktc; i++) {
    ...
    if (u8 == 4) {
      printf(" %s -> %s", inet_ntop(AF_INET, pkt->data + 12, src, sizeof(src)),
             inet_ntop(AF_INET, pkt->data + 16, dst, sizeof(dst)));
    } else if (u8 == 6) {
      printf(" %s -> %s", inet_ntop(AF_INET6, pkt->data + 8, src, sizeof(src)),
...
static void dump_host_rr(scamper_host_rr_t *rr, const char *section) {
  ...
  printf("  %s: %s %u ", section, rr->name != NULL ? rr->name : "<null>",
         rr->ttl);
  ...
  switch (rr->type) {
    ...
    case SCAMPER_HOST_TYPE_AAAA:
      printf("AAAA");
      break;
    case SCAMPER_HOST_TYPE_DS:
      printf("DS");
      break;
    case SCAMPER_HOST_TYPE_SSHFP:
      printf("SSHFP");
      break;
    case SCAMPER_HOST_TYPE_RRSIG:
      printf("RRISG");
      break;
    case SCAMPER_HOST_TYPE_NSEC:
      printf("NSEC");
      break;
    case SCAMPER_HOST_TYPE_DNSKEY:
      printf("DNSKEY");
      break;
    default:
      printf("%d", rr->type);
...
static void dump_host(scamper_host_t *host) {
  ...
  if (host->flags != 0) {
    printf(" flags: ");
    if (host->flags & SCAMPER_HOST_FLAG_NORECURSE) printf("norecurse");
    printf("\n");
  }
  ...
  printf(" stop: ");
  switch (host->stop) {
    case SCAMPER_HOST_STOP_NONE:
      printf("none");
      break;
    case SCAMPER_HOST_STOP_DONE:
      printf("done");
      break;
    case SCAMPER_HOST_STOP_TIMEOUT:
      printf("timeout");
      break;
    case SCAMPER_HOST_STOP_HALTED:
      printf("halted");
      break;
    case SCAMPER_HOST_STOP_ERROR:
      printf("error");
      break;
    default:
      printf("%04x", host->stop);
      break;
  }
  printf("\n");
  ...
  for (i = 0; i < host->qcount; i++) {
    ...
    printf(" query: %u, id: %u, tx: %d.%06d", i, query->id, (int)tv.tv_sec,
           (int)tv.tv_usec);
    ...
    for (j = 0; j < query->ancount; j++) dump_host_rr(query->an[j], "an");
    for (j = 0; j < query->nscount; j++) dump_host_rr(query->ns[j], "ns");
    for (j = 0; j < query->arcount; j++) dump_host_rr(query->ar[j], "ar");
  }
...
static void dump_cycle(scamper_cycle_t *cycle, const char *type) {
  ...
  printf("cycle %s, list %s %d, cycle %d, time %s\n", type, cycle->list->name,
         cycle->list->id, cycle->id, buf);
...
int main(int argc, char *argv[]) {
  ...
  uint16_t filter_types[] = {
      SCAMPER_FILE_OBJ_LIST,          SCAMPER_FILE_OBJ_CYCLE_START,
      SCAMPER_FILE_OBJ_CYCLE_DEF,     SCAMPER_FILE_OBJ_CYCLE_STOP,
      SCAMPER_FILE_OBJ_TRACE,         SCAMPER_FILE_OBJ_PING,
      SCAMPER_FILE_OBJ_TRACELB,       SCAMPER_FILE_OBJ_DEALIAS,
      SCAMPER_FILE_OBJ_NEIGHBOURDISC, SCAMPER_FILE_OBJ_TBIT,
      SCAMPER_FILE_OBJ_STING,         SCAMPER_FILE_OBJ_SNIFF,
