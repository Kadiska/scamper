...
static int check_options(int argc, char *argv[]) {
  ...
  struct in6_addr in6;
  uint16_t types_def[] = {
      SCAMPER_FILE_OBJ_DEALIAS, SCAMPER_FILE_OBJ_PING,    SCAMPER_FILE_OBJ_TBIT,
      SCAMPER_FILE_OBJ_TRACE,   SCAMPER_FILE_OBJ_TRACELB,
  };
  uint16_t *types = NULL;
  int i, typec;
  long lo;
  ...
  slist_free_cb(addrs, free);
  addrs = NULL;

  /* determine where to read the warts file */
  if (opt_infile == NULL) {
    if ((infile = scamper_file_openfd(STDIN_FILENO, "-", 'r', "warts")) ==
        NULL) {
  ...
  if (opt_outfile == NULL || string_isdash(opt_outfile) != 0) {
    ...
    if ((outfile = scamper_file_openfd(STDOUT_FILENO, "-", 'w', "warts")) ==
        NULL) {
...
static int addr_matched(scamper_addr_t *addr) {
  if (SCAMPER_ADDR_TYPE_IS_IPV4(addr)) {
    if (prefixtree_find_ip4(addr_pt4, addr->addr) == NULL) return 0;
  } else if (SCAMPER_ADDR_TYPE_IS_IPV6(addr)) {
    if (prefixtree_find_ip6(addr_pt6, addr->addr) == NULL) return 0;
  } else
    return 0;
  return 1;
}
...
static void process_dealias(scamper_dealias_t *dealias) {
  ...
  if (addrc > 0) {
    if (SCAMPER_DEALIAS_METHOD_IS_MERCATOR(dealias)) {
      mc = dealias->data;
      if (addr_matched(mc->probedef.dst) == 0) goto done;
      ...
      for (i = 0; i < rg->probedefc; i++)
        if (addr_matched(rg->probedefs[i].dst) != 0) break;
      if (i == rg->probedefc) goto done;
    } else if (SCAMPER_DEALIAS_METHOD_IS_PREFIXSCAN(dealias)) {
      pfs = dealias->data;
      for (i = 0; i < pfs->probedefc; i++)
        if (addr_matched(pfs->probedefs[i].dst) != 0) break;
      if (i == pfs->probedefc) goto done;
    } else
      goto done;
  }
...
static void process_ping(scamper_ping_t *ping) {
  if (addrc > 0 && addr_matched(ping->dst) == 0) goto done;
...
static void process_tbit(scamper_tbit_t *tbit) {
  if (addrc > 0 && addr_matched(tbit->dst) == 0) goto done;
